...
DEVICE="iPhone 17"
PUSH_DEMO=""
BUNDLE_ID="com.pryx.mobile"
PORT=7643
SHOT_DIR="${TMPDIR:-/tmp}/pryx-ios-e2e"
...
xcodebuild build \
  -project PryxMobile.xcodeproj \
  -scheme PryxMobile \
  -destination "platform=iOS Simulator,name=$DEVICE" \
  -derivedDataPath .build-ios >/dev/null
APP=".build-ios/Build/Products/Debug-iphonesimulator/PryxMobile.app"
...
mkdir -p "$APPSUP"
printf '%s\n' \
  '[{"host":"127.0.0.1","port":7643,"token":"mocktoken0123456789abcdef","serverName":"mock-pryx","serverVersion":"mock-0.32.0","pairedAt":770000000}]' \
  > "$APPSUP/pryx-servers.json"
