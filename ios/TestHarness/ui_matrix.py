#!/usr/bin/env python3
"""Layout efficiency matrix for the pryx iOS app.
...
IOS = HERE.parent
BUNDLE = "com.pryx.mobile"
APP = IOS / ".build-ios/Build/Products/Debug-iphonesimulator/PryxMobile.app"
PORT = 7643
TOKEN = "mocktoken0123456789abcdef"
CRED = ('[{"host":"127.0.0.1","port":7643,"token":"%s","serverName":'
        '"mock-pryx","serverVersion":"mock-0.32.0","pairedAt":770000000}]' % TOKEN)
...
def build_app(device):
    ...
    r = sh(
        "xcodebuild build -project PryxMobile.xcodeproj -scheme PryxMobile "
...
def seed_and_launch(device, collect_perf=True):
    ...
    (appsup / "pryx-servers.json").write_text(CRED + "\n")
