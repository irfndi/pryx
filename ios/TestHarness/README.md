...
A deterministic, no-LLM harness for developing and validating the pryx iOS
client (`PryxMobile`) end-to-end. It replaces the role of the old Rust
...
- **`mock_gateway.py`** - a self-contained (stdlib-only) mock of the pryx
  server gateway. Speaks the exact wire protocol from
  `crates/pryx-base/src/gateway.rs` on one TCP port:
...
```bash
# Full pipeline, screenshot lands in $TMPDIR/pryx-ios-e2e/chat.png
...
python3 TestHarness/mock_gateway.py &        # or run a real `pryx` gateway
...
`Library/Application Support/pryx-servers.json` when the Keychain is
...
`PryxKit` (the platform-free client core) is fully unit-tested with `swift
