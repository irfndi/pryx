...
pryx collects **anonymous, minimal usage statistics** to help understand how many people use pryx, what providers/models are popular, whether onboarding works, which feature families are used, how often sessions succeed, and whether performance/regressions are improving. This data helps prioritize development. **We do not collect your prompts, your code, or your conversation transcripts.**
...
| `version` | `"0.9.1"` | Current pryx version |
| `from_version` | `"0.8.1"` | Previously recorded pryx version |
...
is also sent to the discovery API as the `x-pryx-discovery-request-id` header,
...
The benchmark runner sets `PRYX_DISCOVERY_BENCHMARK=1`. Discovery requests then
carry `x-pryx-discovery-benchmark: 1`, and the corresponding telemetry event has
`benchmark_run: true`.

When telemetry is enabled, discovery API requests also carry
`x-pryx-session-correlation-id`. It is a fresh random UUID for the current
...
in the `pryx-subscriptions` D1 while this event lands in `pryx-telemetry`, and
nothing on the receiving side reads
`x-pryx-session-correlation-id` yet, so the header is currently sent and
...
Cloudflare at the edge from the connection (`request.cf.country`). pryx itself
...
The UUID is randomly generated on first run and stored at `~/.pryx/telemetry_id`. It is not derived from your machine, username, email, or any identifiable information.

## How We Use and Share Data

Telemetry is used to operate, debug, secure, and improve pryx, and for product and
...
1. On first launch, pryx generates a random UUID and sends an `install` event
2. When a session begins, pryx sends a `session_start` event
3. When a session ends normally, pryx sends a `session_end` event with coarse session metrics
4. When auth succeeds, pryx sends a coarse `auth_success` event for activation-funnel analysis
5. When pryx detects a version change, it sends an `upgrade` event
6. On best-effort crash/signal handling, pryx sends a `session_crash` event
7. pryx may also send one-off onboarding milestone events and explicit feedback events when triggered
...
9. If a request fails (offline, firewall, etc.), pryx silently continues - no retries, no queuing
...
This is open source. The telemetry implementation is in [`crates/pryx-telemetry-core/src/`](./crates/pryx-telemetry-core/src/) - you can read exactly what gets sent. There are no other network calls related to telemetry anywhere in the codebase.
