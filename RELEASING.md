...
pryx has two release paths: a fast local path for hotfixes, and CI for full releases.
...
Tag push (v*)
    ...
    └─► release (after platform jobs finish)
          ...
          ├─► Update Homebrew formula (1jehuang/homebrew-pryx)
          └─► Update AUR package (pryx-bin)
```
...
- **Homebrew**: Updates `Formula/pryx.rb` in `1jehuang/homebrew-pryx` with new SHA256 hashes
- **AUR**: Updates `PKGBUILD` and `.SRCINFO` in the `pryx-bin` AUR repo
...
`cargo-zigbuild` can cross-compile pure Rust code to macOS, but pryx depends on crates that link against macOS system frameworks:
...
The bottleneck is compiling pryx itself (120k lines of Rust). Dependencies are cached and don't need recompilation. The `build.rs` timestamp causes a full recompile of the main crate on every build.
