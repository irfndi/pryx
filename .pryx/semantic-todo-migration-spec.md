...
## Enums (all: snake_case string serde, Ord by declaration order, in pryx-task-types)
...
## Gate semantics (pryx-base todo.rs, app-core, tui)
...
predicates exported from pryx-base::todo, e.g.:
...
- Telemetry: TelemetryScoreSummary stays u8-based; map each enum to a
  ...
  This keeps pryx-telemetry-core, usage-types, and the worker schema untouched.
...
- Commands: `cargo test -p pryx-task-types -p pryx-base`,
  `cargo test -p pryx-app-core todo`, `cargo test -p pryx-tui todo`,
  `cargo check -p pryx-tui -p pryx-telemetry-core`.
