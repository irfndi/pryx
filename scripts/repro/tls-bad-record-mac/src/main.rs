...
//
// We then wrap that error exactly like pryx's anthropic provider does
...
fn classify(error_str: &str) -> bool {
    // VERBATIM copy of pryx's shared is_transient_transport_error
    // (crates/pryx-base/src/provider/routing.rs @ v0.24.0).
...
async fn main() -> anyhow::Result<()> {
    ...
    // ---- 4. Client (mirrors pryx's reqwest+rustls config) ----
