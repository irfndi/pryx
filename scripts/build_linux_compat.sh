...
artifact="${PRYX_COMPAT_ARTIFACT:-pryx-linux-x86_64}"
profile="${PRYX_COMPAT_PROFILE:-release}"
image="${PRYX_COMPAT_IMAGE:-quay.io/pypa/manylinux2014_x86_64}"
cache_root="${PRYX_COMPAT_CACHE_DIR:-$HOME/.cache/pryx-linux-compat}"
...
# metadata file (read by pryx-build-meta/build.rs through
# PRYX_BUILD_METADATA_FILE). The repo is bind-mounted into the container and
...
docker run --rm \
  ...
  -e PRYX_RELEASE_BUILD="${PRYX_RELEASE_BUILD:-1}" \
  -e PRYX_BUILD_SEMVER="${PRYX_BUILD_SEMVER:-}" \
  -e PRYX_BUILD_METADATA_FILE=/pryx-build-meta \
  -e PRYX_BUILD_GIT_HASH="$git_hash" \
  -e PRYX_BUILD_GIT_DATE="$git_date" \
  -e PRYX_BUILD_GIT_TAG="$git_tag" \
  -e PRYX_BUILD_GIT_DIRTY="$git_dirty" \
  -e PRYX_COMPAT_PROFILE="$profile" \
  -e PRYX_COMPAT_TARGET="$target" \
  -e HOST_UID="$host_uid" \
  -e HOST_GID="$host_gid" \
  -v "$repo_root:/work" \
  -v "$metadata_file:/pryx-build-meta:ro" \
  -v "$out_dir:/out" \
  ...
  bash -lc '
    ...
    if [[ ! -x /root/.cargo/bin/cargo ]]; then
      curl https://sh.rustup.rs -sSf | sh -s -- -y --profile minimal --default-toolchain stable
	    ...
	    # (PRYX_BUILD_METADATA_FILE=/pryx-build-meta) is the primary source of
	    ...
	    cargo build --profile "$PRYX_COMPAT_PROFILE" --target "$PRYX_COMPAT_TARGET" \
	      -p pryx --bin pryx --features linux-compat-vendored-openssl

	    cp "$CARGO_TARGET_DIR/$PRYX_COMPAT_TARGET/$PRYX_COMPAT_PROFILE/pryx" "/out/'"$artifact"'.bin"
...
WRAPPER
	    ...
	    # these sibling libraries and sets LD_LIBRARY_PATH for the pryx process.
...
# `pryx-build-meta`'s build script deliberately does not watch .git/HEAD, so a
...
if [[ -n "$git_hash" ]]; then
  ...
  if [[ -n "$embedded" && "$embedded" != *"$git_hash"* ]]; then
    ...
    echo "       (stale build cache; re-run after 'cargo clean' or bump PRYX_BUILD_GIT_HASH)" >&2
    exit 1
  fi
fi
