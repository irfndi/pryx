...
Counts production Rust occurrences of `pub use pryx_<crate>::*;` (cross-crate
...
`pryx_*` cross-crate wildcards are budgeted.
...
PATTERN = re.compile(r"^\s*pub\s+use\s+(?:::)?pryx_[a-z0-9_]+(?:::[A-Za-z0-9_]+)*::\*\s*;")
