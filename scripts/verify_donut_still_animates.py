...
def main() -> int:
    ...
    ap.add_argument("--binary",
                    default=str(REPO_ROOT / "target" / "selfdev" / "pryx"))
    ap.add_argument("--watch-s", type=float, default=2.0)
    args = ap.parse_args()

    flick.ROWS, flick.COLS = 48, 160
    binary = str(Path(args.binary).resolve())
    scratch = Path(os.environ.get("PRYX_SCRATCH_DIR") or tempfile.gettempdir())
    root = Path(tempfile.mkdtemp(prefix="pryx-donut-verify-", dir=str(scratch)))
    ...
    env = os.environ.copy()
    env.update({
        "PRYX_HOME": str(home), "PRYX_RUNTIME_DIR": str(run),
        "PRYX_SOCKET": str(run / "pryx.sock"), "PRYX_NO_TELEMETRY": "1",
        "PRYX_DEBUG_CONTROL": "1", "PRYX_TEMP_SERVER": "1",
        "PRYX_SERVER_OWNER_PID": str(os.getpid()), "PRYX_PERF_TIER": "full",
        "PRYX_THEME": "dark",
        # The donut is opt-in since the config default flipped to false
        # (17e075fb2), and this verifier's temp PRYX_HOME gets the default
        # config. Without the explicit opt-in the "animation expected" half of
        # this verifier silently verifies nothing.
        "PRYX_IDLE_ANIMATION": "1",
    })
    ...
    failures: list[str] = []
    try:
        flick.wait_for_socket(Path(env["PRYX_SOCKET"]))
