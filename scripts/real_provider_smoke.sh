...
if [[ "${PRYX_REAL_PROVIDER_TEST_API:-1}" == "1" ]]; then
  if [[ "${provider}" == "claude" && "${PRYX_USE_DIRECT_API:-0}" != "1" ]]; then
    ...
    echo ""
    echo "Test 1: Skipping test_api (provider=${provider}, PRYX_USE_DIRECT_API=${PRYX_USE_DIRECT_API:-0})"
  fi
fi
...
if [[ "${PRYX_REMOTE_CARGO:-0}" == "1" ]]; then
  (cd "$repo_root" && "$cargo_exec" build --bin pryx-harness)
  (cd "$repo_root" && ./target/debug/pryx-harness -- --include-network)
else
  (cd "$repo_root" && cargo run --bin pryx-harness -- --include-network)
fi

echo ""
echo "Test 3: End-to-end trace"
if [[ ! -x "$repo_root/target/release/pryx" ]]; then
...
set +e
output=$(PRYX_HOME="$workdir" PATH="$repo_root/target/release:$PATH" \
  pryx run --no-update --trace --provider "$provider" "$prompt" 2>&1)
status=$?
set -e
