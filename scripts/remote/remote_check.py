#!/usr/bin/env python3
"""End-to-end check that a pryx gateway on another host is usable.
...
TOKEN_DIR = pathlib.Path.home() / ".pryx" / "remote-tokens"
...
def load_or_pair(host, port, code, device_id, device_name):
    ...
    if not code:
        sys.exit(f"No cached token for {host}:{port}. Run `pryx pair` on the "
...
def main():
    ...
    p.add_argument("--code", help="pairing code from `pryx pair` on the remote host")
    p.add_argument("--working-dir", required=True)
    p.add_argument("--device-id", default="pryx-remote-check")
    p.add_argument("--device-name", default="pryx remote check")
    ...
    print(f"pryx remote check -> {args.host}:{args.port}\n")
