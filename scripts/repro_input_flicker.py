#!/usr/bin/env python3
"""
Reproduce the "typed character flickers in and out" bug in a live pryx TUI.

The symptom
-----------
Spawn a fresh pryx, press `/`, and the slash appears, vanishes, and reappears.
...
class EmulatedClient:
    """A live pryx under a PTY, with its output rendered into a real screen.
...
def launch_client(binary: str, env: dict, session_id: str,
    ...
    cenv = dict(env)
    cenv["PRYX_DEBUG_CMD_PATH"] = str(cmd_path)
    cenv["PRYX_DEBUG_RESPONSE_PATH"] = str(resp_path)
    ...
    cenv.setdefault("PRYX_THEME", "dark")
    proc = subprocess.Popen(
        [binary, "--no-update", "--no-selfdev",
         "--socket", env["PRYX_SOCKET"], "--resume", session_id],
...
def main() -> int:
    ...
    default_bin = REPO_ROOT / "target" / "selfdev" / "pryx"
    if not default_bin.exists():
        default_bin = Path.home() / ".pryx" / "builds" / "current" / "pryx"
    ...
    root = Path(tempfile.mkdtemp(prefix="pryx-input-flicker-"))
    ...
    env = os.environ.copy()
    if args.live:
        real_runtime = Path(env.get("PRYX_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
        env["PRYX_SOCKET"] = env.get("PRYX_SOCKET") or str(real_runtime / "pryx.sock")
        env["PRYX_DEBUG_CONTROL"] = "1"
        debug_sock = real_runtime / "pryx-debug.sock"
    else:
        env["PRYX_HOME"] = str(home)
        env["PRYX_RUNTIME_DIR"] = str(run)
        env["PRYX_SOCKET"] = str(run / "pryx.sock")
        env["PRYX_NO_TELEMETRY"] = "1"
        env["PRYX_DEBUG_CONTROL"] = "1"
        env["PRYX_TEMP_SERVER"] = "1"
        env["PRYX_SERVER_OWNER_PID"] = str(os.getpid())
        if not env.get("ANTHROPIC_API_KEY"):
            env["ANTHROPIC_API_KEY"] = "sk-ant-repro-input-flicker"
        debug_sock = run / "pryx-debug.sock"
    cmd_path, resp_path = run / "client_cmd", run / "client_resp"

    if not args.json:
        print("== pryx input-flicker repro ==")
    ...
    if not args.live:
        server = subprocess.Popen(
            [binary, "serve", "--socket", env["PRYX_SOCKET"], "--debug-socket",
    ...
    try:
        wait_for_socket(Path(env["PRYX_SOCKET"]))
