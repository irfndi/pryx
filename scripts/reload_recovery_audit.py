...
server but not every interrupted session continues. It correlates three sources:
  ...
  1. ~/.pryx/reload-recovery/*.json durable recovery intent records
  2. ~/.pryx/logs/pryx-*.log recovery lifecycle messages
  3. ~/.pryx/sessions/<session>.json coarse transcript markers
...
def pryx_home() -> pathlib.Path:
    return pathlib.Path(os.environ.get("PRYX_HOME", pathlib.Path.home() / ".pryx"))


def log_files(log_dir: pathlib.Path, max_log_files: int) -> list[pathlib.Path]:
    files = sorted(log_dir.glob("pryx-*.log"), key=lambda p: p.stat().st_mtime, reverse=True)
...
def parse_trace_files(records: dict[tuple[str, str], IntentRecord], trace_dir: pathlib.Path) -> None:
    """Merge structured ~/.pryx/reload-traces/*.jsonl lifecycle events."""
    phase_to_kind = {
...
def main() -> int:
    ...
    parser.add_argument("--home", type=pathlib.Path, default=pryx_home(), help="PRYX_HOME path")
    ...
    parser.add_argument("--max-log-files", type=int, default=3, help="Newest pryx logs to scan")
