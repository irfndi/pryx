...
Runs a full-feature selfdev pryx build with Cargo timings enabled and summarizes
...
Options:
  ...
  --package <name>          Cargo package to build (default: pryx)
  --bin <name>              Cargo binary to build (default: pryx)
  --feature-profile <name>  PRYX_DEV_FEATURE_PROFILE for dev_cargo.sh (default: default)
...
Notes:
  ...
  - The "pryx serial stack" summary is not a formal Cargo critical path. It is a
    focused view of the known long-pole crates: pryx-base, pryx-app-core,
    pryx-tui, root pryx lib, and pryx bin.
USAGE
}
...
PY
  ...
  PRYX_DEV_FEATURE_PROFILE="$feature_profile" \
...
def is_pryx_stack_unit(unit: dict[str, Any]) -> bool:
    name = unit.get("name")
    target = unit.get("target") or ""
    if name in {"pryx-base", "pryx-app-core", "pryx-tui"} and "build script" not in target:
        return True
    if name == "pryx" and (target == "" or f'bin "{bin_name}"' in target):
        return True
    return False

pryx_stack = sorted([unit for unit in units if is_pryx_stack_unit(unit)], key=lambda unit: float(unit.get("start", 0.0)))
stack_span = None
if pryx_stack:
    stack_span = max(float(unit["end"]) for unit in pryx_stack) - min(float(unit.get("start", 0.0)) for unit in pryx_stack)
stack_sum = sum(float(unit.get("duration", 0.0)) for unit in pryx_stack)
stack_frontend_sum = sum(float(unit.get("frontend_duration") or 0.0) for unit in pryx_stack)
stack_codegen_sum = sum(float(unit.get("codegen_duration") or 0.0) for unit in pryx_stack)
