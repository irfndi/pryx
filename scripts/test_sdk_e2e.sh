...
socket="${TMPDIR:-/tmp}/pryx-sdk-e2e-$$.sock"
log="${TMPDIR:-/tmp}/pryx-sdk-e2e-$$.log"
...
# `pryx api-bridge`, which only the shipped binary provides.
echo "== building pryx =="
cargo build --profile selfdev -p pryx --bin pryx
pryx_bin="$repo_root/target/selfdev/pryx"

echo "== building bridge =="
# The standalone bridge binary, not `pryx api-bridge`, because the full pryx
...
cargo build --profile selfdev -p pryx-harness-api-server --bin pryx-harness-api-bridge
...
# PRYX_ACTIVE_PROVIDER therefore makes this suite pass or fail based on the
...
PRYX_API_SOCKET="$socket" PRYX_ACTIVE_PROVIDER= ./target/selfdev/pryx-harness-api-bridge >"$log" 2>&1 &
bridge_pid=$!
cleanup() {
...
PRYX_API_SOCKET="$socket" node "$sdk_dir/test/live-turn.mjs"

echo "== exercising the control surface =="
PRYX_API_SOCKET="$socket" node "$sdk_dir/test/live-control.mjs"

echo "== exercising models, effort, compaction, rename, and undo =="
PRYX_API_SOCKET="$socket" node "$sdk_dir/test/live-capabilities.mjs"
...
node "$sdk_dir/test/live-launch.mjs" "$pryx_bin"

echo "== checking instance isolation and path safety =="
node "$sdk_dir/test/live-isolation.mjs" "$pryx_bin"

echo "== checking every documented launch option =="
node "$sdk_dir/test/live-options.mjs" "$pryx_bin"
