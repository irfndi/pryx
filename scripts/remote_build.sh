...
#
# Defaults:
# - Config file: ~/.config/pryx/remote-build.env (override with PRYX_REMOTE_CONFIG)
# - Host: PRYX_REMOTE_HOST from env/config, or --host
# - Remote dir: .cache/remote-builds/pryx/<repo-name> (override with PRYX_REMOTE_DIR or --remote-dir)
#
# Examples:
#   scripts/remote_build.sh --release
#   scripts/remote_build.sh test
#   scripts/remote_build.sh check --all-targets
#   scripts/remote_build.sh --host mybox --remote-dir ~/src/pryx test -- --nocapture
...
Options:
  ...
  --host HOST          Remote SSH host (default: $PRYX_REMOTE_HOST from env/config; required if unset)
  --remote-dir DIR     Remote project directory (default: $PRYX_REMOTE_DIR or .cache/remote-builds/pryx/<repo-name>)
...
Behavior:
  ...
  - For build sync-back, copies target/{debug|release}/<artifact> from remote to local
    (artifact defaults to 'pryx', or '--bin <name>' when provided)
  - Default config file is ~/.config/pryx/remote-build.env
EOF
}
...
REMOTE="${PRYX_REMOTE_HOST:-}"
REMOTE_DIR="${PRYX_REMOTE_DIR:-.cache/remote-builds/pryx/${REPO_NAME}}"
SSH_BIN="${PRYX_REMOTE_SSH_BIN:-ssh}"
RSYNC_BIN="${PRYX_REMOTE_RSYNC_BIN:-rsync}"
...
if [[ -z "$REMOTE" ]]; then
    echo "error: remote host not configured; set PRYX_REMOTE_HOST or pass --host HOST" >&2
    exit 2
fi
...
SSH_SERVER_ALIVE_INTERVAL="${PRYX_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
SSH_SERVER_ALIVE_COUNT_MAX="${PRYX_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
SSH_OPTS=(
    -o BatchMode=yes
...
RSYNC_SSH_COMMAND="${PRYX_REMOTE_RSYNC_SSH:-$SSH_BIN -o BatchMode=yes -o ConnectTimeout=$SSH_CONNECT_TIMEOUT -o ServerAliveInterval=$SSH_SERVER_ALIVE_INTERVAL -o ServerAliveCountMax=$SSH_SERVER_ALIVE_COUNT_MAX}"
...
if ! preflight_output="$(remote_ssh "printf 'pryx-remote-ok\\n'" 2>&1)"; then
    echo "error: remote host '$REMOTE' is not reachable within ${SSH_CONNECT_TIMEOUT}s" >&2
    echo "$preflight_output" >&2
    echo "hint: set PRYX_REMOTE_CARGO=0 to force local cargo, or fix PRYX_REMOTE_HOST/PRYX_REMOTE_CONNECT_TIMEOUT." >&2
    exit 75
fi
...
if [[ "$SYNC_SOURCE" -eq 1 ]]; then
    ...
    trap 'rm -f "$metadata_file"' EXIT
    {
        ...
        printf 'git_tag=%s\n' "$local_git_tag"
        printf 'git_dirty=%s\n' "$local_git_dirty"
        printf 'changelog_raw<<PRYX_CHANGELOG_EOF\n%s\nPRYX_CHANGELOG_EOF\n' "$local_changelog_raw"
    } > "$metadata_file"
    "$RSYNC_BIN" -avz -e "$RSYNC_SSH_COMMAND" "$metadata_file" "$REMOTE:$REMOTE_DIR/.pryx-build-meta"
else
    echo ""
...
printf -v REMOTE_INNER_CMD 'cd %q && env PRYX_BUILD_METADATA_FILE=.pryx-build-meta %s' "$REMOTE_DIR" "$REMOTE_CARGO_CMD"
