...
Why this exists
---------------
`repro_input_lag.py` (isolated mode) creates an empty `PRYX_HOME`, so a fresh
...
def main() -> int:
    ...
    runtime = Path(os.environ.get("PRYX_RUNTIME_DIR")
                   or f"/run/user/{os.getuid()}")
    env = os.environ.copy()
    env["PRYX_SOCKET"] = env.get("PRYX_SOCKET") or str(runtime / "pryx.sock")
    env["PRYX_DEBUG_CONTROL"] = "1"
    env["PRYX_THEME"] = "dark"  # never race the client for the OSC 11 reply
    debug_sock = runtime / "pryx-debug.sock"

    scratch = Path(os.environ.get("PRYX_SCRATCH_DIR") or tempfile.gettempdir())
    run = Path(tempfile.mkdtemp(prefix="pryx-realspawn-", dir=str(scratch)))
