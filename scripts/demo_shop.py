...
def state_path() -> Path:
    configured = os.environ.get("PRYX_DEMO_SHOP_STATE")
    if configured:
        return Path(configured).expanduser()
    return Path(f"/tmp/pryx-demo-shop-{os.getuid()}.json")
...
def command_search(args: argparse.Namespace) -> int:
    ...
    print("Add one with: pryx-demo-shop add <sku>")
    return 0
...
def command_add(args: argparse.Namespace) -> int:
    ...
    print("Review it with: pryx-demo-shop cart")
    print("Continue with: pryx-demo-shop checkout")
    print(
