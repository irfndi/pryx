...
def run(binary: str, session: str, fps: int | None, window_s: float,
    ...
    if fps is None:
        ...
        env["PRYX_ANIMATION_FPS"] = str(fps)

    scratch = Path(os.environ.get("PRYX_SCRATCH_DIR") or tempfile.gettempdir())
    root = Path(tempfile.mkdtemp(prefix="pryx-fps-", dir=str(scratch)))
...
def main() -> int:
    ...
    runtime = Path(os.environ.get("PRYX_RUNTIME_DIR")
                   or f"/run/user/{os.getuid()}")
    session = args.session or recent_session(runtime / "pryx-debug.sock",
                                             str(REPO_ROOT))
    if not session:
