...
Targets:
  ...
  release-pryx    Run scripts/dev_cargo.sh build --release -p pryx --bin pryx --quiet
  selfdev-pryx    Run scripts/dev_cargo.sh build --profile selfdev -p pryx --bin pryx --quiet
...
Examples:
  ...
  scripts/bench_compile.sh release-pryx --json
  scripts/bench_compile.sh selfdev-pryx --json
USAGE
}
...
case "$target" in
  ...
  build)
    ...
    ;;
  release-pryx)
    cmd=(scripts/dev_cargo.sh build --release -p pryx --bin pryx --quiet)
    ;;
  selfdev-pryx)
    cmd=(scripts/dev_cargo.sh build --profile selfdev -p pryx --bin pryx --quiet)
    ;;
  *)
