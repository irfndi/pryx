...
def main() -> int:
    ...
    ap.add_argument("--binary", default=str(REPO_ROOT / "target" / "selfdev" / "pryx"))
    ...
    runtime = Path(os.environ.get("PRYX_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
    env = os.environ.copy()
    ...
    root = Path(tempfile.mkdtemp(prefix="pryx-profile-real-", dir=str(scratch)))
