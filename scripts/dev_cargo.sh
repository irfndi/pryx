...
start_rust_action_log() {
  case "${PRYX_RUST_ACTION_LOG:-1}" in
    0|false|no|off) return ;;
  esac

  local state_root="${PRYX_HOME:-${HOME:+$HOME/.pryx}}"
  [[ -n "$state_root" ]] || state_root="$repo_root/target/pryx-state"
  rust_action_log_path="${PRYX_RUST_ACTION_LOG_PATH:-$state_root/logs/rust-actions.jsonl}"
...
record_rust_action_log() {
  ...
  PRYX_LOG_STARTED_AT="$rust_action_log_started_at" \
  PRYX_LOG_DURATION_MS="$duration_ms" \
  PRYX_LOG_GATE_WAIT_MS="$cargo_gate_wait_ms" \
  PRYX_LOG_EXIT_CODE="$exit_code" \
  PRYX_LOG_ACTION="$action" \
  PRYX_LOG_PROFILE="$profile" \
  PRYX_LOG_REPO="$repo_root" \
  PRYX_LOG_EXECUTION="$rust_action_log_execution" \
...
path = sys.argv[1]
record = {
    "started_at": os.environ["PRYX_LOG_STARTED_AT"],
    "duration_ms": int(os.environ["PRYX_LOG_DURATION_MS"]),
    "gate_wait_ms": int(os.environ["PRYX_LOG_GATE_WAIT_MS"]),
    "execution_duration_ms": max(
        0,
        int(os.environ["PRYX_LOG_DURATION_MS"])
        - int(os.environ["PRYX_LOG_GATE_WAIT_MS"]),
    ),
    "exit_code": int(os.environ["PRYX_LOG_EXIT_CODE"]),
    "success": os.environ["PRYX_LOG_EXIT_CODE"] == "0",
    "action": os.environ["PRYX_LOG_ACTION"],
    "profile": os.environ["PRYX_LOG_PROFILE"],
    "repository": os.environ["PRYX_LOG_REPO"],
    "execution": os.environ["PRYX_LOG_EXECUTION"],
    "argv": sys.argv[2:],
}
...
configure_build_tmpdir() {
  local candidate="${PRYX_BUILD_TMPDIR:-}"
  if [[ -n "$candidate" ]]; then
    ...
    return
  elif [[ -n "${PRYX_SCRATCH_DIR:-}" ]]; then
    candidate="$PRYX_SCRATCH_DIR/cargo-tmp"
    build_tmpdir_status="pryx-scratch"
  elif [[ -n "${PRYX_HOME:-}" ]]; then
    candidate="$PRYX_HOME/scratch/cargo-tmp"
    build_tmpdir_status="pryx-home"
  elif [[ -n "${HOME:-}" ]]; then
    candidate="$HOME/.pryx/scratch/cargo-tmp"
    build_tmpdir_status="home-scratch"
  else
    candidate="$repo_root/target/pryx-scratch/cargo-tmp"
...
maybe_enable_sccache() {
  ...
  # caller explicitly forces it via PRYX_SCCACHE=1/on/force.
  local force_sccache="${PRYX_SCCACHE:-auto}"
  case "$force_sccache" in
    1|true|yes|on|force) force_sccache="1" ;;
    0|false|no|off|never)
      sccache_status="disabled-by-pryx-sccache"
      log "sccache disabled by PRYX_SCCACHE"
      return
      ;;
    *) force_sccache="auto" ;;
  esac
  ...
  if [[ "$force_sccache" != "1" ]] && build_is_incremental "$@"; then
    ...
    log "sccache skipped for incremental build (it cannot cache incremental units; set PRYX_SCCACHE=on to force, or use a non-incremental profile)"
    return
  fi
...
validate_feature_profile() {
  local profile="${PRYX_DEV_FEATURE_PROFILE:-default}"
  case "$profile" in
    ""|default|minimal|none|pdf|embeddings|full)
      ;;
    *)
      printf 'error: unsupported PRYX_DEV_FEATURE_PROFILE=%s (expected default|minimal|pdf|embeddings|full)\n' "$profile" >&2
      exit 1
      ;;
  esac
}
...
selfdev_low_memory_default_needed() {
  ...
  # On small no-swap machines, earlyoom can terminate the root pryx rustc
...
#
# After the monolith was split into the pryx-base/app-core/tui/cli crate DAG,
# the largest single rustc unit is pryx-base, which has grown back to a
...
# coarse sample, and pryx-base keeps growing) so a fresh build under load backs
...
# backs off further. An explicit CARGO_BUILD_JOBS / PRYX_BUILD_JOBS always
...
select_build_jobs() {
  ...
  local override="${PRYX_BUILD_JOBS:-${CARGO_BUILD_JOBS:-}}"
  if [[ -n "$override" ]]; then
  ...
  # rustc unit (pryx-base, ~1.6 GiB RSS sampled) so an idle machine uses nearly
  # every core while a memory-pressured one backs off before earlyoom kills a
  # build. Tunable per host via PRYX_BUILD_MIB_PER_JOB.
  mib_per_job_default=1792
  mib_per_job="${PRYX_BUILD_MIB_PER_JOB:-$mib_per_job_default}"
  ...
  if (( jobs < cpus )); then
    log "limiting cargo to ${jobs} job(s) under memory pressure (${mem_available_mib}MiB available, ~${mib_per_job}MiB/job); override with PRYX_BUILD_JOBS"
  fi
}

# Keep `pryx-build-meta`'s embedded git metadata in lockstep with HEAD without
# making it watch `.git` mtimes.
#
# `pryx-build-meta/build.rs` deliberately does NOT declare `.git/HEAD`/`.git/index`
...
# `cargo:rerun-if-env-changed=PRYX_BUILD_GIT_HASH` (and _DATE), so cargo reruns
...
# export PRYX_BUILD_GIT_DIRTY here: the dirty flag flips on every edit and would
...
export_git_build_metadata() {
  ...
  if [[ -n "${PRYX_BUILD_GIT_HASH:-}" ]]; then
    git_meta_status="external:${PRYX_BUILD_GIT_HASH}"
    return
  fi
  ...
  export PRYX_BUILD_GIT_HASH="$hash"
  date="$(git -C "$repo_root" log -1 --format=%ci 2>/dev/null || true)"
  if [[ -n "$date" ]]; then
    export PRYX_BUILD_GIT_DATE="$date"
  fi
...
maybe_configure_low_memory_selfdev() {
  ...
  local mode="${PRYX_SELFDEV_LOW_MEMORY:-auto}"
  case "$mode" in
    1|true|yes|on|force)
      ;;
    0|false|no|off|never)
    ...
    auto|"")
      ...
      ;;
    *)
      printf 'error: unsupported PRYX_SELFDEV_LOW_MEMORY=%s (expected auto|on|off)\n' "$mode" >&2
      exit 1
      ;;
  esac
...
# builds. The pryx monoliths (pryx-base/app-core/tui) are ~80% single-threaded
...
#
# Controls:
#   PRYX_PARALLEL_FRONTEND=auto|0|1   (default auto)
#   PRYX_FRONTEND_THREADS=<n>         (default 4; diminishing returns past 4)
#   PRYX_DEV_TOOLCHAIN=<name>         (default: nightly when present)
...
configure_parallel_frontend() {
  local requested="${PRYX_PARALLEL_FRONTEND:-auto}"
  local forced="false"
  case "$requested" in
    0|false|no|off)
  ...
  # (`PRYX_PARALLEL_FRONTEND=1`) opts dev/test in too, accepting that potential
  # cache contention.
  local profile
  ...
  local threads="${PRYX_FRONTEND_THREADS:-4}"
...
configure_linux_linker() {
  local requested_mode="${PRYX_FAST_LINKER:-auto}"
  local mode="$requested_mode"

  case "$mode" in
    auto)
      # Prefer mold over lld: on this repo's large statically-linked binary
      # (~300 MB .text), mold links the pryx bin in ~2.0s vs lld's ~2.9s
    ...
    lld|mold|system)
      ;;
    *)
      printf 'error: unsupported PRYX_FAST_LINKER=%s (expected auto|lld|mold|system)\n' "$mode" >&2
      exit 1
      ;;
  esac
...
print_setup() {
  if [[ -n "${PRYX_DEV_FEATURE_PROFILE:-}" && "${PRYX_DEV_FEATURE_PROFILE}" != "default" ]]; then
    feature_profile_status="${PRYX_DEV_FEATURE_PROFILE}"
  fi
  local cargo_gate_mode="${PRYX_CARGO_GATE:-on}"
  local cargo_gate_dir="${PRYX_CARGO_GATE_DIR:-${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}}"
  cat <<EOF
repo_root=$repo_root
os=$(uname -s)
arch=$(uname -m)
...
remote_connect_timeout() {
  local value="${PRYX_REMOTE_CONNECT_TIMEOUT:-5}"
...
remote_tcp_timeout() {
  ...
  local value="${PRYX_REMOTE_TCP_TIMEOUT:-1}"
...
remote_recovery_tcp_timeout() {
  ...
  local value="${PRYX_REMOTE_RECOVERY_TCP_TIMEOUT:-0.3}"
  if [[ ! "$value" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
    value=0.3
  fi
  printf '%s\n' "$value"
...
remote_down_cache_ttl() {
  ...
  local value="${PRYX_REMOTE_DOWN_TTL:-300}"
  if [[ ! "$value" =~ ^[0-9]+$ ]]; then
    value=300
  fi
  printf '%s\n' "$value"
...
remote_cargo_preflight() {
  local remote="${PRYX_REMOTE_HOST:-}"
  if [[ -z "$remote" ]]; then
    log "remote cargo requested but PRYX_REMOTE_HOST is not configured"
    return 1
  fi

  local ssh_bin="${PRYX_REMOTE_SSH_BIN:-ssh}"
  ...
  # disabled via PRYX_REMOTE_TCP_PROBE=0.
  local tcp_probe="${PRYX_REMOTE_TCP_PROBE:-1}"
  case "$tcp_probe" in
  ...
  local server_alive_interval="${PRYX_REMOTE_SERVER_ALIVE_INTERVAL:-10}"
  local server_alive_count="${PRYX_REMOTE_SERVER_ALIVE_COUNT_MAX:-1}"
  local output
  if ! output=$("$ssh_bin" \
    -o BatchMode=yes \
...
remote_cargo_fallback_mode() {
  local mode="${PRYX_REMOTE_CARGO_FALLBACK:-local}"
  case "$mode" in
    local|1|true|yes|on)
      printf 'local\n'
      ;;
    error|fail|0|false|no|off|never)
      ...
      ;;
    *)
      printf 'error: unsupported PRYX_REMOTE_CARGO_FALLBACK=%s (expected local|error)\n' "$mode" >&2
      exit 2
      ;;
  esac
}
...
run_local_cargo() {
  if cargo_test_has_explicit_filter "${cargo_argv[@]}" && [[ "${PRYX_DEV_CARGO_ALLOW_ZERO_TESTS:-0}" != "1" ]]; then
    local output_file
    output_file=$(mktemp "${TMPDIR:-/tmp}/pryx-dev-cargo.XXXXXX")
    local status=0
    ...
    if [[ "$status" -eq 0 ]] \
      ...
      printf 'dev_cargo: explicit cargo test filter matched zero tests; check the test path/name or set PRYX_DEV_CARGO_ALLOW_ZERO_TESTS=1 to allow this intentionally\n' >&2
      rm -f "$output_file"
      return 97
    fi
    rm -f "$output_file"
...
# happen to share those exact directories. Pryx agents also build from scratch
...
#
# Serialize compile-capable local Cargo actions across all pryx worktrees. A
...
# parallelism. Nested wrapper calls inherit PRYX_CARGO_GATE_HELD and cannot
# deadlock. Set PRYX_CARGO_GATE=off for an intentional concurrency experiment.
acquire_cargo_gate() {
  cargo_gate_status="not-needed"
  cargo_gate_wait_ms=0
  cargo_action_needs_gate || return 0

  case "${PRYX_CARGO_GATE:-on}" in
    0|false|no|off|disabled)
      cargo_gate_status="disabled"
      return 0
      ;;
  esac
  if [[ "${PRYX_CARGO_GATE_HELD:-0}" == "1" ]]; then
  ...
  gate_dir="${PRYX_CARGO_GATE_DIR:-${XDG_RUNTIME_DIR:-${TMPDIR:-/tmp}}}"
  mkdir -p "$gate_dir"
  gate_path="${PRYX_CARGO_GATE_PATH:-$gate_dir/pryx-cargo-build.lock}"
...
if [[ "${PRYX_REMOTE_CARGO:-0}" == "1" ]]; then
  ...
  if [[ "$(remote_cargo_fallback_mode)" == "local" ]]; then
    log "remote cargo unavailable; falling back to local cargo (set PRYX_REMOTE_CARGO_FALLBACK=error to fail instead)"
  else
...
acquire_cargo_gate
# Size the in-process parallelism only after competing pryx Cargo processes
