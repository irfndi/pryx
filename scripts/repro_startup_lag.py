...
The hypothesis
--------------
A freshly spawned pryx feels laggy for the first few seconds and then becomes
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="pryx-startup-lag-"))
    run = root / "run"
    ...
    runtime = Path(env.get("PRYX_RUNTIME_DIR") or f"/run/user/{os.getuid()}")
