...
echo "=== Fast test loop (library + primary pryx binary) ==="
...
# overridden with PRYX_DEV_FEATURE_PROFILE=default/full.
export PRYX_DEV_FEATURE_PROFILE="${PRYX_DEV_FEATURE_PROFILE:-minimal}"
echo "Feature profile: $PRYX_DEV_FEATURE_PROFILE"

# Only the primary `pryx` binary contains unit tests. `test_api` and
# `pryx-harness` are executable smoke tools with no #[test] functions, so
# `--bins` needlessly builds and links two additional copies of the full graph.
run_cargo test --lib --bin pryx "$@"

echo ""
if [[ -x "$repo_root/target/release/pryx" ]]; then
  echo "=== Startup regression check (release binary) ==="
  "$repo_root/scripts/check_startup_budget.sh" "$repo_root/target/release/pryx"
  echo ""
else
