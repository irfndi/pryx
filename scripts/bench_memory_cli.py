...
def build_specs() -> dict[str, ToolSpec]:
    pryx = shutil.which("pryx") or str(Path.home() / ".local/bin/pryx")
    ...
    specs = {
        "pryx_memory_off": ToolSpec(
            name="pryx_memory_off",
            argv=[pryx, "--no-update", "--no-selfdev"],
            version_argv=[pryx, "version"],
            env={"PRYX_NO_TELEMETRY": "1", "PRYX_MEMORY_ENABLED": "0"},
            pryx=True,
        ),
        "pryx_memory_on": ToolSpec(
            name="pryx_memory_on",
            argv=[pryx, "--no-update", "--no-selfdev"],
            version_argv=[pryx, "version"],
            env={"PRYX_NO_TELEMETRY": "1", "PRYX_MEMORY_ENABLED": "1"},
            pryx=True,
        ),
        "pi": ToolSpec(
            name="pi",
...
def run_tool(spec: ToolSpec, sessions: int, cwd: Path, timeout_s: float, settle_s: float) -> ToolRunResult:
    ...
    try:
        if spec.pryx:
            temp_root = tempfile.mkdtemp(prefix="pryx-memory-bench-")
            env = os.environ.copy()
            if spec.env:
                env.update(spec.env)
            env["PRYX_HOME"] = os.path.join(temp_root, "home")
            env["PRYX_RUNTIME_DIR"] = os.path.join(temp_root, "run")
            ...
            for auth_name in (
                ...
                real_auth = Path.home() / ".pryx" / auth_name
                bench_auth = Path(env["PRYX_HOME"]) / auth_name
                if real_auth.exists() and not bench_auth.exists():
                    bench_auth.symlink_to(real_auth)
            if spec.name == "pryx_memory_on":
                real_models = Path.home() / ".pryx" / "models"
                bench_models = Path(env["PRYX_HOME"]) / "models"
                if real_models.exists() and not bench_models.exists():
                    bench_models.symlink_to(real_models)
            socket_path = os.path.join(env["PRYX_RUNTIME_DIR"], "bench.sock")
            ...
            if not wait_for_socket(socket_path, timeout_s):
                raise RuntimeError("pryx server did not become ready")
            if spec.name == "pryx_memory_on":
                time.sleep(max(settle_s, 5.0))
            per_session_settle = max(settle_s, 2.0) if spec.name == "pryx_memory_on" else settle_s
