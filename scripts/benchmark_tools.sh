...
# Run from the pryx repo root

set -euo pipefail

ITERATIONS=${1:-5}
RESULTS_FILE="/tmp/pryx_tool_benchmark_$(date +%Y%m%d_%H%M%S).csv"

echo "=== pryx Tool Call Benchmark ==="
...
# Find debug socket
DEBUG_SOCK="${PRYX_DEBUG_SOCK:-/run/user/$(id -u)/pryx-debug.sock}"

if [ ! -S "$DEBUG_SOCK" ]; then
    echo "ERROR: Debug socket not found at $DEBUG_SOCK"
    echo "Make sure pryx is running with debug control enabled."
    exit 1
fi
