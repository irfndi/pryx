...
fixture_root=${PRYX_AUTH_FIXTURE_DIR:-$fixture_root_default}
...
Commands:
  ...
  save <name>                  Save the current sandbox PRYX_HOME as a fixture
  load <name>                  Replace the sandbox PRYX_HOME with a saved fixture
  reset-sandbox                Remove only the current sandbox PRYX_HOME
  delete <name>                Delete a saved fixture
  env <name>                   Print exports for running against a loaded fixture
  run <name> -- <args...>      Load fixture, then run pryx with args in sandbox
  help                         Show this help

Environment overrides:
  PRYX_ONBOARDING_SANDBOX     Sandbox name to load into/from (default: default)
  PRYX_ONBOARDING_DIR         Explicit onboarding sandbox directory
...
run_pryx() {
  ...
  (
    ...
    if [[ -x "$binary_path" ]]; then
      env PRYX_HOME="$pryx_home" PRYX_RUNTIME_DIR="$runtime_dir" "$binary_path" "$@"
    else
      env PRYX_HOME="$pryx_home" PRYX_RUNTIME_DIR="$runtime_dir" cargo run --bin pryx -- "$@"
    fi
  )
}
...
save_fixture() {
  ...
  ensure_parent_dirs
  if [[ ! -d "$pryx_home" ]]; then
    echo "sandbox PRYX_HOME does not exist: $pryx_home" >&2
    exit 1
  fi
  mkdir -p "$(dirname "$dst")"
  copy_dir_contents "$pryx_home" "$dst"
  chmod -R go-rwx "$(dirname "$dst")" 2>/dev/null || true
  cat > "$meta" <<EOF
name=$name
saved_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)
sandbox_name=$sandbox_name
source_pryx_home=$pryx_home
warning=May contain real local auth tokens. Do not commit or share.
EOF
  echo "Saved auth fixture '$name' from $pryx_home"
...
load_fixture() {
  ...
  copy_dir_contents "$src" "$pryx_home"
  chmod -R go-rwx "$pryx_home" 2>/dev/null || true
...
case "$command" in
  ...
  reset-sandbox)
    ...
    echo "Reset sandbox PRYX_HOME: $pryx_home"
    ;;
  delete)
    name=$(require_name "${1:-}")
    delete_fixture "$name"
