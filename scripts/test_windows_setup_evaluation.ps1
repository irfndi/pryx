...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("pryx-windows-setup-eval-{0}" -f ([guid]::NewGuid().ToString('N')))
...
$envNames = @(
    ...
    'TMP',
    'PATH',
    'PRYX_INSTALL_PS1_IMPORT_ONLY',
    'PRYX_UNINSTALL_PS1_IMPORT_ONLY',
    'PRYX_SKIP_SERVER_RELOAD',
    'PRYX_INSTALL_SKIP_BINARY_VALIDATION',
    'PRYX_DISABLE_ENV_BROADCAST',
    'PRYX_WINDOWS_SETUP_SKIP_EXTERNALS'
)
$originalEnv = @{}
...
function Assert-PathCount([string]$PathValue, [string]$Entry, [int]$ExpectedCount, [string]$Message) {
    $entryKey = ConvertTo-PryxPathKey $Entry
    $count = 0
    foreach ($candidate in (Split-PryxPathList $PathValue)) {
        if ((ConvertTo-PryxPathKey $candidate) -eq $entryKey) { $count += 1 }
    }
...
function New-IsolatedWindowsProfile([string]$Name) {
    ...
    $pryxHome = Join-Path $root 'Pryx Home'
    $temp = Join-Path $root 'Temp'
    foreach ($dir in @($local, $profile, $appData, $pryxHome, $temp)) {
    ...
    return [pscustomobject]@{
        ...
        PryxHome = $pryxHome
        Temp = $temp
        InstallDir = Join-Path $local 'pryx\bin'
        LauncherPath = Join-Path $local 'pryx\bin\pryx.exe'
        BuildsDir = Join-Path $local 'pryx\builds'
        SetupHintsPath = Join-Path $pryxHome 'setup_hints.json'
        HotkeyDir = Join-Path $pryxHome 'hotkey'
        StartupShortcutPath = Join-Path $appData 'Microsoft\Windows\Start Menu\Programs\Startup\pryx-hotkey.lnk'
    }
}

function Set-InstallScriptProfileGlobals($Profile) {
    $script:PryxHome = $Profile.PryxHome
...
try {
    $env:PRYX_INSTALL_PS1_IMPORT_ONLY = '1'
    $env:PRYX_SKIP_SERVER_RELOAD = '1'
    $env:PRYX_INSTALL_SKIP_BINARY_VALIDATION = '1'
    . $installScript -SkipAlacrittySetup -SkipHotkeySetup

    Invoke-Case 'release_lookup_avoids_unauthenticated_github_api' {
        Assert-Equal 'v1.2.3' (Resolve-PryxReleaseTagFromUri 'https://github.com/pryx-co/pryx/releases/tag/v1.2.3') 'release redirect parser should extract the stable tag'
        Assert-Equal 'v1.2.3-rc.1' (Resolve-PryxReleaseTagFromUri 'https://github.com/pryx-co/pryx/releases/tag/v1.2.3-rc.1?source=latest') 'release redirect parser should stop before query parameters'
        Assert-Equal $true (Test-PryxReleaseTag 'v1.2.3') 'stable release tags should validate'
        Assert-Equal $false (Test-PryxReleaseTag 'latest') 'unversioned release labels should not validate'
        ...
        Assert-Contains $scriptText 'pryx.sh/releases' 'installer should include independent static release metadata'
        ...
        function Invoke-WebRequest {
            ...
            if ($Uri -eq 'https://pryx.sh/releases/latest/version') {
                return [pscustomobject]@{ Content = "v1.2.3`n" }
            }
            if ($Uri -eq 'https://pryx.sh/releases/v1.2.3/download-bases') {
                return [pscustomobject]@{ Content = "https://mirror.example/releases/v1.2.3`n" }
            }
            if ($Uri -eq 'https://github.com/pryx-co/pryx/releases/latest') {
                throw 'simulated GitHub block'
            }
            throw "unexpected URI: $Uri"
        }
        try {
            Assert-Equal 'v1.2.3' (Get-LatestPryxReleaseTag) 'static metadata should cover a blocked GitHub release lookup'
            $bases = @(Get-PryxReleaseDownloadBases 'v1.2.3')
            Assert-Equal 'https://mirror.example/releases/v1.2.3' $bases[0] 'configured mirror should be preferred'
            Assert-Equal 'https://github.com/pryx-co/pryx/releases/download/v1.2.3' $bases[1] 'GitHub should remain the final fallback'
        } finally {
    ...
    Invoke-Case 'path_persistence_and_deduplication' {
        ...
        $pathUpdate = Resolve-PryxPathUpdate -InstallDir $profile.InstallDir -CurrentPath $currentPath
        ...
        Assert-PathCount $pathUpdate.Path $profile.InstallDir 1 'updated PATH should contain exactly one pryx launcher dir'
        Assert-Equal 2 $pathUpdate.RemovedManagedEntries 'PATH update should remove both stale pryx launcher entries before re-adding one'
        ...
        $first = Set-PryxUserPath -InstallDir $profile.InstallDir -CurrentPath 'C:\Tools' -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
        ...
        $second = Set-PryxUserPath -InstallDir $profile.InstallDir -CurrentPath $script:appliedPath -SetUserPathAction $setPathAction -BroadcastAction $broadcastAction
    ...
    Invoke-Case 'wm_settingchange_broadcast_contract' {
        ...
        $env:PRYX_DISABLE_ENV_BROADCAST = '1'
        Assert-Equal $false (Send-PryxEnvironmentChangedBroadcast) 'broadcast helper should honor the deterministic no-mutate opt-out'
        $env:PRYX_DISABLE_ENV_BROADCAST = $null
    ...
    function Set-PryxUserPath {
        ...
        $update = Resolve-PryxPathUpdate -InstallDir $InstallDir -CurrentPath $script:mockUserPath
        if ($update.Changed) {
    ...
    Invoke-Case 'clean_install_isolated_profile_and_opt_out' {
        ...
        $source = Join-Path $profile.Root 'pryx-v1.exe'
        ...
        Assert-PathExists (Join-Path $profile.BuildsDir 'stable\pryx.exe') 'clean install should populate the stable build channel'
        Assert-PathExists (Join-Path $profile.BuildsDir 'versions\0.0.1-eval\pryx.exe') 'clean install should populate the immutable versioned build'
        ...
        Assert-PathExists $profile.SetupHintsPath 'clean install should write setup hints into isolated PRYX_HOME'
    ...
    Invoke-Case 'upgrade_and_idempotency_do_not_duplicate_path' {
        ...
        Set-InstallScriptProfileGlobals $profile
        $sourceV1 = Join-Path $profile.Root 'pryx-v1.exe'
        $sourceV2 = Join-Path $profile.Root 'pryx-v2.exe'
    ...
    Invoke-Case 'copilot_key_mapping_and_spaces_non_ascii_paths' {
        ...
        Set-InstallScriptProfileGlobals $profile
        $env:PRYX_WINDOWS_SETUP_SKIP_EXTERNALS = '1'
        $pryxExe = Join-Path $profile.Root '路径 With Spaces\pryx.exe'
        New-Item -ItemType Directory -Path (Split-Path -Parent $pryxExe) -Force | Out-Null
        Set-Content -Path $pryxExe -Value 'fake exe' -NoNewline
        New-Item -ItemType Directory -Path $profile.HotkeyDir -Force | Out-Null
        Set-Content -Path (Join-Path $profile.HotkeyDir 'pryx-hotkey.ps1') -Value 'legacy listener' -Force

        $ok = Install-PryxHotkey -PryxExePath $pryxExe
        Assert-Equal $true $ok 'hotkey install should succeed using the deterministic external-command skip hook'
        $vbsPath = Join-Path $profile.HotkeyDir 'pryx-hotkey-launcher.vbs'
        Assert-PathMissing $vbsPath 'hotkey install should remove the legacy hidden VBScript trampoline'
        $shortcutScriptPath = Join-Path $profile.HotkeyDir 'pryx-hotkey-shortcut.ps1'
        Assert-PathExists $shortcutScriptPath 'hotkey install should render the deterministic Startup shortcut script under the isolated PRYX_HOME'
        ...
        Assert-Contains $shortcutScript $pryxExe 'Startup shortcut should preserve spaces and non-ASCII characters in the pryx path'
        Assert-PathMissing (Join-Path $profile.HotkeyDir 'pryx-hotkey.ps1') 'hotkey upgrade should remove the legacy PowerShell listener'
        $scriptText = Get-Content -LiteralPath $installScript -Raw
    ...
    Invoke-Case 'missing_windows_terminal_is_not_required' {
        ...
        $pryxExe = Join-Path $profile.Root 'No Windows Terminal\pryx.exe'
        New-Item -ItemType Directory -Path (Split-Path -Parent $pryxExe) -Force | Out-Null
        Set-Content -Path $pryxExe -Value 'fake exe' -NoNewline

        Assert-Equal $true (Install-PryxHotkey -PryxExePath $pryxExe) 'hotkey setup should not require Windows Terminal to be installed or active'
        $scriptText = Get-Content -LiteralPath $installScript -Raw
    ...
    Invoke-Case 'launcher_rollback_failure_preserves_existing_install' {
        ...
        $threw = $false
        try {
            Install-PryxLauncher -SourcePath $missingSource -LauncherPath $profile.LauncherPath | Out-Null
        } catch {
            $threw = $true
        }
        ...
        $tempLaunchers = @(Get-ChildItem -LiteralPath $profile.InstallDir -Filter '.pryx-launcher-*.tmp.exe' -Force -ErrorAction SilentlyContinue)
    ...
    $env:PRYX_UNINSTALL_PS1_IMPORT_ONLY = '1'
    . $uninstallScript
    ...
    function Remove-PryxUserPath {
        ...
        $update = Resolve-PryxPathRemoval -InstallDir $InstallDir -CurrentPath $script:uninstallUserPath
        if ($update.Changed) {
    ...
    Invoke-Case 'uninstall_guards_purge_paths_and_process_scope' {
        ...
        Assert-Equal $true (Test-PryxSafePurgePath $profile.PryxHome) 'dedicated pryx data directories should be purgeable'
        Assert-Equal $false (Test-PryxSafePurgePath $profile.UserProfile) 'the user profile must never be accepted as a purge target'
        Assert-Equal $false (Test-PryxSafePurgePath $profile.Root) 'a parent workspace must never be accepted as a purge target'
        Assert-Equal $true (Test-PryxManagedExecutablePath -ExecutablePath $profile.LauncherPath -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'the installed launcher should be recognized as managed'
        Assert-Equal $true (Test-PryxManagedExecutablePath -ExecutablePath (Join-Path $profile.InstallDir '.pryx-launcher-old-a1b2c3.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'a renamed live-upgrade launcher should be recognized as managed'
        Assert-Equal $false (Test-PryxManagedExecutablePath -ExecutablePath (Join-Path $profile.InstallDir 'other-tool.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'unrelated executables beside the launcher must not be terminated'
        Assert-Equal $true (Test-PryxManagedExecutablePath -ExecutablePath (Join-Path $profile.BuildsDir 'stable\pryx.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'installed version binaries should be recognized as managed'
        Assert-Equal $false (Test-PryxManagedExecutablePath -ExecutablePath (Join-Path $profile.Root 'development\pryx.exe') -LauncherPath $profile.LauncherPath -BuildsDir $profile.BuildsDir) 'unrelated development binaries must not be terminated'
    }
    ...
    Invoke-Case 'uninstall_cleanup_removes_binaries_path_and_keeps_user_data' {
        ...
        New-Item -ItemType Directory -Path $profile.PryxHome -Force | Out-Null
        New-Item -ItemType Directory -Path $profile.HotkeyDir -Force | Out-Null
        ...
        $oldLauncherPath = Join-Path $profile.InstallDir '.pryx-launcher-old-a1b2c3.exe'
        Set-Content -Path $oldLauncherPath -Value 'previous running launcher' -NoNewline
        Set-Content -Path (Join-Path $profile.BuildsDir 'stable\pryx.exe') -Value 'stable build' -NoNewline
        Set-Content -Path (Join-Path $profile.PryxHome 'config.toml') -Value 'kept = true' -NoNewline
        Set-Content -Path (Join-Path $profile.HotkeyDir 'pryx-hotkey.ps1') -Value 'legacy listener' -NoNewline
        ...
        $exitCode = Invoke-PryxUninstall -InstallDir $profile.InstallDir -Yes
        ...
        Assert-PathMissing (Join-Path $profile.HotkeyDir 'pryx-hotkey.ps1') 'uninstall should remove legacy launch-hotkey artifacts'
        Assert-PathExists (Join-Path $profile.PryxHome 'config.toml') 'uninstall without -Purge should keep user data'
        ...
        Assert-Equal 'C:\Keep' $script:uninstallUserPath 'uninstall should remove all pryx-managed PATH variants and keep unrelated entries'
