#!/usr/bin/env python3
"""
pryx Stress Test: Spawn 40 sessions via debug socket, measure performance.
...
MAIN_SOCK = f"/run/user/{os.getuid()}/pryx.sock"
DEBUG_SOCK = f"/run/user/{os.getuid()}/pryx-debug.sock"
...
def get_server_pid():
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            if "pryx.sock" in line and "LISTEN" in line:
                parts = line.split()
                return int(parts[1])
    except:
        pass
    
    # Fallback: find the oldest pryx process (likely the server)
    try:
        result = subprocess.run(
            ["pgrep", "-o", "pryx"], capture_output=True, text=True, timeout=5
        )
...
print_header(f"pryx Stress Test: {NUM_INSTANCES} sessions")
...
if not os.path.exists(MAIN_SOCK):
    ...
    print("  Start a server with: pryx serve &")
    sys.exit(1)

if not os.path.exists(DEBUG_SOCK):
    print_err(f"No debug socket at {DEBUG_SOCK}")
    print("  Enable with: touch ~/.pryx/debug_control")
    sys.exit(1)
...
baseline_procs = int(subprocess.run(["pgrep", "-c", "pryx"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total pryx procs", baseline_procs)
...
for i in range(1, NUM_INSTANCES + 1):
    t0 = time.monotonic()
    resp = debug_cmd(f"create_session:/tmp/pryx-stress-{i}", timeout=30)
    t1 = time.monotonic()
...
peak_procs = int(subprocess.run(["pgrep", "-c", "pryx"], capture_output=True, text=True).stdout.strip() or "0")
...
print_stat("Total pryx procs", peak_procs)
...
final_procs = int(subprocess.run(["pgrep", "-c", "pryx"], capture_output=True, text=True).stdout.strip() or "0")
