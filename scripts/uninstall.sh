#!/usr/bin/env bash
# Uninstall pryx binaries and (optionally) all user data.
#
...
#
# Flags:
#   --purge     Also delete ~/.pryx (config, auth, sessions, logs, memory).
#   --dry-run   Print what would be removed without deleting anything.
#   --yes       Skip the confirmation prompt.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/pryx-co/pryx/master/scripts/uninstall.sh | bash
...
case "$OS" in
  MINGW*|MSYS*|CYGWIN*)
    PRYX_HOME="${LOCALAPPDATA:?LOCALAPPDATA not set}/pryx"
    LAUNCHER_DIR="${PRYX_INSTALL_DIR:-$LOCALAPPDATA/pryx/bin}"
    LAUNCHER="$LAUNCHER_DIR/pryx.exe"
    BUILDS_DIR="$PRYX_HOME/builds"
    USER_DATA_DIR="$PRYX_HOME"
    ;;
  *)
    PRYX_HOME="$HOME/.pryx"
    LAUNCHER_DIR="${PRYX_INSTALL_DIR:-$HOME/.local/bin}"
    LAUNCHER="$LAUNCHER_DIR/pryx"
    BUILDS_DIR="$PRYX_HOME/builds"
    USER_DATA_DIR="$PRYX_HOME"
    ;;
esac
...
if [ "$OS" = "Darwin" ]; then
  [ -d "$HOME/Applications/Pryx.app" ] && TARGETS+=("$HOME/Applications/Pryx.app (macOS launcher)")
  [ -d "$HOME/Applications/Pryx Notifications.app" ] && TARGETS+=("$HOME/Applications/Pryx Notifications.app (notification broker)")
fi
...
if [ -f "$SELFDEV_WRAPPER" ] && grep -q "pryx" "$SELFDEV_WRAPPER" 2>/dev/null; then
  TARGETS+=("$SELFDEV_WRAPPER (selfdev wrapper)")
fi

if [ ${#TARGETS[@]} -eq 0 ]; then
  info "Nothing to uninstall: no pryx installation found."
  exit 0
fi
...
# Stop any running pryx server so files are not recreated mid-wipe.
if command -v pkill >/dev/null 2>&1; then
  pkill -f 'pryx( .*)? serve' 2>/dev/null || true
fi
...
remove "$LAUNCHER"
if [ "$OS" = "Darwin" ]; then
  remove "$HOME/Applications/Pryx.app"
  remove "$HOME/Applications/Pryx Notifications.app"
fi
if [ "$PURGE" = true ]; then
  ...
  MINGW*|MSYS*|CYGWIN*)
    if command -v powershell.exe >/dev/null 2>&1; then
      ...
      if [ "$new_user_path" != "$current_user_path" ]; then
        if PRYX_NEW_USER_PATH="$new_user_path" powershell.exe -NoProfile -NonInteractive -Command \
...
info "pryx uninstalled."
if [ "$PURGE" = false ]; then
  info "Reinstall with: curl -fsSL https://pryx.sh/install | bash"
else
  info "All pryx data wiped. Reinstall with: curl -fsSL https://pryx.sh/install | bash"
fi
