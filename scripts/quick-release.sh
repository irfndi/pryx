...
if [[ "$MODE" == "prepare-fast" ]]; then
    ...
    PRYX_REMOTE_CARGO=0 scripts/dev_cargo.sh build --profile selfdev -p pryx --bin pryx
    source_bin="target/selfdev/pryx"
...
if [[ "$MODE" == "prepare-fast-macos" ]]; then
    ...
    PRYX_RELEASE_BUILD=1 PRYX_BUILD_SEMVER="$VERSION_NUM" \
        CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-1}" \
        cargo build --release --target aarch64-apple-darwin --bin pryx
    source_bin="target/aarch64-apple-darwin/release/pryx"
...
if [[ "$MODE" == "fast-local" ]]; then
    ...
    [[ "$prepared_sha256" == "$actual_sha256" ]] || {
        echo "Error: target/selfdev/pryx changed after fast-release preparation." >&2
        exit 1
    }
    ...
    cp "$source_bin" "$DIST/pryx-linux-x86_64.bin"
    strip --strip-unneeded "$DIST/pryx-linux-x86_64.bin"
    chmod +x "$DIST/pryx-linux-x86_64.bin"
    cat > "$DIST/pryx-linux-x86_64" <<WRAPPER
#!/usr/bin/env sh
set -eu
export PRYX_RUNTIME_RELEASE_SEMVER="$VERSION_NUM"
export PRYX_RUNTIME_RELEASE_GIT_HASH="$(git rev-parse --short HEAD)"
export PRYX_RUNTIME_RELEASE_GIT_DATE="$(git log -1 --format=%ci)"
export PRYX_RUNTIME_RELEASE_GIT_TAG="$VERSION"
self_dir=\$(CDPATH= cd -- "\$(dirname -- "\$0")" && pwd)
exec "\$self_dir/pryx-linux-x86_64.bin" "\$@"
WRAPPER
    chmod +x "$DIST/pryx-linux-x86_64"
    file "$DIST/pryx-linux-x86_64.bin" | grep -q 'ELF 64-bit' || { echo "Error: bad Linux binary" >&2; exit 1; }
    version_output="$("$DIST/pryx-linux-x86_64" --version)"
    printf '%s\n' "$version_output" | grep -Fq "v$VERSION_NUM" || {
        echo "Error: fast binary reports the wrong version: $version_output" >&2
        exit 1
    }
    (cd "$DIST" && tar -cf - pryx-linux-x86_64 pryx-linux-x86_64.bin | gzip -1 > pryx-linux-x86_64.tar.gz)
    (cd "$DIST" && sha256sum pryx-linux-x86_64.tar.gz > SHA256SUMS)
    echo "  ✅ Linux artifact ready ($(( $(date +%s) - build_start ))s validation/package, $(du -h "$DIST/pryx-linux-x86_64.tar.gz" | cut -f1))"
...
if [[ "$MODE" == "fast-macos-local" ]]; then
    ...
    cp "$source_bin" "$DIST/pryx-macos-aarch64"
    chmod +x "$DIST/pryx-macos-aarch64"
    file "$DIST/pryx-macos-aarch64" | grep -q 'Mach-O 64-bit' || { echo "Error: bad macOS binary" >&2; exit 1; }
    (cd "$DIST" && tar czf pryx-macos-aarch64.tar.gz pryx-macos-aarch64)
    (cd "$DIST" && sha256sum pryx-macos-aarch64.tar.gz > SHA256SUMS)
    ...
    ensure_release_draft
    gh release upload "$VERSION" "$DIST/pryx-macos-aarch64.tar.gz" "$DIST/SHA256SUMS" --clobber
    gh release edit "$VERSION" --draft=false --latest
    echo ""
...
(
    PRYX_RELEASE_BUILD=1 PRYX_BUILD_SEMVER="$VERSION_NUM" scripts/build_linux_compat.sh "$DIST" >/dev/null
...
(
    PRYX_RELEASE_BUILD=1 PRYX_BUILD_SEMVER="$VERSION_NUM" \
        CARGO_INCREMENTAL=0 CARGO_BUILD_JOBS="${CARGO_BUILD_JOBS:-1}" \
        cargo build --release --target aarch64-apple-darwin --bin pryx 2>/dev/null
    cp target/aarch64-apple-darwin/release/pryx "$DIST/pryx-macos-aarch64"
    chmod +x "$DIST/pryx-macos-aarch64"
    (cd "$DIST" && tar czf pryx-macos-aarch64.tar.gz pryx-macos-aarch64)
...
ls -lh "$DIST"/*.tar.gz
file "$DIST/pryx-linux-x86_64.bin" | grep -q 'ELF 64-bit' || { echo "Error: bad Linux binary"; exit 1; }
head -1 "$DIST/pryx-linux-x86_64" | grep -q '^#!/' || { echo "Error: bad Linux wrapper"; exit 1; }
file "$DIST/pryx-macos-aarch64" | grep -q 'Mach-O 64-bit' || { echo "Error: bad macOS binary"; exit 1; }
...
gh release upload "$VERSION" \
    "$DIST/pryx-linux-x86_64.tar.gz" \
    "$DIST/pryx-macos-aarch64.tar.gz" \
    --clobber
