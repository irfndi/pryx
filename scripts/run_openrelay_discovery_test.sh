...
MODEL="${PRYX_OPENRELAY_TEST_MODEL:-gpt-5.6-sol}"
PROVIDER="${PRYX_OPENRELAY_TEST_PROVIDER:-openai}"
...
PROMPT="${PRYX_OPENRELAY_TEST_PROMPT:-$DEFAULT_PROMPT}"
OUTPUT="${PRYX_OPENRELAY_TEST_OUTPUT:-$REPO_ROOT/target/openrelay-discovery-test/latest.ndjson}"
SERVER_LOG="${PRYX_OPENRELAY_TEST_SERVER_LOG:-$REPO_ROOT/target/openrelay-discovery-test/server.jsonl}"
...
Runs a local-only OpenRelay Discovery fixture and an isolated Pryx agent test.
Environment overrides:
  PRYX_BIN, PRYX_OPENRELAY_TEST_MODEL, PRYX_OPENRELAY_TEST_PROVIDER
  PRYX_OPENRELAY_TEST_PROMPT, PRYX_OPENRELAY_TEST_OUTPUT
EOF
    exit 0
    ;;
  --print-prompt)
...
[[ -x "$PRYX_BIN" ]] || { printf 'Pryx binary is not executable: %s\n' "$PRYX_BIN" >&2; exit 1; }

mkdir -p "$(dirname "$OUTPUT")" "$(dirname "$SERVER_LOG")"
test_root="$(mktemp -d "${TMPDIR:-/tmp}/pryx-openrelay-discovery.XXXXXX")"
...
for name in openai-auth.json auth.json auth-refresh-state.json auth-validation.json provider_activity.json; do
  if [[ -f "$HOME/.pryx/$name" ]]; then
    cp -p "$HOME/.pryx/$name" "$test_home/$name"
  fi
done
...
printf 'PRYX_PROGRESS {"percent":10,"message":"Local Discovery fixture ready"}\n'
printf 'PRYX_PROGRESS {"percent":20,"message":"OpenRelay public RPC verified"}\n'
...
printf 'PRYX_PROGRESS {"percent":90,"message":"Validating agent trace"}\n'
set +e
...
if [[ $status -ne 0 ]]; then
  printf 'Pryx exited with status %s\n' "$status" >&2
  exit "$status"
fi
...
printf 'PRYX_CHECKPOINT {"message":"OpenRelay Discovery test passed"}\n'
