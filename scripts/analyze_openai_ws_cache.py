#!/usr/bin/env python3
"""Analyze OpenAI persistent-websocket reuse and KV-cache effectiveness from pryx logs.
...
Use it before/after changing ``PRYX_OPENAI_WS_IDLE_RECONNECT_SECS`` (or the
...
With no arguments it scans ~/.pryx/logs/pryx-*.log.
"""
...
def _log_files(argv: list[str]) -> list[str]:
    ...
    return sorted(glob.glob(os.path.join(home, ".pryx", "logs", "pryx-*.log")))
