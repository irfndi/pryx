...
closes that gap by driving the **real pryx binary** under a pseudo-terminal and
...
Everything runs in a throwaway PRYX_HOME / runtime dir / socket. The user's
...
# protocol pryx turns on at startup):
...
# ──────────────────────────────────────────────────────────────────────────────
# A PTY-driven pryx client. This is the real live-render input path; bytes
...
def launch_client(binary: str, env: dict, session_id: str, name: str,
    ...
    cenv["PRYX_DEBUG_CMD_PATH"] = str(debug_cmd)
    cenv["PRYX_DEBUG_RESPONSE_PATH"] = str(debug_resp)
    proc = subprocess.Popen(
        [
            binary,
            "--no-update",
            "--no-selfdev",
            "--socket", env["PRYX_SOCKET"],
            "--resume", session_id,
        ],
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix="pryx-expand-edit-"))
    home = root / "home"
    run = root / "run"
