#!/usr/bin/env python3
"""
Reproduce and measure input-line lag in a freshly spawned pryx TUI.

Why this exists
---------------
Users report that a *newly spawned* pryx is laggy: the slash-command popup
...
def main() -> int:
    ...
    ap.add_argument("--background-clients", type=int, default=0,
                    help="spawn N extra live TUI clients first. Real users run "
                         "many pryx windows at once, and each one repaints on a "
    ...
    root = Path(tempfile.mkdtemp(prefix="pryx-input-lag-"))
    ...
    if args.live:
        ...
        real_runtime = Path(env.get("PRYX_RUNTIME_DIR")
