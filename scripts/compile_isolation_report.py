#!/usr/bin/env python3
"""Report compile-time isolation risks in the Pryx crate graph.
...
WATCHED_CRATES = ["pryx-base", "pryx-app-core", "pryx-tui", "pryx"]
...
def target_state_violations(stats_by_name: dict[str, CrateStats]) -> list[str]:
    ...
    tui = stats_by_name.get("pryx-tui")
    if tui and "pryx-app-core" in tui.normal_workspace_deps:
        violations.append("target-state: pryx-tui still directly depends on pryx-app-core")

    app_core = stats_by_name.get("pryx-app-core")
    if app_core and "pryx-base" in app_core.normal_workspace_deps:
        violations.append("target-state: pryx-app-core still directly depends on pryx-base")

    base = stats_by_name.get("pryx-base")
    if base:
        for dep in base.normal_workspace_deps:
            if dep in {
                "pryx-azure-auth",
                "pryx-provider-gemini",
                "pryx-provider-openai",
                "pryx-provider-openrouter",
                "pryx-notify-email",
                "pryx-build-support",
            }:
                violations.append(f"target-state: pryx-base still depends on leaf/runtime crate {dep}")
        for dep in base.normal_external_deps:
            if dep.startswith("aws-") or dep in {"aws-types"}:
                violations.append(f"target-state: pryx-base still depends directly on AWS crate {dep}")

    for crate in stats_by_name.values():
        for glob in crate.glob_reexports:
            if crate.name in {"pryx", "pryx-app-core", "pryx-tui"}:
