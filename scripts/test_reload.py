...
def pryx_debug_socket():
    """Find the active pryx debug socket path."""
    runtime_dir = os.environ.get("XDG_RUNTIME_DIR") or f"/run/user/{os.getuid()}"
    return os.path.join(runtime_dir, "pryx-debug.sock")
...
def dbg(command, session_id=None, timeout=TIMEOUT_SECS):
    ...
    return _send_recv(pryx_debug_socket(), req, timeout=timeout)
...
def test_reload_context_roundtrip():
    pryx_dir = pathlib.Path.home() / ".pryx"
    ctx_path = pryx_dir / "reload-context.json"
...
def test_reload_info_file():
    pryx_dir = pathlib.Path.home() / ".pryx"
    info_path = pryx_dir / "reload-info"
...
def test_canary_binary_path():
    ...
    manifest_path = home / ".pryx" / "build-manifest.json"
    ...
    if canary_hash:
        canary_binary = home / ".pryx" / "builds" / "canary" / "pryx"
...
def test_multiple_sessions_responsive():
    ...
    try:
        for i in range(N):
            s = create_session(f"/tmp/pryx-test-{i}")
            sessions.append(s)
...
def check_server_up():
    dbg_sock = pryx_debug_socket()
    if not os.path.exists(dbg_sock):
        raise RuntimeError(
            f"Debug socket not found at {dbg_sock}.\n"
            "  Start pryx server first:\n"
            "    pryx serve   (or just launch pryx in the repo)"
        )
    try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test pryx selfdev reload mechanism",
    ...
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║      pryx selfdev reload test suite                    ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
