...
bin=${PRYX_AUTH_MATRIX_BIN:-}
out_dir=${PRYX_AUTH_MATRIX_OUT:-"$repo_root/target/auth-test-reports"}
prompt=${PRYX_AUTH_MATRIX_PROMPT:-"Reply with exactly AUTH_TEST_OK and nothing else. Do not call tools."}
providers=${PRYX_AUTH_MATRIX_PROVIDERS:-"claude copilot openrouter deepseek zai alibaba-coding-plan openai-compatible"}
mode=${PRYX_AUTH_MATRIX_MODE:-configured}
keep_going=${PRYX_AUTH_MATRIX_KEEP_GOING:-1}
per_command_timeout=${PRYX_AUTH_MATRIX_TIMEOUT:-90}

usage() {
  cat <<'EOF'
Usage: scripts/auth_regression_matrix.sh [options]

Runs pryx auth-test across the auth/provider matrix and writes one JSON report per provider.
...
Options:
  ...
  --bin PATH            pryx binary to run (default: cargo run --bin pryx --)
...
Environment equivalents:
  PRYX_AUTH_MATRIX_BIN=/path/to/pryx
  PRYX_AUTH_MATRIX_OUT=target/auth-test-reports
  PRYX_AUTH_MATRIX_PROVIDERS="claude deepseek zai"
  PRYX_AUTH_MATRIX_MODE=configured|all
  PRYX_AUTH_MATRIX_LOGIN=1
  PRYX_AUTH_MATRIX_NO_SMOKE=1
  PRYX_AUTH_MATRIX_NO_TOOL_SMOKE=1
  PRYX_AUTH_MATRIX_KEEP_GOING=0
  PRYX_AUTH_MATRIX_TIMEOUT=90

Examples:
  scripts/auth_regression_matrix.sh --configured --no-smoke
  scripts/auth_regression_matrix.sh --provider deepseek --provider zai
  PRYX_AUTH_MATRIX_BIN=target/selfdev/pryx scripts/auth_regression_matrix.sh --all
EOF
}
...
if [[ "${PRYX_AUTH_MATRIX_LOGIN:-0}" == "1" ]]; then
  extra_args+=(--login)
fi
if [[ "${PRYX_AUTH_MATRIX_NO_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-smoke)
fi
if [[ "${PRYX_AUTH_MATRIX_NO_TOOL_SMOKE:-0}" == "1" ]]; then
  extra_args+=(--no-tool-smoke)
fi
...
run_pryx() {
  if [[ -n "$bin" ]]; then
    ...
    timeout "$per_command_timeout" cargo run --quiet --bin pryx -- "$@"
  fi
}
...
if [[ "$mode" == "configured" ]]; then
  ...
  if ! run_pryx auth-test --all-configured --no-smoke --no-tool-smoke --json --output "$configured_json" >/tmp/pryx-auth-matrix-discovery.out 2>/tmp/pryx-auth-matrix-discovery.err; then
    if [[ -s "$configured_json" ]]; then
      ...
      cat /tmp/pryx-auth-matrix-discovery.err >&2 || true
