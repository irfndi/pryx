#!/usr/bin/env bash
# memory_probe.sh - Reproducible memory probe harness for pryx TUI clients.
#
...
#   --binary <path>   Client binary (default: ~/.pryx/builds/current/pryx)
#   --pryx <path>    pryx CLI used for `pryx debug ...` (default: ~/.local/bin/pryx)
...
#
# Requirements: a running pryx server with the debug socket enabled, jq,
...
CLIENT_BIN="$HOME/.pryx/builds/current/pryx"
TESTER_CWD="$HOME"
SKIP_TRIM=0
KEEP_TESTER=0
...
[[ -x "$PRYX_BIN" ]] || die "pryx CLI not found at $PRYX_BIN"
[[ -x "$CLIENT_BIN" ]] || CLIENT_BIN="$(command -v pryx)" || die "client binary not found"
[[ -f "$HOME/.pryx/sessions/${SESSION_ID}.json" ]] \
    || log "WARN: $HOME/.pryx/sessions/${SESSION_ID}.json not found; resume may create a new session"

RUN_ID="probe_$(date -u +%Y%m%dT%H%M%SZ)_$$"
SESSION_FILE="$HOME/.pryx/sessions/${SESSION_ID}.json"
...
cleanup() {
    ...
    if [[ -n "$TESTER_ID" && "$KEEP_TESTER" -ne 1 ]]; then
        "$PRYX_BIN" debug "tester:${TESTER_ID}:stop" >/dev/null 2>&1 || true
...
# ===========================================================================
WRAPPER="$(mktemp /tmp/pryx_memory_probe_wrapper.XXXXXX.sh)"
cat > "$WRAPPER" <<EOF
#!/usr/bin/env bash
...
SPAWN_OUT="$("$PRYX_BIN" debug "tester:spawn {\"binary\":\"$WRAPPER\",\"cwd\":\"$TESTER_CWD\",\"cols\":120,\"rows\":40}")" \
...
TESTER_INFO="$("$PRYX_BIN" debug tester:list | jq -c --arg id "$TESTER_ID" '.[] | select(.id == $id)')"
