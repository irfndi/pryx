...
install performed by an agent on the user's behalf is attributable to pryx.
...
`cli_flow_attributable` is the primary check: nearly all pryx-driven signups
...
Live requests carry x-pryx-discovery-benchmark: 1 via
PRYX_DISCOVERY_BENCHMARK=1 semantics: this runner calls the discovery HTTP
...
DEFAULT_ENDPOINT = "https://api.pryx.sh/v1/discovery"
...
def http_get(url: str, timeout: float, benchmark: bool = False) -> tuple[int, str, str]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "pryx-attribution-benchmark"})
    if benchmark:
...
def fetch_live_entry(endpoint: str, sponsor: dict[str, Any], timeout: float) -> dict[str, Any]:
    base = {
        ...
        "reason": (
            ...
            "preserve the pryx referral marker. No account will be created."
        ),
    }
...
def main() -> int:
    ...
    print(f"CLI-flow attribution ({CRITICAL_CHECK}) is the primary signal: "
          "agent CLI signups are how pryx actually drives acquisition.")
    for r in reports:
        print(f"  {r.tool:<14} {r.cli_verdict:<14} {r.cli_detail}")
    attributed = len(reports) - len(not_attributed) - len(unknown_attribution)
    print(f"  => {attributed}/{len(reports)} sponsors credit agent-driven CLI signups to pryx"
