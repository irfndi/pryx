...
Emits niri-valid configs to ~/.pryx/scratch/fuzz719/valid_inputs.json. Feed
that to the committed corpus in
crates/pryx-setup-hints/src/linux_niri_fuzz_corpus.txt to widen coverage.

Differential fuzz: does pryx's insert-point scan agree with real niri?
...
asks it again after pryx splices its managed block in.
...
WORK = pathlib.Path.home() / '.pryx' / 'scratch' / 'fuzz719'
