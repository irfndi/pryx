...
#   2. leaf touch        - 1-line change in the root `pryx` bin crate
#   3. tui touch         - 1-line change in pryx-tui (largest UI crate)
#   4. core touch        - 1-line change in pryx-app-core (mid-stack crate)
#   5. commit blast      - simulate HEAD moving (PRYX_BUILD_GIT_HASH change),
#                          which reruns pryx-build-meta's build script and
...
# `scripts/dev_cargo.sh build --profile selfdev -p pryx --bin pryx` succeeds.
#
...
build_cmd=(scripts/dev_cargo.sh build --profile selfdev -p pryx --bin pryx)
...
tui_file="crates/pryx-tui/src/lib.rs"
echo "3. tui touch (${tui_file}):"
touch_file "$tui_file"
run_build "tui" || true
revert_file "$tui_file"

core_file="crates/pryx-app-core/src/lib.rs"
echo "4. core touch (${core_file}):"
touch_file "$core_file"
run_build "core" || true
revert_file "$core_file"

echo "5. commit blast radius (simulated HEAD move via PRYX_BUILD_GIT_HASH):"
fake_hash="bench$(date +%s | tail -c 4)"
start=$(date +%s.%N)
if PRYX_BUILD_GIT_HASH="$fake_hash" "${build_cmd[@]}" >/tmp/bench_selfdev_build_commit-blast.log 2>&1; then
    end=$(date +%s.%N)
...
echo "    pryx-build-meta embeds GIT_HASH via env! and pryx-base/app-core/"
