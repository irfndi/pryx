...
#              install, does `pryx` resolve in a brand-new login/interactive
...
#   Section C  retention - an upgrade must preserve ~/.pryx config and auth,
...
case "$url" in
  *telemetry.pryx.sh*) ;;
  *pryx.sh/releases/latest/version) printf 'v%s\n' "${EVAL_VERSION:-1.2.3}" ;;
  *pryx.sh/releases/v*/download-bases)
    printf 'https://github.com/pryx-co/pryx/releases/download/v%s\n' "${EVAL_VERSION:-1.2.3}"
    ;;
  *SHA256SUMS)
    ...
    printf '8d57abb57a0dae3ff23c8f0df1f51951b7772822e0d560e860d6f68c24ef6d3d  %s\n' \
      "${EVAL_CHECKSUM_ASSET:-pryx-linux-x86_64.tar.gz}"
    ;;
  *github.com*/releases/latest)
    printf 'https://github.com/pryx-co/pryx/releases/tag/v%s' "${EVAL_VERSION:-1.2.3}"
    ;;
...
artifact="${EVAL_ARCHIVE_ARTIFACT:-pryx-linux-x86_64}"
cat > "$dest/$artifact" <<BIN
#!/usr/bin/env bash
if [ "\${1:-}" = "--version" ]; then printf 'pryx ${EVAL_VERSION:-1.2.3}\n'; fi
exit 0
BIN
...
run_install() {
  ...
  PRYX_HOME="$home/.pryx" \
  PRYX_SKIP_SERVER_RELOAD=1 \
  PRYX_NO_TELEMETRY=1 \
  bash "$install_sh" 2>&1
}

# Probe: does `pryx` resolve and run in a fresh shell of the given kind, with
...
probe_shell() { # probe_shell <home> <shell-cmd...>
  ...
  ENV="$home/.profile" \
  PATH="/usr/bin:/bin" \
  "$@" 'command -v pryx >/dev/null 2>&1 && pryx --version' 2>/dev/null </dev/null
}
...
launcher="$home_a/.local/bin/pryx"
[ -x "$launcher" ]; check "launcher exists and is executable" \
  "executable at ~/.local/bin/pryx" "missing or not executable: $launcher" "$?"

ver=$("$launcher" --version 2>/dev/null || true)
[ "$ver" = "pryx 1.2.3" ]; check "launcher runs and reports the installed version" \
  "pryx 1.2.3" "${ver:-<no output>}" "$?"

# The success message must not dead-end the user: either pryx is already
# resolvable or the copy explicitly says future shells will have it.
printf '%s' "$install_out" | grep -q "Run 'pryx' to get started\|Future terminal sessions will have pryx on PATH automatically"
check "install output gives a working next step (no dead end)" \
  "a 'run pryx' or 'future sessions' line" "neither line found in installer output" "$?"
...
probe_case() { # probe_case <label> <binary> <shell-cmd...>
  ...
  [ "$out" = "pryx 1.2.3" ]
  check "$label" "pryx resolves and prints 'pryx 1.2.3'" "${out:-<not found on PATH>}" "$?"
}

probe_case "bash login shell (bash -lc) finds pryx"        bash bash -lc
probe_case "bash interactive shell (bash -ic) finds pryx"  bash bash -ic
probe_case "sh login shell (sh -lc) finds pryx"            sh   sh -lc
probe_case "fish shell (fish -c) finds pryx"               fish fish -c
probe_case "zsh login shell (zsh -lc) finds pryx"          zsh  zsh -lc
...
for rc in .bashrc .profile .zshenv .config/fish/config.fish; do
  file="$home_a/$rc"
  [ -f "$file" ] || continue
  # Each install appends one "# Added by pryx installer" stanza when missing;
  # a correct idempotency guard leaves exactly one after any number of runs.
  count=$(grep -cF "# Added by pryx installer" "$file" || true)
  [ "$count" -le 1 ]
  check "~/$rc has at most one pryx PATH stanza after 3 installs" \
...
mkdir -p "$home_c/.pryx"
printf 'model = "kept"\n' > "$home_c/.pryx/config.toml"
printf '{"kept":true}\n' > "$home_c/.pryx/auth.json"

run_install "$home_c" "1.3.0" >/dev/null 2>&1

ver=$("$home_c/.local/bin/pryx" --version 2>/dev/null || true)
[ "$ver" = "pryx 1.3.0" ]; check "upgrade switches the launcher to the new version" \
  "pryx 1.3.0" "${ver:-<no output>}" "$?"

[ "$(cat "$home_c/.pryx/config.toml" 2>/dev/null)" = 'model = "kept"' ]
check "upgrade preserves ~/.pryx/config.toml" \
  "file unchanged" "missing or modified" "$?"

[ "$(cat "$home_c/.pryx/auth.json" 2>/dev/null)" = '{"kept":true}' ]
check "upgrade preserves ~/.pryx/auth.json" \
  "file unchanged" "missing or modified" "$?"

[ -x "$home_c/.pryx/builds/versions/1.2.3/pryx" ] && [ -x "$home_c/.pryx/builds/versions/1.3.0/pryx" ]
check "both immutable version binaries kept (rollback possible)" \
  "versions/1.2.3 and versions/1.3.0 both executable" \
  "$(ls "$home_c/.pryx/builds/versions" 2>/dev/null | tr '\n' ' ')" "$?"

stable_ver=$(cat "$home_c/.pryx/builds/stable-version" 2>/dev/null || true)
[ "$stable_ver" = "1.3.0" ]; check "stable channel marker points at the new version" \
  "1.3.0" "${stable_ver:-<missing>}" "$?"

# A second post-upgrade login shell must still resolve pryx (PATH survives
# upgrades, not just fresh installs).
out=$(probe_shell "$home_c" bash -lc)
[ "$out" = "pryx 1.3.0" ]; check "post-upgrade login shell still finds pryx" \
  "pryx 1.3.0" "${out:-<not found on PATH>}" "$?"
...
# uninstall.sh pkills running pryx servers; neuter that inside the sandbox so
...
[ ! -e "$home_c/.local/bin/pryx" ] && [ ! -d "$home_c/.pryx/builds" ]
...
if command -v pwsh >/dev/null 2>&1; then
  broadcast_block=$(awk "/<<'PRYX_PS_BROADCAST_EOF'/{inblock=1; next} /^PRYX_PS_BROADCAST_EOF\$/{inblock=0} inblock" "$install_sh")
  printf '%s' "$broadcast_block" > "$work/broadcast.ps1"
  parse_errors=$(PRYX_EVAL_PS_FILE="$work/broadcast.ps1" pwsh -NoProfile -NonInteractive -Command '
    $errors = $null
    [System.Management.Automation.Language.Parser]::ParseFile($env:PRYX_EVAL_PS_FILE, [ref]$null, [ref]$errors) | Out-Null
    $errors.Count
...
printf 'C:\\Tools;%s\\' "$(printf '%s' "$work/localappdata/pryx/bin" | tr '[:lower:]' '[:upper:]')" > "$win_state/user-path"
...
#   2. write the user PATH  -> persist $PRYX_NEW_USER_PATH
#   3. broadcast            -> record that a broadcast happened
args="$*"
case "$args" in
  *GetEnvironmentVariable*) cat "$EVAL_WIN_STATE/user-path" ;;
  *SetEnvironmentVariable*) printf '%s' "$PRYX_NEW_USER_PATH" > "$EVAL_WIN_STATE/user-path" ;;
...
EVAL_ARCHIVE_ARTIFACT="pryx-windows-x86_64.exe" \
EVAL_CHECKSUM_ASSET="pryx-windows-x86_64.tar.gz" \
EVAL_WIN_STATE="$win_state" \
PATH="$work/bin:/usr/bin:/bin" \
HOME="$home_w" \
LOCALAPPDATA="$work/localappdata" \
PRYX_HOME="$home_w/.pryx" \
PRYX_SKIP_SERVER_RELOAD=1 \
PRYX_NO_TELEMETRY=1 \
