...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Profile a single resumed pryx spawn")
    parser.add_argument("--binary", default="./target/selfdev/pryx")
...
def latest_log_file(log_dir: Path) -> Path:
    logs = sorted(log_dir.glob("pryx-*.log"), key=lambda p: p.stat().st_mtime)
    if not logs:
...
def profile_single_spawn(binary: str, cwd: str, timeout_s: float) -> dict:
    root = Path(tempfile.mkdtemp(prefix="pryx-single-profile-"))
    home = root / "home"
    runtime_dir = root / "run"
    socket_path = runtime_dir / "pryx.sock"
    debug_socket_path = runtime_dir / "pryx-debug.sock"
    ...
    env.update(
        {
            "PRYX_HOME": str(home),
            "PRYX_RUNTIME_DIR": str(runtime_dir),
            "PRYX_SOCKET": str(socket_path),
            "PRYX_DEBUG_SOCKET": str(debug_socket_path),
            "PRYX_SWARM_ENABLED": "0",
            "PRYX_NO_TELEMETRY": "1",
            "PRYX_TRACE": "1",
            "PRYX_TEMP_SERVER": "1",
            "PRYX_SERVER_OWNER_PID": str(os.getpid()),
        }
    )
    ...
    try:
        ...
        while time.perf_counter() < deadline:
            ...
            if rlist:
                ...
                lower = buffer.lower()
                if b"loading session" in lower or b"pryx" in lower or len(buffer) > 4096:
