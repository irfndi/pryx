#!/usr/bin/env python3
"""
Comprehensive memory system test for pryx.

Tests all memory features with both Claude and OpenAI providers via the debug socket.

Usage:
    # With existing server (uses /run/user/1000/pryx-debug.sock)
...
def main():
    parser = argparse.ArgumentParser(description='Test pryx memory system')
    ...
    if args.fresh:
        ...
        test_socket = '/tmp/pryx-memory-test.sock'
        ...
        socket_path = args.socket or '/run/user/1000/pryx-debug.sock'
        if not os.path.exists(socket_path):
            log_fail(f"Debug socket not found: {socket_path}")
            log("Use --fresh to start a test server, or ensure pryx is running")
            sys.exit(1)
