...
case "$url" in
  *telemetry.pryx.sh*) printf '%s\n' "$payload" >> "$INSTALL_TELEMETRY_LOG" ;;
  *pryx.sh/releases/latest/version)
    [ "${FAIL_RELEASE:-0}" != "1" ] || exit 22
    [ "${FAIL_METADATA_RELEASE:-0}" != "1" ] || exit 22
    printf 'v1.2.3\n'
    ;;
  *pryx.sh/releases/v1.2.3/download-bases)
    printf 'https://mirror.invalid/releases/v1.2.3\n'
    printf 'https://github.com/pryx-co/pryx/releases/download/v1.2.3\n'
    ;;
  *pryx.sh/releases/v1.2.3/SHA256SUMS)
    ...
    printf '%s  %s\n' "$checksum" "${TEST_CHECKSUM_ASSET:-pryx-linux-x86_64.tar.gz}"
    ;;
  ...
  *github.com*/releases/latest)
    ...
    printf 'https://github.com/pryx-co/pryx/releases/tag/v1.2.3'
    ;;
...
artifact="${TEST_ARCHIVE_ARTIFACT:-pryx-linux-x86_64}"
cat > "$dest/$artifact" <<'BIN'
#!/usr/bin/env bash
if [ "${1:-}" = "--version" ]; then printf 'pryx 1.2.3\n'; fi
...
PATH="$tmp/bin:$PATH" \
HOME="$tmp/home" \
PRYX_HOME="$tmp/home/.pryx" \
PRYX_INSTALL_DIR="$tmp/install" \
PRYX_INSTALL_CONVERSION_ID="$conversion_id" \
...
test "$(cat "$tmp/home/.pryx/install_conversion_id")" = "$conversion_id"
...
# If GitHub's release page is blocked, the static pryx.sh version endpoint
# must keep the complete install path working.
PATH="$tmp/bin:$PATH" \
HOME="$tmp/home-metadata-fallback" \
PRYX_HOME="$tmp/home-metadata-fallback/.pryx" \
PRYX_INSTALL_DIR="$tmp/install-metadata-fallback" \
PRYX_SKIP_SERVER_RELOAD=1 \
PRYX_NO_TELEMETRY=1 \
FAIL_GITHUB_RELEASE=1 \
bash "$repo_dir/scripts/install.sh" >/dev/null
test -x "$tmp/install-metadata-fallback/pryx"
...
PRYX_HOME="$tmp/home-checksum-fallback/.pryx" \
PRYX_INSTALL_DIR="$tmp/install-checksum-fallback" \
PRYX_SKIP_SERVER_RELOAD=1 \
PRYX_NO_TELEMETRY=1 \
METADATA_CHECKSUM_HTML=1 \
bash "$repo_dir/scripts/install.sh" >/dev/null
test -x "$tmp/install-checksum-fallback/pryx"
...
PRYX_HOME="$tmp/home-windows-arm64/.pryx" \
PRYX_INSTALL_DIR="$tmp/install-windows-arm64" \
PRYX_SKIP_SERVER_RELOAD=1 \
PRYX_NO_TELEMETRY=1 \
...
TEST_ARCHIVE_ARTIFACT=pryx-windows-aarch64.exe \
TEST_CHECKSUM_ASSET=pryx-windows-aarch64.tar.gz \
DOWNLOAD_URL_LOG="$windows_url_log" \
bash "$repo_dir/scripts/install.sh" >/dev/null
grep -q '/pryx-windows-aarch64.tar.gz$' "$windows_url_log"
test -x "$tmp/install-windows-arm64/pryx.exe"

failure_log="$tmp/failure.jsonl"
if PATH="$tmp/bin:$PATH" \
  HOME="$tmp/home-failure" \
  PRYX_HOME="$tmp/home-failure/.pryx" \
  PRYX_INSTALL_DIR="$tmp/install-failure" \
  PRYX_INSTALL_CONVERSION_ID="$conversion_id" \
  PRYX_SKIP_SERVER_RELOAD=1 \
...
if PATH="$tmp/bin:$PATH" \
  ...
  PRYX_HOME="$tmp/home-checksum-failure/.pryx" \
  PRYX_INSTALL_DIR="$tmp/install-checksum-failure" \
  PRYX_INSTALL_CONVERSION_ID="$conversion_id" \
  PRYX_SKIP_SERVER_RELOAD=1 \
...
PRYX_HOME="$tmp/home-private/.pryx" \
PRYX_INSTALL_DIR="$tmp/install-private" \
PRYX_INSTALL_CONVERSION_ID="$conversion_id" \
PRYX_SKIP_SERVER_RELOAD=1 \
PRYX_NO_TELEMETRY=1 \
...
bash "$repo_dir/scripts/install.sh" >/dev/null
test ! -e "$privacy_log"
test ! -e "$tmp/home-private/.pryx/install_conversion_id"
