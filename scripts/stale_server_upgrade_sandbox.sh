...
#   Client: the freshly built current binary (target/debug/pryx, has the fix).
#   Field state: shared-server channel pinned to OLD (v0.14.6); stable -> NEW.
#
# It starts the real old daemon, then runs the NEW client's `pryx server reload`
# (which repairs the stale shared-server channel, then forces a reload). PASS iff
# the resulting daemon is running v0.22.x.
#
# Usage:
#   cargo build -p pryx --bin pryx
#   scripts/stale_server_upgrade_sandbox.sh
#
# Linux x86_64 only (uses the published pryx-linux-x86_64 release asset).
set -uo pipefail

REPO_ROOT="$(cd -- "$(dirname -- "$0")/.." && pwd)"
NEW_BIN="${NEW_BIN:-$REPO_ROOT/target/debug/pryx}"
OLD_VERSION="${OLD_VERSION:-v0.14.6}"
OLD_DIR="${OLD_DIR:-/tmp/pryx-sandbox}"
OLD_WRAP="$OLD_DIR/pryx-linux-x86_64"

[ -x "$NEW_BIN" ] || { echo "missing new client binary: $NEW_BIN (run: cargo build -p pryx --bin pryx)"; exit 2; }

# Fetch + extract the real old release binary if it is not already present.
if [ ! -x "$OLD_WRAP" ]; then
  mkdir -p "$OLD_DIR"
  url="$(curl -fsSL "https://api.github.com/repos/pryx-co/pryx/releases/tags/$OLD_VERSION" \
        | grep -o 'https://[^"]*pryx-linux-x86_64.tar.gz' | head -1)"
...
SANDBOX="$(mktemp -d /tmp/pryx-stale-sandbox.XXXXXX)"
export PRYX_HOME="$SANDBOX/home"
export PRYX_RUNTIME_DIR="$SANDBOX/runtime"
# Hard isolation: pin the socket explicitly so we can NEVER touch the real
# global daemon at /run/user/<uid>/pryx.sock.
export PRYX_SOCKET="$SANDBOX/runtime/pryx.sock"
# Make the new client's clean release version comparable (debug build is dirty).
export PRYX_TEST_CLIENT_VERSION_OVERRIDE="v0.22.0 (sandbox)"
mkdir -p "$PRYX_HOME" "$PRYX_RUNTIME_DIR"

BUILDS="$PRYX_HOME/builds"
...
cp "$OLD_DIR/pryx-linux-x86_64.bin" "$OLD_DIR/libssl.so.10" \
   "$OLD_DIR/libcrypto.so.10" "$BUILDS/versions/0.14.6/"
cat > "$BUILDS/versions/0.14.6/pryx" <<'WRAP'
#!/usr/bin/env sh
set -eu
real=$0
...
WRAP
chmod +x "$BUILDS/versions/0.14.6/pryx"

# --- Install the NEW binary as version 0.22.0 (newer mtime) ----------------
cp "$NEW_BIN" "$BUILDS/versions/0.22.0/pryx"
touch -d "+1 minute" "$BUILDS/versions/0.22.0/pryx"

# --- Field state: shared-server -> OLD, stable/current -> NEW --------------
ln -sf "../versions/0.14.6/pryx" "$BUILDS/shared-server/pryx"
echo "0.14.6" > "$BUILDS/shared-server-version"
ln -sf "../versions/0.22.0/pryx" "$BUILDS/stable/pryx"
echo "0.22.0" > "$BUILDS/stable-version"
ln -sf "../versions/0.22.0/pryx" "$BUILDS/current/pryx"
...
cleanup() {
  ...
  pkill -f "$BUILDS/versions/0.14.6/pryx-linux-x86_64.bin" 2>/dev/null || true
  pkill -f "$BUILDS/versions/0.22.0/pryx" 2>/dev/null || true
  rm -rf "$SANDBOX"
}
trap cleanup EXIT
...
"$BUILDS/shared-server/pryx" --no-update --provider antigravity serve \
...
# --- 2) New client: pryx server reload (repairs channel, then reloads) ----
log "Running NEW client: pryx server reload"
