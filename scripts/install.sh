...
REPO="pryx-co/pryx"
RELEASE_METADATA_BASE="${PRYX_RELEASE_METADATA_BASE:-https://pryx.sh/releases}"
IS_WINDOWS=false
IS_TERMUX=false
...
valid_conversion_id() {
  printf '%s' "${PRYX_INSTALL_CONVERSION_ID:-}" |
...
report_install_funnel() {
  ...
  curl -fsS --max-time 2 -H 'Content-Type: application/json' \
    --data "$payload" https://telemetry.pryx.sh/v1/event >/dev/null 2>&1 || true
}
...
persist_install_conversion_id() {
  ...
  [ "${DO_NOT_TRACK:-}" != "1" ] || return 0
  valid_conversion_id || return 0
  pryx_home="${PRYX_HOME:-$HOME/.pryx}"
  mkdir -p "$pryx_home" 2>/dev/null || return 0
  (umask 077; printf '%s\n' "$PRYX_INSTALL_CONVERSION_ID" > "$pryx_home/install_conversion_id") \
    2>/dev/null || return 0
  chmod 600 "$pryx_home/install_conversion_id" 2>/dev/null || true
}
...
case "$OS" in
  Linux)
    case "$ARCH" in
      x86_64)  ARTIFACT="pryx-linux-x86_64" ;;
      aarch64|arm64) ARTIFACT="pryx-linux-aarch64" ;;
      *)       err "Unsupported Linux architecture: $ARCH" ;;
    esac
    ;;
  Darwin)
    case "$ARCH" in
      arm64)   ARTIFACT="pryx-macos-aarch64" ;;
      x86_64)  ARTIFACT="pryx-macos-x86_64" ;;
  ...
  MINGW*|MSYS*|CYGWIN*)
    ...
    case "$WINDOWS_ARCH" in
      x86_64) ARTIFACT="pryx-windows-x86_64" ;;
      aarch64) ARTIFACT="pryx-windows-aarch64" ;;
...
if [ "$IS_WINDOWS" = true ]; then
  INSTALL_DIR="${PRYX_INSTALL_DIR:-$LOCALAPPDATA/pryx/bin}"
else
  INSTALL_DIR="${PRYX_INSTALL_DIR:-$HOME/.local/bin}"
fi

# Prefer GitHub's stable redirect when it is reachable so publication changes
# are visible immediately. pryx.sh keeps a static copy of the latest published
...
if [ -z "$VERSION" ]; then
  ...
  if valid_release_tag "$GITHUB_VERSION"; then
    ...
    info "GitHub release lookup unavailable; using cached pryx.sh metadata ($VERSION)."
  fi
fi
...
if [ "$IS_WINDOWS" = true ]; then
  EXE=".exe"
  builds_dir="$LOCALAPPDATA/pryx/builds"
else
  EXE=""
...
launcher_path="$INSTALL_DIR/pryx${EXE}"
...
if [ -n "$EXISTING" ]; then
  if echo "$EXISTING" | grep -qF "${VERSION#v}"; then
    info "pryx $VERSION is already installed — reinstalling"
  else
    info "Updating pryx $EXISTING → $VERSION"
  fi
else
  info "Installing pryx $VERSION"
fi
...
for candidate in "$ARTIFACT.tar.gz" "$ARTIFACT$EXE"; do
  while IFS= read -r base; do
    ...
    if curl -fsSL --retry 2 --connect-timeout 10 \
      "${base%/}/$candidate" -o "$tmpdir/pryx.download" 2>/dev/null; then
...
if [ -n "$download_mode" ]; then
  ...
  ACTUAL_SHA256=$(sha256_file "$tmpdir/pryx.download") \
...
if [ "$download_mode" = "tar" ]; then
  tar xzf "$tmpdir/pryx.download" -C "$tmpdir"
  ...
  mv "$tmpdir/pryx.download" "$dest_version_dir/$bin_name"
else
    ...
    if [ -x "$termux_glibc_linker" ]; then
      if command -v patchelf >/dev/null 2>&1; then
        patchelf --set-interpreter "$termux_glibc_linker" "$dest_version_dir/$bin_name" \
          || err "Failed to patch pryx ELF interpreter for Termux glibc"
        info "Patched Termux glibc ELF interpreter: $termux_glibc_linker"
      else
        info "Termux detected: install patchelf with 'pkg install patchelf' and rerun this installer if pryx fails to start."
      fi
    else
      info "Termux detected: install glibc with 'pkg install glibc' if pryx fails due to a missing dynamic linker."
    fi
  fi
fi
...
if [ "$(uname -s)" = "Darwin" ]; then
  ...
  # first interactive pryx launch performs the same version-gated repair.
...
if [ "${PRYX_SKIP_SERVER_RELOAD:-}" != "1" ]; then
  ...
  if [ -x "$reload_bin" ]; then
    if "$reload_bin" server reload </dev/null >/dev/null 2>&1; then
      info "Reloaded the running pryx server onto $VERSION (if one was active)."
    fi
  fi
fi
...
if [ "$IS_WINDOWS" = true ]; then
  ...
  # cmd, Git Bash, Windows Terminal) finds pryx without manual setup. This is
  # the Git Bash (`curl | sh`) counterpart of install.ps1's Set-PryxUserPath:
  # read the user PATH, drop stale pryx launcher entries (case- and trailing-
  ...
  if command -v powershell.exe >/dev/null 2>&1; then
    ...
    if [ "$new_user_path" = "$current_user_path" ]; then
    ...
    elif PRYX_NEW_USER_PATH="$new_user_path" powershell.exe -NoProfile -NonInteractive -Command \
      '[Environment]::SetEnvironmentVariable("Path", $env:PRYX_NEW_USER_PATH, "User")' >/dev/null 2>&1; then
      win_path_persisted=true
      ...
      win_broadcast_ps=$(cat <<'PRYX_PS_BROADCAST_EOF'
$sig = '[DllImport("user32.dll", SetLastError = true, CharSet = CharSet.Auto)] public static extern IntPtr SendMessageTimeout(IntPtr hWnd, uint Msg, UIntPtr wParam, string lParam, uint fuFlags, uint uTimeout, out UIntPtr lpdwResult);'
$type = Add-Type -MemberDefinition $sig -Name 'PryxEnvBroadcast' -Namespace Win32 -PassThru
[UIntPtr]$result = [UIntPtr]::Zero
$type::SendMessageTimeout([IntPtr]0xffff, 0x001A, [UIntPtr]::Zero, 'Environment', 2, 5000, [ref]$result) | Out-Null
PRYX_PS_BROADCAST_EOF
)
  ...
  echo ""
  info "✅ pryx $VERSION installed successfully!"
  echo ""
  if [ "$win_path_persisted" = true ]; then
    info "Added $win_install_dir to your user PATH. New terminals will find pryx automatically."
  fi
  if command -v pryx >/dev/null 2>&1; then
    info "Run 'pryx' to get started."
  else
    echo "  To start using pryx in THIS terminal right now, run:"
    echo ""
    printf '    \033[1;32mexport PATH="%s:$PATH" && pryx\033[0m\n' "$INSTALL_DIR"
    if [ "$win_path_persisted" != true ]; then
      echo ""
      echo "  To add pryx to PATH permanently (PowerShell):"
      echo ""
  ...
  if [ "$(uname -s)" = "Darwin" ]; then
    if [ "$hotkey_setup_ready" = true ]; then
      info "Global hotkey ready: Cmd+; launches a new pryx from anywhere, system-wide"
    else
      info "Tip: run 'pryx setup-hotkey' so Cmd+; launches pryx system-wide on macOS"
    fi
  fi
  ...
  if command -v pryx >/dev/null 2>&1; then
    ...
    echo "  To start using pryx right now, run:"
    echo ""
    printf '    \033[1;32mexport PATH="%s:\$PATH" && pryx\033[0m\n' "$INSTALL_DIR"
    echo ""
    echo "  Future terminal sessions will have pryx on PATH automatically."
  fi
fi
