...
use pryx::message::{ContentBlock, Role, ToolCall};
use pryx::perf::{SyntheticSystemProfile, TuiPerfPolicy, tui_policy_for};
use pryx::prompt::ContextInfo;
use pryx::session::{Session, StoredDisplayRole};
use pryx::side_panel::{
    SidePanelPage, SidePanelPageFormat, SidePanelPageSource, SidePanelSnapshot,
};
use pryx::tui::{DisplayMessage, ProcessingStatus, TuiState, info_widget::InfoWidgetData};
...
struct BenchState {
    ...
    diff_mode: pryx::config::DiffDisplayMode,
    queue_mode: bool,
    ...
    copy_selection_range: Option<pryx::tui::CopySelectionRange>,
...
impl BenchState {
    fn new(
        ...
        Ok(Self {
            ...
            is_processing,
            status,
            diff_mode: pryx::config::DiffDisplayMode::Off,
            queue_mode: true,
    ...
    fn from_session(
        ...
        let session = pryx::replay::load_session(id_or_path)
            .with_context(|| format!("failed to load session '{}'", id_or_path))?;
        let mut side_panel =
            pryx::side_panel::snapshot_for_session(&session.id).unwrap_or_default();
        ...
        Ok(Self {
            ...
            diff_mode: if matches!(mode, BenchMode::FileDiff) {
                pryx::config::DiffDisplayMode::File
            } else {
                pryx::config::DiffDisplayMode::Off
            },
            queue_mode: true,
            context_info: ContextInfo::default(),
            info_widget: InfoWidgetData::default(),
    ...
    fn simulate_linked_refresh(&mut self) -> Result<()> {
        ...
        let _ = pryx::side_panel::refresh_linked_page_content(&mut self.side_panel, None);
        Ok(())
    }

    fn prewarm_side_panel(&self, width: u16, height: u16) -> bool {
        pryx::tui::prewarm_focused_side_panel(
            &self.side_panel,
            width,
            height,
            40,
            pryx::tui::mermaid::protocol_type().is_some(),
            false,
        )
    }
}
...
fn stored_message_visible_text(message: &pryx::session::StoredMessage) -> String {
...
impl TuiState for BenchState {
    ...
    fn side_pane_images(&self) -> Vec<pryx::session::RenderedImage> {
        Vec::new()
    }
    ...
    fn batch_progress(&self) -> Option<pryx::bus::BatchProgress> {
        None
    }
    ...
    fn diff_mode(&self) -> pryx::config::DiffDisplayMode {
        self.diff_mode
    }
    ...
    fn context_limit(&self) -> Option<usize> {
        Some(pryx::provider::DEFAULT_CONTEXT_LIMIT)
    }
    ...
    fn render_streaming_markdown(&self, width: usize) -> Vec<ratatui::text::Line<'static>> {
        ...
        pryx::tui::markdown::render_markdown_with_width(&self.streaming_text, Some(width))
    }
    ...
    fn auth_status(&self) -> pryx::auth::AuthStatus {
        pryx::auth::AuthStatus::default()
    }

    fn diagram_mode(&self) -> pryx::config::DiagramDisplayMode {
        pryx::config::DiagramDisplayMode::Pinned
    }
    ...
    fn diagram_pane_position(&self) -> pryx::config::DiagramPanePosition {
        pryx::config::DiagramPanePosition::default()
    }
    ...
    fn side_panel(&self) -> &pryx::side_panel::SidePanelSnapshot {
        &self.side_panel
    }
    ...
    fn chat_native_scrollbar(&self) -> bool {
        pryx::config::config().display.native_scrollbars.chat
    }

    fn side_panel_native_scrollbar(&self) -> bool {
        pryx::config::config().display.native_scrollbars.side_panel
    }

    fn diff_line_wrap(&self) -> bool {
        true
    }
    fn inline_interactive_state(&self) -> Option<&pryx::tui::InlineInteractiveState> {
        None
    }
    ...
    fn session_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<pryx::tui::session_picker::SessionPicker>> {
        None
    }

    fn login_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<pryx::tui::login_picker::LoginPicker>> {
        None
    }

    fn account_picker_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<pryx::tui::account_picker::AccountPicker>> {
        None
    }

    fn usage_overlay(
        &self,
    ) -> Option<&std::cell::RefCell<pryx::tui::usage_overlay::UsageOverlay>> {
        None
    }
    ...
    fn copy_badge_ui(&self) -> pryx::tui::CopyBadgeUiState {
        pryx::tui::CopyBadgeUiState::default()
    }
    ...
    fn copy_selection_range(&self) -> Option<pryx::tui::CopySelectionRange> {
        self.copy_selection_range
    }

    fn copy_selection_status(&self) -> Option<pryx::tui::CopySelectionStatus> {
        None
    }
    ...
    fn cache_ttl_status(&self) -> Option<pryx::tui::CacheTtlInfo> {
        None
    }
}
...
fn main() -> Result<()> {
    if std::env::var("PRYX_TUI_PROFILE").is_ok() {
        pryx::logging::init();
        if let Some(path) = pryx::logging::log_path() {
    ...
    if matches!(args.mode, BenchMode::MermaidFlicker) {
        let result = pryx::tui::mermaid::debug_flicker_benchmark(args.frames.max(4));
    ...
    if matches!(args.mode, BenchMode::ImageScroll) {
        let result = pryx::tui::mermaid::debug_image_scroll_benchmark(
            args.images,
            args.frames.max(4),
            args.images_visible,
        );
    ...
    if matches!(args.mode, BenchMode::FileDiff) {
        state.diff_mode = pryx::config::DiffDisplayMode::File;
    }
    ...
    if profile_side_panel {
        pryx::tui::mermaid::init_picker();
        pryx::tui::mermaid::clear_active_diagrams();
        pryx::tui::mermaid::clear_streaming_preview_diagram();
        pryx::tui::clear_side_panel_render_caches();
        pryx::tui::reset_side_panel_debug_stats();
        pryx::tui::markdown::reset_debug_stats();
        pryx::tui::mermaid::reset_debug_stats();
        if !args.keep_mermaid_cache {
            let _ = pryx::tui::mermaid::clear_cache();
        }
    ...
    for frame in 0..args.frames {
        ...
        if matches!(args.mode, BenchMode::CopySelection) {
            ...
            state.copy_selection_range = Some(pryx::tui::CopySelectionRange {
                start: pryx::tui::CopySelectionPoint {
                    pane: pryx::tui::CopySelectionPane::Chat,
                    abs_line: start_line,
                    column: 0,
                },
                end: pryx::tui::CopySelectionPoint {
                    pane: pryx::tui::CopySelectionPane::Chat,
                    abs_line: end_line,
                    column: usize::MAX / 4,
                },
        ...
        let markdown_before = profile_side_panel.then(pryx::tui::markdown::debug_stats);
        let mermaid_before = profile_side_panel.then(pryx::tui::mermaid::debug_stats);
        let side_panel_before = profile_side_panel.then(pryx::tui::side_panel_debug_stats);
        let frame_start = Instant::now();
        terminal.draw(|f| pryx::tui::render_frame(f, &state))?;
            ...
            && let Some(range) = state.copy_selection_range
        {
            ...
            if let Some(text) = pryx::tui::debug_copy_selection_text_for_bench(range) {
        ...
        {
            let markdown_after = pryx::tui::markdown::debug_stats();
            let mermaid_after = pryx::tui::mermaid::debug_stats();
            let side_panel_after = pryx::tui::side_panel_debug_stats();
    ...
    let side_panel_final_stats = profile_side_panel.then(pryx::tui::side_panel_debug_stats);
    let markdown_final_stats = profile_side_panel.then(pryx::tui::markdown::debug_stats);
    let mermaid_final_stats = profile_side_panel.then(pryx::tui::mermaid::debug_stats);
    let mermaid_ui_summary = if profile_mermaid_ui {
        Some(summarize_mermaid_ui(
            &side_panel_profiles,
            pryx::tui::mermaid::protocol_type().is_some(),
            pryx::tui::mermaid::protocol_type().map(|p| format!("{:?}", p)),
        ))
    } else {
        None
    };
    let actual_policy = summarize_policy("detected", pryx::perf::tui_policy());
    let synthetic_policy = args.synthetic_profile.map(|kind| {
        let synthetic = pryx::perf::synthetic_profile(kind.to_system_profile());
        summarize_policy(
            kind.to_system_profile().label(),
            tui_policy_for(&synthetic, &pryx::config::config().display),
        )
    });
