...
use clap::Parser;
use pryx::id::new_id;
use pryx::message::{Message, ToolDefinition};
use pryx::provider::{EventStream, Provider};
use pryx::tool::{Registry, ToolContext, ToolExecutionMode};
use serde_json::json;
use std::path::PathBuf;
use std::sync::Arc;

#[derive(Parser)]
#[command(name = "pryx-harness")]
...
fn create_temp_workspace() -> Result<PathBuf> {
    ...
    path.push(format!("pryx-harness-{}", new_id("run")));
    Ok(path)
}
