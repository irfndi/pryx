...
//! REAL pryx retrieval primitives:
//!   - `pryx::memory_graph::MemoryGraph` deserialization (real on-disk graphs)
//!   - `pryx::embedding::embed` (real all-MiniLM-L6-v2 ONNX model)
//!   - `pryx::memory::format_context_for_relevance` (real live query window)
//!   - a faithful re-implementation of `score_and_filter` (cosine + gap filter)
//!
//! Privacy: all data lives OUTSIDE the repo (default `~/pryx-memory-bench`).
...
use pryx::embedding;
use pryx::memory::format_context_for_relevance;
use pryx::memory_graph::MemoryGraph;
use pryx::session::Session;
...
fn bench_root() -> PathBuf {
    std::env::var("MEMORY_BENCH_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| dirs_home().join("pryx-memory-bench"))
}
...
fn cmd_queries(args: &[String]) -> Result<()> {
    ...
    let sessions_dir = opts
        .get("sessions")
        .cloned()
        .unwrap_or_else(|| format!("{}/.pryx/sessions", dirs_home().display()));
    ...
    for (path, _) in sessions {
        ...
        let user_turns: Vec<usize> = messages
            .iter()
            .enumerate()
            .filter(|(_, m)| matches!(m.role, pryx::message::Role::User))
            .map(|(i, _)| i)
...
// ---------------- LLM judge (direct Anthropic via pryx Sidecar) ----------------
...
//
// The prompt/parse/rerank logic lives in the shared `pryx::memory_rerank`
...
use pryx::memory_rerank::{LLM_RERANK_SYSTEM, build_rerank_prompt, parse_rerank_response};
            ...
            .map(|input| {
                ...
                async move {
                    let sidecar = if backend == "openai" {
                        ...
                        pryx::sidecar::Sidecar::with_openai_model(&model, eff)
                    } else {
                        pryx::sidecar::Sidecar::with_claude_model(&model)
                    };
...
fn cmd_metrics(args: &[String]) -> Result<()> {
    ...
    // For `prod_hybrid`: seed a temp PRYX_HOME project graph with the corpus and
    ...
    let prod_mgr = if config == "prod_hybrid" {
        ...
        unsafe { std::env::set_var("PRYX_HOME", &tmp) };
        let project_dir = "/bench/prod-validate";
        let mgr = pryx::memory::MemoryManager::new().with_project_dir(project_dir);
    ...
    let alt_embedder = opts.get("embedder").map(|dir| {
        ...
        pryx::embedding::Embedder::load_from_dir(Path::new(dir)).expect("load alt embedder")
    });
    ...
    let openai_backend = if config == "openai_dense" || config == "openai_hybrid" {
        use pryx::embedding_backend::{DEFAULT_OPENAI_EMBEDDING_MODEL, OpenAiEmbeddingBackend};
        let model = opts
            .get("openai_model")
            .cloned()
        ...
        let key = opts
            ...
            .or_else(|| {
                pryx::provider_catalog::load_api_key_from_env_or_config(
                    "OPENAI_API_KEY",
                    "openai.env",
                )
            })
    ...
    let openai_corpus_emb: Vec<(String, Vec<f32>)> = match openai_backend.as_ref() {
        Some(b) => {
            use pryx::embedding_backend::EmbeddingBackend;
    ...
    let ce_reranker = opts.get("reranker").map(|dir| {
        ...
        pryx::embedding::CrossEncoder::load_from_dir(Path::new(dir)).expect("load reranker")
    });
        ...
        let raw: Vec<RecallResultRow> = rt.block_on(async {
            ...
            stream::iter(jobs)
                .map(|(qid, query, cands)| {
                    ...
                    async move {
                        let sidecar = if backend == "openai" {
                            pryx::sidecar::Sidecar::with_openai_model(&model, None)
                        } else {
                            pryx::sidecar::Sidecar::with_claude_model(&model)
                        };
                        let n = cands.len();
            ...
            stream::iter(jobs)
                .map(|(qid, query, cands)| {
                    ...
                    async move {
                        ...
                        let mut out_chars = 0usize;
                        for _ in 0..n_ens {
                            let sidecar = pryx::sidecar::Sidecar::with_claude_model(&model);
                            for attempt in 0..2 {
                                match sidecar.complete(JUDGE_SYSTEM, &prompt).await {
                                    Ok(resp) => {
                                        out_chars += resp.len();
...
fn cmd_probe(args: &[String]) -> Result<()> {
    ...
    let e = pryx::embedding::Embedder::load_from_dir(Path::new(&dir))?;
