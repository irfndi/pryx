...
pub use pryx_selfdev_types::CLIENT_SELFDEV_ENV;
pub use pryx_selfdev_types::client_selfdev_requested;

const PRYX_REPO_URL: &str = "https://github.com/pryx-co/pryx.git";

fn selfdev_clone_dir() -> Result<PathBuf> {
    Ok(crate::storage::pryx_dir()?.join("source").join("pryx"))
}
...
fn resolve_or_clone_repo_dir() -> Result<PathBuf> {
    ...
    if repo_dir.exists() {
        ...
        anyhow::bail!(
            "Self-dev source directory exists but is not a pryx repository: {}\n\
    ...
    output::stderr_info(format!(
        "No local pryx checkout found; cloning self-dev source into {}...",
        repo_dir.display()
    ));
    ...
    if !build::is_pryx_repo(&repo_dir) {
        anyhow::bail!(
            "Cloned self-dev source is not a valid pryx repository: {}",
            repo_dir.display()
        );
    }
...
pub async fn run_self_dev(should_build: bool, resume_session: Option<String>) -> Result<()> {
    ...
    crate::env::set_var("PRYX_REPO_DIR", &repo_dir);
    ...
    if !target_binary.exists() {
        anyhow::bail!(
            "No binary found at {:?}\n\
             Run 'pryx self-dev --build' first, or build with '{}' and then publish current.",
            target_binary,
    ...
    if is_resume {
        crate::env::set_var("PRYX_RESUMING", "1");
    }
    ...
    super::tui_launch::run_tui_client(Some(session_id), None, !server_running, false, None, false)
        .await
}
#[cfg(test)]
