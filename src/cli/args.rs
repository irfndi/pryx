...
#[derive(Parser, Debug)]
#[command(name = "pryx")]
#[command(version = pryx_build_meta::version())]
#[command(about = "J-Code: A coding agent using Claude Max or ChatGPT Pro subscriptions")]
pub(crate) struct Args {
    /// Initial provider to use (pryx, claude, openai, openai-api, openrouter, azure, opencode, opencode-go, zai, 302ai, baseten, cortecs, comtegra, deepseek, fpt, firmware, huggingface, moonshotai, nebius, scaleway, stackit, groq, mistral, perplexity, togetherai, deepinfra, xai, grok-build, nvidia-nim, lmstudio, ollama, chutes, cerebras, alibaba-coding-plan, openai-compatible, cursor, copilot, gemini, antigravity, google, or auto-detect). Interactive sessions can switch providers with /model.
    ...
    /// Disable auto-detection of pryx repository and self-dev mode
    #[arg(long, global = true)]
...
pub(crate) enum Command {
    ...
    /// Run as an Agent Client Protocol (ACP) adapter backed by the Pryx daemon
    Acp,

    /// Manage the background server daemon (e.g. `pryx server stop`).
    Server {
        #[command(subcommand)]
    ...
    Login {
        /// Provider to log in to. Equivalent to --provider for this command, e.g. `pryx login google`.
        // Distinct clap id: the global `--provider` flag also has id "provider";
        // sharing the id makes clap drop the flag inside `login` (so
        // `pryx login --provider x` errors) and propagate the global default
        ...
        /// OpenAI-compatible API key. If omitted, pryx prompts securely when needed.
        #[arg(long)]
    ...
    /// Log in to and manage your Pryx account
    Account {
        #[command(subcommand)]
    ...
    /// Update pryx to the latest version
    Update,
    ...
    /// Debug socket CLI - interact with running pryx server
    Debug {
    ...
    /// Optional Pryx Cloud/Jade integration commands
    #[command(subcommand)]
    Cloud(CloudCommand),
    ...
    /// Inject externally transcribed text into the active Pryx TUI
    Transcript {
        /// Transcript text. If omitted, reads from stdin.
        text: Option<String>,

        /// How to apply the transcript inside Pryx
    ...
    /// Run configured dictation: send to last-focused pryx client or type raw text
    Dictate {
        /// Type the transcript into the focused app instead of sending to pryx
        #[arg(long)]
        r#type: bool,
    },

    /// Set up the platform global hotkey to launch pryx
    SetupHotkey {
    ...
    /// Install a launcher so pryx appears in your app launcher
    SetupLauncher,
    ...
    /// Save or restore the current set of open pryx windows across a system reboot
    Restart {
        #[command(subcommand)]
    ...
    ///
    /// This is the endpoint the TypeScript SDK (`@1jehuang/pryx-sdk`) connects to. It
    ...
    ApiBridge {
        /// Path of the API socket to listen on (default: $XDG_RUNTIME_DIR/pryx-api.sock)
        ///
...
pub(crate) enum AccountCommand {
    ...
    /// Open the public Pryx account management page
    Manage,
...
pub(crate) enum CloudSessionsCommand {
    ...
    Upload {
        /// Path to a local Pryx session JSON file
        session_file: String,
    ...
    /// Upload the newest local Pryx session to Jade cloud storage
    UploadLatest {
        /// Directory containing local Pryx session JSON files
        #[arg(long, default_value = "~/.pryx/sessions")]
        sessions_dir: String,
    ...
    Sync {
        /// Directory containing local Pryx session JSON files (default: ~/.pryx/sessions)
        #[arg(long)]
...
pub(crate) struct JadeCloudOptions {
    ...
    /// Path to the private Jade session helper. Defaults to $PRYX_JADE_SESSIONS_HELPER or ~/jade/scripts/jade_sessions.py.
    #[arg(long)]
...
pub(crate) enum RestartCommand {
    /// Save a reboot snapshot of currently active pryx windows
    Save {
        /// Restore this reboot snapshot automatically the next time plain `pryx` starts
        #[arg(long)]
        auto_restore: bool,
    },
...
pub(crate) enum ProviderCommand {
    ...
    Add {
        ...
        /// API key value to store in pryx's private provider env file. Prefer --api-key-stdin for shell history safety.
        #[arg(long, conflicts_with_all = ["api_key_stdin", "no_api_key"])]
        api_key: Option<String>,

        /// Read the API key from stdin and store it in pryx's private provider env file
        #[arg(long, conflicts_with = "no_api_key")]
        api_key_stdin: bool,
        ...
        /// Private env file name under pryx's app config directory for stored API keys
        #[arg(long)]
