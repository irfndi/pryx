...
pub async fn run() -> Result<()> {
    ...
    std::thread::Builder::new()
        .name("pryx-memlog-cleanup".to_string())
    ...
    std::thread::Builder::new()
        .name("pryx-session-bak-prune".to_string())
        .spawn(crate::session::prune_old_session_backups)
        .ok();
    logging::info("pryx starting");
    ...
    // provider registry. These crates sit downstream of pryx-base (so
    ...
    // particular, a first-ever `pryx telemetry disable` must not emit the
...
/// Register provider runtimes that live downstream of `pryx-base` with the
...
pub fn register_external_provider_runtimes() {
    crate::provider::external::register_external_provider(
        ...
        || std::sync::Arc::new(pryx_provider_grok_build_runtime::GrokBuildProvider::new()),
    );
    crate::provider::external::register_external_provider(
    ...
    crate::provider::external::register_external_provider(
        ...
        || std::sync::Arc::new(pryx_provider_antigravity_runtime::AntigravityProvider::new()),
    );
    crate::provider::external::register_external_provider(
        crate::provider::external::CLAUDE_CLI_RUNTIME,
        || std::sync::Arc::new(pryx_provider_claude_cli_runtime::ClaudeProvider::new()),
    );
    crate::provider::external::register_external_provider(
        crate::provider::external::ANTHROPIC_RUNTIME,
        || std::sync::Arc::new(pryx_provider_anthropic_runtime::AnthropicProvider::new()),
    );
    ...
    crate::provider::external::register_external_provider_fallible(
        ...
        || {
            let provider = match crate::auth::codex::load_credentials() {
                Ok(credentials) => pryx_provider_openai_runtime::OpenAIProvider::new(credentials),
                Err(_) => pryx_provider_openai_runtime::OpenAIProvider::new_browser_only(),
            };
    ...
    crate::provider::external::register_external_provider_fallible(
        ...
        || {
            let provider = std::sync::Arc::new(
                pryx_provider_copilot_runtime::CopilotApiProvider::new().ok()?,
            );
            let eager_tier_detection = std::env::var("PRYX_NON_INTERACTIVE").is_err();
...
fn parse_and_prepare_args() -> Result<Args> {
    ...
    if args.trace {
        crate::env::set_var("PRYX_TRACE", "1");
    }
                ...
                && update_available
            {
                ...
                if hot_exec::local_commits_ahead_of_upstream() == Some(true) {
                    ...
                    Bus::global().publish(BusEvent::UpdateStatus(UpdateStatus::Available {
                        current: pryx_build_meta::version().to_string(),
                    ...
                    if auto_update {
                        ...
                        logging::info(
                            "Update available! Run `pryx update` or `/reload` to update.",
                        );
                    }
                }
            } else {
...
fn report_main_error(error: &anyhow::Error) {
    ...
    if let Some(session_id) = terminal::get_current_session() {
        ...
        output::stderr_info(format!("  pryx --resume {}", session_id));
        output::stderr_blank_line();
    }
}
...
mod tests {
    ...
    fn auto_install_allowed_without_live_terminal() {
        let args = parse_args(&["pryx", "login"]);
    ...
    fn auto_install_respects_explicit_disable_even_without_terminal() {
        let mut args = parse_args(&["pryx", "login"]);
    ...
    fn update_command_still_skips_background_check_before_auto_install_logic() {
        let args = parse_args(&["pryx", "update"]);
    ...
    fn hidden_spawn_hotkey_argument_is_global_and_preserves_canonical_text() {
        let args = parse_args(&["pryx", "--spawn-hotkey", "shift+cmd+'", "self-dev"]);
