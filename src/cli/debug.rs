...
pub async fn run_debug_command(
    ...
    if !crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("\nMake sure:");
        eprintln!("  1. A pryx server is running (pryx or pryx serve)");
        eprintln!("  2. debug_socket is enabled in ~/.pryx/config.toml");
        eprintln!("     [display]");
        eprintln!("     debug_socket = true");
        eprintln!("\nOr use 'pryx debug start' to start a server.");
        eprintln!("Use 'pryx debug list' to see running servers.");
...
async fn debug_list_servers() -> Result<()> {
    ...
    if servers.is_empty() {
        println!("No running pryx servers found.");
        println!("\nStart one with: pryx debug start");
        return Ok(());
    }

    println!("Running pryx servers:\n");
    ...
    println!("  pryx debug -s /path/to/socket.sock sessions");
...
async fn debug_start_server(arg: &str, socket_path: Option<String>) -> Result<()> {
            ...
            .is_ok()
    {
        ...
        eprintln!("Use 'pryx debug list' to see all servers.");
        return Ok(());
    }
    ...
    if crate::transport::is_socket_path(&debug_socket) {
        ...
        eprintln!("⚠ Debug socket not enabled. Add to ~/.pryx/config.toml:");
        eprintln!("  [display]");
        eprintln!("  debug_socket = true");
    }
