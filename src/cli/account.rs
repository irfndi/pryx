...
pub(crate) async fn run_login(no_browser: bool) -> Result<()> {
    super::login::run_pryx_account_login(no_browser).await
}

pub(crate) async fn run_status(json: bool) -> Result<()> {
    let Some(api_key) = crate::subscription_catalog::configured_api_key() else {
        anyhow::bail!(
            "No Pryx account credential is configured. Run `pryx account login` to sign in."
        );
    };
    ...
    match subscription_api::fetch_subscription_me_with(&client, &api_base, &api_key).await {
        ...
        Err(AccountApiError::Unauthorized) => {
            ...
            anyhow::bail!(
                "The Pryx account key was revoked or expired. Local credentials were cleared. Run `pryx account login` to sign in again."
            )
        }
...
pub(crate) fn run_manage() -> Result<()> {
    ...
    println!("Opening Pryx account management: {url}");
...
pub(crate) async fn run_logout() -> Result<()> {
    ...
    crate::subscription_catalog::clear_account_credentials()
        .context("Failed to securely clear local Pryx account credentials")?;
    crate::auth::AuthStatus::invalidate_cache();

    match (api_key.is_some(), remote) {
        (false, _) => {
            println!("No local Pryx account credential was present. Local account cache is clear.")
        }
        (true, Ok(())) => println!(
            "Pryx account key revoked. Local credentials and account cache were securely cleared."
        ),
        (true, Err(AccountApiError::Unauthorized)) => println!(
            "The Pryx account key was already revoked. Local credentials and account cache were securely cleared."
        ),
            ...
            matches!(
                ...
                Ok(parsed)
                    if parsed.scheme() == "https"
                        && matches!(parsed.host_str(), Some("pryx.sh" | "www.pryx.sh" | "solosystems.dev"))
                        && parsed.username().is_empty()
                        && parsed.password().is_none()
            )
        })
        .unwrap_or(crate::subscription_catalog::PRYX_ACCOUNT_URL)
}
...
mod tests {
    ...
    fn manage_url_accepts_only_public_allowlisted_https_origins() {
        assert_eq!(
            public_manage_url(Some("https://pryx.sh/account")),
            "https://pryx.sh/account"
        );
        assert_eq!(
            public_manage_url(Some("https://evil.example/?key=jck_live_secret")),
            crate::subscription_catalog::PRYX_ACCOUNT_URL
        );
        assert_eq!(
            public_manage_url(Some("https://user:pass@pryx.sh/account")),
            crate::subscription_catalog::PRYX_ACCOUNT_URL
        );
    }
}
