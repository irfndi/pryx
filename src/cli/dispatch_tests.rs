...
fn only_file_controlled_debug_clients_need_parent_lifetime_binding() {
    ...
    let previous = std::env::var_os("PRYX_DEBUG_CMD_PATH");
    crate::env::remove_var("PRYX_DEBUG_CMD_PATH");
    assert!(!is_file_controlled_debug_client());

    crate::env::set_var("PRYX_DEBUG_CMD_PATH", "/tmp/pryx-test-debug-command");
    assert!(is_file_controlled_debug_client());

    if let Some(previous) = previous {
        crate::env::set_var("PRYX_DEBUG_CMD_PATH", previous);
    } else {
        crate::env::remove_var("PRYX_DEBUG_CMD_PATH");
    }
}
...
fn init_and_systemd_are_recognized_as_orphan_adopters() {
    ...
    assert!(!is_orphan_adopter_name("pryx\n"));
}
...
fn auth_doctor_positional_provider_wins_over_global_provider() {
    assert_eq!(
        ...
        Some("openai"),
        "`pryx --provider cerebras auth doctor openai` should diagnose the explicit positional provider"
    );
}
...
impl Drop for ReloadTestEnv {
    fn drop(&mut self) {
        ...
        if let Some(prev_socket) = &self.prev_socket {
            crate::env::set_var("PRYX_SOCKET", prev_socket);
        } else {
...
fn resume_failure_defers_to_server_during_reload_handoff() {
    ...
    // update/restart handoff is in progress (PRYX_RESUMING set), we must defer
    // to the server instead of exiting with "No session found matching ...".
    let with_resuming = |key: &str| {
        if key == "PRYX_RESUMING" {
