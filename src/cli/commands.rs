...
fn cloud_sessions_config_path() -> Result<PathBuf> {
    Ok(crate::storage::pryx_dir()?.join("cloud_sessions.json"))
}
...
fn cloud_sessions_sync_state_path() -> Result<PathBuf> {
    Ok(crate::storage::pryx_dir()?.join("cloud_sessions_sync.json"))
}
...
fn resolve_sync_sessions_dir(override_path: Option<&str>) -> Result<PathBuf> {
    ...
    Ok(crate::storage::pryx_dir()?.join("sessions"))
}
...
fn resolve_jade_sessions_helper(override_path: Option<&str>) -> Result<PathBuf> {
    ...
    if let Some(path) = std::env::var_os("PRYX_JADE_SESSIONS_HELPER")
    ...
    anyhow::bail!(
        "Could not find Jade session helper. Set --helper PATH or PRYX_JADE_SESSIONS_HELPER. Expected a private helper like ~/jade/scripts/jade_sessions.py"
    );
}
...
async fn run_ambient_visible() -> Result<()> {
    ...
    let _ = crossterm::execute!(
        std::io::stdout(),
        crossterm::terminal::SetTitle(terminal_title("🤖 pryx ambient cycle"))
    );
...
pub fn run_memory_command(cmd: MemorySubcommand) -> Result<()> {
    ...
    match cmd {
        ...
        MemorySubcommand::ClearTest => {
            let test_dir = storage::pryx_dir()?.join("memory").join("test");
            if test_dir.exists() {
...
pub fn run_pair_command(list: bool, revoke: Option<String>) -> Result<()> {
    ...
    if connect_host == gateway::UNKNOWN_CONNECT_HOST {
        eprintln!(
            "\n  \x1b[33mTip:\x1b[0m set PRYX_GATEWAY_HOST to your reachable Tailscale hostname."
        );
    }
    ...
    match action {
        ...
        "status" => {
            ...
            if status.ready {
                ...
                println!(
                    "\nThe browser bridge is connected, but the installed Firefox extension is out of date for this pryx build. Run `pryx browser setup` to repair or update it."
                );
            } else {
                println!("\nRun `pryx browser setup` to install or repair it.");
            }
        }
        other => {
...
pub async fn run_server_reload_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    // `pryx server reload` unconditionally after swapping the binary.
    if !crate::server::has_live_listener(&socket).await {
        ...
        let detail = if reaped {
            "No running pryx server found; cleared a stale socket.".to_string()
        } else {
            "No running pryx server found; nothing to reload.".to_string()
        };
    ...
    if skipped && !reloading {
        return emit(ServerReloadReport {
            ...
            handoff_ready: true,
            detail: "pryx server is already running the newest binary; no reload needed."
                .to_string(),
        });
    }
    ...
    let detail = if handoff_ready {
        "pryx server reloaded onto the newest binary.".to_string()
    } else {
        "pryx server reload requested; the new server is still coming up.".to_string()
    };
...
pub async fn run_server_stop_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    if !force {
        let msg = "`pryx server stop` terminates the daemon and drops any live headless/swarm sessions. \
Prefer `pryx server reload` to pick up an upgrade gracefully. \
Re-run with `--force` if you really want to stop the server.";
    ...
    if let Some(info) = server_info.as_ref() {
        ...
        if crate::platform::is_process_running(pid) {
            #[cfg(unix)]
            {
                ...
                match crate::platform::signal_detached_process_group(pid, libc::SIGTERM) {
                    Ok(()) => {
                        ...
                        detail = format!("Sent SIGTERM to pryx server (pid {pid}).");
                    }
                    Err(e) => {
                        detail = format!("Failed to signal pryx server (pid {pid}): {e}");
                    }
                }
            }
            #[cfg(not(unix))]
            {
                match crate::platform::signal_detached_process_group(pid, 0) {
                    Ok(()) => {
                        signaled_pid = Some(pid);
                        detail = format!("Terminated pryx server (pid {pid}).");
                    }
                    Err(e) => {
                        detail = format!("Failed to terminate pryx server (pid {pid}): {e}");
                    }
                }
            }
        } else {
            detail = format!("Registered pryx server (pid {pid}) is not running.");
        }
        ...
        detail = "No running pryx server found.".to_string();
    }
    ...
    if emit_json {
        ...
        if stopped && signaled_pid.is_some() {
            println!("pryx server stopped.");
        } else if stopped && !had_listener && signaled_pid.is_none() {
            // Nothing was running; this is still a success for an installer.
        } else if !stopped {
            println!(
                "pryx server did not exit cleanly; it may still be shutting down. Re-run if needed."
            );
        }
        if reaped {
            println!("Cleared a stale pryx socket.");
        }
    }
...
pub async fn run_single_message_command(
    ...
    // Load MCP servers from ~/.pryx/mcp.json so headless `pryx run` has the
    ...
    if run_command_mcp_enabled() {
        ...
        // for it, and a single-turn `pryx run` locks its tool snapshot before
        // the background connection finishes, so the model would never see those
        // tools. Long-lived sessions recover on a later turn, but `pryx run`
...
fn run_command_auto_poke_enabled() -> bool {
    std::env::var("PRYX_RUN_AUTO_POKE")
        .ok()
        .map(|value| {
...
/// Whether headless `pryx run` should load MCP servers from `~/.pryx/mcp.json`.
/// Enabled by default; set `PRYX_RUN_MCP=0` (or `false`/`off`/`no`) to skip MCP
/// registration for latency-sensitive scripting. (#390)
fn run_command_mcp_enabled() -> bool {
    std::env::var("PRYX_RUN_MCP")
        .ok()
        .map(|value| {
            let value = value.trim().to_ascii_lowercase();
            !matches!(value.as_str(), "0" | "false" | "off" | "no")
...
/// Max time `pryx run` waits for cold-cache MCP servers to register their
/// tools before running the single turn. Override with `PRYX_RUN_MCP_WAIT_MS`
/// (0 disables the wait).
fn run_command_mcp_cold_wait() -> std::time::Duration {
    let ms = std::env::var("PRYX_RUN_MCP_WAIT_MS")
        .ok()
...
/// Bridge the cold-cache gap for `pryx run`: if any configured MCP server has
...
async fn wait_for_cold_cache_mcp_tools(registry: &crate::tool::Registry) {
    ...
    crate::logging::info(&format!(
        "pryx run: waiting up to {}ms for cold-cache MCP server(s) to register tools: {}",
        budget.as_millis(),
        cold_servers.join(", ")
    ));
    ...
    loop {
        ...
        if covered {
            crate::logging::info(
                "pryx run: cold-cache MCP server(s) registered tools; proceeding",
            );
            return;
        }
        if std::time::Instant::now() >= deadline {
            crate::logging::warn(
                "pryx run: timed out waiting for cold-cache MCP server(s); \
...
fn run_command_auto_poke_max_turns() -> Option<usize> {
    std::env::var("PRYX_RUN_AUTO_POKE_MAX_TURNS")
        .ok()
    ...
    loop {
        ...
        match build_run_auto_poke_follow_up_from_todos(
            ...
            Some(RunAutoPokeFollowUp::GateDigest { message }) => {
                ...
                eprintln!(
                    "We asked the agent to double-check this turn's weak points. Set PRYX_RUN_AUTO_POKE=0 to disable."
                );
                continue;
            }
            Some(RunAutoPokeFollowUp::ConfidenceSummary {
                ...
                next_message = message;
                eprintln!(
                    "Todos are done. Asking the agent for a final confidence check. Set PRYX_RUN_AUTO_POKE=0 to disable."
                );
                continue;
            }
            Some(RunAutoPokeFollowUp::Incomplete { count, message }) => {
                ...
                next_message = message;
                eprintln!(
                    "{} incomplete todo(s). We poked the agent for you. Set PRYX_RUN_AUTO_POKE=0 to disable.",
                    count
                );
            }
            None => break,
