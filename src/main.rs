...
fn configure_system_allocator() {
    ...
    let arena_max = parse_alloc_tuning_env("PRYX_GLIBC_ARENA_MAX", 4);
    ...
    let mmap_threshold = parse_alloc_tuning_env("PRYX_GLIBC_MMAP_THRESHOLD", 256 * 1024);
...
fn main() -> Result<()> {
    ...
    // for every auxiliary binary and run the Pryx entry point on a deliberately
    // sized stack instead.
    const WINDOWS_MAIN_STACK_SIZE: usize = 8 * 1024 * 1024;
    match std::thread::Builder::new()
        .name("pryx-main".to_string())
...
fn run_main() -> Result<()> {
    ...
    // Codex. Handle this tiny callback before the Tokio runtime and normal Pryx
    ...
    if let Some(source) = cli_launch_hint_source_invocation() {
        return pryx::setup_hints::run_setup_hotkey(false, false, false, Some(&source));
    }
    ...
    if is_macos_hotkey_listener_invocation() {
        return pryx::setup_hints::run_macos_hotkey_listener_main_thread();
    }
    ...
    if pryx::cli::macos_notification_broker::is_invocation() {
        return pryx::cli::macos_notification_broker::run();
    }
    ...
    runtime.block_on(async { pryx::run().await })
}

/// True when invoked as `pryx setup-hotkey --listen-macos-hotkey`.
