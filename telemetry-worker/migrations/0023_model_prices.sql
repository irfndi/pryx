...
-- token flow through pryx: `events.input_tokens` / `output_tokens` /
...
-- crates/pryx-base/src/model_pricing.rs).
--
-- `model` is the raw telemetry label, NOT a models.dev id: users run pryx
...
--
-- Prices are list/rack rates. Most pryx traffic runs on subscriptions
...
CREATE TABLE IF NOT EXISTS model_prices (
    ...
    -- See crates/pryx-compaction-core/src/lib.rs (estimate_compaction_tokens)
    -- and pryx-provider-openai/src/stream.rs (extract_cached_input_tokens).
