...
[![Latest Release](https://badgen.net/github/release/pryx-co/pryx?icon=github)](https://github.com/pryx-co/pryx/releases)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue?style=flat-square)](LICENSE)
[![Platforms](https://img.shields.io/badge/platforms-Linux%20%7C%20macOS%20%7C%20Windows-blue?style=flat-square)](https://github.com/pryx-co/pryx/releases)
[![Last Commit](https://badgen.net/github/last-commit/pryx-co/pryx/master?icon=github)](https://github.com/pryx-co/pryx/commits/master)
[![GitHub Stars](https://badgen.net/github/stars/pryx-co/pryx?icon=github)](https://github.com/pryx-co/pryx/stargazers)
...
<a href="https://trendshift.io/repositories/25042?utm_source=repository-badge&amp;utm_medium=badge&amp;utm_campaign=badge-repository-25042" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/repositories/25042" alt="pryx-co/pryx | Trendshift" width="250" height="55"></a>

<a href="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-memory-demo.mp4">
  <img src="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-memory-demo.webp" alt="pryx memory demonstration" width="800">
</a>

<br>

[Website](https://pryx.sh) · [Docs](https://pryx.sh/docs) · [SDK](https://pryx.sh/sdk) · [Benchmarks](https://pryx.sh/bench) · [Features](#features) · [Install](#installation) · [Quick Start](#quick-start) · [Further Reading](#further-reading) · [Contributing](CONTRIBUTING.md)
...
```bash
# macOS & Linux
curl -fsSL https://pryx.sh/install | bash
```

```powershell
# Windows 11 (PowerShell 5.1+)
irm https://pryx.sh/install.ps1 | iex
```
...
pryx is built to be as performant and resource efficient as possible. Every metric is optimized to the bone, which is important for scaling multi-session workflows. Here we sample a few metrics to show the difference: RAM usage and boot up.
  ...
  <tr>
    <td valign="top" align="center" width="50%">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td><strong>pryx (local embedding off)</strong></td>
            <td align="right"><strong>27.8 MB</strong></td>
            <td align="right">baseline</td>
          </tr>
          <tr>
            <td><strong>pryx</strong></td>
...
|---|---:|---:|---:|
| **pryx** | **14.0 ms** | 10.1–19.3 ms | baseline |
...
|---|---:|---:|---:|
| **pryx** | **48.7 ms** | 30.3–62.7 ms | baseline |
...
|---|---:|---:|
| **pryx (local embedding off)** | **~9.9 MB** | baseline |
| **pryx** | **~10.4 MB** | **1.1× more RAM** |
...
<div align="center">
  ...
  <a href="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-performance-demo.mp4">
    <img src="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-performance-demo.webp" alt="pryx performance demonstration" width="900">
  </a>

  <p><em>pryx performance demonstration</em></p>
...
<div align="center">
  ...
  <a href="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-memory-demo.mp4">
    <img src="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-memory-demo.webp" alt="pryx memory demonstration" width="900">
  </a>

  <p><em>pryx memory demonstration</em></p>
...
The side panel is a place for auxiliary information. Tell your pryx agent to load a file into the side panel and see it update in real time, or tell your agent to write directly to the side panel, or use it as a diff viewer. The side panel (and chat) is able to render mermaid diagrams inline. 
...
Pryx can render at over a thousand fps. Your monitor will not have the refresh rate to show you, but this means you will not have silly flicker problems. 

The custom scrollback implementation of pryx allows it to do much more than a native scrollback. However, it is a terminal-level limitation that I cannot have smooth, partial line scrolling with a custom scrollback. To fix this, I made my own terminal. Handterm https://github.com/1jehuang/handterm implements a native scroll api, and also happens to be very efficient. This is a work in progress. Scrolling is still well implemented for normal terminals.

Pryx is left-aligned by default. You can switch to centered mode with the `Alt+C` hotkey, with the `/alignment` command, or in the config.

To disable emoji globally in TUI and CLI output, set `emoji = false` under `[display]` in `~/.pryx/config.toml`, or launch with `PRYX_NO_EMOJI=1`. Pryx replaces emoji with compact ASCII markers while preserving other Unicode text.
...
<div align="center">
  ...
  <a href="https://github.com/pryx-co/pryx/releases/download/readme-assets/swarm-demo.mp4">
    <img src="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-swarm-demonstration.webp" alt="pryx swarm demonstration" width="900">
  </a>

  <p><em>pryx swarm demonstration</em></p>
...
pryx works with subscription-backed OAuth flows and many provider integrations, so you can use the models you already pay for and still fall back to direct API providers when needed.

### Supported built-in login flows

- **Claude** (`pryx login --provider claude`)
- **OpenAI / ChatGPT / Codex** (`pryx login --provider openai`)
- **Google Gemini** (`pryx login --provider gemini`)
- **GitHub Copilot** (`pryx login --provider copilot`)
- **Azure OpenAI** (`pryx login --provider azure`)
- **Alibaba Cloud Coding Plan** (`pryx login --provider alibaba-coding-plan`)
- **Fireworks** (`pryx login --provider fireworks`)
- **MiniMax** (`pryx login --provider minimax`)
- **Meta Model API / Muse** (`pryx login --provider meta-muse`)
- **LM Studio** (`pryx login --provider lmstudio`)
- **Ollama** (`pryx login --provider ollama`)
- **Custom OpenAI-compatible endpoint** (`pryx login --provider openai-compatible`)

For custom OpenAI-compatible endpoints, pryx now prompts for the API base and supports local localhost servers without requiring an API key.

### Config-file setup for self-hosted endpoints and MCP

If you prefer to configure things by editing files instead of using the login UI, pryx supports both a custom OpenAI-compatible endpoint config and MCP config files.

#### OpenAI-compatible providers

Many hosted services speak the standard OpenAI `/v1/chat/completions` API. pryx talks to them through one shared OpenAI-compatible provider, so you can use almost any such endpoint without waiting for a dedicated integration.

There are two ways to set one up:

- **Built-in named profiles** — pryx ships ready-made profiles for several popular OpenAI-compatible services. Log in by id and pryx fills in the base URL and key environment variable for you:

  ```bash
  pryx login --provider <profile-id>
  # for example:
  pryx login --provider openrouter
  pryx login --provider deepseek
  pryx login --provider opencode      # OpenCode Zen
  pryx login --provider moonshotai
  pryx login --provider meta-muse     # Meta Model API / Muse Spark
  ```

  Built-in OpenAI-compatible profile ids include: `openrouter`, `deepseek`, `zai`, `kimi`, `moonshotai`, `meta-muse` (Meta Model API / Muse Spark), `opencode` (OpenCode Zen), `opencode-go`, `302ai`, `baseten`, `cortecs`, `huggingface`, `nebius`, `scaleway`, `stackit`, and `firmware`. Each profile only sets the endpoint and key variable; you still pick the model with `/model` (or `--model`). Run `pryx login` with no provider to see the interactive list.

- **Any other endpoint** — point pryx at an arbitrary OpenAI-compatible API (hosted or local) with `pryx login --provider openai-compatible` or the scriptable `pryx provider add` command described below.

Useful environment overrides for these endpoints:

- `PRYX_STREAM_IDLE_TIMEOUT_SECS` — raise the base streaming idle timeout (default 180s) for slow reasoning models that think silently before emitting tokens. High reasoning efforts scale this automatically (high 2x, xhigh 3x, max 4x). Also settable as `[provider] stream_idle_timeout_secs` in `config.toml`.
- Per-model `context_window` (alias `context_limit`) in a `[[providers.<name>.models]]` entry — set the context window when the endpoint has no usable `/v1/models` response, so pryx does not fall back to the generic 200k default.
...
For agents and scripts, the preferred path is the one-shot provider profile command. It writes a named profile to `~/.pryx/config.toml`, stores secrets in pryx's private app config directory when requested, and prints exact run/validation commands:

```bash
# Secret-safe setup for a hosted OpenAI-compatible API.
printf '%s' "$MY_API_KEY" | pryx provider add my-api \
...
pryx --provider-profile my-api auth-test --prompt 'Reply exactly PRYX_PROVIDER_SETUP_OK'

# Use it directly.
pryx --provider-profile my-api run 'hello'
```

For local servers that do not require auth:

```bash
pryx provider add local-vllm \
...
pryx login --provider ollama
pryx --provider ollama --model llama3.2 run 'hello'
...
pryx login --provider lmstudio
pryx --provider lmstudio --model '<model-id>' run 'hello'
```

Ollama and LM Studio both expose OpenAI-compatible `/v1/models` and `/v1/chat/completions` endpoints. pryx uses streaming chat completions, function/tool calling, and OpenAI-style image content for vision-capable local models. If a local server requires a token, enter it during `pryx login` or create a named profile with `--api-key-stdin`.
...
The generated profile can also be edited manually in `~/.pryx/config.toml`:
...
api_key_env = "PRYX_PROVIDER_MY_API_API_KEY"
...
2. For built-in profiles (e.g. `nvidia-nim`) or any endpoint, via the `PRYX_OPENAI_EXTRA_BODY` environment variable (a JSON object string). It can live in the provider's env file (`~/.config/pryx/nvidia-nim.env`) next to the API key:

   ```bash
   PRYX_OPENAI_EXTRA_BODY={"chat_template_kwargs":{"thinking":true,"reasoning_effort":"high"}}
   ```

Keys from `extra_body` are merged last and override any pryx-generated body field with the same name (`PRYX_OPENAI_EXTRA_BODY` wins over the config `extra_body` on key collisions). Invalid values are logged and ignored rather than failing the request.

The custom OpenAI-compatible provider reads overrides from environment variables or from an env file in pryx's app config directory. On Linux this is usually `~/.config/pryx/`, so the default file is usually:

```text
~/.config/pryx/openai-compatible.env
```

Example for a local or LAN vLLM server:

```bash
PRYX_OPENAI_COMPAT_API_BASE=http://192.168.1.50:8000/v1
PRYX_OPENAI_COMPAT_DEFAULT_MODEL=Qwen/Qwen3-Coder-30B-A3B-Instruct
...
- `pryx login --provider openai-compatible` can create or update this for you.
...
- `~/.pryx/mcp.json` for global MCP servers
- `.pryx/mcp.json` for project-local MCP servers
...
Claude Code config is read live on every load rather than copied into pryx's
...
For migration from Codex CLI, pryx still performs a one-time import from
`~/.codex/config.toml` into `~/.pryx/mcp.json` when the latter does not exist.
That imported file is then pryx-owned; later Codex changes are not synced
automatically. Imported environment values are copied too and may contain
secrets.

Both the canonical `mcpServers` key and pryx's historical `servers` key are accepted. pryx currently supports stdio (command-based) servers only; HTTP/SSE entries (`"type": "http"`/`"sse"`) are recognized and skipped with a log line.
...
For headless or SSH sessions, OAuth-style providers support `pryx login --provider <provider> --no-browser` (alias: `--headless`) so pryx prints the auth URL/QR and falls back to manual code or callback paste instead of trying to launch a local browser.
...
pryx login --provider openai --print-auth-url --json

# Step 2: complete later with the callback URL or auth code
pryx login --provider openai --callback-url 'http://localhost:1455/auth/callback?...'
pryx login --provider gemini --auth-code '...'
```
...
pryx login --provider copilot --print-auth-url --json
pryx login --provider copilot --complete

# Gmail/Google OAuth after credentials are already configured
pryx login --provider google --print-auth-url --google-access-tier readonly
pryx login --provider google --callback-url 'http://127.0.0.1:8456?...'
```

Pending scriptable login state is stored under `~/.pryx/pending-login/`, automatically expires, and stale entries are cleaned up when new scriptable logins start or resume.

For the built-in OpenAI login flow, pryx opens a local callback on
...
Pryx also supports easy multi-account switching. Ran out of tokens on your first ChatGPT Pro subscription? /account and quickly switch to your second. 

---

## Customizability / Self-Dev

Pryx is inventing a new form of customizability. One that doesn't limit you to what a plugin or extension can do. Tell your pryx agent to enter self dev mode, and it will start modifying its own source code. Pryx is optimized to iterate on itself. There is significant infrastructure around self developement, which allows it to edit, build, and test its own source code, then reload its own binary and continue work in your (potentially many) sessions, fully automatically. 

It is reccomended that you use a frontier model for this. The pryx codebase is not a simple one, and weaker models can make subtle, breaking changes. GPT 5.5 or the latest available frontier model works well.
...
The devil is in the details. There are many undocumented optimizations and niceties that pryx implements. Some examples: 
...
pryx comes with instructions on how to set up Firefox Agent Bridge. Ask you agent to set it up, and then you will have browser automation in pryx as well. 

Agent grep is a grep tool I made for the pryx agent. It adds file strucuture information (ie the list of functions, their displacement, etc) to the grep return, so that the agent can infer more of what the file doesn without actually reading the file. It also implements a harness-level integration that adaptively truncates returns based on what the agent has already seen. This saves on context a lot. 
...
Resume sessions from different harnesses. Claude code broke on you? Resume the session from pryx and continue where you left off. Session resume is supported for codex, claude code, opencode, and pi. 
...
A native iOS application version of pryx is coming soon. This will allow you to work with pryx on your personal machine's environment from your phone, via Tailscale. Openclaw like features will be bundled with this iOS application. 
...
pryx supports interactive TUI use, non-interactive runs, persistent server/client workflows,
and hotkey-friendly dictation without requiring a bundled speech-to-text stack.

<div align="center">

  <a href="https://github.com/pryx-co/pryx/releases/download/readme-assets/workflow.mp4">
    <img src="https://github.com/pryx-co/pryx/releases/download/readme-assets/pryx-workflow-demonstration.webp" alt="pryx workflow demonstration" width="900">
  </a>

  <p><em>pryx workflow demonstration</em></p>
...
pryx includes a first-class built-in `browser` tool for browser control inside agent sessions.
...
- [pryx.sh/docs](https://pryx.sh/docs) — install, providers, configuration, keybindings
- [pryx.sh/swarm](https://pryx.sh/swarm) — many coding agents in one repository
- [pryx.sh/sdk](https://pryx.sh/sdk) — TypeScript SDK: drive pryx sessions from your own program
- [pryx.sh/bench](https://pryx.sh/bench) — benchmark methodology and results
...
If you want another agent to set up pryx for you, give it this prompt:

```text
Set up pryx on this machine for me.

1. Detect the operating system, available package managers, and shell environment, then install pryx using the best matching command below instead of referring me somewhere else:

   - macOS with Homebrew available:
     brew tap pryx-co/pryx
     brew install pryx
   ...
   - From source if the above paths are not appropriate:
     git clone https://github.com/pryx-co/pryx.git
     cd pryx
     cargo build --release
     scripts/install_release.sh

   - For local self-dev / refactor work on Linux x86_64, prefer:
     scripts/dev_cargo.sh build --release -p pryx --bin pryx
     scripts/dev_cargo.sh --print-setup
     scripts/install_release.sh

2. Verify that `pryx` is on my `PATH`.
3. Launch `pryx` once in a new terminal window/session to confirm it starts successfully.
4. Before attempting any interactive login flow, assess which providers are already available non-interactively and prefer those first. Check existing local credentials, config files, CLI sessions, and environment variables such as:
   - Claude: `~/.pryx/auth.json`, `~/.claude/.credentials.json`, `~/.local/share/opencode/auth.json`, `ANTHROPIC_API_KEY`
   - OpenAI: `~/.pryx/openai-auth.json`, `~/.codex/auth.json`, `OPENAI_API_KEY`
   - Gemini: `~/.pryx/gemini_oauth.json`, `~/.gemini/oauth_creds.json`
   - GitHub Copilot: existing auth under `~/.config/github-copilot/`
   - Azure OpenAI: `~/.config/pryx/azure-openai.env`, `AZURE_OPENAI_*`, or an existing `az login`
   - OpenRouter: `OPENROUTER_API_KEY`
   - Fireworks: `~/.config/pryx/fireworks.env`, `FIREWORKS_API_KEY`
   - MiniMax: `~/.config/pryx/minimax.env`, `MINIMAX_API_KEY`
   - NVIDIA NIM: `~/.config/pryx/nvidia-nim.env`, `NVIDIA_API_KEY`
   - Alibaba Cloud Coding Plan: existing pryx config/env if present
5. Prefer whichever provider is already configured and verify it with `pryx auth-test --all-configured` or a provider-specific auth test when appropriate.
6. Only if no usable provider is already configured, guide me through the minimal manual step needed:
   - Claude: `pryx login --provider claude`
   - GitHub Copilot: `pryx login --provider copilot`
   - OpenAI: `pryx login --provider openai`
   - Gemini: `pryx login --provider gemini`
   - Azure OpenAI: `pryx login --provider azure`
   - Fireworks: `pryx login --provider fireworks`
   - MiniMax: `pryx login --provider minimax`
   - NVIDIA NIM: `pryx login --provider nvidia-nim`
   - Alibaba Cloud Coding Plan: `pryx login --provider alibaba-coding-plan`
   - OpenRouter: help me set `OPENROUTER_API_KEY`
   - Anthropic direct API: help me set `ANTHROPIC_API_KEY`
7. After setup, run a simple smoke test with `pryx run "say hello"` and confirm it works.
8. If I want browser automation, also check `pryx browser status`. If browser automation is not ready, run `pryx browser setup`, verify the built-in `browser` tool works, and explain any remaining manual step.
...
This is intended to be a copy-paste bootstrap prompt for pryx itself or any other coding agent.
...
```bash
curl -fsSL https://raw.githubusercontent.com/pryx-co/pryx/master/scripts/uninstall.sh | bash -s -- --yes
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/pryx-co/pryx/master/scripts/uninstall.sh | bash -s -- --purge --yes
```
