...
Status: draft
Owner: pryx
Audience: pryx core, browser bridge authors, adapter authors

## Why this exists

pryx should expose a single first-class `browser` tool while remaining compatible with multiple browser automation backends:
...
The protocol in this document defines the **normalized contract** between pryx and a browser provider.

This is intentionally **not** a demand that every bridge speak exactly the same native command language. Instead:

- pryx defines a **core semantic layer** it can rely on
...
1. **One first-class tool in pryx**
...
3. **Capability negotiation**
   - pryx should know what each provider can and cannot do.
...
- **browser tool**: the user/model-facing pryx tool.
...
- **browser session**: the provider's isolated session or attachment scope for a pryx session.
...
- direct Rust trait calls inside pryx
...
pryx should not care whether a provider uses tabs, contexts, profiles, or remote targets internally.
It only needs a stable handle it can reuse.

### `session.ensure`

Creates or reuses a browser session for a pryx session.

Request:

```json
{
  "client_session_id": "pryx_session_123",
  "browser_preference": "auto",
  "isolation": "per_pryx_session",
  "attach": "prefer",
  "persist": true,
  "metadata": {
    "owner": "agent",
...
pryx must not assume identifier shape or encode browser semantics into them.

---

## Normalized core methods

These are the semantics pryx can rely on.
...
### Rule 3: pryx core should only rely on normalized methods by default
...
- a pryx-side adapter knows how to use them safely
...
Semantics or qualities that influence pryx behavior:
...
## Recommended pryx integration policy

The pryx `browser` tool should:
...
4. Should `page.snapshot` support multiple named formats beyond `pryx.page_snapshot.v1`?
