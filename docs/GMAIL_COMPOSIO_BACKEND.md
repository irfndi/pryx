...
|---|---|---|---|
| `direct` (default) | Local Google OAuth tokens (`pryx login google`) | No third party in the loop | Unverified-app warning; 7-day refresh-token expiry in Google "Testing" mode |
...
- `PRYX_GMAIL_BACKEND=direct` (or unset) -> direct Google backend.
- `PRYX_GMAIL_BACKEND=composio` -> Composio backend (requires `COMPOSIO_API_KEY`).

If `composio` is requested but `COMPOSIO_API_KEY` is missing, pryx warns and
...
1. pryx calls Composio's `POST /connected_accounts/link` (hosted "Connect
...
4. pryx polls `GET /connected_accounts/{id}` until the connection is `ACTIVE`,
   then persists it to `~/.pryx/composio_gmail.json`.
...
3. Export the variables:
   ```bash
   export PRYX_GMAIL_BACKEND=composio
...
- `direct`: honors the access tier chosen at `pryx login google`
