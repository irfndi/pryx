...
`AXIsProcessTrustedWithOptions(prompt=true)` (adds pryx to the list + shows the
...
Important: the permission attaches to the **host binary/terminal** running pryx.
For a stable experience we likely want a signed pryx.app with a fixed bundle id so
...
6. Signed pryx.app bundle for durable permissions.
...
2. **Requests** what it can programmatically:
   - `AXIsProcessTrustedWithOptions(prompt=true)` — shows the Accessibility dialog
     and pre-adds pryx to the list (toggled off).
...
- Ship/install pryx as a **signed `.app` bundle with a stable bundle id**
  (e.g. `com.pryx.app`) and a Designated Requirement, so the Accessibility /
