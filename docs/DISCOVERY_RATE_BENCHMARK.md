...
```bash
python scripts/benchmark_discovery_rate.py --provider pryx --model claude-haiku-4-5-20251001
...
`PRYX_DISCOVERY_BENCHMARK=1`, so every request carries
`x-pryx-discovery-benchmark: 1` and telemetry carries `benchmark_run: true`.
...
`pryx-base/src/prompt_tests.rs` asserts Discovery is never injected into the
...
```bash
python scripts/verify_discovery_select.py ./target/selfdev/pryx
```
...
```bash
PRYX_BIN=<after-bin> python scripts/benchmark_discovery_rate.py \
