...
`/remote` is the single activation entry point for Pryx Cloud. It opens the Pryx account activation page, where a user signs in, confirms the subscription entitlement, chooses a region, and creates or wakes their host. `/remote status`, `/remote on`, `/remote pair`, and `/remote revoke` remain the explicit self-hosted gateway controls.

Cloud access is bundled into paid Pryx subscriptions rather than sold as a second product. The subscription pays for the control plane and a bounded amount of host runtime/storage. Model-token budgets remain governed by the existing subscription tier.

### User journey

1. Run `/remote` on desktop or select **Pryx Cloud** in a client.
2. Browser opens `https://pryx.sh/account`. During early access, the account
   page handles sign-in and plan management but does not yet provision a host.
3. Sign in with the existing Pryx device/account identity. If needed, subscribe or upgrade.
4. Pick the nearest supported region. Defaults are automatic and reversible.
5. Pryx provisions an isolated host, imports only credentials or repository access the user explicitly approves, and displays progress.
...
```mermaid
flowchart LR
  C[Pryx clients] -->|OAuth/device credential| CF[CloudFront]
...
- **Identity:** Cognito user pool federated from the existing Pryx account during migration. Long term, Cognito is the account identity authority. Device authorization is implemented by Lambda/API Gateway with hashed, short-lived codes.
...
Authenticated endpoints under the existing `api.pryx.sh/v1` origin:
...
1. **Internal alpha:** keep the existing guarded `pryx-phone` EC2 deployment as the reference host. Validate wake, SSM, gateway, Bedrock, idle stop, and breaker paths.
