...
This RFC describes a modular target architecture for pryx that matches the current codebase, preserves the existing product model, and gives us a safe migration path from today's mostly-monolithic root crate to a layered workspace.
...
Today, pryx is best described as a **modular monolith with a growing workspace shell**:

- The root `pryx` crate still owns most runtime orchestration and product behavior.
...
4. **Composition layer** where the top-level `pryx` package wires the product together.
...
- `pryx` is a **single-server, multi-client** application.
...
- **Root crate `pryx`** still contains most product logic.
...
- root package: `pryx`
- foundation/runtime support: `pryx-agent-runtime`, `pryx-core`, `pryx-storage`, `pryx-terminal-launch`, `pryx-tool-core`
- data-contract crates: `pryx-ambient-types`, `pryx-auth-types`, `pryx-background-types`, `pryx-batch-types`, `pryx-config-types`, `pryx-gateway-types`, `pryx-memory-types`, `pryx-message-types`, `pryx-selfdev-types`, `pryx-session-types`, `pryx-side-panel-types`, `pryx-task-types`, `pryx-tool-types`, `pryx-usage-types`
- protocol and planning: `pryx-protocol`, `pryx-plan`
- heavy or optional integrations: `pryx-embedding`, `pryx-pdf`, `pryx-notify-email`
- auth and providers: `pryx-azure-auth`, `pryx-provider-core`, `pryx-provider-metadata`, `pryx-provider-openrouter`, `pryx-provider-gemini`
- TUI extraction seams: `pryx-tui-core`, `pryx-tui-markdown`, `pryx-tui-mermaid`, `pryx-tui-render`, `pryx-tui-workspace`
- product surfaces outside the main TUI binary: `pryx-desktop`
...
- provider implementation composition and runtime provider wiring; the shared `Provider` trait now lives in `pryx-provider-core`
...
| Crate | Current role |
|---|---|
| `pryx-agent-runtime` | shared interrupt and lightweight runtime primitives for agent execution |
| `pryx-ambient-types` | usage and rate-limit records shared by ambient/background flows |
| `pryx-auth-types` | provider-neutral auth state and credential metadata |
| `pryx-background-types` | background-task status and progress DTOs |
| `pryx-batch-types` | batch tool progress DTOs, currently depending only on message types internally |
| `pryx-config-types` | stable configuration data contracts |
| `pryx-core` | low-level utilities such as IDs, env helpers, fs helpers, stdin detection, and formatting |
| `pryx-gateway-types` | gateway-facing data contracts |
| `pryx-memory-types` | memory subsystem data contracts |
| `pryx-message-types` | message content and transport-adjacent data contracts |
| `pryx-protocol` | client/server protocol surface built from stable type crates and provider-core values |
| `pryx-plan` | plan/task graph data model shared across coordination flows |
| `pryx-selfdev-types` | self-development request/status data contracts |
| `pryx-session-types` | session DTOs, currently depending only on message types internally |
| `pryx-side-panel-types` | side-panel page and update data contracts |
| `pryx-task-types` | task/tool scheduling data contracts |
| `pryx-tool-core` | runtime tool contracts such as the `Tool` trait and execution context |
| `pryx-tool-types` | stable tool output/image DTOs |
| `pryx-usage-types` | usage accounting data contracts |
| `pryx-storage` | storage helpers layered on `pryx-core` |
| `pryx-embedding` | ONNX/tokenizer-based embedding implementation and heavy inference deps |
| `pryx-pdf` | PDF text extraction |
| `pryx-azure-auth` | Azure bearer token retrieval |
| `pryx-notify-email` | SMTP/IMAP/mail transport |
| `pryx-provider-metadata` | provider/login catalog and profile metadata |
| `pryx-provider-core` | shared provider contract (`Provider`/`EventStream`), value types, route/cost/model helpers, shared HTTP client, schema helpers |
| `pryx-provider-openrouter` | OpenRouter-specific catalog/cache/support helpers |
| `pryx-provider-gemini` | Gemini schema/model/support helpers |
| `pryx-tui-core` | low-level terminal UI primitives that do not need full app state |
| `pryx-tui-markdown` | markdown wrapping/rendering, layered on mermaid/workspace support |
| `pryx-tui-mermaid` | mermaid parsing, rendering, caching, viewport, and widget support |
| `pryx-tui-render` | reusable TUI layout/render helpers |
| `pryx-tui-workspace` | workspace-map data/model/widget rendering |
| `pryx-terminal-launch` | terminal process launch helpers |
| `pryx-desktop` | desktop app surface and session/workspace rendering experiments |
...
flowchart TD
  ...
  J --> AR[pryx-agent-runtime]
  J --> Emb[pryx-embedding]
  J --> PDF[pryx-pdf]
  J --> Azure[pryx-azure-auth]
  J --> Mail[pryx-notify-email]
  J --> PMeta[pryx-provider-metadata]
  J --> PCore[pryx-provider-core]
  J --> POR[pryx-provider-openrouter]
  J --> PGem[pryx-provider-gemini]
  J --> TW[pryx-tui-workspace]
```
...
```mermaid
flowchart TD
  App[pryx top-level package]

  subgraph L2[Layer 2: interfaces and product surfaces]
    TUI[pryx-tui]
    SelfDev[pryx-selfdev]
    CLI[pryx-cli or root CLI modules]
  end

  subgraph L1[Layer 1: domain/runtime]
    Server[pryx-server]
    Agent[pryx-agent]
    Provider[pryx-provider]
    Session[pryx-session]
  end

  subgraph L0[Layer 0: foundation and support]
    Core[pryx-core]
    AR[pryx-agent-runtime]
    Emb[pryx-embedding]
    PDF[pryx-pdf]
    Azure[pryx-azure-auth]
    Mail[pryx-notify-email]
    PMeta[pryx-provider-metadata]
    PCore[pryx-provider-core]
    POR[pryx-provider-openrouter]
    PGem[pryx-provider-gemini]
    TW[pryx-tui-workspace]
  end
...
- `pryx-provider`: provider composition, provider routing, streaming contract adapters, and concrete runtime implementations layered on the `pryx-provider-core` trait.
- `pryx-agent`: turn loop, compaction orchestration, provider/tool interaction, recovery logic.
- `pryx-session`: session model, state transitions, persistence-facing session operations.
- `pryx-server`: daemon lifecycle, client attachment, swarm/background coordination, service registries.
- `pryx-tools` or narrower `pryx-tool-core` plus `pryx-tool-impl`: tool registry contracts and tool implementations.
- `pryx-auth`: root auth orchestration after provider-neutral data lives in `pryx-auth-types` and heavy leaf SDKs stay separate.
- `pryx-memory`: memory graph/log/search orchestration once its contracts are stable enough.
...
- `pryx-cli`: parsing and command dispatch if CLI keeps growing.
- `pryx-tui`: app state, reducers, key handling, command/input handling, UI orchestration.
- `pryx-desktop`: already a separate surface.
- `pryx-selfdev`: self-dev build/reload/customization workflows if they remain a substantial product surface.
...
- `pryx-notify-email`
- `pryx-tui-mermaid`
- provider support crates such as `pryx-provider-openrouter` and `pryx-provider-gemini`
...
The top-level `pryx` package should eventually become mostly:
...
```text
pryx binary/composition
  -> pryx-cli, pryx-tui, pryx-server, pryx-selfdev

pryx-cli / pryx-tui
  -> pryx-protocol, pryx-*-types, pryx-server-client contracts

pryx-server
  -> pryx-agent, pryx-session, pryx-provider, pryx-tools, pryx-storage

pryx-agent
  -> pryx-provider, pryx-tools, pryx-session, pryx-agent-runtime

pryx-provider
  -> pryx-provider-core, pryx-provider-* leaves, pryx-auth-types

pryx-session
  -> pryx-session-types, pryx-message-types, pryx-storage, optional leaf adapters
...
- **One mega `pryx-common` crate.** It becomes the new root crate and invalidates everything.
...
1. **Provider contracts:** keep shrinking `src/provider/mod.rs` until a `pryx-provider` trait/runtime crate can depend only on `pryx-message-types`, `pryx-provider-core`, and small runtime primitives.
...
6. **Auth facade:** keep provider-neutral auth data in `pryx-auth-types`, heavy SDKs in leaf crates, and move root auth orchestration only after provider contracts stabilize.
...
- `cargo tree -p pryx --edges normal --depth 1` before/after for dependency surprises
...
- If introducing `pryx-session` feels too early, the same boundary should still be established internally first and extracted later.
...
- Existing crates `pryx-provider-core`, `pryx-provider-metadata`, `pryx-provider-openrouter`, and `pryx-provider-gemini` remain useful under this layer.
...
- `pryx-agent-runtime` remains the low-level runtime primitive crate below it.
...
- session persistence internals that belong in `pryx-session`
...
- `pryx-tui-workspace` can remain a leaf crate or become a child dependency of `pryx-tui`.
...
- socket/session attachment state, fanout senders, debug socket helpers, and daemon lifecycle code must not appear in `pryx-core`, `pryx-provider-core`, or provider leaf crates

### Rule 4: Provider implementation crates depend on contracts, not on the server or TUI

- provider leaf crates may depend on `pryx-core`, `pryx-provider`, and `pryx-provider-core`
- they must not depend on `pryx-server` or `pryx-tui`

### Rule 5: Async/network-heavy dependencies do not belong in `pryx-core`

`pryx-core` should stay cheap to compile and highly reusable.
...
- protocol/data types -> `pryx-core`
- session persistence -> `pryx-session`
- provider route/schema helpers -> provider crates
- rendering helpers -> `pryx-tui`

### Rule 8: The root package may compose many crates, but peer crates should stay narrow

The top-level `pryx` package can wire multiple domains together. Peer crates should not casually depend on each other sideways when a lower-level contract would do.
...
|---|---|
| `src/id.rs`, protocol/message/config primitives | `pryx-core` |
| `src/session.rs`, parts of `storage`, restart snapshot concerns | `pryx-session` |
| `src/agent/*`, parts of `compaction`, tool orchestration seams | `pryx-agent` |
| `src/server/` + shrinking `src/server.rs` facade | `pryx-server` |
| `src/provider/mod.rs` trait/contracts plus provider composition seams | `pryx-provider` |
| existing provider helper crates | remain leaf/provider support crates |
| `src/tui/*` + `pryx-tui-workspace` | `pryx-tui` + leaf workspace widget crate |
| `src/cli/*` | stay in root initially or become `pryx-cli` later if justified |
| `src/tool/selfdev/*`, self-dev workflow/productization | `pryx-selfdev` |
...
- Should `pryx-session` become an explicit crate, or remain an internal boundary until later?
- Should CLI remain in the top-level package permanently, or eventually become `pryx-cli`?
- Should `message` and `protocol` remain together in `pryx-core`, or split into separate contract crates if they evolve at different rates?
- Should `pryx-tui-workspace` remain a separate leaf crate long-term, or fold into `pryx-tui` once the larger TUI extraction lands?
