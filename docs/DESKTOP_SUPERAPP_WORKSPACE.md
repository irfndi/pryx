...
This document refines the Pryx Desktop product direction from a single chat-like app into a **Niri-like agent workspace superapp**.
...
The goal is not to clone a general-purpose window manager. The goal is to give Pryx users a fast, keyboard-driven, spatial environment for supervising multiple agent sessions and related development tools inside one custom desktop app.
...
Pryx Desktop should be a **local AI development superapp**:
...
Niri's useful idea for Pryx is not the compositor implementation. It is the mental model:
...
Pryx Desktop can bring that workflow to macOS users who do not have a Niri-like environment, while still working well on Linux.
...
| Session surface | A surface attached to a server-owned Pryx session |
...
This is important because Pryx users may run many agents/tasks concurrently.
...
It should be backed by a shared command registry in `pryx-client-core`, not hardcoded separately per UI.

## Data model additions

`pryx-client-core` should include a workspace layout state model:
...
Pryx Desktop should become a **keyboard-driven, Niri-like agent workspace superapp**.
