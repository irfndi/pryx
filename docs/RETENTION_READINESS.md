...
pryx separates two questions that should not share a label:
...
```bash
cargo test -p pryx-app-core --lib retention_readiness -- --nocapture
```
...
pryx already records coarse active-day helpers (`active_days_7d` and `active_days_30d`). Those are useful engagement signals, but they are not cohort retention by themselves because they do not anchor the denominator to an activation event or require a meaningful successful outcome.
