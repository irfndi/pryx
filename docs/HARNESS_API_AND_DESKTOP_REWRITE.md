...
- The agent runtime ("harness") is already client/server: NDJSON over Unix
  socket (`~/.pryx/pryx.sock`), with `Request` / `ServerEvent` in
  `crates/pryx-protocol`. But it is an *internal* wire format: unversioned,
  ~147 variants, TUI-shaped, and coupled to client rendering assumptions.
- The current desktop app (`crates/pryx-desktop`, ~44k lines) hand-rolls
...
Introduce `crates/pryx-harness-api`:
...
- **Relationship to `pryx-protocol`.** Short term the server adapter maps
...
1. `crates/pryx-harness-api`: types, version constants, handshake,
...
```
pryx-desktop2 (new crate, old crate untouched until parity)
...
Key invariants:
- The desktop talks to the runtime **only** through `pryx-harness-api`.
...
2. `pryx-desktop2` skeleton: window, Vello "hello scene", Parley paragraph.
...
6. Parity checklist vs old desktop, then delete `pryx-desktop`.
