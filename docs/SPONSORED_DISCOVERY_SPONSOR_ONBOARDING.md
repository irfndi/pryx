...
This runbook is the source of truth for adding a tool sponsor to pryx's
...
It should not require a pryx release.

A pryx code change is required when the sponsor needs:
...
- `crates/pryx-app-core/src/tool/discover.rs`;
- `crates/pryx-base/src/sponsors.rs`;
- `crates/pryx-base/src/sponsors/provenance.rs`;
- `crates/pryx-tui/src/tui/app/sponsor_disclosure.rs`; and
- `TELEMETRY.md`.
...
5. The setup does not bypass pryx's confirmation requirements. Signups,
...
`crates/pryx-base/src/sponsors.rs`. Category values are lowercase slugs.
...
5. Ship the pryx change before publishing entries that rely on the category.
...
measurement-sensitive. pryx records discovery provenance only when a later MCP
...
The default client sends `GET https://api.pryx.sh/v1/discovery` with a
three-second timeout and a 64 KiB maximum response. It sends a
`User-Agent: pryx/<version>` header and a random
`x-pryx-discovery-request-id` correlation header.
...
`x-pryx-discovery-benchmark: 1`. The service must retain that marker with its
...
suggestions are sent only to Pryx maintainers. The schema tells the model to
...
marked `x-pryx-discovery-benchmark: 1` remain stored with
...
receipt. Suggestions go only to Pryx maintainers, never to sponsors. Receipt
...
- suggest requires a prior successful browse in the same category, persists one
  actionable proposal per browse, and sends it only to Pryx maintainers;
...
Then validate through pryx by pointing a test config at staging:
...
In a disposable pryx session, browse the category, select the sponsor or submit
...
```bash
cargo test -p pryx-app-core tool::discover
cargo test -p pryx-base sponsors
cargo test -p pryx-base discovery_provenance
cargo test -p pryx-tui sponsor_disclosure
cargo check -p pryx
```
...
- [ ] Staging pryx validation passes, including disclosure and confirmation.
