...
This demo gives Pryx a deterministic local storefront. The agent can search,
...
> Use `./bin/pryx-demo-shop` to see whether this shop has a USB-C laptop charger
...
1. Pryx inspects the CLI, searches the shop, and chooses the qualifying 65W
   charger.
2. The shop verifies the simulated `$43.19` total and reports that no payment
   method is available.
3. Pryx independently browses `payments`, receives AgentCard, selects it, and
