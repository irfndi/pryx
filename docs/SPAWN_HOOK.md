...
pryx opens new terminal windows in several flows: swarm agent spawning
(`swarm spawn` with `spawn_mode=visible`), resume-in-new-terminal, self-dev
sessions, restart restores, and jade relay launches. By default pryx detects
...
```bash
export PRYX_SPAWN_HOOK="tmux new-window"
# An empty value disables a config-file hook:
export PRYX_SPAWN_HOOK=
```
...
When a headed spawn happens and a hook is configured, pryx runs:

```
<spawn_hook> <pryx-binary> <args...>
```

- The hook command is parsed shell-style (quotes and backslash escapes work),
  but it is executed directly, not through a shell.
- The pryx binary and its full argument list are appended as extra argv
  entries (the familiar `$TERMINAL -e <cmd>` convention).
- The hook's working directory is the session working directory.
- The hook process is detached; pryx does not wait for it.
- If the hook fails to start (binary missing, parse error), pryx logs a
...
| Variable | Meaning |
| --- | --- |
| `PRYX_SPAWN_KIND` | Why the spawn happened: `swarm-agent`, `resume`, `selfdev`, `restart`, `jade-relay` |
| `PRYX_SPAWN_SESSION_ID` | The pryx session the window will run |
| `PRYX_SPAWN_TITLE` | Suggested window/tab title (includes session icon + name) |
| `PRYX_SPAWN_CWD` | Session working directory |
| `PRYX_SPAWN_PROGRAM` | Path of the pryx binary to execute |
| `PRYX_SPAWN_COMMAND` | Full command line, shell-escaped, for hooks that take one shell string |
| `PRYX_SPAWN_SWARM_ID` | (swarm spawns) The swarm the agent joins |
| `PRYX_SPAWN_COORDINATOR_SESSION_ID` | (swarm spawns) The coordinator session that requested the spawn |
| `PRYX_FRESH_SPAWN` | `1` when the spawn is a fresh window handoff |

### Client terminal environment (multi-terminal routing)

The pryx server process is long-lived: it captures terminal-identifying env
...
- A `PRYX_CLIENT_<NAME>` alias (e.g. `PRYX_CLIENT_ZELLIJ_SESSION_NAME`,
  `PRYX_CLIENT_TMUX`, `PRYX_CLIENT_DISPLAY`) is also exported so a hook can
...
When pryx detects that the requesting client is inside tmux, its built-in
...
self-dev, and visible agent spawns. `PRYX_TERMINAL` can explicitly choose a
...
`tmux new-window <pryx> --resume ses_x` runs the command in a new window of
...
```toml
[terminal]
spawn_hook = "~/bin/pryx-spawn-router"
```

```bash
#!/usr/bin/env bash
# ~/bin/pryx-spawn-router
# argv: the pryx command to run ("$@"). Env: PRYX_SPAWN_* metadata.

case "$PRYX_SPAWN_KIND" in
  swarm-agent)
    # Swarm workers as tmux panes in a window named after the swarm.
    tmux new-window -n "swarm:${PRYX_SPAWN_SWARM_ID:0:8}" "$@" 2>/dev/null \
      || tmux split-window "$@"
    ;;
  *)
    # Everything else as a normal terminal window.
    kitty --title "$PRYX_SPAWN_TITLE" -e "$@" &
    ;;
esac
```

A hook that exits non-zero after launching nothing will NOT trigger the
built-in fallback (pryx only falls back when the hook process cannot be
...
```bash
#!/usr/bin/env bash
zellij action new-pane -- bash -lc "$PRYX_SPAWN_COMMAND"
```

## Programmatic discovery

Programs that wrap pryx (e.g. herd-style session managers) can set
`PRYX_SPAWN_HOOK` in the environment of the `pryx` server process they
...
When pryx wants to bring an existing session window to the foreground (e.g.
...
[terminal]
spawn_hook = "tmux new-window"
focus_hook = "~/bin/pryx-focus"   # env: PRYX_FOCUS_SESSION_ID, PRYX_FOCUS_TITLE
```
...
tmux select-window -t "$(tmux list-windows -F '#{window_id} #{window_name}' \
  | grep -F "$PRYX_FOCUS_TITLE" | head -1 | cut -d' ' -f1)"
```

Env override: `PRYX_FOCUS_HOOK` (empty value disables a config-file hook).
If the hook fails to start, pryx falls back to the built-in focus path.
