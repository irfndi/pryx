...
pryx uses a **single-server, multi-client** architecture. One server process
...
│                                                                             │
│  pryx serve                                                                │
│  ├── Unix socket:  /run/user/$UID/pryx.sock                                │
│  ├── Debug socket: /run/user/$UID/pryx-debug.sock                          │
│  ├── Registry:     ~/.pryx/servers.json                                    │
...
The server name persists across reloads via the registry (`~/.pryx/servers.json`).
...
```
  ...
  ─────                          ───────                     ──────
  pryx (first run)              pryx (subsequent)          /reload
       │                              │                          │
...
When you run `pryx`, it checks if a server is already running:

1. **Server exists**: connect directly as a client
2. **No server**: spawn `pryx serve` as a detached daemon (with `setsid`),
...
`pryx serve --server-name <name>` or the `PRYX_SERVER_NAME` environment
variable. The optional `PRYX_SERVER_DISPLAY_NAME` environment variable is also
...
```bash
pryx --socket /tmp/pryx.sock -C /local/checkout --remote-working-dir /remote/checkout
```
...
```
/run/user/$UID/
├── pryx.sock          # Main communication socket
└── pryx-debug.sock    # Debug/testing socket
```

## Self-Dev Mode

When running `pryx` inside the pryx repository:

1. Auto-detects the repo and enables self-dev mode
2. Connects to the normal shared pryx server
...
| Scenario | Behavior |
|----------|----------|
| First `pryx` run | Spawns server daemon, connects |
| Subsequent `pryx` | Connects to existing server |
...
| Resume session | `pryx --resume fox` reconnects to existing session |
