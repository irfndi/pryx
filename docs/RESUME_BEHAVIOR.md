...
- Pryx sessions switch the current workspace/client to that session.
- Importable external sessions are first converted to a Pryx session, then resumed in place.
...
- If terminal launch is unavailable, the UI prints manual `pryx --resume <id>` commands.
