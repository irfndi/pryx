...
- Client telemetry pipeline: `crates/pryx-telemetry-core/src/lib.rs`
  (`record_auth_success` @1409, `sanitize_telemetry_label` applied to all
  auth labels @398-400; `AuthEvent` schema in
  `crates/pryx-usage-types/src/lib.rs:270-284` has NO account_id/email slot,
  by design — see the TODO at `src/cli/login/pryx_device.rs:370-374`).
- Onboarding auth funnel: `auth_provider`, `auth_method`,
  `auth_failure_reason` on onboarding step events
  (`crates/pryx-telemetry-core/src/lib.rs:369-400`).
- Failure classification labels: `crates/pryx-base/src/auth/login_diagnostics.rs`
  (`AuthFailureReason::label`) — the only failure detail that may be reported.
- Local logs: `~/.pryx/logs/pryx-YYYY-MM-DD.log`.
...
- `api_key` (PRYX_API_KEY) — full or truncated beyond `jc_...last4`.
...
- Env-file contents (`pryx-subscription` env file) and `Authorization`
...
`auth_method` ("pryx-subscription", "device_code_magic_link"), failure
...
- Do NOT add `account_id` to `AuthEvent` (the existing TODO at
  `src/cli/login/pryx_device.rs:370` should be resolved as "backend audit
...
- `me_requests_total{status}`; alert on 401 spike (mass revocation bug) and
  ...
  `crates/pryx-base/src/subscription_api.rs:14`).
...
| Local client logs | `~/.pryx/logs/` | user-owned; must satisfy never-log list |
| Support bundles | created on demand | delete after case close (<= 90 days) |

## Support diagnostics

`pryx doctor`-style account diagnostics (extend existing provider doctor,
`crates/pryx-provider-doctor/`): prints auth_base, masked key
...
- Add `record_auth_failure` for the pryx device flow (labels only).
- Resolve the `account_linked` TODO in `pryx_device.rs` as "won't add to
