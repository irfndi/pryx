...
This document audits the current provider, session, and shared-contract seams in the pryx workspace and recommends the next **realistic** crate moves that improve modularity without creating high-churn dependency cycles.
...
1. **Add a small `pryx-shared-contracts` crate** for the serde-only protocol/session overlap types that already act like shared contracts.
2. **After that, add a narrow `pryx-session-contracts` crate** for session metadata/replay/view structs that are widely reused but do not need the full `Session` runtime.
3. **If we want one more provider-side move before a larger provider refactor, extract the pure provider identity/selection layer** into `pryx-provider-core` or a small `pryx-provider-selection` crate.
...
- `pryx-provider-core`: no local workspace deps
- `pryx-provider-metadata`: no local workspace deps
- `pryx-provider-openrouter`: no local workspace deps
- `pryx-provider-gemini`: no local workspace deps
...
- `pryx-provider-metadata` holds stable login/profile catalog data
- `pryx-provider-core` holds route/cost/shared HTTP client/core value types
- `pryx-provider-openrouter` holds OpenRouter-specific catalog/cache/ranking/model-spec support
- `pryx-provider-gemini` holds Gemini Code Assist schema/types/support helpers
...
- either a new `crates/pryx-provider-selection`
- or a small `provider_identity` / `selection` module inside `pryx-provider-core`
...
### Option B: narrow `pryx-session-contracts`
...
### Option C: add `pryx-shared-contracts`
...
Create `crates/pryx-shared-contracts`.
...
Create `crates/pryx-session-contracts`.

Do this only after Phase 1, so session replay types can point at `pryx_shared_contracts::*` instead of `crate::protocol::*` or `crate::plan::*`.
...
1. Add `crates/pryx-shared-contracts` with serde-only types from `plan.rs` and the small protocol/session overlap set.
...
3. Re-measure touched-file compile times for:
   ...
   - `src/protocol.rs`
   - `src/provider/mod.rs`
4. If the new seam stays clean, follow with a narrow `pryx-session-contracts` extraction.
