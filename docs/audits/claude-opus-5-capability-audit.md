...
This exists because pryx's Claude capability tables are hand-maintained, and
...
| Capability | Probe | Result | pryx encoding |
|---|---|---|---|
...
pryx previously sent a flat `max_tokens = 32768` for every Claude model. Opus 5
...
```bash
set -a; source ~/.config/pryx/anthropic.env; set +a
