...
Pryx supports Windows as a first-class platform. The Windows implementation uses native named pipes, Windows process management, PowerShell installation, and platform-specific launch-hotkey integration.
...
| `pryx update` | Supported with SHA-256 verification |
...
4. Installs immutable, stable, and launcher copies under `%LOCALAPPDATA%\pryx`.
5. Adds `%LOCALAPPDATA%\pryx\bin` to the user `PATH`.
...
```powershell
$script = [scriptblock]::Create((irm https://pryx.sh/install.ps1))
& $script -ConfigureAlacritty -ConfigureHotkey
```

Pryx can also offer these options interactively after launch.
...
- Launcher: `%LOCALAPPDATA%\pryx\bin\pryx.exe`
- Stable binary: `%LOCALAPPDATA%\pryx\builds\stable\pryx.exe`
- Versioned binary: `%LOCALAPPDATA%\pryx\builds\versions\<version>\pryx.exe`
- User data and configuration: `%USERPROFILE%\.pryx`
...
pryx --version
Get-Command pryx
Get-FileHash (Get-Command pryx).Source -Algorithm SHA256
```

Compare the hash with `SHA256SUMS` on the matching [GitHub release](https://github.com/pryx-co/pryx/releases/latest).

After Authenticode signing is enabled, this must report `Valid`:

```powershell
Get-AuthenticodeSignature (Get-Command pryx).Source | Format-List Status,StatusMessage,SignerCertificate
```
...
2. Create a Microsoft Entra application or managed identity with a federated credential for `pryx-co/pryx` GitHub Actions.
...
Unix domain sockets are replaced by Windows named pipes under `crates/pryx-base/src/transport/windows.rs`. Platform-specific filesystem, process, update, and replacement behavior is selected at compile time with `#[cfg(windows)]`, so Windows support does not add runtime branching to Unix builds.

Windows launch-hotkey setup is implemented in `crates/pryx-setup-hints/src/windows_setup.rs` and is only installed after explicit user consent.
...
- Pryx version from `pryx --version`
- Installation method
...
Do not upload private configuration, credentials, session transcripts, or `.pryx` authentication files.
