...
This document defines the target structure for keeping `pryx` modular without turning shared crates into a dumping ground. It is intentionally practical: use it when deciding whether to move a type, helper, or behavior out of the root crate.
...
- Avoid cyclic dependencies and hidden coupling through broad `pryx-core` re-exports.
...
Examples: `pryx-session-types`, `pryx-side-panel-types`, `pryx-selfdev-types`, `pryx-background-types`.
...
### `pryx-core` is for genuinely shared primitives

`pryx-core` should contain:
...
`pryx-core` should not accumulate every extracted DTO indefinitely. Once a cluster grows, split it into a focused domain crate.
...
4. Validate.
   ...
   - [ ] Did `cargo check --profile selfdev -p <type-crate> -p pryx --bin pryx` pass?
...
The guard blocks direct dependencies from `pryx-*-types` crates to root/runtime-heavy internal crates such as `pryx`, `pryx-core`, provider crates, TUI crates, protocol/runtime crates, and desktop/mobile crates. Type crates may depend on external lightweight libraries and other type crates. If a new internal dependency is needed, first decide whether it should itself be a type crate.
...
| --- | --- | --- | --- |
| Usage DTOs | `cargo check --profile selfdev -p pryx-usage-types -p pryx --bin pryx` | Prefer exact tests under usage/copilot usage modules. Avoid bare `usage` as a required gate because it selects display/UI tests too. | DTO crate owns report and local counter contracts. Runtime fetch/cache/display stay root. |
| Gateway DTOs | `cargo check --profile selfdev -p pryx-gateway-types -p pryx --bin pryx` | Focus gateway persistence/auth tests by exact test names when available. | Pairing/token HTTP/WebSocket behavior stays root. |
| Ambient DTOs | `cargo check --profile selfdev -p pryx-ambient-types -p pryx --bin pryx` | Scheduler/type consumers only. | Ambient DTO crate owns usage records only. Queue/runtime/prompt behavior stays root. |
| Ambient behavior modules | `cargo check --profile selfdev -p pryx --bin pryx` | `cargo test --profile selfdev -p pryx ambient::ambient_tests --lib`; `cargo test --profile selfdev -p pryx ambient::scheduler::tests --lib`; `cargo test --profile selfdev -p pryx ambient::runner::runner_tests --lib` | Avoid bare `ambient` as a required gate for module-only refactors because it selects cross-module TUI/config state tests. |
| Memory activity DTOs | `cargo check --profile selfdev -p pryx-memory-types -p pryx-core -p pryx --bin pryx` | `cargo test --profile selfdev -p pryx runtime_memory_log --lib`; `cargo test --profile selfdev -p pryx tui::info_widget::tests --lib` | `memory::activity` currently matches no tests, so use consumer tests. |
| Goal/todo/catchup core DTOs | `cargo check --profile selfdev -p pryx-core -p pryx --bin pryx` | Exact goal/todo/catchup filters if behavior changes. | Currently small/stable enough to leave in `pryx-core`; revisit if churn grows. |


## Compile baseline observations

Measured on 2026-04-30 with `scripts/dev_cargo.sh check --profile selfdev -p pryx --bin pryx` after the compile-speed boundary doc commit. This is a coarse mtime-touch benchmark, not a full statistical study, but it is enough to guide priorities.
...
| Touch `crates/pryx-core/src/usage_types.rs` | ~65.35s | Editing `pryx-core` invalidates broad downstream dependents. Avoid adding high-churn domain DTOs to `pryx-core`. |

Implication: the compile-speed target is not simply "move things out of root". Moving stable, low-churn contracts out of root is good, but putting many high-churn domain DTOs into `pryx-core` can be counterproductive because `pryx-core` has high fan-out. Prefer focused leaf crates such as `pryx-usage-types`, `pryx-gateway-types`, and `pryx-ambient-types` for domain DTOs that are likely to change.

## `pryx-core` fan-out audit

At this checkpoint, the root crate is the only direct Cargo dependency on `pryx-core`, but root re-exports many `pryx-core` modules and root is the high-cost recompilation target. A touch to `pryx-core` invalidated broad downstream checks in the baseline above. Therefore `pryx-core` should be treated as a high-fan-out crate even if Cargo.toml direct dependents are currently few.
...
1. Move clustered, likely-changing domain DTOs from `pryx-core` to focused leaf crates.
2. Keep stable general utilities in `pryx-core`.
3. Avoid adding new domain DTOs to `pryx-core` unless they are very stable or temporary staging.

| Module | Current contents | Preferred long-term home | Notes |
| --- | --- | --- | --- |
| `ambient_usage_types` | Ambient scheduler usage records/rate limit DTOs | moved to `pryx-ambient-types` | Compatibility re-export remains in root module. |
| `catchup_types` | Catch-up persisted state and rendered brief DTOs | `pryx-catchup-types` or stay in core | Small and low churn. Split only if catch-up grows. |
| `copilot_usage_types` | Local Copilot usage counters | moved to `pryx-usage-types` | Compatibility re-export remains in root module. |
| `gateway_types` | Paired device and pairing code persisted records | moved to `pryx-gateway-types` | Pairing/token behavior remains root. |
| `goal_types` | Goal state, milestones, status, updates | `pryx-goal-types` or `pryx-task-types` | Larger domain. Worth splitting if goal/tool work grows. |
| `memory_types` | Memory activity DTOs | moved to `pryx-memory-types` | Memory has enough domain weight for its own type crate. |
| `todo_types` | Todo item DTO | `pryx-task-types`, `pryx-todo-types`, or core | Tiny. Could join goal/catchup task-state crate. |
| `usage_types` | Provider usage report DTOs | moved to `pryx-usage-types` | Runtime fetch/cache/display remain root. |
...
- public report DTOs in `pryx-usage-types`
...
- WebSocket relay
- public gateway DTOs in `pryx-gateway-types`
...
- `pryx-core` contains only true primitives or documented temporary staging modules.
