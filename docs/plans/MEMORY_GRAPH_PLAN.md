...
## Two operating modes (gate: `agents.memory_sidecar_enabled`, env `PRYX_MEMORY_SIDECAR_ENABLED`, default off)
...
Benchmark (Mode 1, private ~/pryx-memory-bench, Sonnet judge):
...
Benchmark-driven results (Sonnet judge, 28 judged queries, pryx self-dev corpus):
...
- Shared: pryx-base/src/memory_rerank.rs (prompt + parse + rerank_candidates),
...
~/pryx-memory-bench/results/BASELINE_SUMMARY.md):
