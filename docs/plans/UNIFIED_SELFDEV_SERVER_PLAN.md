...
> Any references below to `/tmp/pryx-selfdev.sock`, `canary-wrapper`, or
> `PRYX_SELFDEV_MODE` describe the pre-merge architecture or transition steps,
...
### Normal mode
- Main socket: runtime `pryx.sock`
- Debug socket: runtime `pryx-debug.sock`
- Startup path: `pryx` -> default client flow -> spawn `pryx serve` if needed

### Self-dev mode
- Main socket: `/tmp/pryx-selfdev.sock`
- Debug socket: `/tmp/pryx-selfdev-debug.sock`
- Startup path:
  - repo auto-detection or `pryx self-dev`
...
- explicit `pryx self-dev`
- current working directory is the pryx repo (auto-detected)
...
Expected result:
- opening pryx inside the repo uses the shared server path by default
- session still becomes canary/self-dev
- explicit `pryx self-dev` command may still use legacy wrapper temporarily

### Phase 2 - Move explicit `pryx self-dev` onto shared server path
...
Expected result:
- both auto-detected self-dev and explicit `pryx self-dev` share one server
...
Changes:
- deprecate `/tmp/pryx-selfdev.sock` and `/tmp/pryx-selfdev-debug.sock`
...
There are multiple `PRYX_SELFDEV_MODE` checks across startup, hot reload, and server behavior; these need to be separated into:
...
2. Land explicit `pryx self-dev` shared-path behavior
