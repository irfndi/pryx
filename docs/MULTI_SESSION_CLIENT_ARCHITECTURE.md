...
This document describes a proposed evolution of pryx's UI architecture from the
...
Today, pryx is effectively organized like this:
...
- a independent pryx window attached to one session
...
- current independent pryx window
...
2. Client spawns a independent pryx client attached to the same session.
...
This can initially be provided through existing pryx control channels such as:
...
- Preserve current `pryx --resume <session>` workflow.
...
- Should workspace mode be a new command (`pryx workspace`) or a runtime mode of
  the normal client?
...
This gives pryx a portable built-in multi-session workspace without sacrificing
