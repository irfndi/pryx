# pryx wrapper / scripting guide

This document describes the non-interactive CLI surface intended for wrappers, scripts, and other tools that invoke `pryx`.
...
```bash
pryx --quiet --no-update --no-selfdev ...
```
...
```bash
pryx --quiet model list
pryx --quiet model list --json
pryx --quiet --provider openai model list --json
```
...
```bash
pryx --quiet provider list
pryx --quiet provider list --json
```

Inspect the currently requested and resolved provider/model selection:

```bash
pryx --quiet provider current
pryx --quiet --provider openai --model gpt-5.4 provider current --json
```

Verbose human summary:

```bash
pryx --quiet model list --verbose
```

## Run one prompt and return JSON

```bash
pryx --quiet run --json "Reply with exactly OK"
```

## Stream one prompt as NDJSON

```bash
pryx --quiet run --ndjson "Reply with exactly OK"
```
...
```bash
pryx --quiet auth status
pryx --quiet auth status --json
```
...
- `pryx model list` and `pryx run --json` do not require the TUI
- `pryx model list` does not require an already-running shared server
