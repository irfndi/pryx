...
A human-in-the-loop safety layer for unmonitored agent operations. Designed as an independent subsystem that any pryx feature can integrate with. Currently the only consumer is ambient mode, but the system is intentionally decoupled so it can be reused for future features.
...
sequenceDiagram
    ...
    NF->>US: Email: "pryx ambient wants to create a PR"
...
url = "https://example.com/pryx-safety"
secret = "..."
...
- Extracted 3 memories from crashed session pryx-red-fox-1234
...
```bash
pryx safety review           # Interactive review of pending requests
pryx safety list             # List all pending requests
pryx safety approve <id>     # Approve a specific request
pryx safety deny <id>        # Deny a specific request
pryx safety log              # View decision history
```
...
The safety system exposes a simple API for any pryx feature to use:
...
- [ ] TUI review panel
- [ ] CLI commands (`pryx safety review/list/approve/deny/log`)
