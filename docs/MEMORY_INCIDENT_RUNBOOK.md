# Pryx Server Memory Incident Runbook
...
1. What is using the pryx server's memory?
...
```bash
pryx debug 'server:memory-incident'
```
...
```bash
python scripts/analyze_runtime_memory_log.py --days 1 --json > /tmp/pryx-memory-analysis.json
```
...
2. Run `pryx debug 'swarm:list'` and inspect the largest live swarm.
...
```bash
pryx debug 'server:memory-incident' > /tmp/before.json
pryx debug 'allocator:purge'
pryx debug 'server:memory-incident' > /tmp/after.json
```
...
1. Run `pryx debug 'server:memory'` for the full attribution walk.
