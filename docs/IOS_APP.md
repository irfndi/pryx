...
A native iOS remote control for pryx servers running on your own machines.
...
ios/
  ...
  Sources/PryxKit/          platform-free client core (no UIKit)
  ...
  Sources/PryxMobile/       SwiftUI app shell (iOS only)
    PryxMobileApp.swift     entry
  ...
  Tests/PryxKitTests/       swift test on macOS: codec fixtures, reducer, pairing
  project.yml                XcodeGen spec for the app target
```

Rules:

- `PryxKit` never imports UIKit/SwiftUI. It must compile for macOS so the
...
- `pryx pair` CLI generates a 6-digit code (5 min TTL) and QR with
  `pryx://pair?host=H&port=P&code=C`.
...
- `ws://host:7643/ws?token=...` upgrades to a WebSocket carrying the same
  ...
  (`crates/pryx-protocol/src/wire.rs`, `#[serde(tag = "type")]`).
