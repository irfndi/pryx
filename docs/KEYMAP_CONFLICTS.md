...
pryx runs inside a terminal, which runs inside an OS. Both layers can claim a
key chord before it ever reaches pryx (for example Ghostty binding `Ctrl+Tab`
to "next tab", or macOS binding `Cmd+Space` to Spotlight). When that happens, a
pryx keybinding silently does nothing and it is not obvious why.

This feature discovers the key bindings that exist on the machine, compares them
against pryx's own configured bindings, and warns about overlaps.
...
It is a snapshot taken at startup, not a live hook, so changes made while pryx
...
- **Startup notice.** On launch, if the set of conflicts has *changed* since the
  last time we warned, pryx shows a one-time heads-up pointing at `/keys`. It is
...
The report names the conflicting pryx action and its config field, e.g.:

```
  ⚠ Ctrl+Tab
      pryx: Switch to next model (keybindings.model_switch_next = "ctrl+tab")
      taken by terminal: next_tab
```

To fix, either:

- rebind the pryx action in `~/.pryx/config.toml` under `[keybindings]`
  (e.g. `model_switch_next = "ctrl+shift+m"`), or
- change the conflicting shortcut in your terminal or OS settings.

## Implementation

All logic lives in `crates/pryx-setup-hints/src/keymap/`:
...
- `chord.rs` - `KeyChord`, a normalized `(cmd/ctrl/alt/shift + key)` that unifies
  ...
  pryx's own binding-string grammar.
...
- `mod.rs` - `collect_snapshot` / `refresh_and_save` / `snapshot_cached_or_refresh`
  (persisted to `~/.pryx/keymap-snapshot.json`).
