...
This document translates the current Pryx TUI architecture into a concrete codebase plan for a future custom desktop app.
...
```text
crates/
  pryx-protocol/             # eventually extracted from src/protocol.rs
  pryx-client-core/          # surface-independent client state/reducers/view models
  pryx-desktop-ui/           # custom UI tree, layout, input routing, style tokens
  pryx-desktop-renderer/     # wgpu renderer, display list, glyph/image atlases
  pryx-desktop-platform/     # winit/AppKit/Linux shell abstraction, menus, clipboard
  pryx-desktop/              # product app: windows, panels, protocol client, composition
```

Initial implementation may keep some of these as modules inside `crates/pryx-desktop` to reduce early friction, but the boundaries should be designed as if they were separate crates.
...
```text
pryx-desktop
  -> pryx-desktop-platform
  -> pryx-desktop-renderer
  -> pryx-desktop-ui
  -> pryx-client-core
  -> pryx-protocol
```

`pryx-client-core` must not depend on:
...
`pryx-desktop-ui` should not depend on the Pryx server runtime. It can depend on client-core view models and generic UI types.

`pryx-desktop-renderer` should not know what a Pryx session is. It renders display lists, text runs, images, clips, and primitives.
...
```text
crates/pryx-protocol/src/lib.rs
  Request
  ServerEvent
  HistoryMessage
...
`pryx-desktop` should be product composition, not renderer internals.

Suggested modules:

```text
crates/pryx-desktop/src/
  main.rs
...
`pryx-desktop-ui` is the framework-like internal layer, but it is product-owned and small.

```text
crates/pryx-desktop-ui/src/
  lib.rs
  id.rs
  geometry.rs
  color.rs
...
`pryx-desktop-renderer` should know nothing about Pryx.

```text
crates/pryx-desktop-renderer/src/
  lib.rs
  gpu.rs
  surface.rs
  pipeline.rs
...
```text
crates/pryx-desktop-platform/src/
  lib.rs
  event.rs
  window.rs
  clipboard.rs
...
Once the desktop reducer shape is clear, extract shared pieces from TUI and desktop into `pryx-client-core`:
...
At that point the TUI can gradually become another presentation of `pryx-client-core`, but it does not have to be converted all at once.
...
> Desktop always connects to a local Pryx server/daemon. It does not embed the agent runtime in-process.
...
```text
crates/pryx-desktop-ui       # pure-ish UI/layout model
crates/pryx-desktop-renderer # wgpu display-list renderer
crates/pryx-desktop          # fake-data app shell
```

Then connect to the server protocol.

Only after the desktop reducer/view model shape is proven should shared `pryx-client-core` extraction begin. This avoids prematurely extracting the wrong abstraction from the current TUI.
