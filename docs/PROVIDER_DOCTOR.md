...
`pryx provider-doctor` is a user-facing diagnostic that answers one question:
...
tracks (`pryx provider-test-coverage`), but as an interactive command you can run
...
```bash
# Validate pryx's own wiring for a provider, no API key, no spend:
pryx provider-doctor cerebras --tier offline

# Validate the key + live model catalog (needs a key, negligible spend):
pryx provider-doctor cerebras --tier catalog

# Full readiness, including real chat, streaming, and tool calls (spends balance):
pryx provider-doctor cerebras --tier full

# Pin a specific model and emit JSON for scripting/CI:
pryx provider-doctor cerebras --model gpt-oss-120b --tier full --json
```
...
| --- | --- | --- | --- | --- |
| `offline` | no | no | pryx-side wiring against a synthetic catalog | catalog reload, picker rendering, fallback labeling, and model-switch routing bugs for this provider |
...
12. `real_pryx_tool_smoke` - an end-to-end tool smoke passed (full tier)
...
`pryx provider-test-coverage` shows a cumulative "Recorded spend" footer
...
1. **"My picker is broken / shows the wrong models."**
   ...
   `model_switch_route` fail, it's a pryx-side routing bug for that provider:
...
`pryx provider-test-coverage`. Lighter tiers record the API-dependent
checkpoints as skipped, so they never over-credit a pair.

`pryx provider-test-coverage` renders the same 11 checkpoints as an 11-stage
...
```
  ...
  6/11   nvidia-nim / gemma-4-31b  failed at `streaming reply`; run `pryx provider-doctor nvidia-nim --model gemma-4-31b --tier full`; last tested 2 days ago ...
```
