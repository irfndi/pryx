...
use pryx::tui::login_picker::{LoginPicker, LoginPickerItem, LoginPickerSummary};
...
impl TestEnv {
    fn new() -> Result<Self> {
        ...
        pryx::env::set_var("HOME", temp.path());
        pryx::env::set_var("XDG_CONFIG_HOME", temp.path().join("config"));
        pryx::env::set_var("APPDATA", temp.path().join("AppData").join("Roaming"));
        pryx::env::set_var("PRYX_HOME", temp.path().join("pryx-home"));
        pryx::env::set_var("NO_PROXY", "127.0.0.1,localhost");
        pryx::env::set_var("no_proxy", "127.0.0.1,localhost");
    ...
    fn configure_openai_compatible_runtime(
        ...
        pryx::env::set_var("PRYX_OPENROUTER_API_BASE", api_base);
        pryx::env::set_var("PRYX_OPENROUTER_API_KEY_NAME", "AUTH_FLOW_TEST_KEY");
        pryx::env::set_var("PRYX_OPENROUTER_ENV_FILE", "auth-flow-test.env");
        pryx::env::set_var("PRYX_OPENROUTER_CACHE_NAMESPACE", cache_namespace);
        pryx::env::set_var("PRYX_OPENROUTER_PROVIDER_FEATURES", "0");
        pryx::env::set_var("PRYX_OPENROUTER_MODEL_CATALOG", "1");
        if let Some(key) = key {
            pryx::env::set_var("AUTH_FLOW_TEST_KEY", key);
        } else {
            pryx::env::remove_var("AUTH_FLOW_TEST_KEY");
        }
        if allow_no_auth {
            pryx::env::set_var("PRYX_OPENROUTER_ALLOW_NO_AUTH", "1");
        } else {
            pryx::env::remove_var("PRYX_OPENROUTER_ALLOW_NO_AUTH");
        }
        AuthStatus::invalidate_cache();
    }
}
...
fn live_models_contract_supports_api_key_header_mode() -> Result<()> {
    ...
    pryx::env::set_var("PRYX_OPENROUTER_AUTH_HEADER", "api-key");
    pryx::env::set_var("PRYX_OPENROUTER_AUTH_HEADER_NAME", "x-api-key");
...
fn model_picker_cache_miss_schedules_single_background_refresh_and_updates_routes() -> Result<()> {
    ...
    pryx::env::set_var("PRYX_OPENROUTER_MODEL", "background-race-selected-model");
...
fn live_model_catalog_failure_keeps_static_and_selected_model_picker_fallbacks() -> Result<()> {
    ...
    pryx::env::set_var("PRYX_OPENROUTER_MODEL", "selected-fallback-model");
    pryx::env::set_var(
        "PRYX_OPENROUTER_STATIC_MODELS",
