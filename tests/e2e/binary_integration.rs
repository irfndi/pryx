...
// These tests run the actual pryx binary and require real credentials.
...
/// Test that the pryx binary can run independent with Claude provider
#[tokio::test]
...
/// Test that the pryx binary can run with OpenAI provider
#[tokio::test]
...
/// Test that pryx version command works
#[tokio::test]
async fn binary_version_command() -> Result<()> {
    use std::process::Command;
    let _env = setup_test_env()?;

    let output = Command::new(env!("CARGO_BIN_EXE_pryx"))
        .arg("--version")
        .output()?;
    ...
    assert!(
        stdout.contains("pryx") || stdout.contains("20"),
        "Version should contain 'pryx' or date. Got: {}",
        stdout
    );
...
///
/// Requires a built release binary at target/release/pryx because the reload
...
async fn binary_integration_reload_handoff() -> Result<()> {
    ...
    let release_binary =
        pryx::build::release_binary_path(std::path::Path::new(env!("CARGO_MANIFEST_DIR")));
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_pryx"))
        .arg("--no-update")
        .arg("--socket")
        .arg(&socket_path)
        .arg("serve")
        ...
        .env_remove("PRYX_TEST_SESSION")
        .env("PRYX_HOME", &home_dir)
        .env("PRYX_RUNTIME_DIR", &runtime_dir)
        .env("PRYX_INSTALL_DIR", &install_dir)
        .env("PRYX_DEBUG_CONTROL", "1")
        .env("PRYX_TEMP_SERVER", "1")
        .env("PRYX_SERVER_OWNER_PID", std::process::id().to_string())
        .stdin(Stdio::null())
        .stdout(Stdio::null())
    ...
    let test_result = async {
        ...
        while pryx::server::reload_marker_active(Duration::from_secs(30)) {
...
///
/// Requires a built release binary at target/release/pryx because the
...
async fn binary_integration_selfdev_reload_reconnects_quickly() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("pryx-selfdev-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let _home_guard = EnvVarGuard::set("PRYX_HOME", &home_dir);
    let _runtime_guard = EnvVarGuard::set("PRYX_RUNTIME_DIR", &runtime_dir);
    let _install_guard = EnvVarGuard::set("PRYX_INSTALL_DIR", &install_dir);
...
async fn binary_integration_selfdev_client_reload_resumes_session() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("pryx-selfdev-client-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let socket_path = runtime_dir.join("pryx.sock");
    let debug_socket_path = runtime_dir.join("pryx-debug.sock");
    let starter_binary = temp_root.path().join("pryx-selfdev-client-starter");
    std::fs::copy(env!("CARGO_BIN_EXE_pryx"), &starter_binary)?;
...
async fn binary_integration_selfdev_full_reload_resumes_session_quickly() -> Result<()> {
    ...
    let temp_root = tempfile::Builder::new()
        .prefix("pryx-selfdev-full-reload-e2e-")
        .tempdir()?;
    let runtime_dir = temp_root.path().join("runtime");
    let home_dir = temp_root.path().join("home");
    let install_dir = temp_root.path().join("install");
    ...
    let socket_path = runtime_dir.join("pryx.sock");
    let debug_socket_path = runtime_dir.join("pryx-debug.sock");
    let starter_binary = temp_root.path().join("pryx-selfdev-full-reload-starter");
    std::fs::copy(env!("CARGO_BIN_EXE_pryx"), &starter_binary)?;
    let starter_mtime = std::fs::metadata(&release_binary)?
        .modified()?
        .checked_sub(Duration::from_secs(60))
