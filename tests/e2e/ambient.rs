...
fn test_ambient_state_lifecycle() {
    use pryx::ambient::{AmbientCycleResult, AmbientState, AmbientStatus, CycleStatus};
...
fn test_ambient_scheduled_queue() {
    use pryx::ambient::{Priority, ScheduledItem, ScheduledQueue};

    let tmp = std::env::temp_dir().join("pryx-test-queue.json");
    ...
    queue.push(ScheduledItem {
        ...
        target: pryx::ambient::ScheduleTarget::Ambient,
...
fn test_adaptive_scheduler_intervals() {
    use pryx::ambient_scheduler::{AdaptiveScheduler, AmbientSchedulerConfig};
...
async fn test_ambient_end_cycle_tool() -> Result<()> {
    ...
    let result = pryx::tool::ambient::take_cycle_result();
...
async fn test_ambient_request_permission_tool() -> Result<()> {
    ...
    pryx::tool::ambient::register_ambient_session(ambient_session_id.clone());

    let response = agent.run_once_capture("Request permission").await?;
    pryx::tool::ambient::unregister_ambient_session(&ambient_session_id);
...
async fn test_ambient_runner_status() {
    use pryx::ambient_runner::AmbientRunnerHandle;
...
async fn test_ambient_runner_trigger_and_stop() {
    use pryx::ambient::AmbientStatus;
    use pryx::ambient_runner::AmbientRunnerHandle;
    use pryx::safety::SafetySystem;
...
fn test_memory_reinforcement_provenance() {
    use pryx::memory::{MemoryCategory, MemoryEntry};
...
fn test_ambient_lock() {
    use pryx::ambient::AmbientLock;
    let _env = setup_test_env().expect("failed to setup isolated PRYX_HOME");
...
async fn test_full_ambient_cycle_simulation() -> Result<()> {
    ...
    agent.set_system_prompt("You are the pryx ambient maintenance agent.");
