...
async fn test_resume_restores_model_and_tool_history() -> Result<()> {
    ...
    session.add_message(
        pryx::message::Role::User,
        vec![pryx::message::ContentBlock::Text {
            text: "Run a tool".to_string(),
            cache_control: None,
        }],
    );
    session.add_message(
        pryx::message::Role::Assistant,
        vec![
            pryx::message::ContentBlock::Text {
                text: "Running...".to_string(),
                cache_control: None,
            },
            pryx::message::ContentBlock::ToolUse {
                id: "tool-1".to_string(),
                name: "bash".to_string(),
    ...
    session.add_message(
        pryx::message::Role::User,
        vec![pryx::message::ContentBlock::ToolResult {
...
async fn test_resume_session_with_local_history_uses_metadata_only_history() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn pryx::provider::Provider> = provider.clone();
...
/// The agent should identify as "pryx" or just a generic "coding assistant powered by Claude"
#[tokio::test]
async fn test_system_prompt_no_claude_code_identity() -> Result<()> {
    ...
    let provider_dyn: Arc<dyn pryx::provider::Provider> = provider;
    ...
    // Should identify as pryx
    assert!(
        lower_identity.contains("pryx"),
        "System prompt should identify as pryx. Found: {}",
        identity_portion
    );
