...
impl SpawnedWindowsServer {
    fn pryx_binary() -> std::path::PathBuf {
        ...
        std::path::PathBuf::from(env!("CARGO_BIN_EXE_pryx"))
    }
    ...
    fn spawn(prefix: &str) -> Result<Self> {
        ...
        let socket_path = runtime_dir.join("pryx-windows-lifecycle.sock");
        let debug_socket_path = runtime_dir.join("pryx-windows-lifecycle-debug.sock");

        let stdout_file = std::fs::File::create(&stdout_path)?;
        let stderr_file = std::fs::File::create(&stderr_path)?;
        let mut command = Command::new(Self::pryx_binary());
        command
            .arg("--no-update")
            .arg("--socket")
            .arg(&socket_path)
            ...
            .env("PRYX_OPENAI_COMPAT_API_BASE", "http://127.0.0.1:9/v1")
            .env("PRYX_OPENAI_COMPAT_DEFAULT_MODEL", "windows-e2e-model")
            .env("PRYX_OPENAI_COMPAT_LOCAL_ENABLED", "1")
    ...
    fn apply_env<'a>(&self, command: &'a mut Command) -> &'a mut Command {
        command
            .env_remove("PRYX_TEST_SESSION")
            .env("PRYX_HOME", &self.home_dir)
            .env("PRYX_RUNTIME_DIR", &self.runtime_dir)
            .env("PRYX_INSTALL_DIR", &self.install_dir)
            .env("PRYX_NO_TELEMETRY", "1")
            .env("PRYX_OPENAI_COMPAT_API_BASE", "http://127.0.0.1:9/v1")
            .env("PRYX_OPENAI_COMPAT_DEFAULT_MODEL", "windows-e2e-model")
            .env("PRYX_OPENAI_COMPAT_LOCAL_ENABLED", "1")
    ...
    fn dump_logs(&self, label: &str) {
        ...
        if let Ok(artifact_root) = std::env::var("PRYX_E2E_ARTIFACT_DIR") {
            ...
            if let Ok(entries) = std::fs::read_dir(logs_dir) {
                let copied_logs_dir = artifact_dir.join("pryx-logs");
...
async fn windows_binary_server_accepts_clients_and_debug_cli() -> Result<()> {
    ...
    let server = SpawnedWindowsServer::spawn("pryx-windows-lifecycle-")?;
    ...
    let result = async {
        ...
        anyhow::ensure!(
            info.get("debug_control_enabled")
                ...
                == Some(true),
            "server should honor PRYX_DEBUG_CONTROL in Windows e2e"
        );
...
async fn windows_binary_server_rebinds_named_pipe_after_exit() -> Result<()> {
    let _env = setup_test_env()?;
    let mut first = SpawnedWindowsServer::spawn("pryx-windows-rebind-")?;
