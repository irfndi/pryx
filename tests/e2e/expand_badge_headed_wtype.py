...
Uses kitty remote control to launch a real pryx TUI window, prepares the same
...
def main():
    ...
    binary = os.environ.get("PRYX_E2E_BIN", str(REPO / "target" / "selfdev" / "pryx"))
    kitty_sock = os.environ.get("KITTY_E2E_SOCKET", find_kitty_socket())
    title = f"PRYX_EXPAND_E2E_{int(time.time())}"
