//! End-to-end tests for pryx using a mock provider
//!
...
pub(crate) use pryx::agent::Agent;
pub(crate) use pryx::message::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
pub(crate) use pryx::protocol::{Request, ServerEvent};
pub(crate) use pryx::provider::{EventStream, Provider};
pub(crate) use pryx::server;
pub(crate) use pryx::session::{Session, StoredCompactionState};
pub(crate) use pryx::tool::Registry;
...
static PRYX_HOME_LOCK: std::sync::OnceLock<Mutex<()>> = std::sync::OnceLock::new();
...
fn lock_pryx_home() -> std::sync::MutexGuard<'static, ()> {
    let mutex = PRYX_HOME_LOCK.get_or_init(|| Mutex::new(()));
...
impl TestEnvGuard {
    pub(crate) fn new() -> Result<Self> {
        let lock = lock_pryx_home();
        let temp_home = tempfile::Builder::new()
            .prefix("pryx-e2e-home-")
            .tempdir()?;
        let prev_home = std::env::var_os("PRYX_HOME");
        let prev_runtime_dir = std::env::var_os("PRYX_RUNTIME_DIR");
        let prev_test_session = std::env::var_os("PRYX_TEST_SESSION");
        let prev_debug_control = std::env::var_os("PRYX_DEBUG_CONTROL");
        ...
        let prev_openrouter_cache_namespace = std::env::var_os("PRYX_OPENROUTER_CACHE_NAMESPACE");
        let runtime_dir = temp_home.path().join("runtime");
        std::fs::create_dir_all(&runtime_dir)?;

        pryx::env::set_var("PRYX_HOME", temp_home.path());
        pryx::env::set_var("PRYX_RUNTIME_DIR", &runtime_dir);
        pryx::env::set_var("PRYX_TEST_SESSION", "1");
        pryx::env::set_var("PRYX_DEBUG_CONTROL", "1");
        pryx::env::remove_var("PRYX_RUNTIME_PROVIDER");
        pryx::env::remove_var("PRYX_ACTIVE_PROVIDER");
        pryx::env::remove_var("PRYX_OPENROUTER_CACHE_NAMESPACE");
        ...
        pryx::env::set_var("PRYX_MEMORY_ENABLED", "0");
        pryx::env::set_var("PRYX_MEMORY_SIDECAR_ENABLED", "0");
...
impl Drop for TestEnvGuard {
    fn drop(&mut self) {
        if let Some(prev_home) = &self.prev_home {
            pryx::env::set_var("PRYX_HOME", prev_home);
        } else {
            pryx::env::remove_var("PRYX_HOME");
        }

        if let Some(prev_runtime_dir) = &self.prev_runtime_dir {
            pryx::env::set_var("PRYX_RUNTIME_DIR", prev_runtime_dir);
        } else {
            pryx::env::remove_var("PRYX_RUNTIME_DIR");
        }

        if let Some(prev_test_session) = &self.prev_test_session {
            pryx::env::set_var("PRYX_TEST_SESSION", prev_test_session);
        } else {
            pryx::env::remove_var("PRYX_TEST_SESSION");
        }

        if let Some(prev_debug_control) = &self.prev_debug_control {
            pryx::env::set_var("PRYX_DEBUG_CONTROL", prev_debug_control);
        } else {
            pryx::env::remove_var("PRYX_DEBUG_CONTROL");
        }

        if let Some(prev_runtime_provider) = &self.prev_runtime_provider {
            pryx::env::set_var("PRYX_RUNTIME_PROVIDER", prev_runtime_provider);
        } else {
            pryx::env::remove_var("PRYX_RUNTIME_PROVIDER");
        }

        if let Some(prev_active_provider) = &self.prev_active_provider {
            pryx::env::set_var("PRYX_ACTIVE_PROVIDER", prev_active_provider);
        } else {
            pryx::env::remove_var("PRYX_ACTIVE_PROVIDER");
        }
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = &self.prev {
            pryx::env::set_var(self.name, prev);
        } else {
            pryx::env::remove_var(self.name);
        }
    }
}
...
fn pair_test_device(token: &str) -> Result<()> {
    let mut registry = pryx::gateway::DeviceRegistry::load();
    ...
    registry.devices.push(pryx::gateway::PairedDevice {
...
pub(crate) async fn run_unix_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    std::fs::create_dir_all(&runtime_dir)?;
    let socket_path = runtime_dir.join("pryx.sock");
    let debug_socket_path = runtime_dir.join("pryx-debug.sock");
    ...
    let provider: Arc<dyn pryx::provider::Provider> = Arc::new(provider);
    let server_instance =
    ...
    let result = async {
        ...
        if !saw_message_done {
            ...
            if !response_persisted {
                ...
                let logs = std::env::var_os("PRYX_HOME")
...
pub(crate) async fn run_websocket_transport_scenario() -> Result<TransportScenarioResult> {
    ...
    let server_instance =
        server::Server::new_with_paths(provider, socket_path.clone(), debug_socket_path.clone())
            .with_gateway_config(pryx::gateway::GatewayConfig {
                port: gateway_port,
...
pub(crate) async fn wait_for_selfdev_reload_cycle(
    ...
    while Instant::now() < deadline {
        let marker_active = pryx::server::reload_marker_active(Duration::from_secs(30));
