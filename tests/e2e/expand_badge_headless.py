...
This starts a real pryx TUI client inside a pseudo-terminal, prepares a real
...
Run from repo root after building pryx:
...
SOCKET_PATH = os.path.join(RUNTIME_DIR, "pryx-debug.sock")
...
def main():
    binary = os.environ.get("PRYX_E2E_BIN")
    if not binary:
        candidates = [
            REPO / "target" / "selfdev" / "pryx",
            Path.home() / ".pryx" / "builds" / "current" / "pryx",
        ]
        binary = next((str(p) for p in candidates if p.exists()), None)
    if not binary:
        raise SystemExit("No pryx binary found. Set PRYX_E2E_BIN or build first.")
    ...
    try:
        ...
        env.setdefault("PRYX_CLIENT_SELFDEV_MODE", "1")
