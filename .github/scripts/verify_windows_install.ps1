...
$originalEnvironment = @{
    ...
    PRYX_HOME = $env:PRYX_HOME
    PRYX_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE = $env:PRYX_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE
}
...
if (-not $Version) {
    ...
    if ($artifactVersionText -notmatch '(?i)\bpryx\s+v?(\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?)\b') {
        throw "Could not parse pryx version from local artifact output: $artifactVersionText"
    }
    $Version = 'v' + $Matches[1]
} else {
...
$tempRoot = Join-Path $tempBase ("pryx-windows-install-verify-" + [guid]::NewGuid().ToString('N'))
$localAppData = Join-Path $tempRoot 'localappdata'
$appData = Join-Path $tempRoot 'appdata'
$userProfile = Join-Path $tempRoot 'userprofile'
$pryxHome = Join-Path $tempRoot '.pryx'
$installDir = Join-Path $localAppData 'pryx\bin'

try {
New-Item -ItemType Directory -Force -Path $localAppData, $appData, $userProfile, $pryxHome | Out-Null

$env:LOCALAPPDATA = $localAppData
$env:APPDATA = $appData
$env:USERPROFILE = $userProfile
$env:PRYX_HOME = $pryxHome
$env:PRYX_WINDOWS_SETUP_SKIP_PROCESS_LIFECYCLE = '1'
...
$versionDir = Join-Path $localAppData ('pryx\builds\versions\' + $Version.TrimStart('v') + '\pryx.exe')
$stablePath = Join-Path $localAppData 'pryx\builds\stable\pryx.exe'
...
$startupShortcut = Join-Path $appData 'Microsoft\Windows\Start Menu\Programs\Startup\pryx-hotkey.lnk'
...
if ($versionOutput -notmatch 'pryx') {
...
$legacyVbs = Join-Path $hotkeyDir 'pryx-hotkey-launcher.vbs'
