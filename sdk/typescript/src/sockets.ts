/**
 * Socket path resolution, mirroring `pryx-harness-api::sockets`.
 *
...
export function runtimeDir(): string {
  const explicit = process.env.PRYX_RUNTIME_DIR;
  ...
  return path.join(os.tmpdir(), `pryx-${userDiscriminator()}`);
}
 ...
 * `pryx-transport`'s derivation exactly, and the two are pinned together by a
 * test on each side.
 */
export function transportEndpoint(socketPath: string): string {
  ...
  const stem =
    (path.parse(socketPath).name.match(/[A-Za-z0-9\-_]/g) ?? []).join("").slice(0, 32) || "pryx";
  const normalized = socketPath.replace(/\\/g, "/").toLowerCase();
  const hash = createHash("sha256").update(normalized).digest("hex").slice(0, 16);
  return `\\\\.\\pipe\\${stem}-${hash}`;
}

/** Path of the versioned harness API socket. `PRYX_API_SOCKET` overrides. */
export function apiSocketPath(): string {
  return process.env.PRYX_API_SOCKET ?? path.join(runtimeDir(), "pryx-api.sock");
}

/** Path of the internal daemon socket. `PRYX_SOCKET` overrides. */
export function legacySocketPath(): string {
  return process.env.PRYX_SOCKET ?? path.join(runtimeDir(), "pryx.sock");
}
