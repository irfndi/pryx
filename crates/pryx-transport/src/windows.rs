...
///
/// e.g. `/run/user/1000/pryx.sock` -> `\\.\pipe\pryx`
/// e.g. `/run/user/1000/pryx/myserver.sock` -> `\\.\pipe\pryx-myserver`
fn path_to_pipe_name(path: &Path) -> String {
    let stem: String = path
        ...
        .take(32)
        .collect();
    let stem = if stem.is_empty() { "pryx" } else { &stem };
    let normalized = path
        .to_string_lossy()
        .replace('\\', "/")
        .to_ascii_lowercase();
...
impl Stream {
    ...
    pub fn pair() -> io::Result<(Self, Self)> {
        ...
        let pipe_name = format!(r"\\.\pipe\pryx-pair-{}-{}", std::process::id(), counter);
...
mod tests {
    ...
    fn pipe_name_matches_the_typescript_sdk() {
        for (path, expected) in [
            (
                r"C:\Users\jeremy\AppData\Local\pryx\run\pryx-api.sock",
                r"\\.\pipe\pryx-api-5e00c01702e8cfe4",
            ),
            (r"C:\a\b\pryx.sock", r"\\.\pipe\pryx-52dfdb00b2f35a71"),
        ] {
            assert_eq!(
    ...
    fn pipe_name_is_stable_and_normalizes_case_and_separators() {
        let a = path_to_pipe_name(Path::new(r"C:\Temp\Pryx\server.sock"));
        let b = path_to_pipe_name(Path::new("c:/temp/pryx/server.sock"));
