...
//! Gemini/Claude/gpt-oss upstreams), moved out of `pryx-base` so provider
...
//! [`AntigravityProvider`] with `pryx_base::provider::external` at startup.

use anyhow::{Context, Result};
use async_trait::async_trait;
use pryx_base::auth::antigravity as antigravity_auth;
use pryx_message_types::{ConnectionPhase, Message, StreamEvent, ToolDefinition};
use pryx_provider_antigravity::{
...
impl AntigravityProvider {
    fn load_persisted_catalog() -> Option<PersistedCatalog> {
        pryx_base::provider::antigravity::load_persisted_catalog()
    }

    fn persist_catalog(snapshot: &CatalogSnapshot) {
        pryx_base::provider::antigravity::persist_catalog(snapshot);
    }
    ...
    pub fn new() -> Self {
        let model =
            std::env::var("PRYX_ANTIGRAVITY_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
    ...
    /// by `pryx login --provider antigravity`; there is no API-key path. This
    ...
    fn resolve_model_for_request(&self, model: &str) -> String {
        ...
        // Claude models on the Cloud Code backend currently reject pryx's tool
    ...
    async fn fetch_available_models(&self) -> Result<CatalogSnapshot> {
        pryx_base::provider::antigravity::fetch_catalog_snapshot(&self.client).await
    }

    /// Recover from a tool-schema rejection by learning what the backend
    /// refused and re-sending the turn without it.
    ///
    /// Because pryx advertises every tool on every request, one construct the
    ...
    /// #446). `pryx-schema-dialect` instead parses the construct out of the
    ...
    async fn retry_after_schema_rejection(
        ...
        signature_policy: pryx_provider_gemini::SignaturePolicy,
    ) -> Option<Result<CodeAssistGenerateResponse>> {
        let resolved = self.resolve_model_for_request(model);
        let dialect = pryx_provider_antigravity::antigravity_dialect(&resolved);
        match pryx_schema_dialect::recover_from_error(error, dialect) {
            pryx_schema_dialect::RecoveryAction::NotSchemaRelated => None,
            pryx_schema_dialect::RecoveryAction::Unrecoverable { hint } => {
                pryx_base::logging::warn(&format!("Antigravity tool-schema rejection: {hint}"));
                None
            }
            pryx_schema_dialect::RecoveryAction::RetryWithoutConstruct { description } => {
                pryx_base::logging::warn(&format!("Antigravity {description}"));
                Some(
    ...
    async fn generate_content(
        ...
        let mut tools = pryx_provider_gemini::build_tools(tools);
        ...
        // construct. Gemini-native accepts everything pryx emits, so Gemini
        ...
        let request = CodeAssistGenerateRequest {
            ...
            request: VertexGenerateContentRequest {
                contents: pryx_provider_gemini::build_contents_with_signature_policy(
                    messages,
                    signature_policy,
                ),
                system_instruction: pryx_provider_gemini::build_system_instruction_with_tool_guard(
                    system,
                    !tools_is_empty,
                ),
                tools,
...
impl Provider for AntigravityProvider {
    async fn complete(
        ...
        let (tx, rx) = mpsc::channel::<Result<pryx_message_types::StreamEvent>>(100);
        ...
        tokio::spawn(async move {
            ...
            let mut signature_policy = pryx_provider_gemini::SignaturePolicy::ReplayCarriedForward;
                ...
                .await
            {
                ...
                Err(err) => {
                        ...
                        .await
                    {
                    ...
                    } else if !pryx_provider_gemini::is_missing_thought_signature_error(
                        &err.to_string(),
                    ) {
                        ...
                        signature_policy =
                            pryx_provider_gemini::SignaturePolicy::DowngradeToolCallsToText;
                        match provider
                            .generate_content(
                                &model,
                                &messages,
            ...
            if pryx_provider_antigravity::is_pseudo_tool_call_turn(&response) {
            ...
            if let Some(content) = candidate.content {
                ...
                // calls through the standard ToolUseStart/End path so pryx
                ...
                for part in content.parts {
                    ...
                    if let Some(function_call) = part.function_call {
                        ...
                        let call_id = pryx_message_types::sanitize_tool_id(&raw_call_id);
                        let _ = tx
            ...
            if !produced_output {
                ...
                if abnormal {
                    ...
                    let detail = candidate
                        ...
                        .map(|msg| format!(": {}", pryx_base::util::truncate_str(msg.trim(), 300)))
                        .unwrap_or_default();
                    let _ = tx
                        .send(Err(anyhow::anyhow!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        // resumed turn 404.
        let trimmed = pryx_provider_core::strip_own_model_prefix(model, "antigravity:");
        if trimmed.is_empty() {
    ...
    fn model_routes(&self) -> Vec<pryx_provider_core::ModelRoute> {
        let catalog = self.fetched_catalog();
        if !catalog.is_empty() {
            return catalog
                .into_iter()
                .map(|model| pryx_provider_core::ModelRoute {
