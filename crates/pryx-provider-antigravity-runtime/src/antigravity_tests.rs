...
use pryx_provider_core::Provider;
...
fn available_models_display_seeds_from_persisted_catalog() {
    ...
    pryx_base::env::set_var("PRYX_HOME", temp.path());

    let path = pryx_base::provider::antigravity::persisted_catalog_path().expect("catalog path");
    ...
    if let Some(previous) = previous {
        pryx_base::env::set_var("PRYX_HOME", previous);
    } else {
        pryx_base::env::remove_var("PRYX_HOME");
    }
}
...
fn resolve_model_for_request_default_prefers_gemini_catalog_model() {
    ...
    *provider.fetched_catalog.write().expect("catalog lock") = vec![
    ...
    // Cloud Code backend. Claude on this backend rejects pryx's tool schemas.
    assert_eq!(
        provider.resolve_model_for_request("default"),
        "gemini-3-flash"
    );
...
fn antigravity_gemini_uses_the_same_dialect_as_the_native_gemini_provider() {
    ...
    assert_eq!(
        out,
        pryx_provider_gemini::gemini_compatible_schema(&schema),
...
fn pseudo_tool_call_turn_only_matches_the_recovery_marker_as_a_call() {
    ...
    assert!(pryx_provider_antigravity::is_pseudo_tool_call_turn(
