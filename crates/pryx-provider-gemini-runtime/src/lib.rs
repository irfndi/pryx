...
//! key), moved out of `pryx-base` so provider edits compile only this crate
...
impl GeminiProvider {
    fn persisted_catalog_path() -> Result<std::path::PathBuf> {
        Ok(pryx_base::storage::app_config_dir()?.join("gemini_models_cache.json"))
    }
    ...
    pub fn new() -> Self {
        let model = std::env::var("PRYX_GEMINI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.into());
        let provider = Self {
    ...
    /// cloudcode-pa tier. Set `PRYX_GEMINI_FORCE_OAUTH=1` to pin OAuth even when
    /// a key is present.
    fn auth_mode() -> GeminiAuthMode {
        let force_oauth = std::env::var("PRYX_GEMINI_FORCE_OAUTH")
            .map(|value| {
    ...
    ///
    /// pryx advertises every tool on every request, so one construct the
    ...
    /// `pryx-schema-dialect` remembers it so later requests never send it.
    ///
    ...
    async fn retry_after_schema_rejection(
        ...
        let dialect = &pryx_schema_dialect::registry::GEMINI;
        match pryx_schema_dialect::recover_from_error(error, dialect) {
            ...
            pryx_schema_dialect::RecoveryAction::Unrecoverable { hint } => {
                pryx_base::logging::warn(&format!("Gemini tool-schema rejection: {hint}"));
                None
            }
            pryx_schema_dialect::RecoveryAction::RetryWithoutConstruct { description } => {
                pryx_base::logging::warn(&format!("Gemini {description}"));
                Some(
...
impl Provider for GeminiProvider {
                ...
                if !produced_output {
                    ...
                    if abnormal {
                        ...
                        let detail = candidate
                            ...
                            .filter(|msg| !msg.trim().is_empty())
                            .map(|msg| {
                                format!(": {}", pryx_base::util::truncate_str(msg.trim(), 300))
                            })
                            .unwrap_or_default();
                        let _ = tx
                            .send(Err(anyhow::anyhow!(
    ...
    fn set_model(&self, model: &str) -> Result<()> {
        ...
        let trimmed = pryx_provider_core::strip_own_model_prefix(model, "gemini:");
        if trimmed.is_empty() {
...
fn gemini_http_client() -> reqwest::Client {
    reqwest::Client::builder()
        ...
        .build()
        .unwrap_or_else(|_| pryx_provider_core::shared_http_client())
}
...
fn is_transient_gemini_transport_error(err: &reqwest::Error) -> bool {
    ...
    err.is_connect()
        || err.is_timeout()
        || pryx_provider_core::is_transient_transport_error(&err.to_string())
}
