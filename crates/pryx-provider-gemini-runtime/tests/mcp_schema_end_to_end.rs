...
//!
//! The unit tests in `pryx-schema-dialect` work on schema values, and the
//! sweep in `pryx-app-core` works on pryx's own tools. Neither covers the
...
//! wire and asserts on the bytes pryx sends to the provider.

use pryx_base::mcp::McpToolDef;
...
fn playwright_schema_reaches_gemini_without_the_keyword_that_caused_issue_754() {
    ...
    let built = pryx_provider_gemini::build_tools(&defs).expect("gemini tools");
...
fn playwright_schema_reaches_openai_without_an_unsupported_construct() {
    let defs = playwright_tool_definitions();
    let built = pryx_provider_openai::request::build_tools(&defs);
...
fn the_normalized_playwright_schema_conforms_to_every_dialect() {
    ...
    for spec in pryx_schema_dialect::registry::ALL {
        let normalized = pryx_schema_dialect::dialect::apply(schema, spec);
        let errors =
        ...
        let lost = pryx_schema_dialect::must_preserve_meaning(schema, &normalized);
        assert!(
            lost.is_empty(),
...
fn recovery_triggers_on_the_error_string_the_runtime_really_builds() {
    ...
    let rejection = pryx_schema_dialect::classify(&with_context)
