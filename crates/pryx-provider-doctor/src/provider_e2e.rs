...
//!
//! This powers `pryx provider-doctor`: it walks the same strict provider/model
...
//! - [`DoctorTier::Offline`]: no API key, no network, no spend. Validates pryx's
...
use pryx_base::protocol::{AuthChanged, CatalogNamespace, RuntimeProviderKey};
use pryx_base::provider::ModelRoute;
use pryx_base::provider_catalog::OpenAiCompatibleProfile;

/// How much of the strict pipeline to exercise.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DoctorTier {
    /// No key, no network, no spend. Validates pryx-side wiring only.
    Offline,
...
const FULL_PIPELINE_LABELS: &[(&str, &str)] = &[
    ...
    (checkpoints::REAL_PRYX_TOOL_SMOKE, "Real Pryx tool smoke"),
...
fn tool_stage_detail(stage: &pryx_base::live_tests::LiveVerificationStage) -> String {
...
/// doctor report.
fn reasoning_stage_detail(stage: &pryx_base::live_tests::LiveVerificationStage) -> String {
    match stage
        .evidence
...
const API_DEPENDENT_CHECKPOINTS: &[&str] = &[
    ...
    checkpoints::REAL_PRYX_TOOL_SMOKE,
    checkpoints::REASONING_CAPABILITY,
];
...
///
/// The predicate itself lives in `pryx_base::auth::doctor` so base-internal
...
pub use pryx_base::auth::doctor::native_doctor_supports_provider;
...
pub async fn run_claude_native_e2e(
    ...
    use pryx_base::provider::Provider;
    use pryx_provider_anthropic_runtime::AnthropicProvider;

    let normalized = pryx_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id))
        .unwrap_or("claude");
    let provider_label = pryx_base::auth::lifecycle::provider_display_label(Some(normalized))
    ...
    // `PRYX_RUNTIME_PROVIDER=claude-api` would silently test the API-key path
        ...
        checks.push(DoctorCheck::failed(
            ...
            format!(
                "could not select the Claude OAuth credential path: {error}. \
                 Run `pryx login --provider claude` to mint a fresh OAuth token."
            ),
        ));
        return Ok(finish_report(
            provider_id,
    ...
    let catalog_models: Vec<String> = if tier.requires_api_key() {
        match provider_runtime.fetch_live_model_ids_for_doctor().await {
            ...
            Ok(_) => {
                ...
                let fallback = pryx_base::provider::known_anthropic_model_ids();
                checks.push(DoctorCheck::passed(
                    checkpoints::MODEL_CATALOG_LIVE_ENDPOINT,
                    label_for(checkpoints::MODEL_CATALOG_LIVE_ENDPOINT),
                    format!(
...
/// Google OAuth tokens minted by `pryx login --provider antigravity`; the
...
/// accepts every schema construct pryx emits), then any Gemini model, then any
...
pub async fn run_antigravity_native_e2e(
    ...
    use pryx_base::provider::Provider;
    use pryx_provider_antigravity_runtime::AntigravityProvider;
    ...
    let _ = pryx_base::auth::lifecycle::normalized_auth_provider_id(Some(provider_id));
    let provider_label = pryx_base::auth::lifecycle::provider_display_label(Some("antigravity"))
                ...
                checks.push(DoctorCheck::failed(
                    ...
                    format!(
                        "could not resolve an Antigravity credential: {error}. \
                         Run `pryx login --provider antigravity` to sign in."
                    ),
                ));
                return Ok(finish_report(
                    provider_id,
...
impl NativeProviderKind {
    ...
    fn spec(self) -> NativeProviderSpec {
        match self {
            Self::OpenAi => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "pryx login --provider openai",
            },
            Self::Gemini => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "pryx login --provider gemini",
            },
            Self::Cursor => NativeProviderSpec {
                ...
                login_hint: "pryx login --provider cursor",
            },
            Self::Copilot => NativeProviderSpec {
                ...
                auth_env_key: None,
                login_hint: "pryx login --provider copilot",
            },
            Self::Bedrock => NativeProviderSpec {
                ...
                login_hint: "pryx login --provider bedrock",
            },
            Self::Pryx => NativeProviderSpec {
                provider_id: "pryx",
                label: "Pryx Subscription",
                // The transport is OpenAI-compatible internally, but the public
                // route identity is the managed Pryx subscription. Model
                // switches use a bare model id so they stay on that runtime.
                contract: WiringContract {
                    api_method: pryx_base::subscription_catalog::PRYX_ROUTE_API_METHOD
                        .to_string(),
                    route_provider: pryx_base::subscription_catalog::PRYX_PROVIDER_DISPLAY_NAME
                        .to_string(),
                    expected_runtime: "pryx",
                    expected_namespace: None,
                    switch_prefix: String::new(),
                },
                auth_source: "Pryx subscription API key (PRYX_API_KEY)",
                auth_env_key: Some("PRYX_API_KEY"),
                login_hint: "pryx login --provider pryx",
            },
            Self::Azure => NativeProviderSpec {
                ...
                login_hint: "pryx login --provider azure",
            },
        }
    }
    ...
    fn build_runtime(self) -> anyhow::Result<std::sync::Arc<dyn pryx_base::provider::Provider>> {
        use anyhow::Context as _;
        use pryx_base::provider::Provider;
        let runtime: std::sync::Arc<dyn Provider> = match self {
            Self::OpenAi => {
                let credentials =
                    pryx_base::auth::codex::load_credentials().unwrap_or_else(|_| {
                        pryx_base::auth::codex::CodexCredentials {
                ...
                std::sync::Arc::new(pryx_provider_openai_runtime::OpenAIProvider::new(
                    credentials,
                ))
            }
            Self::Gemini => {
                std::sync::Arc::new(pryx_provider_gemini_runtime::GeminiProvider::new())
            }
            Self::Cursor => {
                std::sync::Arc::new(pryx_provider_cursor_runtime::CursorCliProvider::new())
            }
            Self::Copilot => {
                ...
                pryx_base::env::set_var("PRYX_COPILOT_PREFETCH_STARTUP_GRACE_MS", "0");
                let runtime = match pryx_provider_copilot_runtime::CopilotApiProvider::new() {
                    Ok(runtime) => runtime,
                    Err(_) => pryx_provider_copilot_runtime::CopilotApiProvider::new_with_token(
                        String::new(),
                    ),
                };
            ...
            Self::Bedrock => {
                std::sync::Arc::new(pryx_base::provider::bedrock::BedrockProvider::new())
            }
            Self::Pryx => std::sync::Arc::new(pryx_base::provider::pryx::PryxProvider::new()),
            Self::Azure => {
                ...
                pryx_base::auth::azure::apply_runtime_env()
                    .context("apply Azure OpenAI runtime env")?;
                let runtime = pryx_provider_openrouter_runtime::OpenRouterProvider::new()
                ...
                if let Some(model) = pryx_base::auth::azure::load_model() {
    ...
    async fn resolve_credential(self) -> anyhow::Result<String> {
        ...
        match self {
            Self::OpenAi => {
                let credentials = pryx_base::auth::codex::load_credentials()
                    .context("load OpenAI credentials (run `pryx login --provider openai`)")?;
            ...
            Self::Gemini => {
                let tokens = pryx_base::auth::gemini::load_or_refresh_tokens()
                    .await
            ...
            Self::Cursor => {
                let key = pryx_base::auth::cursor::load_api_key()
                    .context("load Cursor credential (run `pryx login --provider cursor`)")?;
            ...
            Self::Copilot => {
                let token = pryx_base::auth::copilot::load_github_token()
                    .context("load GitHub Copilot token (run `pryx login --provider copilot`)")?;
            ...
            Self::Bedrock => {
                if !pryx_base::provider::bedrock::BedrockProvider::has_credentials() {
                    anyhow::bail!(
            ...
            Self::Pryx => {
                if !pryx_base::subscription_catalog::has_credentials() {
                    anyhow::bail!(
                        "no Pryx subscription credential found (set PRYX_API_KEY or run \
                         `pryx login --provider pryx`)"
                    );
                }
                Ok("Pryx subscription credential resolved".to_string())
            }
            Self::Azure => {
                if !pryx_base::auth::azure::has_configuration() {
                    anyhow::bail!(
                        "Azure OpenAI is not fully configured (need AZURE_OPENAI_ENDPOINT plus an \
                         API key or Entra ID); run `pryx login --provider azure`"
                    );
                }
                Ok(format!(
                    "Azure OpenAI configured ({})",
                    pryx_base::auth::azure::method_detail()
                ))
            }
        }
    }
    ...
    fn cheapest_model(self, catalog: &[String]) -> Option<String> {
        ...
        let cheap_markers: &[&str] = match self {
            ...
            Self::Pryx => &["mini", "flash", "haiku", "lite", "nano"],
...
struct NativeProviderSpec {
    ...
    label: &'static str,
    /// pryx-side routing/activation contract for the wiring checkpoints.
    contract: WiringContract,
    ...
    /// `pryx login` hint surfaced when the credential cannot be resolved.
...
async fn run_generic_native_api_checks(
    provider: &dyn pryx_base::provider::Provider,
    selected: &str,
    label: &str,
    checks: &mut Vec<DoctorCheck>,
    spend: &mut DoctorSpend,
...
/// The pryx-side wiring a given compat profile is expected to activate.
///
...
/// and pryx remaps them to their native runtimes. The doctor must assert the
...
fn wiring_contract(profile: OpenAiCompatibleProfile) -> WiringContract {
    match pryx_base::auth::lifecycle::normalized_auth_provider_id(Some(profile.id)) {
...
fn run_wiring_checks_for_contract(
    ...
    let auth = AuthChanged {
        provider: pryx_base::protocol::AuthProviderId::new(auth_provider_id),
...
mod tests {
    ...
    fn native_provider_specs_are_self_consistent() {
        ...
        for kind in [
            ...
            if kind == NativeProviderKind::Pryx {
                assert!(
                    spec.contract.switch_prefix.is_empty(),
                    "Pryx switches must use bare managed model ids"
                );
            } else {
                assert!(
    ...
    /// `native_doctor_supports_provider` lives in `pryx_base::auth::doctor`
    ...
    #[test]
    fn native_pryx_contract_uses_managed_subscription_identity() {
        let contract = NativeProviderKind::Pryx.spec().contract;
        assert_eq!(
            contract.api_method,
            pryx_base::subscription_catalog::PRYX_ROUTE_API_METHOD
        );
        assert_eq!(
            contract.route_provider,
            pryx_base::subscription_catalog::PRYX_PROVIDER_DISPLAY_NAME
        );
        assert_eq!(contract.expected_runtime, "pryx");
        assert!(contract.expected_namespace.is_none());
