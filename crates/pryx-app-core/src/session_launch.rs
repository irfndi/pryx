//! Launching pryx sessions in new terminal windows.
//!
//! These helpers spawn a fresh `pryx` process (resume or self-dev) inside a
...
/// spawn hooks and spawned terminals as `PRYX_SPAWN_*` env vars so external
...
pub struct SessionSpawnContext {
    ...
    /// Extra `PRYX_SPAWN_*` env entries (e.g. swarm/coordinator ids).
...
/// `PRYX_FOCUS_SESSION_ID` and `PRYX_FOCUS_TITLE` env vars.
...
/// var is exported natively and under a `PRYX_CLIENT_<NAME>` alias.
pub fn focus_session_via_hook_with_env(
    ...
    cmd.args(args)
        .env("PRYX_FOCUS_SESSION_ID", session_id)
        .env("PRYX_FOCUS_TITLE", title)
...
fn focus_title_best_effort(title: &str) {
    ...
    cmd.arg("-c")
        .arg(
            "sleep 0.4; \
             if command -v wmctrl >/dev/null 2>&1; then wmctrl -a \"$PRYX_WINDOW_TITLE\" >/dev/null 2>&1 && exit 0; fi; \
             if command -v xdotool >/dev/null 2>&1; then xdotool search --name \"$PRYX_WINDOW_TITLE\" windowactivate >/dev/null 2>&1 && exit 0; fi; \
             exit 0",
        )
        .env("PRYX_WINDOW_TITLE", title)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn resume_terminal_candidates_windows() -> Vec<String> {
    std::env::var("PRYX_RESUME_TERMINAL")
        .ok()
        .map(|value| {
            value
                .split(',')
...
pub fn spawn_resume_in_new_terminal_with_context(
    ...
    let mut pryx_args: Vec<String> = Vec::new();
    if let Some(provider_arg) = resume_provider_arg(provider_key) {
        pryx_args.push("--provider".to_string());
        pryx_args.push(provider_arg.to_string());
    }
    pryx_args.push("--resume".to_string());
    pryx_args.push(session_id.to_string());

    let hook_command = crate::terminal_launch::TerminalCommand::new(exe, pryx_args.clone())
