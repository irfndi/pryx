...
pub(super) async fn await_reload_signal(
    ...
    loop {
        ...
        if std::env::var("PRYX_TEST_SESSION")
            .map(|value| {
        ...
        {
            crate::logging::info(
                "Server: PRYX_TEST_SESSION set, skipping process exec for reload test",
            );
            continue;
        }
        ...
        if let Some((binary, label)) = super::reload_exec_target(prefers_selfdev) {
            if binary.exists() {
                ...
                {
                    cmd.env("PRYX_RELOAD_AUTH_STATUS", auth_status);
                }
