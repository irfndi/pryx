...
//! `pryx_plan::dag`. Each handler lifts the swarm's current `VersionedPlan` into
//! a `TaskGraph` (via `pryx_plan::bridge`), applies the engine op (which enforces
...
use pryx_plan::MAX_PLAN_ITEMS;
use pryx_plan::bridge::{apply_task_graph, parse_kind, to_task_graph};
use pryx_plan::dag::{self, HandoffArtifact, NodeSpec, NodeStatus, TaskGraph};
