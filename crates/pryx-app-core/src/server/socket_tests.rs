...
fn sibling_socket_path_roundtrip() {
    let main = std::path::PathBuf::from("/tmp/pryx.sock");
    let debug = std::path::PathBuf::from("/tmp/pryx-debug.sock");
...
fn cleanup_socket_pair_removes_main_and_debug_files() {
    ...
    let main = dir.join(format!("pryx-test-{}.sock", stamp));
    let debug = dir.join(format!("pryx-test-{}-debug.sock", stamp));
...
async fn reap_stale_socket_removes_dead_socket_pair_and_lock() {
    ...
    let socket = temp.path().join("pryx.sock");
    let debug = temp.path().join("pryx-debug.sock");
...
fn existing_server_start_errors_are_detected() {
    assert!(server_start_matches_existing_server(
        "Error: Another pryx server process is already running for runtime dir /run/user/1000"
    ));
    assert!(server_start_matches_existing_server(
        "Error: Refusing to replace active server socket at /run/user/1000/pryx.sock"
    ));
    assert!(!server_start_matches_existing_server(
