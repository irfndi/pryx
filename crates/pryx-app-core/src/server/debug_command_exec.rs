...
use anyhow::Result;
use pryx_agent_runtime::{InterruptSignal, SoftInterruptSource};
...
pub(super) fn debug_message_timeout_secs() -> Option<u64> {
    let raw = std::env::var("PRYX_DEBUG_MESSAGE_TIMEOUT_SECS").ok()?;
...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "mcp" || trimmed == "mcp:servers" {
        ...
        let config_path = if let Ok(pryx_dir) = crate::storage::pryx_dir() {
            let path = pryx_dir.join("mcp.json");
            if path.exists() {
        ...
        let default_model = match provider.as_str() {
            "claude" | "anthropic" => {
                if claude_usage_exhausted {
                    ...
                    pryx_provider_core::DEFAULT_CLAUDE_MODEL
                }
            }
            "openai" | "codex" => pryx_provider_core::DEFAULT_OPENAI_MODEL,
    ...
    if trimmed == "reload" {
        let repo_dir = crate::build::get_repo_dir()
            .ok_or_else(|| anyhow::anyhow!("Could not find pryx repository directory"))?;

        let target_binary = crate::build::find_dev_binary(&repo_dir)
            .unwrap_or_else(|| build::release_binary_path(&repo_dir));
        if !target_binary.exists() {
            return Err(anyhow::anyhow!(format!(
                "No binary found at {}. Run 'pryx self-dev --build' first, or build with 'scripts/dev_cargo.sh build --profile selfdev -p pryx --bin pryx' and publish current.",
        ...
        let pryx_dir = crate::storage::pryx_dir()?;
        let info_path = pryx_dir.join("reload-info");
...
mod tests {
    ...
    use pryx_agent_runtime::InterruptSignal;
    ...
    /// which set `PRYX_HOME` under `storage::lock_test_env`). Two mutexes
    ...
    async fn debug_tool_selfdev_reload_returns_promptly_for_direct_execution() {
        ...
        let _test_session = EnvGuard::set("PRYX_TEST_SESSION", "1");
        let _debug_control = EnvGuard::set("PRYX_DEBUG_CONTROL", "1");
