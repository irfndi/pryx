...
pub(crate) fn debug_control_allowed() -> bool {
    ...
    if std::env::var("PRYX_DEBUG_CONTROL")
        .ok()
    ...
    if let Ok(pryx_dir) = crate::storage::pryx_dir()
        && pryx_dir.join("debug_control").exists()
    {
        return true;
    }
    false
...
pub(crate) fn embedding_idle_unload_secs() -> u64 {
    parse_embedding_idle_unload_secs(
        std::env::var("PRYX_EMBEDDING_IDLE_UNLOAD_SECS")
            .ok()
            .as_deref(),
    )
}
...
pub(crate) fn swarm_id_for_dir(dir: Option<PathBuf>) -> Option<String> {
    if let Ok(sw_id) = std::env::var("PRYX_SWARM_ID") {
...
/// default. `PRYX_SWARM_ID` remains an explicit opt-in to a shared swarm.
...
pub(crate) fn startup_headless_recovery_test_delay() -> Option<std::time::Duration> {
    let raw = std::env::var("PRYX_TEST_HEADLESS_STARTUP_RECOVERY_DELAY_MS").ok()?;
...
mod newer_binary_tests {
    ...
    fn reports_update_when_candidate_is_strictly_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/pryx"), Some(t(200)))];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/pryx")),
            candidates,
        ));
    }
    ...
    fn ignores_candidate_that_is_not_newer() {
        let candidates = vec![(PathBuf::from("/x/stable/pryx"), Some(t(100)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/pryx")),
            candidates,
    ...
    fn never_reloads_into_self_even_if_paths_were_equal() {
        ...
        let candidates = vec![(PathBuf::from("/x/current/pryx"), Some(t(999)))];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/pryx")),
            candidates,
    ...
    fn suppresses_update_when_candidate_mtime_unavailable() {
        ...
        let candidates = vec![(PathBuf::from("/x/stable/pryx"), None)];
        assert!(!newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/pryx")),
            candidates,
    ...
    fn reports_update_if_any_candidate_is_newer() {
        let candidates = vec![
            (PathBuf::from("/x/stable/pryx"), None),
            (PathBuf::from("/x/shared/pryx"), Some(t(300))),
        ];
        assert!(newer_binary_available(
            Some(t(100)),
            Some(std::path::Path::new("/x/current/pryx")),
    ...
    fn newer_server_is_not_outdated_by_older_channel_binary() {
        ...
        assert!(!newer_binary_available(
            Some(t(300)),
            Some(std::path::Path::new("/x/builds/versions/dev/pryx")),
            candidates,
        ));
    }
...
mod reload_target_tests {
    ...
    fn same_binary_is_always_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/current/pryx"),
            Path::new("/x/current/pryx"),
            Some(Path::new("/x/current/pryx")),
            Some(Path::new("/x/current/pryx")),
            Some(t(200)),
            Some(t(100)),
        );
    ...
    fn newer_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/pryx"),
            Path::new("/x/builds/versions/new/pryx"),
            Some(Path::new("/x/builds/versions/old/pryx")),
            Some(Path::new("/x/builds/versions/old/pryx")),
            Some(t(100)),
            Some(t(200)),
        );
        match decision {
            ReloadTargetDecision::UseCandidate((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/shared-server/pryx"));
            }
    ...
    fn equal_mtime_candidate_is_used() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/pryx"),
            Path::new("/x/builds/versions/same/pryx"),
            Some(Path::new("/x/builds/versions/current/pryx")),
            Some(Path::new("/x/builds/versions/current/pryx")),
            Some(t(100)),
            Some(t(100)),
        );
    ...
    fn strictly_older_candidate_is_blocked_and_uses_current_exe() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/pryx"),
            Path::new("/x/builds/versions/old-0.14.3/pryx"),
            Some(Path::new("/x/builds/versions/new/pryx")),
            Some(Path::new("/x/builds/versions/new/pryx")),
            Some(t(300)),
            Some(t(100)),
        );
        match decision {
            ReloadTargetDecision::DowngradeBlockedUseCurrent((path, _)) => {
                assert_eq!(path, PathBuf::from("/x/builds/versions/new/pryx"));
            }
    ...
    fn unreadable_candidate_mtime_is_treated_as_downgrade() {
        let decision = guarded_reload_target(
            candidate("/x/shared-server/pryx"),
            Path::new("/x/builds/versions/unknown/pryx"),
            Some(Path::new("/x/builds/versions/new/pryx")),
            Some(Path::new("/x/builds/versions/new/pryx")),
            Some(t(300)),
            None,
    ...
    fn downgrade_without_current_exe_falls_back_to_candidate() {
        ...
        let decision = guarded_reload_target(
            candidate("/x/shared-server/pryx"),
            Path::new("/x/builds/versions/old/pryx"),
            None,
            None,
            None,
            Some(t(100)),
...
mod pick_newest_candidate_tests {
    ...
    fn other_flavor_wins_when_strictly_newer() {
        ...
        let chosen = pick_newest_candidate([
            entry(
                "/x/versions/old-selfdev/pryx",
                "shared-server",
                Some(t(100)),
            ),
            entry("/x/versions/new-release/pryx", "stable", Some(t(200))),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/new-release/pryx"));
    }
    ...
    fn own_flavor_wins_on_tie() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/selfdev/pryx", "shared-server", Some(t(200))),
            entry("/x/versions/release/pryx", "stable", Some(t(200))),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/selfdev/pryx"));
    }
    ...
    fn own_flavor_wins_when_strictly_newer() {
        ...
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/fresh-selfdev/pryx"));
    }
    ...
    fn unknown_other_mtime_never_displaces_preferred() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/selfdev/pryx", "shared-server", Some(t(100))),
            entry("/x/versions/release/pryx", "stable", None),
        ])
        .expect("a candidate");
        assert_eq!(chosen.0, PathBuf::from("/x/versions/selfdev/pryx"));
    }
    ...
    fn duplicate_canonical_paths_collapse() {
        ...
        let chosen = pick_newest_candidate([
            entry("/x/versions/same/pryx", "shared-server", Some(t(100))),
            entry("/x/versions/same/pryx", "stable", Some(t(999))),
        ])
        .expect("a candidate");
...
mod newest_reload_candidate_integration_tests {
    ...
    //! a temp `PRYX_HOME`. This reproduces the field "/update -> new client,
    ...
    fn selfdev_daemon_reloads_into_fresh_release_after_update() {
        ...
        if let Some(prev_home) = prev_home {
            crate::env::set_var("PRYX_HOME", prev_home);
        } else {
    ...
    /// Install a release-archive-style version dir: a tiny `pryx` wrapper
    /// script plus the real `pryx-linux-x86_64.bin` payload, with independently
    ...
    fn install_release_style_binary(
        ...
        let dir = build::builds_dir()
            ...
            .join(version);
        std::fs::create_dir_all(&dir).expect("create version dir");
        let payload = dir.join("pryx-linux-x86_64.bin");
...
mod deleted_suffix_tests {
    ...
    fn strips_linux_deleted_marker() {
        let p = PathBuf::from("/home/u/.pryx/builds/versions/abc/pryx (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/home/u/.pryx/builds/versions/abc/pryx")
        );
    }

    #[test]
    fn leaves_normal_paths_untouched() {
        let p = PathBuf::from("/home/u/.pryx/builds/versions/abc/pryx");
    ...
    fn only_strips_trailing_marker() {
        ...
        let p = PathBuf::from("/home/u/pryx (deleted)/pryx");
        assert_eq!(strip_deleted_suffix(p.clone()), p);
    }
}
