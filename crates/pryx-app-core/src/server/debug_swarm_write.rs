...
fn debug_specs(specs: Vec<DebugNodeSpec>) -> Vec<pryx_plan::dag::NodeSpec> {
    specs
        .into_iter()
        .map(|s| pryx_plan::dag::NodeSpec {
            id: Some(s.id),
            content: s.content,
            kind: pryx_plan::bridge::parse_kind(s.kind.as_deref()),
...
async fn handle_debug_graph_op(arg: &str, ctx: &DebugSwarmWriteContext<'_>) -> String {
    use pryx_plan::bridge::{apply_task_graph, to_task_graph};
    use pryx_plan::dag;
...
mod tests {
    ...
    fn isolated_runtime(dir: &tempfile::TempDir) -> EnvGuard {
        ...
        let runtime = std::env::var_os("PRYX_RUNTIME_DIR");
        crate::env::set_var("PRYX_RUNTIME_DIR", dir.path());
        EnvGuard {
            _lock: lock,
            runtime,
        }
