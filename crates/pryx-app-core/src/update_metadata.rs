//! On-disk state shared by every pryx process that checks for updates.
//!
//! Update checks are cheap but rate limited, and several pryx processes can run
...
pub struct UpdateMetadata {
    ...
    /// the metadata file so every pryx process on the machine backs off, not
...
pub(super) fn metadata_path() -> Result<PathBuf> {
    Ok(storage::pryx_dir()?.join("update_metadata.json"))
}
