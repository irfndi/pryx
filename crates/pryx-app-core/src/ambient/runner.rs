...
use chrono::Utc;
use pryx_agent_runtime::{SoftInterruptMessage, SoftInterruptQueue, SoftInterruptSource};
use std::sync::Arc;
...
impl AmbientRunnerHandle {
    ...
    pub async fn log_json(&self) -> String {
        let transcripts_dir = match crate::storage::pryx_dir() {
    ...
    async fn run_cycle_visible(
        ...
        let pryx_bin =
            std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("pryx"));

        // Spawn kitty with `pryx ambient run-visible`
        logging::info("Ambient visible: spawning kitty with pryx TUI");
        let child = std::process::Command::new("kitty")
            .args([
                "--title",
                "🤖 pryx ambient cycle",
                "-e",
                &pryx_bin.to_string_lossy(),
                "ambient",
                "run-visible",
            ])
