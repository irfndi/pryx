...
//!
//! When a user asks pryx to change a setting, the agent writes
//! `~/.pryx/config.toml` and then has to guess whether the change took
...
///
/// Compares resolved paths so `~/.pryx/config.toml`, a relative path, and a
/// symlinked pryx home all resolve to the same file.
...
pub fn config_edit_notice(path: &Path, before: &str, after: &str) -> Option<String> {
    ...
    if let Err(error) = crate::config::Config::load_strict() {
        return Some(format!(
            "\n\nWARNING: {} no longer parses as TOML, so pryx is falling back to \
