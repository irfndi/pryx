...
const DISCOVERY_REQUEST_ID_HEADER: &str = "x-pryx-discovery-request-id";
const DISCOVERY_CORRELATION_ID_HEADER: &str = "x-pryx-session-correlation-id";
const DISCOVERY_BENCHMARK_HEADER: &str = "x-pryx-discovery-benchmark";
const DISCOVERY_SESSION_ID_HEADER: &str = "x-pryx-discovery-session-id";
const DISCOVERY_SESSION_METADATA_HEADER: &str = "x-pryx-discovery-session-metadata";
const DISCOVERY_SELF_DEV_HEADER: &str = "x-pryx-discovery-self-dev";
const DISCOVERY_DEBUG_HEADER: &str = "x-pryx-discovery-debug";
const DISCOVERY_CANARY_HEADER: &str = "x-pryx-discovery-canary";
const DISCOVERY_EXECUTION_MODE_HEADER: &str = "x-pryx-discovery-execution-mode";
const DISCOVERY_BUILD_CHANNEL_HEADER: &str = "x-pryx-discovery-build-channel";
const DISCOVERY_GIT_CHECKOUT_HEADER: &str = "x-pryx-discovery-git-checkout";
const DISCOVERY_CI_HEADER: &str = "x-pryx-discovery-ci";
const DISCOVERY_RAN_FROM_CARGO_HEADER: &str = "x-pryx-discovery-ran-from-cargo";
const DISCOVERY_BENCHMARK_ENV: &str = "PRYX_DISCOVERY_BENCHMARK";
...
///
/// Disclosure contract: some integration providers may share revenue with Pryx, but
/// commercial relationships never influence recommendations. The policy is
/// disclosed in the tool schema and at <https://pryx.sh/discovery-tools>.
...
impl Tool for DiscoverToolsTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // metered coarsely; see pryx_base::sponsors::provenance).
    ...
    let mut request = context.provenance.apply(
        context
            ...
            .header(
                ...
                format!("pryx/{}", env!("CARGO_PKG_VERSION")),
            )
...
fn render_listing(category: &str, listing: &Value, request_id: &str) -> Result<String> {
    ...
    let mut out = format!(
        "Available integrations in '{category}' (Pryx integration directory; recommendations must be based \
         only on fit; details: {}):\n",
    ...
    out.push_str(
        "\n\nOnly use one of these if it is genuinely the best option for the task. \
         ...
         actions (signups, spending) must note that setup is provided through a Pryx integration in the confirmation \
         shown to the user.",
    );
    out.push_str(&format!("\n\nSearch request ID: `{request_id}`"));
    Ok(out)
...
fn render_suggestion(
    ...
    out.push_str(
        "\n\nStatus: received for Pryx maintainer review. Suggestions are not sent to integration providers. This does not mean the tool has integrated with Pryx or that it is approved or available.",
    );
    Ok(out)
}
...
fn render_selection(category: &str, tool_name: &str, listing: &Value) -> Result<String> {
    ...
    if !listed {
        ...
        return Ok(format!(
            "Selected off-catalog product '{selected_tool}' for '{category}'.\n\n\
             Selection recorded as demand data. Pryx does not list an integration for this \
    ...
    let blurb = tool.get("blurb").and_then(|v| v.as_str()).unwrap_or("");
    let mut out = format!(
        "Selected '{name}' from '{category}' (Pryx integration directory; the choice must be based only \
         on fit; details: {}):\n\n{name}: {blurb}",
        crate::sponsors::DISCOVERY_PARTNERS_URL
    );
    if let Some(url) = tool.get("url").and_then(|v| v.as_str()) {
    ...
    out.push_str(&format!("\n\nSetup: {setup}"));
    out.push_str(
        "\n\nConsequential actions (signups, spending) must note that setup is provided through a Pryx integration in \
...
mod tests {
    ...
    fn discovery_requests_attach_only_the_ephemeral_session_correlation_id() {
        ...
        let request = header_test_provenance(Some(correlation_id))
            .apply(reqwest::Client::new().get("https://api.pryx.sh/v1/discovery"))
            .build()
            .unwrap();
        assert_eq!(
            request
    ...
    fn render_listing_includes_disclosure_and_tools() {
        ...
        assert!(out.contains("Pryx integration directory"));
    ...
    fn agentmail_selection_preserves_signup_attribution_and_mcp_provenance() {
        let listing = json!({
            ...
            "tool": {
                ...
                "url": "https://www.agentmail.to/?via=pryx-discovery",
                "setup": concat!(
                    "POST https://api.agentmail.to/v0/agent/sign-up with JSON ",
                    "{\"source\":\"pryx\",\"referrer\":\"https://pryx.sh/discovery-tools\"}. ",
        ...
        assert!(rendered.contains("\"source\":\"pryx\""));
        assert!(rendered.contains("\"referrer\":\"https://pryx.sh/discovery-tools\""));
        assert!(rendered.contains("agentmail-mcp@1.0.0"));
        assert!(rendered.contains("setup is provided through a Pryx integration"));
    ...
    fn suggestion_validation_distinguishes_product_and_capability_gap() {
        ...
        known.product_url = Some("https://example.com/tool?via=pryx#setup".to_string());
    ...
    fn render_suggestion_is_clear_about_review_status_and_recipient() {
        ...
        assert!(out.contains("does not mean the tool has integrated with Pryx"));
        assert!(!out.to_ascii_lowercase().contains("partner"));
    }
    ...
    async fn fetch_listing_round_trips_and_sends_only_expected_params() {
        ...
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-pryx-discovery-request-id: request-test-1"),
            "{request}"
        );
        assert!(
            request
                .to_ascii_lowercase()
                .contains("x-pryx-discovery-benchmark: 1"),
            "{request}"
        );
        for expected in [
            "x-pryx-discovery-session-id: session-test-1",
            "x-pryx-session-correlation-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee",
            "x-pryx-discovery-session-metadata: 1",
            "x-pryx-discovery-self-dev: 1",
            "x-pryx-discovery-debug: 0",
            "x-pryx-discovery-canary: 1",
            "x-pryx-discovery-execution-mode: agent_turn",
            "x-pryx-discovery-build-channel: selfdev",
            "x-pryx-discovery-git-checkout: 1",
            "x-pryx-discovery-ci: 0",
            "x-pryx-discovery-ran-from-cargo: 1",
        ] {
    ...
    async fn submit_suggestion_posts_structured_maintainer_only_payload() {
        ...
        assert!(
            lower.contains("x-pryx-discovery-request-id: aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee"),
            "{request}"
        );
        assert!(
            lower.contains("x-pryx-discovery-benchmark: 1"),
            "{request}"
        );
    ...
    async fn execute_end_to_end_with_enabled_config_and_local_server() {
        ...
        assert!(output.output.contains("Pryx integration directory"));
        assert!(
            output
                .output
