...
/// Point the process at a temp pryx home and return it with a restore guard.
fn temp_pryx_home() -> (tempfile::TempDir, Option<std::ffi::OsString>) {
...
fn restore_pryx_home(prev: Option<std::ffi::OsString>) {
...
fn writing_the_active_config_reports_what_changed() {
    ...
    restore_pryx_home(prev);
}

#[test]
fn writing_an_unrelated_file_reports_nothing() {
    let _guard = crate::storage::lock_test_env();
    let (dir, prev) = temp_pryx_home();
