...
pub(crate) use pryx_tool_core::intent_schema_property;
pub use pryx_tool_core::{StdinInputRequest, Tool, ToolContext, ToolExecutionMode};
pub use pryx_tool_types::{ToolImage, ToolOutput};
...
pub(crate) fn accept_large_output_schema_property_for_test() -> Value {
    pryx_tool_core::accept_large_output_schema_property()
}
...
fn accepts_large_output(input: &Value) -> bool {
    ...
    match input.get(pryx_tool_core::ACCEPT_LARGE_OUTPUT_KEY) {
...
impl Registry {
    ...
    /// correctly.
    ///
    /// The canonical mapping lives in `pryx-tool-types::resolve_tool_name` so
    ...
    pub(crate) fn resolve_tool_name(name: &str) -> &str {
        pryx_tool_types::resolve_tool_name(name)
    }
    ...
    /// (`.mcp.json`, `.pryx/mcp.json`, `.claude/mcp.json`) against
