...
use super::osa;
use anyhow::Result;
use pryx_tool_types::ToolOutput;
use serde_json::json;
...
fn screen_recording_ok() -> bool {
    let tmp = std::env::temp_dir().join(format!("pryx_setup_{}.png", std::process::id()));
...
pub fn setup() -> Result<ToolOutput> {
    ...
    // in screen_recording_ok). For Accessibility, prompt + pre-add pryx by
    ...
    if !ax0 {
        ...
        let _ = Command::new("/usr/bin/open")
            ...
            .status();
        log.push(
            "Opened Privacy & Security > Accessibility. Add and enable your terminal/pryx there."
                .into(),
        );
    }
    if !screen0 {
        let _ = Command::new("/usr/bin/open")
            ...
            .status();
        log.push(
            "Opened Privacy & Security > Screen Recording. Add and enable your terminal/pryx there."
                .into(),
        );
    }
