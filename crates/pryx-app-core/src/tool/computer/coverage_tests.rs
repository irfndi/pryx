...
//! run explicitly:
//!   cargo test -p pryx-app-core tool::computer::coverage -- --ignored --nocapture --test-threads=1
//!
...
use super::*;
use pryx_tool_core::{ToolContext, ToolExecutionMode};
...
async fn coverage_clipboard_scripting_system() {
    ...
    ok(json!({"action":"notify","text":"pryx coverage test","title":"pryx"})).await;
