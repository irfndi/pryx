...
include!(concat!(env!("OUT_DIR"), "/pryx_docs.rs"));

const DEFAULT_LIMIT: usize = 5;
const MAX_LIMIT: usize = 10;
const MAX_SECTION_CHARS: usize = 4_000;

pub struct PryxDocsTool;
...
#[async_trait]
impl Tool for PryxDocsTool {
    fn name(&self) -> &str {
        "pryx_docs"
    }

    fn description(&self) -> &str {
        "Search bundled, version-matched Pryx documentation. Use this first for questions about Pryx features, configuration, architecture, tools, or behavior."
    }
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput> {
        let params: PryxDocsInput = serde_json::from_value(input)?;
        ...
        Ok(ToolOutput::new(output).with_title(format!("pryx docs {}", params.action)))
    }
}

fn list_docs() -> String {
    let mut output = format!(
        "Bundled Pryx documentation ({} files):\n",
        PRYX_DOCS.len()
    );
    for (path, body) in PRYX_DOCS {
        let title = body
            .lines()
...
fn read_doc(path: &str) -> Result<String> {
    ...
    Ok(format!(
        "Source: `{path}` (bundled with this Pryx build)\n\n{body}"
    ))
}
...
fn search(query: &str, limit: Option<usize>) -> String {
    ...
    let mut output = format!("Pryx docs results for {query:?} (bundled with this Pryx build):\n");
...
fn terms(query: &str) -> Vec<String> {
    let stop: HashSet<&str> = [
        "a", "an", "and", "about", "does", "for", "how", "i", "in", "is", "pryx", "of", "on",
        "the", "to", "what", "with",
    ]
    .into_iter()
    .collect();
...
fn sections() -> Vec<Section<'static>> {
    ...
    for (path, document) in PRYX_DOCS {
...
mod tests {
    ...
    fn corpus_includes_current_docs_but_not_plans() {
        assert!(PRYX_DOCS.iter().any(|(path, _)| *path == "README.md"));
        assert!(PRYX_DOCS.iter().any(|(path, _)| *path == "docs/README.md"));
        assert!(
            !PRYX_DOCS
                .iter()
    ...
    fn search_finds_relevant_version_matched_documentation() {
        ...
        assert!(output.contains("bundled with this Pryx build"));
    }

    #[test]
    fn exact_document_can_be_read() {
        let output = read_doc("docs/README.md").unwrap();
        assert!(output.contains("# pryx Docs"));
    }
}
