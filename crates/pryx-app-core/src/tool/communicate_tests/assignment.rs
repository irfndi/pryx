...
async fn communicate_assign_task_can_spawn_fallback_agent() {
    ...
    let socket_path = runtime_dir.path().join("pryx.sock");
    let _runtime = EnvGuard::set("PRYX_RUNTIME_DIR", runtime_dir.path());
    let _socket = EnvGuard::set("PRYX_SOCKET", &socket_path);
    let _debug = EnvGuard::set("PRYX_DEBUG_CONTROL", "1");
