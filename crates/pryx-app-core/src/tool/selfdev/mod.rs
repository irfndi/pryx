...
//! Self-development tool - manage canary builds when working on pryx itself
...
/// Public GitHub source used when cloning the pryx repository for self-dev.
pub const PRYX_REPO_URL: &str = "https://github.com/pryx-co/pryx.git";
...
impl BuildRequest {
    ...
    fn requests_dir() -> Result<PathBuf> {
        let dir = storage::pryx_dir()?.join("selfdev-build-requests");
    ...
    fn terminal_history_limit() -> usize {
        std::env::var("PRYX_SELFDEV_REQUEST_HISTORY_LIMIT")
            .ok()
...
impl SelfDevTool {
    ...
    pub fn description_for(is_selfdev: bool) -> &'static str {
        if is_selfdev {
            "Manage self-dev builds, tests, and reloads while working on pryx itself."
        } else {
            "Enter self-dev mode to work on pryx itself: setup, reload, find config/paths."
        }
    }
            ...
            json!({
                ...
                "properties": {
                    ...
                    "target": {
                        ...
                        "description": "Build target for action=build. auto chooses from changed paths; tui builds pryx; desktop2 builds pryx-desktop2; all builds every binary."
                    },
                    "command": {
                        "type": "string",
            ...
            json!({
                ...
                "properties": {
                    ...
                    "action": {
                        ...
                        "description": "Action. `enter` starts a self-dev session; `setup` installs prerequisites; `reload` restarts pryx."
                    },
                    "prompt": {
                        "type": "string",
...
impl Tool for SelfDevTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        let result = match action.as_str() {
            ...
            // access only make sense once you are working on pryx itself.
            "build" => {
                self.do_build(
                    params.reason,
                    params.target,
...
impl SelfDevTool {
    ...
    fn reload_timeout_secs() -> u64 {
        std::env::var("PRYX_SELFDEV_RELOAD_TIMEOUT_SECS")
            .ok()
    ...
    fn build_reload_wait_secs() -> u64 {
        std::env::var("PRYX_SELFDEV_BUILD_WAIT_SECS")
            .ok()
            .and_then(|raw| raw.trim().parse::<u64>().ok())
            .filter(|secs| *secs > 0)
            .unwrap_or(1800)
    ...
    fn resolve_repo_dir(working_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
        if let Some(dir) = working_dir {
            for ancestor in dir.ancestors() {
                if build::is_pryx_repo(ancestor) {
    ...
    fn launch_binary() -> Result<std::path::PathBuf> {
        build::client_update_candidate(true)
            ...
            .ok_or_else(|| anyhow::anyhow!("Could not resolve pryx executable to launch"))
    }
    ...
    fn build_lock_path(worktree_scope: &str) -> Result<PathBuf> {
        let dir = storage::pryx_dir()?.join("selfdev-build-locks");
        storage::ensure_dir(&dir)?;
