...
impl SelfDevTool {
    ...
    /// building pryx from source: the rust toolchain, git, and a local repo
    ...
    pub(super) async fn do_setup(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        // Rust toolchain: cargo + rustc are required to build pryx.
        ...
        if repo_dir.is_none() {
            ...
            if SelfDevTool::is_test_session() {
                clone_note = Some("Test mode: skipped cloning the pryx source.".to_string());
            } else if git_available {
                match Self::clone_selfdev_source() {
                    Ok(path) => {
                        clone_note = Some(format!("Cloned pryx source into {}.", path.display()));
                        repo_dir = Some(path);
                    }
                    Err(err) => {
                        clone_note =
                            Some(format!("Could not clone pryx source automatically: {err}",));
                    }
                }
            }
        }
        ...
        match &repo_dir {
            ...
            None => {
                let target = Self::selfdev_clone_dir()
                    ...
                    .unwrap_or_else(|_| "~/.pryx/source/pryx".to_string());
                checks.push(SetupCheck::missing(
                    "repository",
                    "no local pryx checkout found",
                    format!(
        ...
        if let Some(repo) = repo_dir.as_deref() {
            match build::find_dev_binary(repo) {
                ...
                None => checks.push(SetupCheck::missing(
                    ...
                    "Build it once with `pryx self-dev --build`, or inside a \
        ...
        if !failing.is_empty() {
            ...
            output.push_str(
                "\nAll prerequisites satisfied. Use `selfdev enter` to start working on pryx.\n",
            );
        }
    ...
    /// `selfdev find-config`: report the key pryx paths (config file, home,
    ...
    pub(super) async fn do_find_config(&self, ctx: &ToolContext) -> Result<ToolOutput> {
        let pryx_home = storage::pryx_dir().ok();
        let config_path = pryx_home.as_ref().map(|home| home.join("config.toml"));
        ...
        let mut output = String::from("## pryx config & paths\n\n");
        output.push_str(&format!(
            "**Config file:** {}\n",
        ...
        output.push_str(&format!(
            "**pryx home:** {}\n",
            format_path(pryx_home.as_deref())
        ));
        output.push_str(&format!(
            "**Logs dir:** {}\n",
        ...
        let metadata = json!({
            ...
            "pryx_home": pryx_home.as_ref().map(|p| p.display().to_string()),
    ...
    pub(super) async fn do_reload_to_newer_build(&self, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        if !server::server_has_newer_binary() {
            return Ok(ToolOutput::new(
                "Already running the newest installed pryx build; no reload needed.",
            ));
        }

        let hash = pryx_build_meta::git_hash().to_string();
    ...
    fn selfdev_clone_dir() -> Result<std::path::PathBuf> {
        Ok(storage::pryx_dir()?.join("source").join("pryx"))
    }

    /// Clone the pryx source into the default self-dev source directory.
    fn clone_selfdev_source() -> Result<std::path::PathBuf> {
        let repo_dir = Self::selfdev_clone_dir()?;
        if repo_dir.exists() {
            if build::is_pryx_repo(&repo_dir) {
                return Ok(repo_dir);
            }
            anyhow::bail!(
                "{} exists but is not a pryx repository; move it aside and retry",
                repo_dir.display()
            );
        }
        ...
        if !build::is_pryx_repo(&repo_dir) {
            anyhow::bail!(
                "cloned source at {} is not a valid pryx repository",
                repo_dir.display()
            );
        }
        Ok(repo_dir)
