...
//! Application core for pryx (upper layer).
//!
...
//! message, memory, telemetry, ...) lives in the `pryx-base` crate and is
//! re-exported here via `pub use pryx_base::*`, so every existing
//! `crate::<module>` path (e.g. `crate::config`, `crate::provider`) keeps
//! resolving unchanged across this crate and the root `pryx` crate, which in
//! turn re-exports this crate via `pub use pryx_app_core::*`.

// Foundational layer: re-export every `pryx-base` module so `crate::<module>`
