...
fn main() {
    ...
    let mut generated = String::from("pub(crate) static PRYX_DOCS: &[(&str, &str)] = &[\n");
    for path in files {
        let relative = path
            .strip_prefix(&repo)
    ...
    let out = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR")).join("pryx_docs.rs");
    fs::write(out, generated).expect("write generated Pryx documentation corpus");
}
