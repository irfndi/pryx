...
//!
//! Run with:
//!   cargo test -p pryx-tui-markdown --test highlight_backend_probe -- --ignored --nocapture

use pryx_tui_markdown::highlight_line;
...
fn workload() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            "sh",
            "for pid in $(pgrep -f pryx); do awk '/VmRSS/{print $2}' /proc/$pid/status; done",
        ),
        (
            "toml",
