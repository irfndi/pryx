...
//! renderer at a hanging toolchain via process-global `PRYX_*_COMMAND`
...
fn a_deferred_formula_resolves_into_an_image_once_the_worker_finishes() {
    ...
    unsafe {
        std::env::remove_var("PRYX_LATEX_COMMAND");
        std::env::remove_var("PRYX_DVIPNG_COMMAND");
        std::env::remove_var("PRYX_PDFLATEX_COMMAND");
        std::env::remove_var("PRYX_PDFTOCAIRO_COMMAND");
    }
    ...
    let render = || {
        pryx_tui_mermaid::with_image_protocol_override(Some(true), || {
            pryx_tui_markdown::render_markdown_with_width(&text, Some(90))
        })
    };
    ...
    let pending = first
        .iter()
        .any(pryx_tui_markdown::line_is_mermaid_pending_placeholder);
    if !pending {
...
fn rendered_a_formula_image(lines: &[ratatui::text::Line<'static>]) -> bool {
    let has_placeholder = lines.iter().any(|line| {
        pryx_tui_mermaid::parse_image_placeholder(line).is_some()
            || pryx_tui_mermaid::parse_inline_image_placeholder(line).is_some()
    });
