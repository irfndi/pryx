//! Runtime startup and private pryx instances.
//!
...
/// Options for starting a private pryx instance.
pub struct LaunchOptions {
    ...
    /// pryx executable. Defaults to `pryx` on `$PATH`.
    ...
    /// Client identity used by [`crate::PryxClient::launch`].
    pub client_name: String,
    /// Request timeout used by [`crate::PryxClient::launch`].
...
impl Default for LaunchOptions {
    fn default() -> Self {
        Self {
            ...
            client_name: concat!("pryx-sdk-rs/", env!("CARGO_PKG_VERSION")).to_string(),
...
impl LaunchedInstance {
    fn shutdown(&self) {
        ...
        stop_instance_daemon(&self.pryx_home, &self.runtime_dir);
        ...
        if self.ephemeral {
            remove_ephemeral_home(&self.pryx_home, self.cleanup_timeout);
        }
    }
}
...
pub fn launch_instance(options: &LaunchOptions) -> Result<LaunchedInstance> {
    let ephemeral = options.pryx_home.is_none();
    let pryx_home = match &options.pryx_home {
        Some(path) => {
            fs::create_dir_all(path).map_err(launch_io)?;
            path.clone()
        }
        None => tempfile::Builder::new()
            .prefix("pryx-sdk-instance-")
            .tempdir()
            .map_err(launch_io)?
            .keep(),
    };
    set_owner_only_dir(&pryx_home)?;

    let runtime_dir = pryx_home.join("run");
    fs::create_dir_all(&runtime_dir).map_err(launch_io)?;
    set_owner_only_dir(&runtime_dir)?;
    let socket_path = runtime_dir.join("pryx-api.sock");
    for stale in [
        "pryx-api.sock",
        "pryx.sock",
        "pryx-debug.sock",
    ...
    let cleanup_on_error = || {
        if ephemeral {
            remove_ephemeral_home(&pryx_home, Duration::ZERO);
        }
    };
    if options.inherit_logins {
        if let Err(error) = inherit_credentials(&user_pryx_home(), &pryx_home) {
            cleanup_on_error();
            return Err(error);
        }
    }
    ...
    let binary = options
        .binary
        .clone()
        .unwrap_or_else(|| PathBuf::from("pryx"));
    ...
    command
        ...
        .env("PRYX_HOME", &pryx_home)
        .env("PRYX_RUNTIME_DIR", &runtime_dir)
        .env("PRYX_API_SOCKET", &socket_path)
        .env("PRYX_SOCKET", runtime_dir.join("pryx.sock"))
    ...
    let mut child = match command.spawn() {
        ...
        Err(cause) => {
            ...
            let message = if cause.kind() == std::io::ErrorKind::NotFound {
                format!(
                    "could not run `{}`: pryx is not installed, or not on PATH. Install it from https://pryx.sh, or pass `binary` with its full path.",
                    binary.display()
                )
            } else {
                format!("could not run `{}`: {cause}", binary.display())
            };
            return Err(Error::new(ErrorKind::PryxNotFound, message));
        }
    };
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                return Err(Error::new(
                    ...
                    format!(
                        "pryx exited during startup ({status}){}",
    ...
    Err(Error::new(
        ...
        format!(
            "pryx did not create its API socket at {} within {:?}{}",
            socket_path.display(),
...
/// Resolve the user's normal pryx home before a private instance overrides it.
pub fn user_pryx_home() -> PathBuf {
    std::env::var_os("PRYX_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".pryx"))
}
...
pub fn inherit_credentials(from_home: &Path, to_home: &Path) -> Result<Vec<PathBuf>> {
    ...
    if let Ok(entries) = fs::read_dir(user_app_config_dir()) {
        for entry in entries.flatten() {
            ...
            let relative = PathBuf::from("config").join("pryx").join(&name);
...
pub fn ensure_runtime(options: &LaunchOptions, progress: &Progress<'_>) -> Result<()> {
    let api = options
        .api_socket
        .clone()
        .unwrap_or_else(pryx_harness_api::api_socket_path);
    if socket_accepts(&api) {
        return Ok(());
    }
    let legacy = pryx_harness_api::legacy_socket_path();
    if !socket_accepts(&legacy) {
        progress("starting pryx runtime...");
        spawn_detached(&sibling_exe("pryx"), &["serve"]).map_err(|error| {
            Error::new(
                ErrorKind::LaunchFailed,
                format!("could not start the pryx runtime: {error}"),
            )
        })?;
        wait_for_socket(&legacy, "pryx runtime", options.start_timeout)?;
    }
    progress("starting harness API bridge...");
    spawn_detached(&sibling_exe("pryx-harness-api-bridge"), &[]).map_err(|error| {
        Error::new(
            ErrorKind::LaunchFailed,
            format!("could not start pryx-harness-api-bridge: {error}"),
        )
    })?;
...
fn read_daemon_pid(home: &Path, runtime_dir: &Path) -> Option<i32> {
    ...
    let socket = runtime_dir.join("pryx.sock");
...
fn remove_ephemeral_home(home: &Path, timeout: Duration) {
    ...
    if home.parent() != Some(std::env::temp_dir().as_path())
        || !name.starts_with("pryx-sdk-instance-")
    {
        return;
    }
    let deadline = Instant::now() + timeout;
...
/// Resolve the user's platform pryx config directory.
pub fn user_app_config_dir() -> PathBuf {
    #[cfg(target_os = "macos")]
    {
        return home_dir().join("Library/Application Support/pryx");
    }
...
mod tests {
    ...
    fn dropping_an_ephemeral_instance_stops_its_bridge_and_removes_its_home() {
        ...
        let instance = LaunchedInstance {
            socket_path: runtime_dir.join("pryx-api.sock"),
            pryx_home: home.clone(),
            runtime_dir,
            child: Mutex::new(Some(child)),
            ephemeral: true,
