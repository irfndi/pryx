...
//!
//! Mirrors the TypeScript SDK's `PryxClient` capability for capability. The
            ...
            Error::new(
                ...
                match cause.kind() {
                    std::io::ErrorKind::NotFound => format!(
                        "no harness API socket at {}: the pryx harness is not running. \
                         Start it with `pryx serve` and `pryx-harness-api-bridge`, or \
...
/// Discovery and buffering controls for [`PryxClient::global_events`].
...
struct GlobalEventControl {
    ...
    children: Mutex<HashMap<String, PryxClient>>,
...
impl PryxClient {
    /// Start and own a private pryx instance, then connect to it.
    ///
    /// Its state and sockets are isolated from the user's interactive pryx.
    ...
    pub fn launch(options: LaunchOptions) -> Result<Self> {
        ...
        client.instance_home = Some(instance.pryx_home.clone());
        client.instance = Some(instance);
        Ok(client)
    }

    /// Connect to the pryx running on this machine.
    ///
    /// Use this to automate the user's own pryx: a desktop app, an editor
    ...
    /// Persist an API key in pryx's owner-only provider store and hot-reload
...
/// Runtime identity and protocol metadata returned by [`PryxClient::get_runtime_info`].
...
/// Content and truncation metadata returned by [`PryxClient::read_file`].
...
/// Optional constraints for [`PryxClient::search_text`].
...
/// Safe filesystem metadata returned by [`PryxClient::file_status`].
#[derive(Debug, Clone, PartialEq, Eq)]
...
fn start_global_child(parent: &PryxClient, control: &Arc<GlobalEventControl>, session_id: String) {
    if control
        .children
        .lock()
    ...
    let child = match PryxClient::connect(ConnectOptions {
    ...
    if let Err(error) = child.attach_session(&session_id) {
        if !matches!(
            error.kind,
            ErrorKind::Harness(pryx_harness_api::ErrorCode::UnknownSession)
        ) {
            stop_global_stream(control, Some(error));
        }
        return;
