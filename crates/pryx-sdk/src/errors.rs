...
pub enum ErrorKind {
    ...
    Harness(pryx_harness_api::ErrorCode),
    /// I/O or JSON trouble on the transport.
    Transport,
    /// Starting a runtime (daemon or bridge) failed.
    LaunchFailed,
    /// The configured pryx executable could not be started.
    PryxNotFound,
...
impl ErrorKind {
    ...
    pub fn code(&self) -> &'static str {
        match self {
            ...
            Self::PryxNotFound => "pryx_not_found",
            ...
            Self::Harness(code) => match code {
                pryx_harness_api::ErrorCode::UnsupportedVersion => "unsupported_version",
                pryx_harness_api::ErrorCode::UnknownRequest => "unknown_request",
                pryx_harness_api::ErrorCode::UnknownSession => "unknown_session",
                pryx_harness_api::ErrorCode::InvalidRequest => "invalid_request",
                pryx_harness_api::ErrorCode::Internal => "internal",
            },
        }
    }
}
...
impl From<pryx_harness_api::FrameError> for Error {
    fn from(error: pryx_harness_api::FrameError) -> Self {
        let kind = match error {
            pryx_harness_api::FrameError::Eof => ErrorKind::Disconnected,
