...
//! and "attach to the user's pryx" versus "provision a private instance" are
...
struct Capability {
    /// Rust method on `PryxClient`.
    rust: &'static str,
    /// TypeScript method on `PryxClient`.
    ts: &'static str,
}
...
fn neither_sdk_has_an_untriaged_public_capability() {
    ...
    assert!(
        untriaged.is_empty(),
        "these public PryxClient methods are in the Rust SDK but not in the \
    ...
    assert!(
        untriaged.is_empty(),
        "these public PryxClient methods are in the TypeScript SDK but not in the \
...
/// Public method names in `impl PryxClient`.
fn rust_public_methods() -> Vec<String> {
    let source = rust_client_source();
    let mut methods = std::collections::BTreeSet::new();
    let mut remaining = source.as_str();
    while let Some(start) = remaining.find("impl PryxClient {") {
...
/// Public method names in the TypeScript `PryxClient` class.
///
...
fn ts_public_methods() -> Option<Vec<String>> {
    ...
    let start = source.find("export class PryxClient")?;
    let mut methods = std::collections::BTreeSet::new();
