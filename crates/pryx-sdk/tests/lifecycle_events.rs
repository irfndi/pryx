...
use pryx_sdk::{
    ConnectOptions, GlobalEventsOptions, PryxClient, LaunchOptions, inherit_credentials,
};
...
impl UnixHarness {
    ...
    fn connect(&self) -> PryxClient {
        PryxClient::connect(ConnectOptions {
...
fn missing_launch_binary_is_typed_and_cleans_up_its_temporary_home() {
    let before = std::fs::read_dir(std::env::temp_dir())
        ...
        .filter(|entry| {
            entry
                .file_name()
                .to_string_lossy()
                .starts_with("pryx-sdk-instance-")
        })
        .count();
    let error = match PryxClient::launch(LaunchOptions {
        binary: Some(PathBuf::from("pryx-definitely-not-installed-sdk-test")),
    ...
    assert_eq!(error.code(), "pryx_not_found");
