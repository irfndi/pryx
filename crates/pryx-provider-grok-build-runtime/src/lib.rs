...
use pryx_message_types::{
    ContentBlock as PryxContentBlock, Message, Role, StreamEvent, ToolDefinition,
};
use pryx_provider_core::{EventStream, ModelRoute, Provider};
...
impl GrokBuildProcess {
    pub fn from_env() -> Self {
        let command = std::env::var_os("PRYX_GROK_CLI_PATH")
...
impl Provider for GrokBuildProvider {
    async fn complete(
        ...
        let thread = std::thread::Builder::new()
            .name("pryx-grok-build-acp".to_string())
            .spawn(move || {
...
async fn initialize_and_authenticate(
    ...
    let initialize = acp::InitializeRequest::new(acp::ProtocolVersion::V1)
        .client_info(acp::Implementation::new("pryx", env!("CARGO_PKG_VERSION")).title("Pryx"));
...
async fn run_on_acp_thread_with_process<T: Send + 'static>(
    ...
    std::thread::Builder::new()
        .name("pryx-grok-build-acp-probe".to_string())
        .spawn(move || {
            let result = (|| {
                let runtime = tokio::runtime::Builder::new_current_thread()
                    .enable_all()
...
fn message_text(message: &Message) -> String {
    message
        ...
        .filter_map(|block| match block {
            PryxContentBlock::Text { text, .. } => Some(text.clone()),
            PryxContentBlock::ToolResult { content, .. } => Some(content.clone()),
            _ => None,
        })
        .collect::<Vec<_>>()
        .join("\n\n")
