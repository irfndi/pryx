...
//! persistent WebSocket), moved out of `pryx-base` so provider edits compile
...
//! [`OpenAIProvider`] with `pryx_base::provider::external` at startup.
//!
//! Model-catalog/account-availability state stays in `pryx_base::provider`
//! (it is shared vocabulary for routing), as does the pure request shaping in
//! `pryx_base::provider::openai_request`.

use anyhow::{Context, Result};
use async_trait::async_trait;
use futures::{FutureExt, SinkExt, StreamExt as FuturesStreamExt};
use pryx_base::auth::codex::CodexCredentials;
use pryx_base::auth::oauth;
use pryx_base::provider::openai_request::{build_responses_input, build_tools};
#[cfg(test)]
use pryx_message_types::TOOL_OUTPUT_MISSING_TEXT;
use pryx_message_types::{Message as ChatMessage, StreamEvent, ToolDefinition};
...
const DEFAULT_MODEL: &str = pryx_provider_core::DEFAULT_OPENAI_MODEL;
const ORIGINATOR: &str = "codex_cli_rs";

pub(crate) const CHATGPT_WEB_MODEL: &str = pryx_provider_core::CHATGPT_WEB_MODEL;
...
/// `PRYX_OPENAI_WS_IDLE_RECONNECT_SECS` (0 disables the idle reconnect entirely,
...
static WEBSOCKET_PERSISTENT_IDLE_RECONNECT_SECS: LazyLock<Option<u64>> = LazyLock::new(|| {
    match std::env::var("PRYX_OPENAI_WS_IDLE_RECONNECT_SECS") {
        Ok(raw) => match raw.trim().parse::<u64>() {
            Ok(0) => {
                pryx_base::logging::info(
                    "OpenAI persistent WS idle reconnect disabled (PRYX_OPENAI_WS_IDLE_RECONNECT_SECS=0); relying on healthcheck + max-age",
                );
                None
            }
            Ok(secs) => {
                pryx_base::logging::info(&format!(
                    "OpenAI persistent WS idle reconnect threshold set to {}s (PRYX_OPENAI_WS_IDLE_RECONNECT_SECS)",
                    secs
                ));
                Some(secs)
            }
            Err(_) => {
                pryx_base::logging::info(&format!(
                    "Warning: invalid PRYX_OPENAI_WS_IDLE_RECONNECT_SECS '{}'; using default {}s",
        ...
        match raw.trim().to_ascii_lowercase().as_str() {
            ...
            other => {
                pryx_base::logging::warn(&format!(
                    "Unknown PRYX_OPENAI_TRANSPORT '{}'; using auto. Use: auto, websocket, or https.",
                    other
                ));
                Self::Auto
            }
...
/// Shared dual-auth credential pin (see `pryx_provider_core::CredentialMode`).
/// The OpenAI-specific alias is kept so existing call sites read naturally.
pub(crate) use pryx_provider_core::CredentialMode as OpenAICredentialMode;
...
pub(crate) fn load_credentials_for_mode(mode: OpenAICredentialMode) -> Result<CodexCredentials> {
    match mode {
        OpenAICredentialMode::Auto => pryx_base::auth::codex::load_credentials(),
        OpenAICredentialMode::OAuth => pryx_base::auth::codex::load_oauth_credentials(),
        OpenAICredentialMode::ApiKey => pryx_base::auth::codex::load_api_key_credentials(),
    }
}
...
fn log_openai_stream_lifecycle(
    level: pryx_base::logging::LogLevel,
    phase: &str,
    ...
    pryx_base::logging::event(level, "PROVIDER_STREAM_LIFECYCLE", owned);
}
...
async fn emit_connection_phase(
    ...
    phase: pryx_message_types::ConnectionPhase,
) {
        ...
        loop {
                ...
                >= Duration::from_secs(WEBSOCKET_PERSISTENT_MAX_AGE_SECS)
            {
                log_openai_stream_lifecycle(
                    pryx_base::logging::LogLevel::Info,
            ...
            match ensure_persistent_ws_is_healthy(state, false).await {
                ...
                Err(error) => {
                    ...
                    log_openai_stream_lifecycle(
                        pryx_base::logging::LogLevel::Warn,
                        "persistent_state_reset",
                        vec![
                            ("model", model.clone()),
...
impl OpenAIProvider {
    pub(crate) fn supports_extended_prompt_cache_retention(model_id: &str) -> bool {
        pryx_base::provider::openai::supports_extended_prompt_cache_retention(model_id)
    }
    ...
    fn new_inner(credentials: CodexCredentials, browser_only: bool) -> Self {
        let credential_mode = if browser_only {
            ...
            OpenAICredentialMode::from_runtime_env(pryx_provider_core::DualAuthProvider::OpenAI)
        };
        ...
        let mut model = if browser_only {
            ...
            std::env::var("PRYX_OPENAI_MODEL")
                .unwrap_or_else(|_| DEFAULT_MODEL.to_string())
                .trim()
                .to_string()
        };
        if !is_chatgpt_web_model(&model)
            && !pryx_base::provider::known_openai_model_ids()
                .iter()
        ...
        let prompt_cache_key = std::env::var("PRYX_OPENAI_PROMPT_CACHE_KEY")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = std::env::var("PRYX_OPENAI_PROMPT_CACHE_RETENTION")
            .ok()
            .map(|v| v.trim().to_string())
            .filter(|v| !v.is_empty());
        let prompt_cache_retention = match prompt_cache_retention.as_deref() {
            ...
            Some(other) => {
                pryx_base::logging::info(&format!(
                    "Warning: Unsupported PRYX_OPENAI_PROMPT_CACHE_RETENTION '{}'; expected 'in_memory' or '24h'",
                    other
                ));
                None
            }
        ...
        let native_compaction_threshold_tokens = pryx_base::config::config()
            .provider
            .openai_native_compaction_threshold_tokens
            .max(1000);
        let model_reasoning_efforts =
            pryx_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    pub(crate) fn set_credential_mode(&self, mode: OpenAICredentialMode) -> Result<()> {
        ...
        if let Some(route) = mode.auth_route(pryx_provider_core::DualAuthProvider::OpenAI) {
    ...
    fn reload_cached_reasoning_efforts(&self) {
        let cached = pryx_base::provider::cached_openai_reasoning_efforts().unwrap_or_default();
    ...
    async fn clear_persistent_ws(&self, reason: &str) {
        ...
        if persistent_ws.is_some() {
            pryx_base::logging::info(&format!("Clearing persistent OpenAI WS state: {}", reason));
        }
        *persistent_ws = None;
    }
    ...
    fn default_reasoning_effort_for_model(model: &str) -> Option<String> {
        let key = pryx_provider_core::model_id::canonical(model);
    ...
    fn revalidate_reasoning_effort(&self) {
        ...
        if pryx_base::prompt::is_swarm_effort(&current)
    ...
    fn api_reasoning_effort(&self, effort: Option<&str>) -> Option<String> {
        match effort {
            Some(e) if pryx_base::prompt::is_swarm_effort(e) => {
                let available = self.available_efforts();
                pryx_provider_core::OPENAI_SELECTABLE_EFFORTS
                    .iter()
                    .rev()
                    .find(|candidate| {
                        !pryx_base::prompt::is_swarm_effort(candidate)
    ...
    fn parse_max_output_tokens(raw: Option<&str>) -> Option<u32> {
        ...
        match raw.parse::<u32>() {
            ...
            Err(_) => {
                pryx_base::logging::warn(&format!(
                    "Invalid PRYX_OPENAI_MAX_OUTPUT_TOKENS='{}'; using default {}",
    ...
    fn load_max_output_tokens() -> Option<u32> {
        let raw = std::env::var("PRYX_OPENAI_MAX_OUTPUT_TOKENS").ok();
        let parsed = Self::parse_max_output_tokens(raw.as_deref());
        if raw.is_some() {
            match parsed {
                Some(value) => pryx_base::logging::info(&format!(
                    "OpenAI max_output_tokens configured to {}",
                    value
                )),
                None => pryx_base::logging::info(
                    "OpenAI max_output_tokens disabled (PRYX_OPENAI_MAX_OUTPUT_TOKENS=0)",
                ),
            }
        }
        parsed
    ...
    /// `PRYX_OPENAI_API_BASE`, `OPENAI_BASE_URL`, `OPENAI_API_BASE`.
    ///
    ...
    pub(crate) fn resolve_api_base() -> String {
        pryx_base::provider::openai::resolve_api_base()
    }
    ...
    async fn model_id(&self) -> String {
        ...
        let availability = pryx_base::provider::model_availability_for_account(&current);

        match availability.state {
            pryx_base::provider::AccountModelAvailabilityState::Unavailable => {
                ...
                if let Some(fallback) = pryx_base::provider::get_best_available_openai_model()
                    && fallback != current
                {
                    pryx_base::logging::info(&format!(
            ...
            pryx_base::provider::AccountModelAvailabilityState::Unknown => {
                if pryx_base::provider::should_refresh_openai_model_catalog()
                    && pryx_base::provider::begin_openai_model_catalog_refresh()
                {
                    let is_chatgpt_mode = {
                    ...
                    pryx_base::provider::refresh_openai_model_catalog_in_background(
                        token,
                        is_chatgpt_mode,
            ...
            pryx_base::provider::AccountModelAvailabilityState::Available => {}
        }
