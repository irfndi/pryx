...
use pryx_message_types::{ContentBlock, Message, Role, StreamEvent, ToolDefinition};
use pryx_provider_core::EventStream;
...
const TOOL_CALL_START: &str = "<pryx_tool_call>";
const TOOL_CALL_END: &str = "</pryx_tool_call>";
...
/// state, so parallel pryx agents do not cross streams.
...
impl ChatGptWebState {
    ...
    async fn run_turn(
        ...
        let status = pryx_base::browser::ensure_browser_ready_noninteractive()
            .await
            .context(
                "ChatGPT web transport needs the Firefox Browser Agent Bridge. Run `pryx browser status`, start Firefox, and log in at chatgpt.com",
            )?;
        if !status.ready {
            anyhow::bail!(
                "Firefox Browser Agent Bridge is not ready. Run `pryx browser status`, start Firefox, and log in at chatgpt.com"
            );
        }
        ...
        let result = async {
            send_phase(tx, pryx_message_types::ConnectionPhase::Authenticating).await?;
            ...
            send_phase(tx, pryx_message_types::ConnectionPhase::SendingRequest).await?;
            if tx.is_closed() {
                anyhow::bail!("ChatGPT web response consumer was closed before submission");
            }
            ...
            send_phase(tx, pryx_message_types::ConnectionPhase::WaitingForResponse).await?;
        ...
        match (result, cleanup) {
            ...
            (Ok(_), Err(cleanup_err)) => Err(cleanup_err.context(
                "GPT-5.6 Pro answered, but pryx could not securely close its browser tab",
            )),
...
fn next_owned_tab_name() -> String {
    ...
    let millis = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    format!("pryx-chatgpt-web-{millis}-{sequence}")
}
...
"#,
    ...
    if preparation.get("onboarding").and_then(Value::as_bool) == Some(true) {
        anyhow::bail!(
            "ChatGPT is waiting for a workspace onboarding choice. Open chatgpt.com in Firefox and finish onboarding; pryx will not merge or move your personal chat history automatically"
        );
    }
    if preparation.get("signedOut").and_then(Value::as_bool) == Some(true) {
        anyhow::bail!(
            "Firefox is not logged in to ChatGPT. Log in at chatgpt.com, then retry the pryx turn"
        );
    }
...
"#,
    ...
    if verification.get("temporary").and_then(Value::as_bool) != Some(true) {
        anyhow::bail!(
            "ChatGPT did not enter Temporary Chat. Refusing to send the pryx system prompt into persistent web history"
        );
    }
    Ok(())
}
...
async fn poll_for_response(tab_id: u64, tx: &mpsc::Sender<Result<StreamEvent>>) -> Result<String> {
    let timeout_secs = std::env::var("PRYX_CHATGPT_WEB_TIMEOUT_SECS")
        .ok()
...
"#,
        ...
        if Instant::now() >= deadline {
            anyhow::bail!(
                "GPT-5.6 Pro web response timed out after {} seconds (override with PRYX_CHATGPT_WEB_TIMEOUT_SECS)",
                timeout_secs
            );
        }
...
"#,
        ...
        if !text.is_empty() && !streaming_emitted {
            send_phase(tx, pryx_message_types::ConnectionPhase::Streaming).await?;
...
async fn emit_response(
    ...
    if let Some(parsed) = parse_tool_call(response)? {
        if !advertised_tools.iter().any(|name| name == &parsed.name) {
            anyhow::bail!(
                "GPT-5.6 Pro requested unknown pryx tool '{}'; advertised tools were: {}",
                parsed.name,
...
fn parse_tool_call(response: &str) -> Result<Option<ParsedToolCall>> {
    ...
    if !trimmed.starts_with(TOOL_CALL_START) || !trimmed.ends_with(TOOL_CALL_END) {
        anyhow::bail!(
            "GPT-5.6 Pro mentioned a pryx tool-call envelope without emitting it as the entire response"
        );
    }
    ...
    let payload: Value = serde_json::from_str(payload_text.trim())
        .context("GPT-5.6 Pro emitted invalid JSON in a pryx tool-call envelope")?;
    let name = payload
        .get("name")
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|name| !name.is_empty())
        .ok_or_else(|| anyhow::anyhow!("pryx tool-call envelope is missing a non-empty name"))?
        .to_string();
    let input = payload.get("input").cloned().unwrap_or_else(|| json!({}));
    if !input.is_object() {
        anyhow::bail!("pryx tool-call envelope input must be a JSON object");
    }
...
fn build_web_prompt(
    ...
    prompt.push_str("# Pryx system instructions\n\n");
    prompt.push_str(system);
    prompt.push_str("\n\n# Pryx tools available for this turn\n\n");
...
# Mandatory Pryx web-transport protocol

Act as the assistant for the conversation above and obey the Pryx system instructions.
You do not have native access to Pryx tools in this web transport. When a tool is needed,
request exactly one tool and stop by emitting this exact envelope with raw JSON and no code fence:

<pryx_tool_call>{"name":"tool_name","input":{"argument":"value"}}</pryx_tool_call>

The name must exactly match one of the advertised Pryx tools. Put every required field,
including `intent` when its schema requires one, inside `input`. Pryx will execute the tool
...
async fn bridge_command(action: &str, params: Value) -> Result<Value> {
    let binary = pryx_base::browser::browser_binary_path();
    if !binary.exists() {
        anyhow::bail!(
            "Browser bridge binary is not installed. Run `pryx browser setup` once, then log in at chatgpt.com in Firefox"
        );
    }
...
mod tests {
    ...
    fn tool_call_parser_accepts_valid_exact_envelope() {
        let parsed = parse_tool_call(
            "  <pryx_tool_call>{\"name\":\"bash\",\"input\":{\"command\":\"pwd\",\"intent\":\"Check cwd\"}}</pryx_tool_call>\n",
        )
        .unwrap()
        .unwrap();
    ...
    fn tool_call_parser_rejects_incomplete_envelope() {
        let err = parse_tool_call("<pryx_tool_call>{\"name\":\"bash\"}")
            .err()
    ...
    fn tool_call_parser_rejects_quoted_or_suffixed_envelope() {
        for response in [
            "Example: <pryx_tool_call>{\"name\":\"bash\",\"input\":{}}</pryx_tool_call>",
            "<pryx_tool_call>{\"name\":\"bash\",\"input\":{}}</pryx_tool_call> done",
        ] {
    ...
    fn tool_call_parser_rejects_non_object_input() {
        let err = parse_tool_call(
            "<pryx_tool_call>{\"name\":\"bash\",\"input\":[\"not\",\"an\",\"object\"]}</pryx_tool_call>",
        )
        .err()
    ...
    fn tool_call_parser_allows_marker_text_inside_json_string_values() {
        let parsed = parse_tool_call(
            "<pryx_tool_call>{\"name\":\"bash\",\"input\":{\"command\":\"printf '</pryx_tool_call>'\"}}</pryx_tool_call>",
        )
        .unwrap()
        .unwrap();
        assert_eq!(parsed.input["command"], "printf '</pryx_tool_call>'");
    }
