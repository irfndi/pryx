...
fn test_websocket_continuation_request_excludes_transport_fields() {
    ...
    assert_eq!(continuation["prompt_cache_key"], "pryx-test-cache");
...
/// GPT-5.6 Sol is pryx's default OpenAI model and defaults to `low`
...
fn gpt_5_6_sol_defaults_to_low_reasoning_effort() {
    ...
    pryx_base::auth::codex::set_active_account_override(Some("sol-low-default-test".to_string()));
...
/// `.pryx/prompt-overlay.md`) were silently dropped in ChatGPT/Codex OAuth
