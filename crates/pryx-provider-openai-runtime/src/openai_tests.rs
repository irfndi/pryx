...
impl LiveOpenAITestEnv {
    fn new() -> Result<Option<Self>> {
        let lock = pryx_base::storage::lock_test_env();
        let Some(source_auth) = real_codex_auth_path() else {
            return Ok(None);
        };

        let temp = tempfile::Builder::new()
            .prefix("pryx-openai-live-")
            .tempdir()?;
        ...
        let pryx_home = EnvVarGuard::set_path("PRYX_HOME", temp.path());
        let transport = EnvVarGuard::set("PRYX_OPENAI_TRANSPORT", "https");

        Ok(Some(Self {
            _lock: lock,
            _pryx_home: pryx_home,
...
async fn live_openai_catalog() -> Result<Option<pryx_base::provider::OpenAIModelCatalog>> {
    let Some(_env) = LiveOpenAITestEnv::new()? else {
        return Ok(None);
    };
    let creds = pryx_base::auth::codex::load_credentials()?;
    ...
    Ok(Some(
        pryx_base::provider::fetch_openai_model_catalog(&token).await?,
    ))
}
...
fn openai_credential_mode_runtime_provider_identity_round_trips() {
    ...
    pryx_base::env::set_var("PRYX_RUNTIME_PROVIDER", "openai");
    assert_eq!(
    ...
    pryx_base::env::set_var("PRYX_RUNTIME_PROVIDER", "openai-api");
    assert_eq!(
        OpenAICredentialMode::from_runtime_env(pryx_provider_core::DualAuthProvider::OpenAI),
