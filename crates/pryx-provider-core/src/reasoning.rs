...
//! same ordered values. `swarm` and `swarm-deep` are Pryx UI sentinels rather
//! than wire-level provider values, but they belong in the selectable ladder.

/// OpenAI Responses API effort levels, followed by Pryx's swarm modes.
...
/// Direct DeepSeek effort levels, followed by Pryx's swarm modes.
