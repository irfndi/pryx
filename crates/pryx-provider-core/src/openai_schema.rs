...
///
/// The subset is now an allow-list in `pryx-schema-dialect`, shared with every
...
pub fn openai_compatible_schema(schema: &Value) -> Value {
    pryx_schema_dialect::normalize(schema, &pryx_schema_dialect::registry::OPENAI)
}
...
pub fn schema_supports_strict(schema: &Value) -> bool {
    fn check_map(map: &serde_json::Map<String, Value>) -> bool {
        ...
        // same rule the registry-wide sweep already enforces on pryx's own
        // tools.
...
mod tests {
    ...
    /// constructs from a real MCP catalog that pryx marked `strict: true` and
