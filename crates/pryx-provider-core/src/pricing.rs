...
/// API spells the latency-optimized tier `auto`; pryx also accepts `priority`
...
/// tier since pryx cannot see the per-request tier split.
...
pub fn copilot_pricing(model: &str, zero_premium_mode: bool) -> RouteCheapnessEstimate {
    ...
    RouteCheapnessEstimate::included_quota(
        ...
        Some(if zero_premium_mode {
            "Copilot zero-premium mode: pryx will send requests as agent/non-premium when possible"
                .to_string()
