...
pub trait Provider: Send + Sync {
    ...
    /// Returns true if pryx should use its own compaction for this provider.
    fn supports_compaction(&self) -> bool {
        false
    }

    /// Returns true if pryx should proactively run its own summary-based compaction.
...
impl CredentialMode {
    /// Resolve the runtime-env pin (PRYX_*_AUTH / route aliases) for a
...
/// Native tool result to send back to provider bridges that delegate tool execution to pryx.
...
/// Canonical User-Agent for generic outbound Pryx HTTP requests.
pub const PRYX_USER_AGENT: &str = concat!("pryx/", env!("CARGO_PKG_VERSION"));
    ...
    CLIENT
        .get_or_init(|| {
            reqwest::Client::builder()
                ...
                .build()
                .unwrap_or_else(|err| {
                    eprintln!("pryx: failed to build shared provider HTTP client: {err}");
                        ...
                        .build()
                    {
                        ...
                        Err(fallback_err) => {
                            eprintln!(
                                "pryx: failed to build fallback provider HTTP client: {fallback_err}"
                            );
                            reqwest::Client::new()
                        }
                    }
...
impl RuntimeKey {
    pub fn from_api_method(api_method: &ModelRouteApiMethod, _provider_label: &str) -> Self {
        match api_method {
            ModelRouteApiMethod::PryxSubscription => Self::PryxSubscription,
    ...
    pub fn stable_id(&self) -> String {
        match self {
            Self::PryxSubscription => "pryx-subscription".to_string(),
...
impl RouteSelection {
    ...
    pub fn routed_model_spec(&self) -> String {
        ...
        match &self.runtime_key {
            RuntimeKey::PryxSubscription => model.to_string(),
...
/// through. Mirrors `pryx_base::provider::openrouter_catalog_model_id` but
...
impl ModelRouteApiMethod {
    ...
    pub fn parse(value: &str) -> Self {
        ...
        match lower.as_str() {
            "pryx-subscription" => Self::PryxSubscription,
    ...
    pub fn display_label(&self) -> String {
        match self {
            Self::PryxSubscription => "subscription".to_string(),
...
mod tests {
    ...
    #[test]
    fn canonical_user_agent_identifies_pryx() {
        assert!(PRYX_USER_AGENT.starts_with("pryx/"));
    }
