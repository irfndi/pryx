...
//! gallery the way it looks live in the pryx TUI, without touching any running
...
//!
//! Run with:
//!   cargo run --profile selfdev -p pryx-tui-render --example swarm_gallery_live
//!
//! Controls:
//!   q / Esc      quit
//!   + / -        more / fewer agents
...
use pryx_tui_render::swarm_gallery::{GalleryMember, humanize_age, render_gallery};
    ...
    let pool: Vec<(&str, Option<&'static str>, Vec<&str>)> = vec![
        (
            ...
            vec![
                ...
                "Reading crates/pryx-app-core/src/auth.rs",
        ...
        (
            ...
            None,
            vec![
                "Editing crates/pryx-base/src/config.rs",
...
fn draw(f: &mut Frame, workers: &[MockWorker], max_pct: usize, paused: bool) {
    ...
    let help = vec![
        ...
        Line::from(""),
        Line::from(vec![Span::styled(
            "  The band above is the same renderer the pryx TUI shows above your chat",
