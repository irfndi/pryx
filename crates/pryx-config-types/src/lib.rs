...
pub struct NamedProviderConfig {
    ...
    /// Must be a JSON object; keys here override pryx-generated body fields.
    ...
    /// `reasoning_effort` request field (`/effort` support). When unset, pryx
...
pub struct AuthConfig {
    /// External auth source ids that the user has approved pryx to read/use.
...
pub struct AgentsConfig {
    ...
    /// local. Env override: `PRYX_MEMORY_EMBEDDING_BACKEND`.
    ...
    /// Unset = `text-embedding-3-small`. Env: `PRYX_MEMORY_EMBEDDING_MODEL`.
    #[serde(default)]
    ...
    /// Env: `PRYX_MEMORY_EMBEDDING_BASE_URL`.
    #[serde(default)]
...
/// pane by the built-in launcher. `PRYX_TERMINAL` explicitly selects a terminal
/// emulator instead.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(default)]
pub struct TerminalConfig {
    ...
    /// opening a terminal emulator itself, with `PRYX_SPAWN_*` metadata env
    ...
    /// tmux window in the current server. If the hook fails to launch, pryx
    ...
    ///
    /// When set, pryx runs the hook (instead of wmctrl/xdotool) whenever it
    /// wants to bring a session's window to the foreground, with
    /// `PRYX_FOCUS_SESSION_ID` and `PRYX_FOCUS_TITLE` env vars. Pair this
    ...
    /// which terminal pryx launches into and is preferred over the legacy
    /// `~/.pryx/preferred_terminal.json` file. Re-run `pryx setup-hotkey`
...
/// Lifecycle hooks: external commands pryx runs at well-defined points.
///
/// Hook commands are parsed shell-style (quotes work) but executed directly,
/// with `PRYX_HOOK_*` env vars describing the event (`PRYX_HOOK_EVENT`,
/// `PRYX_HOOK_SESSION_ID`, `PRYX_HOOK_CWD`, event-specific fields, and a
/// `PRYX_HOOK_PAYLOAD` JSON mirror). Hook processes get
/// `PRYX_HOOKS_DISABLED=1` so nested pryx invocations don't recurse.
///
/// All hooks except `pre_tool` are observers: detached, fire-and-forget,
/// failures only logged. `pre_tool` is a gate: pryx waits for it and exit
...
pub struct HooksConfig {
    ...
    /// ("chat"/"resume"/"ambient"). Env override: PRYX_HOOK_TURN_START.
    ...
    /// Env override: PRYX_HOOK_PRE_TOOL.
...
/// providers may share revenue with Pryx when a referred user becomes a
...
pub struct SponsorsConfig {
    ...
    /// `discover_tools` tool is not registered, and pryx never contacts the
    /// discovery endpoint.
    pub enabled: bool,
...
impl Default for SponsorsConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            endpoint: "https://api.pryx.sh/v1/discovery".to_string(),
        }
    }
}
...
pub struct KeybindingsConfig {
    ...
    /// Spawn a fresh pryx session in a new terminal window (default: unbound).
    /// Example: "alt+enter".
...
pub enum WebSearchEngine {
    ...
    /// `PRYX_SEARXNG_URL` env var) to point at a SearXNG instance. Useful on
...
impl Default for WebSearchConfig {
    fn default() -> Self {
        Self {
            ...
            bing_api_key: None,
            bing_api_key_env: "PRYX_BING_API_KEY".to_string(),
            bing_market: "en-US".to_string(),
            searxng_url: None,
            searxng_url_env: "PRYX_SEARXNG_URL".to_string(),
        }
    }
}
...
pub struct ProviderConfig {
    ...
    /// Whether pryx should automatically try another account on the same provider
    ...
    /// automatically (see `pryx_base::provider::stream_idle_timeout_for_effort`).
    /// Default: 180. Overridable via `PRYX_STREAM_IDLE_TIMEOUT_SECS`.
...
pub struct SafetyConfig {
    ...
    /// SMTP password (prefer PRYX_SMTP_PASSWORD env var)
    ...
    /// Jade relay bearer token (prefer PRYX_JADE_RELAY_TOKEN env var)
...
pub struct PowerConfig {
    /// Prevent automatic system sleep while any pryx session is actively
    ...
    ///
    /// Honored by the shared `pryx serve` daemon. The `PRYX_DISABLE_POWER_INHIBIT`
...
/// A single global launch hotkey: a chord plus the directory it opens pryx in.
///
...
/// - `$LAST_DIR` -> the most recent non-home project directory pryx ran in.
/// - `$LAST_REPO` -> the most recent pryx repo (for self-dev).
///
...
pub struct LaunchHotkeyEntry {
    /// pryx-style chord string, e.g. `cmd+;`, `cmd+[`, `cmd+shift+'`.
    pub chord: String,
...
/// Configuration for the global "launch a new pryx" hotkeys (macOS).
///
/// When `entries` is empty, pryx uses its built-in defaults (`Cmd+;` -> home,
