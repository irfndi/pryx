...
pub(super) fn semver() -> &'static str {
    ...
    SEMVER.get_or_init(|| format!("v{}", pryx_build_meta::semver()))
}

/// True when this process is running from the stable release binary path.
/// Only matches the explicit ~/.pryx/builds/stable/pryx path, NOT
/// ~/.local/bin/pryx launcher path (which now points to current).
