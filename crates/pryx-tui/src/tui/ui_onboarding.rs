...
//!   2. "Welcome to pryx onboarding" title.
...
/// everywhere. Override with `PRYX_ROUNDED_PILLS=on|off`.
fn rounded_pill_caps_supported() -> bool {
    ...
    *SUPPORTED.get_or_init(|| {
        ...
        if let Ok(raw) = std::env::var("PRYX_ROUNDED_PILLS") {
...
/// use a Pryx subscription, with secondary controls for a selective import and
/// telemetry settings.
...
fn telemetry_settings_lines(
    ...
    if env_forced_off {
        lines.push(
            Line::from(Span::styled(
                "Your environment already disables telemetry (PRYX_NO_TELEMETRY).",
                dim,
            ))
            .alignment(align),
        );
...
fn telemetry_header_lines(width: u16) -> Vec<Line<'static>> {
    ...
    let dim = Style::default().fg(dim_color());
    let lines = vec![
        "pryx collects anonymous usage statistics (version, OS, session",
        "activity, and crash reasons). No code, prompts, or personal data.",
        "Change anytime: /telemetry (or export PRYX_NO_TELEMETRY=1)",
    ];
    lines
        .into_iter()
        .map(|text| {
        ...
        OnboardingWelcomeKind::Login {
            ...
            match import {
                ...
                None if error.is_some() => {
                    ...
                    // to hand the fix to it (it can run pryx's auth-test and add
