...
// `pryx-terminal-image` crate (shared with the `read` tool). Re-exported here
// so existing `crate::tui::image` / `crate::tui::image_metadata` paths keep working.
pub use pryx_terminal_image::display as image;
use pryx_terminal_image::metadata as image_metadata;
pub mod info_widget;
...
pub use pryx_tui_workspace::workspace_map;
pub use pryx_tui_workspace::workspace_map_widget;
...
pub trait TuiState {
    ...
    /// from the `PRYX_RUNTIME_PROVIDER` env var. The header must prefer this
    /// over its own env-based heuristic: the TUI client process often does not
    /// inherit `PRYX_RUNTIME_PROVIDER` (it is set inside the agent/server
    ...
    fn active_dual_credential(
        &self,
        _provider: pryx_provider_core::ActiveProvider,
    ...
    /// Number of pryx compactions already applied to this session, when known.
...
fn supports_reliable_zero_cache_read_warning(
    ...
    // OpenRouter/Pryx-subscription routes can only be treated as reliable for zero-read
...
pub struct LoginImportPrompt {
    ...
    /// Whether the environment (PRYX_NO_TELEMETRY / DO_NOT_TRACK) already
...
pub enum ImportSummaryPill {
    ...
    Continue,
    /// Sign in with a Pryx subscription instead of importing.
    Subscription,
...
pub(crate) fn subscribe_metadata(
    ...
    resolve_subscribe_metadata(
        ...
        remote_working_dir,
        pryx_selfdev_types::client_selfdev_requested(),
    )
}
...
pub(crate) fn resolve_subscribe_metadata(
    ...
    if !selfdev && let Some(dir) = client_working_dir {
        ...
        while let Some(path) = current {
            if crate::build::is_pryx_repo(path) {
                selfdev = true;
                break;
            }
            current = path.parent();
...
pub fn display_messages_from_session(session: &crate::session::Session) -> Vec<DisplayMessage> {
    let mut messages = pryx_tui_messages::display_messages_from_rendered_messages(
