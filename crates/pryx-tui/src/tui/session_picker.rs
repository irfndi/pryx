...
use pryx_session_types::SessionStatus;
use ratatui::{
    Frame,
...
pub use pryx_tui_session_picker::{
...
pub enum PickerResult {
    ...
    /// to Pryx. This is never emitted by ordinary Enter/resume behavior.
...
impl SessionPicker {
    ...
    fn ensure_selected_preview_loading(&mut self) {
        ...
        let _ = std::thread::Builder::new()
            .name("pryx-session-preview-loader".to_string())
            .spawn(move || {
        ...
        let body = vec![
            Line::from(Span::styled(
                format!(
                    ...
                    pryx_core::util::truncate_str(session_id, 12)
                ),
                Style::default()
                    .fg(Color::White)
                    .add_modifier(Modifier::BOLD),
            ...
            Line::from(""),
            Line::from(Span::styled(
                "Pryx will prepare the transcript first, then ask Claude to exit.",
