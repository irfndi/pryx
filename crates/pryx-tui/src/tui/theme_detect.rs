...
//!
//! 1. `PRYX_THEME=dark|light` env override (also accepts `auto`).
...
//! 4. Fallback: dark (pryx's native palette).
//!
//! The result is stored in `pryx_tui_style::theme_mode` where the renderer
//! adapts colors for light backgrounds at frame time.

use pryx_tui_style::ThemeMode;
...
pub fn prewarm_theme_mode() {
    ...
    if let Ok(handle) = std::thread::Builder::new()
        .name("pryx-theme-detect".to_string())
...
pub fn init_theme_mode() -> ThemeMode {
    ...
    pryx_tui_style::set_theme_mode(mode);
    init_palette();
    mode
}
...
pub fn init_palette() {
    ...
    let (palette, errors) = pryx_tui_style::Palette::from_pairs(
        configured
            .iter()
...
pub fn current_theme_label() -> &'static str {
    match pryx_tui_style::theme_mode() {
        ThemeMode::Dark => "dark",
...
fn resolve_configured_theme(query_terminal: bool) -> ThemeMode {
    let configured = std::env::var("PRYX_THEME")
        .ok()
