...
//!      login (`pryx auth-test --json`, `pryx provider add --api-key-stdin`,
//!      `pryx login`). The agent can add the key/login for the user itself.
//!
...
/// An external coding agent pryx can hand a repair task to. These map to the
...
/// `PRYX_HOME`/external isolation, matching the onboarding import detectors.
...
pub(crate) fn build_repair_brief(
    ...
    let log_line = pryx_logging::log_path()
        .map(|p| format!("Logs:    {}", p.display()))
        .unwrap_or_else(|| "Logs:    ~/.pryx/logs/pryx-<date>.log".to_string());

    let mut brief = String::new();
    brief.push_str(
        "pryx (a terminal coding agent) could not import or validate a login \
during first-run onboarding. Please fix the login for the user.\n\n",
    ...
    if provider_hint.is_some() {
        brief.push_str(&format!("  pryx auth-test --provider {provider} --json\n",));
    } else {
        brief.push_str("  pryx auth-test --all-configured --json   # which provider is broken?\n");
    }
    brief.push_str("  pryx auth doctor   # human-readable, structured recovery steps\n\n");
...
\u{20}     --base-url https://api.example.com/v1 --model <model> --api-key-stdin\n\n",
    ...
    brief.push_str(
        "When auth-test reports success, tell the user to restart pryx (or press Enter on \
...
/// to paste anything. Lives under the pryx home so it honors `PRYX_HOME`.
...
mod tests {
    ...
    fn brief_lists_the_non_interactive_commands_and_log_path() {
        ...
        assert!(
            brief.contains("pryx auth-test --provider openai --json"),
            "{brief}"
        );
        assert!(brief.contains("pryx login --provider openai"), "{brief}");
        assert!(brief.contains("--api-key-stdin"), "{brief}");
        assert!(brief.contains("pryx provider add"), "{brief}");
        // Points at the logs.
    ...
    fn brief_without_provider_or_agent_is_still_actionable() {
        ...
        assert!(
            brief.contains("pryx auth-test --all-configured --json"),
            "{brief}"
        );
        assert!(brief.contains("pryx auth doctor"), "{brief}");
        // No agent label, but still tells the user what to do.
        assert!(brief.contains("restart pryx"), "{brief}");
    }
