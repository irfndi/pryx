...
**`/remote`** - use pryx from any device

`/remote` opens your Pryx account, the entry point for Pryx Cloud access.
Cloud is being bundled with eligible Pryx subscriptions. The managed host
control plane is currently in early access.

- `/remote` or `/remote cloud` - activate or manage Pryx Cloud
...
const CLOUD_ACCOUNT_URL: &str = "https://pryx.sh/account";
...
fn activate_cloud(app: &mut App) {
    ...
    let account_line = if signed_in {
        match tier {
            ...
            None => "Signed in to your Pryx account.".to_string(),
        }
    } else {
    ...
    let open_line = if opened {
        "Opened your secure Pryx account page in the browser."
    } else {
        "Open your secure Pryx account page:"
    };
    app.push_display_message(DisplayMessage::system(format!(
        "**Pryx Cloud early access**\n\n{account_line}\n\n{open_line}\n\n{CLOUD_ACCOUNT_URL}\n\n\
    ...
    app.set_status_notice(if opened {
        "Pryx Cloud account opened"
    } else {
        "Pryx Cloud account link ready"
    });
}
