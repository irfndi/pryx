...
fn create_test_app() -> crate::tui::app::App {
    ...
    // from its own PRYX_HOME (scheduled reminders read as a notification).
...
/// Point PRYX_HOME at a per-process temp dir when the environment does not
/// already pin one, so tests never read the developer's real `~/.pryx`
...
fn reload_handoff_inactive_without_flag_or_marker() {
    ...
    // `pryx.reload` left by a live self-dev reload on this machine cannot
    // leak into the assertion.
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().expect("create temp dir");
    let prev_runtime = std::env::var_os("PRYX_RUNTIME_DIR");
    crate::env::set_var("PRYX_RUNTIME_DIR", temp.path());

    let inactive = !reconnect::reload_handoff_active(&RemoteRunState::default());

    if let Some(prev_runtime) = prev_runtime {
        crate::env::set_var("PRYX_RUNTIME_DIR", prev_runtime);
    } else {
...
fn remote_skill_invocation_with_prompt_sends_remote_turn() {
    ...
    let skill_dir = temp.path().join(".pryx/skills/remote-skill");
