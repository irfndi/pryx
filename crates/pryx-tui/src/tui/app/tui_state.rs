...
impl App {
    ...
    /// the header uses (session stub, `PRYX_MODEL`, config default) so effort
    ...
    ///   [`AuthStatus`] snapshot (cached up to 60s) or a `PRYX_RUNTIME_PROVIDER`
    ...
    fn dual_credential_active(
        ...
        if route.is_remote {
            ...
            return self
                ...
                .route_api_method
                .as_deref()
                .and_then(pryx_provider_core::AuthRoute::parse)
    ...
    fn widget_auth_method(&self, route: WidgetRouteInfo) -> crate::tui::info_widget::AuthMethod {
        ...
        match route.provider {
            WidgetProviderKind::Anthropic => {
                match self
                    .dual_credential_active(route, pryx_provider_core::ActiveProvider::Claude)
                {
            ...
            WidgetProviderKind::OpenAI => {
                match self
                    .dual_credential_active(route, pryx_provider_core::ActiveProvider::OpenAI)
                {
...
impl crate::tui::TuiState for App {
    ...
    fn session_compaction_count(&self) -> usize {
        if self.is_remote || !self.provider.uses_pryx_compaction() {
            return 0;
        }
        self.registry
            .compaction()
    ...
    fn status_notice(&self) -> Option<String> {
        if !self.is_remote
            && self.provider.uses_pryx_compaction()
    ...
    fn context_snapshot(&self) -> crate::tui::ContextSnapshot {
        ...
        let (compaction_count, compaction_summary_chars, is_compacting, compaction_fresh) =
            if self.is_remote {
                (0, 0, false, true)
            } else if self.provider.uses_pryx_compaction() {
        ...
        if self.is_remote {
            ...
            let skip = if self.provider.uses_pryx_compaction() {
    ...
    fn info_widget_data(&self) -> crate::tui::info_widget::InfoWidgetData {
                ...
                || !managed_members.is_empty()
            {
                ...
                Some(crate::tui::info_widget::SwarmInfo {
                    ...
                    spinner_frame: (self.animation_elapsed()
                        * pryx_tui_render::swarm_gallery::STRIP_SPINNER_FPS)
                        as usize,
                    managed_members,
                })
            } else {
        ...
        let compaction_info = if !self.is_remote && self.provider.uses_pryx_compaction() {
            let compaction = self.registry.compaction();
        ...
        crate::tui::info_widget::InfoWidgetData {
            ...
            cache_hit_info,
            compaction_info,
            is_compacting: if !self.is_remote && self.provider.uses_pryx_compaction() {
                let compaction = self.registry.compaction();
                compaction
                    .try_read()
...
impl App {
    ...
    pub(crate) fn pop_out_selected_swarm_agent(&mut self) {
        ...
        let exe = std::env::current_exe().unwrap_or_else(|_| std::path::PathBuf::from("pryx"));
        let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
        match pryx_app_core::session_launch::spawn_resume_in_new_terminal(&exe, &session_id, &cwd)
        {
