...
///
/// Layout (macOS):
///   title:    pryx · <session> · done in <dur>
...
pub(super) fn build_turn_notification(
    ...
    let mut title = String::from("pryx");
...
mod tests {
    ...
    fn title_includes_session_and_compact_duration() {
        ...
        assert_eq!(n.title, "pryx · fox · done in 12m 34s");
    ...
    fn subtitle_holds_progress_and_body_names_the_work() {
        ...
        assert_eq!(n.title, "pryx · done in 3m 20s");
    ...
    fn empty_inputs_fall_back_to_minimal_body() {
        ...
        assert_eq!(n.title, "pryx · done in 1m 5s");
        assert_eq!(n.subtitle, None);
    ...
    fn kitty_notification_is_one_completed_clickable_message() {
        let n = TurnNotification {
            title: "pryx · fox".to_string(),
            subtitle: Some("2/3 todos".to_string()),
            body: "Finished parser".to_string(),
        };
        assert_eq!(
            kitty_notification_sequence(&n, "session:fox/123"),
            "\x1b]99;i=pryx-turn-sessionfox123:d=0:e=1:p=title;amNvZGUgwrcgZm94\x1b\\\x1b]99;i=pryx-turn-sessionfox123:d=1:e=1:p=body:a=focus;Mi8zIHRvZG9zCkZpbmlzaGVkIHBhcnNlcg==\x1b\\"
        );
    }
