...
//   cargo test -p pryx-tui onboarding_golden -- --nocapture
...
fn onboarding_golden_walks_every_phase() {
    ...
    // import action, with a Pryx subscription alternative and secondary
    // import/telemetry controls beside it.
    {
        ...
        // The primary actions explicitly offer import or a Pryx subscription.
        assert!(text.contains("Import"), "import pill label: {text}");
        assert!(
            text.contains("Pryx subscription"),
    ...
    // 5. Suggestions (resting state).
    {
        ...
        assert!(text.contains("Welcome to pryx onboarding"), "{text}");
    }
}
...
fn onboarding_golden_walks_failure_and_async_states() {
    ...
    let assert_guided_polish = |title: &str, text: &str| {
        assert!(
            text.contains("Welcome to pryx onboarding"),
...
fn onboarding_golden_telemetry_settings_page() {
    ...
    assert!(text.contains("Helps pryx the most"), "caption 1: {text}");
    assert!(
