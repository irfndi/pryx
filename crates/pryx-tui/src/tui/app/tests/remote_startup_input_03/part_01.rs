...
fn test_recover_session_without_tools_preserves_debug_and_canary_flags() {
    ...
    app.session.working_dir = Some("/tmp/pryx-test".to_string());
    ...
    assert_eq!(app.session.working_dir.as_deref(), Some("/tmp/pryx-test"));
...
fn test_selfdev_command_spawns_session_in_test_mode() {
    ...
    let prev_test = std::env::var_os("PRYX_TEST_SESSION");
    crate::env::set_var("PRYX_HOME", temp_home.path());
    crate::env::set_var("PRYX_TEST_SESSION", "1");

    let repo = create_pryx_repo_fixture();
    let mut app = create_test_app();
    ...
    let sessions_dir = crate::storage::pryx_dir().unwrap().join("sessions");
    ...
    if let Some(prev_test) = prev_test {
        crate::env::set_var("PRYX_TEST_SESSION", prev_test);
    } else {
        crate::env::remove_var("PRYX_TEST_SESSION");
    }
}
...
fn test_restore_reload_state_supports_legacy_input_format() {
    ...
    let pryx_dir = crate::storage::pryx_dir().unwrap();
    let path = pryx_dir.join(format!("client-input-{}", session_id));
    std::fs::write(&path, "2\nhello").unwrap();
