...
/// RAII guard for the PRYX_PIN_TODOS env override used by the band tests.
struct PinTodosEnvGuard;

impl PinTodosEnvGuard {
    fn enable() -> Self {
        crate::env::set_var("PRYX_PIN_TODOS", "1");
        // pryx-base's config cache throttles env re-checks (the zero
        // interval under cfg!(test) applies only when pryx-base itself is
        // the crate under test), so force a reload or a sibling test's
        // PRYX_PIN_TODOS state leaks into this one for up to 500ms.
        crate::config::invalidate_config_cache();
        Self
    }

    fn disable() -> Self {
        crate::env::set_var("PRYX_PIN_TODOS", "0");
        crate::config::invalidate_config_cache();
        Self
    }
}
...
impl Drop for PinTodosEnvGuard {
    fn drop(&mut self) {
        crate::env::remove_var("PRYX_PIN_TODOS");
