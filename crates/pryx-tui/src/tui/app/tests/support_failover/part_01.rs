...
fn cleanup_background_task_files(task_id: &str) {
    let task_dir = std::env::temp_dir().join("pryx-bg-tasks");
...
/// Point `PRYX_HOME` at a per-process scratch directory if nothing set one.
///
...
/// that scope their own `PRYX_HOME` restore it by *removing* the variable, so
/// without serialization this function would observe the gap and repoint
/// `PRYX_HOME` at the shared scratch home while that test was still running.
/// That is the race behind pryx-tui's intermittent failures in unrelated
/// ambient, header, and model-picker tests, which all read files under
/// `PRYX_HOME` mid-assertion.
///
...
/// `with_temp_pryx_home` hold this same non-reentrant mutex for their whole
...
fn ensure_test_pryx_home_if_unset() {
    ...
    // own PRYX_HOME under `lock_test_env`. The mutex is not reentrant and
    // several tests hold it while calling `create_test_app` (e.g. the
    // pinned-todo-band test), so a blocking `lock_test_env()` here would
    // self-deadlock whenever a preceding test removed PRYX_HOME on drop.
...
fn clear_persisted_test_ui_state() {
    if let Ok(home) = crate::storage::pryx_dir() {
...
fn with_temp_pryx_home<T>(f: impl FnOnce() -> T) -> T {
...
/// Run `f` in a hermetic `PRYX_HOME` with reasoning display pinned to
/// `current`.
///
...
fn create_pryx_repo_fixture() -> tempfile::TempDir {
