...
fn swarm_prompt_edit_path_prefers_nonblank_project_override() {
    ...
    let pryx_home = tempfile::tempdir().expect("pryx tempdir");
    let project_prompt = project.path().join(".pryx/swarm-prompt.md");
    ...
    std::fs::write(pryx_home.path().join("swarm-prompt.md"), "global routing")
        .expect("write global prompt");

    let path = ensure_swarm_prompt_edit_path(project.path().to_str(), pryx_home.path())
...
fn swarm_prompt_edit_path_falls_back_to_nonblank_global_override() {
    ...
    let global_prompt = pryx_home.path().join("swarm-prompt.md");
...
fn swarm_prompt_edit_path_materializes_builtin_default_globally() {
    ...
    assert_eq!(path, pryx_home.path().join("swarm-prompt.md"));
...
/// error handling.
///
/// These use an isolated `PRYX_HOME` (`create_test_app` sets one) so they write
...
mod colors {
    ...
    fn with_clean_config(body: impl FnOnce()) {
        ...
        impl Drop for Restore {
            fn drop(&mut self) {
                ...
                let _ = config.save();
                pryx_tui_style::set_palette(pryx_tui_style::Palette::default());
            }
        }
    ...
    fn colors_lists_every_role_with_its_hex_value() {
        ...
        for role in pryx_tui_style::ALL_ROLES.iter().copied() {
            assert!(
    ...
    fn setting_a_role_persists_it_and_reports_the_new_score() {
        with_clean_config(|| {
            ...
            assert!(
                pryx_tui_style::palette().is_overridden(pryx_tui_style::Role::Error),
    ...
    fn generate_writes_a_complete_palette_and_scores_it() {
        with_clean_config(|| {
            ...
            for role in pryx_tui_style::ALL_ROLES.iter().copied() {
                ...
                assert!(
                    pryx_tui_style::palette::parse_hex(value).is_some(),
