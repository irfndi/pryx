...
fn parse_account_command(trimmed: &str) -> Option<Result<AccountCommand, String>> {
    ...
    if let Some(provider) = resolve_account_provider_descriptor(first) {
        ...
        let parsed = match subcommand {
            ...
            "status" if provider.id == "pryx" => AccountCommand::PryxStatus,
            "manage" if provider.id == "pryx" => AccountCommand::PryxManage,
            "logout" if provider.id == "pryx" => AccountCommand::PryxLogout,
...
pub(crate) fn execute_account_command_local(app: &mut App, command: AccountCommand) {
    match command {
        ...
        AccountCommand::PryxStatus => app.show_pryx_subscription_status(),
        AccountCommand::PryxManage => app.open_pryx_account_management(),
        AccountCommand::PryxLogout => app.start_pryx_account_logout(),
...
fn save_default_provider_setting(app: &mut App, provider: Option<&str>) {
    ...
    let provider = match normalized.as_deref() {
        ...
        Some(other) if pryx_provider_core::AuthRoute::parse(other).is_some() => normalized,
        Some(other) => {
            app.push_display_message(DisplayMessage::error(format!(
...
fn save_openai_compat_setting(app: &mut App, setting: OpenAiCompatSetting, value: Option<&str>) {
    ...
    let (env_key, normalized_value) = match setting {
        OpenAiCompatSetting::ApiBase => {
            ...
            ("PRYX_OPENAI_COMPAT_API_BASE", normalized)
        }
...
mod tests {
    ...
    #[test]
    fn parse_native_pryx_account_actions() {
        assert!(matches!(
            parse_account_command("/account pryx login"),
            Some(Ok(AccountCommand::Login { provider_id })) if provider_id == "pryx"
        ));
        assert!(matches!(
            parse_account_command("/account pryx status"),
            Some(Ok(AccountCommand::PryxStatus))
        ));
        assert!(matches!(
            parse_account_command("/account pryx manage"),
            Some(Ok(AccountCommand::PryxManage))
        ));
        assert!(matches!(
            parse_account_command("/account pryx logout"),
            Some(Ok(AccountCommand::PryxLogout))
        ));
    }
    ...
    fn render_auth_doctor_markdown_includes_recovery_steps() {
        ...
        assert!(markdown.contains("pryx login --provider openai"));
        assert!(markdown.contains("Review current state: pryx auth status --json"));
    }
}
