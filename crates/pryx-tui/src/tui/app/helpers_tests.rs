...
fn shell_command_quotes_single_quotes_for_handterm_exec() {
    let command = shell_command(&[
        "/tmp/pryx binary".to_string(),
        "--resume".to_string(),
        "session'quote".to_string(),
    ]);
    assert_eq!(
        command,
        "'/tmp/pryx binary' '--resume' 'session'\"'\"'quote'"
    );
}
...
fn resume_invocation_args_includes_socket_when_present() {
    let args = resume_invocation_args("ses_123", Some("/tmp/pryx-test.sock"));
    assert_eq!(
        args,
        vec![
...
/// Pin PRYX_HOME to a tempdir containing a `builds/current/pryx` binary so
...
/// other tests mutating PRYX_HOME in parallel. Returns the guards that keep
...
fn pinned_resume_test_home() -> (
    ...
    std::fs::write(current.join("pryx"), b"#!/bin/sh\n").expect("write fake pryx binary");
    let home = EnvVarGuard::set_path("PRYX_HOME", temp.path());
    (env_lock, temp, home)
}

#[test]
fn build_resume_command_uses_imported_pryx_session_for_claude_code() {
...
#[test]
fn build_resume_command_uses_imported_pryx_session_for_codex() {
    let _pinned = pinned_resume_test_home();
    let (program, args, title) = build_resume_command(
...
fn gather_ambient_info_filters_to_session_reminders_when_ambient_disabled() {
    ...
    // Read synchronously while the temporary PRYX_HOME and its files are pinned:
    // an unrelated in-flight cache refresh can otherwise overwrite the cleared
    // process-global cache with data loaded under another test's PRYX_HOME.
...
fn fresh_session_command_includes_fresh_spawn_and_socket() {
    ...
    assert_eq!(command.title.as_deref(), Some("pryx · new session"));
    assert_eq!(
        command.args,
        vec![
            "--fresh-spawn".to_string(),
