...
//!     pryx account credentials, never in replay/test runtimes.
//!
...
pub(super) const RATE_LIMIT_NUDGE_LINE: &str =
    "Avoid provider rate limits with Pryx hosted models: /hosted";
/// Status-line tail shared by trigger B.
const SUPPORT_NUDGE_NOTICE: &str = "Try Pryx hosted models: /hosted";
...
/// Persisted nudge bookkeeping (one small JSON file under the pryx dir).
...
pub(super) fn subscribe_pitch_markdown() -> String {
    let mut message = String::from("Pryx hosted models\n\n");
    ...
    message.push_str("  - Sign in once: Pryx saves the hosted API key and routes the rest\n");
    ...
    message.push_str("  - Funds Pryx development while the software stays open source\n");

    message.push_str("\nStart: /login pryx (browser approval, no key pasted into the terminal)\n");
...
impl App {
    ...
    pub(super) fn claim_subscribe_nudge(&mut self, trigger: SubscribeNudgeTrigger) -> bool {
        ...
        // Never pitch existing pryx account holders.
    ...
    pub(super) fn show_subscribe_pitch(&mut self) {
        ...
        self.set_status_notice("Hosted models: /login pryx to start");
    }
}
...
mod tests {
    ...
    fn long_task_message_reads_support_wording_with_elapsed() {
        ...
        assert_eq!(
            message,
            "✦ pryx just worked 1h 23m for you. Try Pryx hosted models: /hosted"
        );
    ...
    fn pitch_explains_limits_warnings_tranches_and_next_step() {
        ...
        assert!(pitch.contains("/login pryx"));
