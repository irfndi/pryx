...
#[test]
fn tui_openai_compatible_api_base_keeps_pryx_docs_and_remote_endpoint() -> anyhow::Result<()> {
    with_temp_pryx_home(|| {
        ...
        assert!(resolved.requires_api_key);
        assert!(resolved.setup_url.contains("github.com/pryx-co/pryx"));
...
#[test]
fn tui_pryx_subscription_logout_clears_credentials_and_preserves_api_base() -> anyhow::Result<()> {
    with_temp_pryx_home(|| {
        ...
        assert!(std::env::var_os(crate::subscription_catalog::PRYX_API_KEY_ENV).is_none());
        assert_eq!(
            std::env::var(crate::subscription_catalog::PRYX_API_BASE_ENV).as_deref(),
            Ok("https://subscription.example/v1")
        );
        assert!(std::env::var_os(crate::subscription_catalog::PRYX_ACCOUNT_ID_ENV).is_none());
        assert!(std::env::var_os(crate::subscription_catalog::PRYX_ACCOUNT_EMAIL_ENV).is_none());
