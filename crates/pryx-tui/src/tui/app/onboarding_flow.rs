...
pub(crate) enum SummaryPill {
    ...
    Continue,
    /// Skip importing and sign in with a Pryx subscription instead (default).
    Subscription,
...
impl ImportReview {
    ...
    /// starting on the summary screen with the Pryx subscription preselected.
...
/// `PRYX_HOME`/external isolation, matching the import detectors.
...
mod tests {
    ...
    fn external_oauth_present_requires_nonempty_file() {
        let dir = std::env::temp_dir().join(format!("pryx-onb-test-{}", std::process::id()));
