...
fn onboarding_external_filter_picks_latest_visible_transcript() {
    ...
    let pryx = make_session("pryx_one", "pryx", false, SessionStatus::Closed);

    let mut picker = SessionPicker::new(vec![older, pryx, newer]);
...
fn onboarding_external_filter_with_no_matches_has_no_target() {
    let pryx = make_session("pryx_only", "pryx", false, SessionStatus::Closed);
    let mut picker = SessionPicker::new(vec![pryx]);
...
fn onboarding_banner_renders_prompt_and_both_action_rows() {
    ...
    let welcome_y = lines
        .iter()
        .position(|line| line.contains("Welcome to pryx"))
        .expect("welcome row");
    let review_y = lines
        .iter()
...
//
//   cargo test -p pryx-tui --lib --release -- --ignored --nocapture benchmark_resume_op
//
