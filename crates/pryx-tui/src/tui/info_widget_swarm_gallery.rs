...
//! [`pryx_tui_render::swarm_gallery`] module so the live TUI and the
...
use pryx_tui_core::keybind::alt_chord_lower;
use pryx_tui_render::swarm_gallery::{
    ...
    members
        ...
        .map(|member| GalleryMember {
            ...
            todo_items: member
                .todo_items
                .iter()
                .map(|t| pryx_tui_render::swarm_gallery::GalleryTodo {
                    content: t.content.clone(),
                    status: t.status.clone(),
                    tool_intents: t
                        .tool_intents
                        .iter()
                        .map(|tool| pryx_tui_render::swarm_gallery::GalleryToolIntent {
...
pub(crate) fn render_swarm_chat_card_lines(
    ...
    pryx_tui_render::swarm_gallery::render_swarm_chat_cards(&gallery_members, width)
}
...
mod tests {
    use super::*;
    use pryx_tui_render::swarm_gallery::members_to_tiles;
