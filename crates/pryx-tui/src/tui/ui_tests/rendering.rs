...
fn test_render_memory_tiles_allows_boxes_below_other_boxes() {
    let tiles = group_into_tiles(vec![
        ...
        (
            "preference".to_string(),
            "User prefers quick verification that pryx is up-to-date.".to_string(),
        ),
        ("fact".to_string(), "Jeremy".to_string()),
        (
            "entity".to_string(),
...
fn test_light_theme_adapted_frame_has_readable_contrast() {
    ...
    terminal
        .draw(|frame| {
            ...
            pryx_tui_style::theme_mode::adapt_buffer(
                frame.buffer_mut(),
                pryx_tui_style::ThemeMode::Light,
            );
        })
        .expect("draw");
...
///
/// The unit tests in `pryx-tui-style` cover the substitution in isolation, but
...
fn test_configured_palette_recolors_a_real_rendered_frame() {
    ...
    let mut palette = pryx_tui_style::Palette::default();
    palette.set(pryx_tui_style::Role::User, (250, 40, 200));
    pryx_tui_style::set_palette(palette);
