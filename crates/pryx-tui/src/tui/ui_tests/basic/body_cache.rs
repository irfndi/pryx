...
fn test_prepare_body_anchors_tool_image_after_tool_message() {
    ...
    assert_eq!(region.render, pryx_tui_messages::ImageRegionRender::Fit);
...
fn test_prepare_body_incremental_anchors_image_on_new_tool_message() {
    ...
    assert_eq!(
        ...
        pryx_tui_messages::ImageRegionRender::Fit
    );
...
fn test_prepare_body_with_math_never_blocks_on_a_stalled_tex_toolchain() {
    ...
    unsafe {
        std::env::set_var("PRYX_LATEX_COMMAND", &stub);
        std::env::set_var("PRYX_DVIPNG_COMMAND", &stub);
        std::env::set_var("PRYX_PDFLATEX_COMMAND", &stub);
        std::env::set_var("PRYX_PDFTOCAIRO_COMMAND", &stub);
    }
