...
//!
//! Ollama's `/v1/models` response carries no `context_length`, so pryx used to
...
//! the conversation" (issue: Ollama conversations appear stateless in pryx).
//!
...
/// user knows to raise `OLLAMA_CONTEXT_LENGTH` instead of assuming pryx lost
/// the conversation.
