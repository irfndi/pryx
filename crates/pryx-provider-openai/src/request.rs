...
use pryx_provider_core::openai_schema::{
...
pub fn openai_encrypted_content_fallback_summary(encrypted_content_len: usize) -> String {
    format!(
        "OpenAI native compaction state was discarded because its encrypted payload was {} chars, above Pryx's safe replay limit of {} chars (provider hard limit: {} chars). Earlier compacted details may be unavailable; use the recent visible messages and session search/tools if exact prior details are needed.",
...
mod tests {
    ...
    ///
    /// The schema-level tests in `pryx-provider-core` cover normalization, but
