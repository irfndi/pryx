...
mod tests {
    ...
    fn summarize_update_error_is_always_one_short_line() {
        let inputs = [
            "Update failed: Update check failed: error sending request for url (https://api.github.com/repos/pryx-co/pryx/releases/latest)\n\nCaused by:\n    dns error: failed to lookup address information",
            "cargo build failed: error[E0308]: mismatched types\n  --> src/lib.rs:1:1",
            "Checksum mismatch for pryx-linux-x86_64.tar.gz: expected aaa, got bbb",
            "Failed to install /home/u/.pryx/builds/versions/0.1.0/pryx: Permission denied (os error 13)",
    ...
    fn summarize_update_error_maps_known_causes() {
        ...
        assert_eq!(
            summarize_update_error("Checksum mismatch for pryx: expected a, got b"),
            "download failed checksum verification"
        );
        assert_eq!(
            summarize_update_error("No asset found for platform: pryx-linux-x86_64"),
    ...
    fn asset_name_is_supported() {
        assert_ne!(get_asset_name(), "pryx-unknown");
    }
    ...
    fn sha256sums_accepts_standard_and_binary_lines() {
        let checksums = parse_sha256sums(
            "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  pryx-linux-x86_64\n\
             bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb *pryx-macos-aarch64\n",
        )
        .unwrap();
        assert_eq!(
            checksums.get("pryx-linux-x86_64").map(String::as_str),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        );
        assert_eq!(
            checksums.get("pryx-macos-aarch64").map(String::as_str),
    ...
    fn checksum_verification_accepts_matching_digest() {
        ...
        let sums = format!("{}  pryx-linux-x86_64\n", digest);
        verify_asset_checksum_text(&sums, "pryx-linux-x86_64", bytes).unwrap();
    }

    #[test]
    fn checksum_verification_rejects_mismatch() {
        let sums = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  pryx-linux-x86_64\n";
        let err = verify_asset_checksum_text(sums, "pryx-linux-x86_64", b"hello")
            .unwrap_err()
            .to_string();
    ...
    fn sha256sums_rejects_invalid_digest() {
        let err = parse_sha256sums("not-a-digest  pryx\n")
            .unwrap_err()
            .to_string();
