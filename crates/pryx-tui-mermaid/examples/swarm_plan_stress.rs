//! Stress probe: build the mermaid source that `pryx_plan::mermaid::
...
//!
//!   scripts/dev_cargo.sh run -p pryx-tui-mermaid --features renderer \
//!       --example swarm_plan_stress
//!
//! Cases:
//!   (a) real deep-mode plan fixture (examples/swarm_plan_fixture.json,
//!       snapshot of ~/.pryx/state/swarm/_home_jeremy_pryx__git.json)
...
use pryx_plan::PlanItem;
use pryx_plan::mermaid::swarm_plan_mermaid;
...
fn probe(name: &str, src: &str) -> bool {
    match pryx_tui_mermaid::render_mermaid_untracked(src, Some(100)) {
        ...
        pryx_tui_mermaid::RenderResult::Error(err) => {
