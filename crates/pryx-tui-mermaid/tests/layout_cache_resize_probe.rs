...
//!
//! Run with:
//!   cargo test --release -p pryx-tui-mermaid --test layout_cache_resize_probe -- --ignored --nocapture
//!
//! NOTE: uses the real pryx mermaid cache dir (like the crate's unit tests);
...
use pryx_tui_mermaid::{RenderResult, debug_stats, render_mermaid_untracked};

/// Deterministic ~40-node / 45-edge flowchart. There is no `infra` fixture in
/// this crate (checked `crates/pryx-tui-mermaid/` for fixtures/), so this is
