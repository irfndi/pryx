...
//! analyzer. Run with:
//!   cargo run -p pryx-tui-style --example light_bench

use pryx_tui_style::palette::{ALL_ROLES, Palette, Role, parse_hex, to_hex};
use pryx_tui_style::{HarmonyReport, Oklab, analyze_harmony};
...
fn main() {
    ...
    let generated =
        pryx_tui_style::harmony::generate_from_seed(Role::User.default_rgb(), LIGHT_BG);
    let tuned = tune(&hand_tuned());

    report("derived (runtime flip of dark defaults)", &derived);
    println!();
    report("generated (generate_from_seed, pryx blue)", &generated);
    println!();
