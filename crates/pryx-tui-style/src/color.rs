...
/// and Apple Terminal; see `pryx_app_core::perf` and issue #330). These
/// renderers key their rasterized-glyph cache on the full 24-bit color, so the
/// continuous color animations pryx emits (shimmer, rainbow, pulsing tool
...
/// full truecolor.
///
/// Overridable with `PRYX_GLYPH_SAFE_MODE=on|off` (shared with the perf
...
fn fragile_glyph_cache_terminal() -> bool {
    ...
    // Mirror of `pryx_app_core::perf::detect_terminal` for the two affected
...
mod tests {
    ...
    fn test_fragile_terminal_override_on_forces_quantize() {
        temp_env_scope(&[("PRYX_GLYPH_SAFE_MODE", Some("on"))], || {
            assert!(fragile_glyph_cache_terminal());
        });
    }
