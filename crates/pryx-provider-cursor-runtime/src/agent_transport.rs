...
use pryx_message_types::StreamEvent;
...
fn agent_host_from_cursor_cli_config() -> Option<String> {
    let path = match pryx_base::storage::user_home_path(".cursor/cli-config.json") {
        Ok(path) => path,
        Err(err) => {
...
/// 1. `PRYX_CURSOR_AGENT_HOST` / `CURSOR_AGENT_HOST` (explicit override)
...
fn agent_host() -> String {
    for var in ["PRYX_CURSOR_AGENT_HOST", "CURSOR_AGENT_HOST"] {
...
mod tests {
    ...
    fn agent_host_normalizes_env_overrides() {
        ...
        // shared lock in pryx-base.
        let _guard = pryx_base::storage::lock_test_env();
        let prev = std::env::var_os("PRYX_CURSOR_AGENT_HOST");

        pryx_base::env::set_var(
            "PRYX_CURSOR_AGENT_HOST",
            "https://agentn.us.api5.cursor.sh/agent.v1.AgentService/Run",
        );
        assert_eq!(agent_host(), "agentn.us.api5.cursor.sh");

        pryx_base::env::set_var("PRYX_CURSOR_AGENT_HOST", "agentn.eu.api5.cursor.sh:443");
        assert_eq!(agent_host(), "agentn.eu.api5.cursor.sh");

        // A blank override must not win; it falls through to the next source.
        pryx_base::env::set_var("PRYX_CURSOR_AGENT_HOST", "   ");
        assert_ne!(agent_host(), "   ");

        match prev {
            Some(value) => pryx_base::env::set_var("PRYX_CURSOR_AGENT_HOST", value),
            None => pryx_base::env::remove_var("PRYX_CURSOR_AGENT_HOST"),
        }
    }
