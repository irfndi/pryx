...
pub async fn send_email(request: SendEmailRequest<'_>) -> Result<()> {
    ...
    if let Some(cid) = request.cycle_id {
        let msg_id = format!("<ambient-{}@pryx>", cid);
...
pub fn poll_imap_once(host: &str, port: u16, user: &str, pass: &str) -> Result<Vec<ReplyAction>> {
    ...
    let reply_search = session.search("UNSEEN HEADER In-Reply-To \"@pryx>\"")?;
    let button_search = session.search("UNSEEN SUBJECT \"[pryx-perm:\"")?;
    ...
    for message in messages.iter() {
            ...
            && let Some(parsed) = mail_parser::MessageParser::default().parse(body)
        {
            ...
            let cycle_id = if in_reply_to.contains("@pryx>") {
                in_reply_to
                    .trim_start_matches("<ambient-")
                    .trim_end_matches("@pryx>")
                    .to_string()
            } else if let Some(start) = subject.find("[pryx-perm:") {
                let rest = &subject[start + "[pryx-perm:".len()..];
...
pub fn build_permission_email_html(
    ...
    let approve_subj_raw = format!("[pryx-perm:{}] Approved", request_id);
    let deny_subj_raw = format!("[pryx-perm:{}] Denied", request_id);
...
mod tests {
    ...
    fn test_build_permission_email_html() {
        ...
        assert!(html.contains("mailto:pryx@example.com"));
    }
}
