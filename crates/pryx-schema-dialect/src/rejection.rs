...
//! construct, but it cannot be complete: a provider may reject something pryx
...
//! same turn recover, and lets pryx report exactly which keyword to add.
//!
...
pub fn classify(message: &str) -> Option<SchemaRejection> {
    ...
    // reaching here means a schema pryx did not expect.
...
/// avoid a pointless retry when the rejection names something pryx did not
