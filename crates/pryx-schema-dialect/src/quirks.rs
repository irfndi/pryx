...
//! knowledge dies with the process. Writing it to `~/.pryx/schema-quirks.json`
//! makes the second request the fast path and means the fleet self-heals in
//! front of a provider change instead of waiting for a pryx release.
//!
...
fn store_path() -> Option<PathBuf> {
    ...
    // developer's real `~/.pryx/schema-quirks.json`, silently stripping that
    ...
    {
        if let Ok(explicit) = std::env::var("PRYX_SCHEMA_QUIRKS_PATH") {
            return Some(PathBuf::from(explicit));
        }
        let home = if let Ok(pryx_home) = std::env::var("PRYX_HOME") {
            PathBuf::from(pryx_home)
        } else {
            dirs::home_dir()?.join(".pryx")
        };
...
/// developer's real `~/.pryx/schema-quirks.json`.
#[cfg(any(test, feature = "test-support"))]
...
/// alternate `PRYX_HOME`) never read each other's state.
