...
///
/// Disable entirely with `PRYX_WATCHDOG=0`. Tune with
/// `PRYX_WATCHDOG_STALL_SECS` and `PRYX_WATCHDOG_HEARTBEAT_SECS`.
pub fn start() {
    if std::env::var("PRYX_WATCHDOG").is_ok_and(|value| value == "0" || value == "off") {
        return;
    }
    ...
    let stall = Duration::from_secs(env_secs("PRYX_WATCHDOG_STALL_SECS", DEFAULT_STALL_SECS));
    let heartbeat = Duration::from_secs(env_secs(
        "PRYX_WATCHDOG_HEARTBEAT_SECS",
        DEFAULT_HEARTBEAT_SECS,
    ));

    std::thread::Builder::new()
        .name("pryx-watchdog".to_string())
