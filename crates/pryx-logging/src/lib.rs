//! Logging infrastructure for pryx
//!
//! Logs to ~/.pryx/logs/ with automatic rotation
//!
...
impl LogLevel {
    ...
    fn is_enabled(self) -> bool {
        !matches!(self, Self::Debug) || std::env::var("PRYX_TRACE").is_ok()
    }
}
...
fn log_dir() -> Option<PathBuf> {
    pryx_storage::logs_dir().ok()
}

impl Logger {
    fn new() -> Option<Self> {
        let log_dir = log_dir()?;
        pryx_storage::ensure_dir(&log_dir).ok()?;

        // Use date-based log file
        let date = Local::now().format("%Y-%m-%d");
        let path = log_dir.join(format!("pryx-{}.log", date));
    ...
    fn write(&mut self, level: &str, message: &str) {
        ...
        if let Err(err) = self.file.write_all(line.as_bytes()) {
            eprintln!("pryx logger write failed: {err}");
            return;
        }
        if let Err(err) = self.file.flush() {
            eprintln!("pryx logger flush failed: {err}");
        }
    }
}
...
pub fn init() {
    ...
    // `pryx-*.log` files, so it is safe to run unconditionally.
    ...
    if !CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
        std::thread::Builder::new()
            .name("pryx-log-cleanup".to_string())
...
/// Log a debug message (only if PRYX_TRACE is set)
#[expect(
    clippy::collapsible_if,
    reason = "Debug logging keeps env gating and logger access explicit"
)]
pub fn debug(message: &str) {
    if std::env::var("PRYX_TRACE").is_ok() {
        if let Ok(mut guard) = LOGGER.lock() {
            if let Some(logger) = guard.as_mut() {
...
fn structured_json_enabled() -> bool {
    matches!(
        std::env::var("PRYX_LOG_JSON").as_deref(),
...
pub fn log_path() -> Option<PathBuf> {
    let log_dir = log_dir()?;
    let date = Local::now().format("%Y-%m-%d");
    Some(log_dir.join(format!("pryx-{}.log", date)))
}

/// Remove daily `pryx-*.log` / `pryx-desktop-*.log` files older than 7 days.
///
...
fn cleanup_old_logs_in(log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
    ...
    for entry in entries.flatten() {
        ...
        let is_pryx_log = (name.starts_with("pryx-") || name.starts_with("pryx-desktop-"))
            && name.ends_with(".log");
        if !is_pryx_log {
            continue;
        }
            ...
            && let Ok(modified) = metadata.modified()
        {
            ...
            {
                eprintln!("pryx logger cleanup failed: {err}");
            }
        }
    }
}
...
fn truncate(s: &str, max_len: usize) -> String {
    if s.len() > max_len {
        format!("{}...", pryx_core::util::truncate_str(s, max_len))
    } else {
        s.to_string()
    }
}
...
mod tests {
    ...
    #[test]
    fn cleanup_removes_only_old_pryx_logs() {
        use std::time::{Duration, SystemTime};

        let dir = std::env::temp_dir().join(format!(
            "pryx-log-cleanup-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let old_log = write("pryx-2000-01-01.log", true);
        let old_desktop = write("pryx-desktop-2000-01-01.log", true);
        // Recent log file that SHOULD survive.
        let new_log = write("pryx-2099-01-01.log", false);
        ...
        assert!(!old_log.exists(), "old pryx log should be deleted");
        assert!(!old_desktop.exists(), "old desktop log should be deleted");
        assert!(new_log.exists(), "recent pryx log must survive");
