...
//!
//! Run with: cargo test -p pryx-logging --test watchdog_stall_probe -- --ignored
...
fn stalled_process_emits_watchdog_stall_event() {
    let dir = std::env::temp_dir().join(format!("pryx-wd-probe-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    unsafe {
        std::env::set_var("HOME", &dir);
        std::env::set_var("PRYX_WATCHDOG_STALL_SECS", "5");
        std::env::set_var("PRYX_WATCHDOG_HEARTBEAT_SECS", "5");
    }

    pryx_logging::init();
    let _work = pryx_logging::watchdog::begin_work("probe.phase");
    pryx_logging::watchdog::set_detail("probe detail");
    ...
    while Instant::now() < deadline {
        ...
        let Some(path) = pryx_logging::log_path() else {
            continue;
        };
