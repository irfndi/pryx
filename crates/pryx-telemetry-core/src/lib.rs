...
pub use pryx_usage_types::{ErrorCategory, SessionEndReason};
...
const TELEMETRY_ENDPOINT: &str = "https://telemetry.pryx.sh/v1/event";
...
const DEFAULT_DISCOVERY_ENDPOINT: &str = "https://api.pryx.sh/v1/discovery";
...
/// Its presence is equivalent to setting `PRYX_NO_TELEMETRY=1`, but persists
...
fn opt_out_marker_path() -> Option<std::path::PathBuf> {
    storage::pryx_dir().ok().map(|d| d.join("no_telemetry"))
}
...
pub fn opt_out_forced_by_env() -> bool {
    std::env::var("PRYX_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok()
}
...
pub fn content_sharing_enabled() -> bool {
    ...
    if std::env::var("PRYX_NO_TELEMETRY").is_ok() || std::env::var("DO_NOT_TRACK").is_ok() {
        return false;
    }
...
fn detect_project_profile() -> ProjectProfile {
    ...
    profile.repo_present = root.join(".git").exists() || is_pryx_repo_dir(root);
...
fn post_payload(payload: serde_json::Value, timeout: Duration) -> bool {
    ...
    let client = TELEMETRY_HTTP_CLIENT.get_or_init(|| {
        reqwest::blocking::Client::builder()
            .user_agent(pryx_provider_core::PRYX_USER_AGENT)
            .build()
...
pub fn record_install_if_first_run() {
    ...
    // fresh ~/.pryx (so a new telemetry_id) on every job, which would otherwise
...
pub fn record_auth_started(provider: &str, method: &str) {
    pryx_logging::auth_event("auth_started", provider, &[("method", method)]);
...
pub fn record_auth_cancelled(provider: &str, method: &str) {
    pryx_logging::auth_event("auth_cancelled", provider, &[("method", method)]);
...
pub fn record_auth_surface_blocked(provider: &str, method: &str) {
    pryx_logging::auth_event("auth_surface_blocked", provider, &[("method", method)]);
...
pub fn record_auth_success(provider: &str, method: &str) {
    pryx_logging::auth_event("auth_success", provider, &[("method", method)]);
    if !is_enabled() {
        return;
    }
    let Some(id) = get_or_create_id() else {
...
fn show_first_run_notice() {
    ...
    let (dim, reset) = if pryx_core::console::stderr_supports_ansi() {
        ("\x1b[90m", "\x1b[0m")
    } else {
        ("", "")
    };
    eprintln!("{dim}");
    eprintln!("  pryx collects anonymous usage statistics (install count, version, OS,");
    eprintln!("  session activity, tool counts, and crash/exit reasons). No code, filenames,");
    eprintln!("  prompts, or personal data is sent.");
    eprintln!("  To opt out: export PRYX_NO_TELEMETRY=1");
    eprintln!("  Details: https://github.com/pryx-co/pryx/blob/master/TELEMETRY.md");
    eprintln!("{reset}");
}
