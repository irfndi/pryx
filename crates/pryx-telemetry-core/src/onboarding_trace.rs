...
//! `pryx-tui::tui::app::onboarding_graph`. The interesting signal is the
...
use pryx_usage_types::{OnboardingTraceEvent, OnboardingTraceStep};
...
impl TraceRecorder {
    ...
    /// Exactly the bytes that would be sent, for `pryx telemetry show-last-trace`.
    ///
