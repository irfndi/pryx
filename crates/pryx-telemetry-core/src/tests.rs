...
// flip `PRYX_NO_TELEMETRY` / `DO_NOT_TRACK`, while the session tests drive the
...
impl Drop for TestEnvironment {
    fn drop(&mut self) {
        restore_env_var("PRYX_HOME", self.previous_home.take());
        restore_env_var("PRYX_NO_TELEMETRY", self.previous_no_telemetry.take());
...
fn global_test_lock() -> TestEnvironment {
    ...
    let previous_no_telemetry = std::env::var_os("PRYX_NO_TELEMETRY");
    let previous_do_not_track = std::env::var_os("DO_NOT_TRACK");
    pryx_core::env::set_var("PRYX_HOME", home.path());
    pryx_core::env::remove_var("PRYX_NO_TELEMETRY");
    pryx_core::env::remove_var("DO_NOT_TRACK");
...
fn telemetry_endpoint_uses_production_custom_domain() {
    assert_eq!(TELEMETRY_ENDPOINT, "https://telemetry.pryx.sh/v1/event");
}
...
fn test_opt_out_env_var() {
    ...
    pryx_core::env::set_var("PRYX_NO_TELEMETRY", "1");
    assert!(!is_enabled());
    pryx_core::env::remove_var("PRYX_NO_TELEMETRY");
}
...
fn test_do_not_track() {
    let _guard = lock_test_env();
    pryx_core::env::set_var("DO_NOT_TRACK", "1");
    assert!(!is_enabled());
    pryx_core::env::remove_var("DO_NOT_TRACK");
}
...
fn test_is_ci_detects_ci_env() {
    ...
    pryx_core::env::set_var("CI", "true");
    assert!(
        is_ci(),
        "CI env var should mark the run as CI (gates install skip)"
    );
    pryx_core::env::remove_var("CI");
    assert!(!is_ci());
    ...
    pryx_core::env::set_var("NEXTEST", "1");
    assert!(is_ci(), "nextest runs should be tagged as automation");
    pryx_core::env::remove_var("NEXTEST");
    assert!(!is_ci());
}
...
fn test_onboarding_step_milestone_key_includes_provider_and_method() {
    assert_eq!(
        onboarding_step_milestone_key("auth_success", Some("pryx"), Some("API key")),
        "auth_success_pryx_api_key"
    );
    assert_eq!(
...
fn test_install_marker_tracks_current_telemetry_id() {
    ...
    if let Some(prev_home) = prev_home {
        pryx_core::env::set_var("PRYX_HOME", prev_home);
    } else {
        pryx_core::env::remove_var("PRYX_HOME");
    }
}
...
fn test_begin_session_closes_superseded_session() {
    let _guard = lock_telemetry_test_state();
    pryx_core::env::set_var("PRYX_TELEMETRY_DISABLED", "1");
    ...
    pryx_core::env::remove_var("PRYX_TELEMETRY_DISABLED");
}
