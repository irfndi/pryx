...
//! consumes them. These live in `pryx-render-core` (a backend-neutral, pure
//! crate) rather than in `pryx-tui-markdown` so the foundation/streaming layer
//! can format reasoning lines without depending on any `pryx-tui-*` crate.
...
///
/// Lives here (a backend-neutral, pure crate) rather than in `pryx-tui-markdown`
/// so the foundation/streaming layer can format the summary without depending on
/// any `pryx-tui-*` crate. Re-exported from `pryx-tui-markdown` for the
/// existing `pryx_tui_markdown::reasoning_summary_line_markup` path.
