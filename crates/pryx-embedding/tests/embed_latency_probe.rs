...
//!
//! Run with:
//!   cargo test -p pryx-embedding --test embed_latency_probe -- --ignored --nocapture
//!
//! Requires the MiniLM model to be installed (~/.pryx/models/all-MiniLM-L6-v2).
...
fn model_dir() -> Option<PathBuf> {
    ...
    let dir = home
        .join(".pryx")
        .join("models")
        .join(pryx_embedding::MODEL_NAME);
...
fn embed_latency_probe() {
    ...
    let embedder = pryx_embedding::Embedder::load_from_dir(&dir).expect("load model");
