...
/// the `cli` subsystem.
pub const CLIENT_SELFDEV_ENV: &str = "PRYX_CLIENT_SELFDEV_MODE";
...
impl SelfDevBuildTarget {
    pub fn parse(value: Option<&str>) -> Result<Self> {
        match value.unwrap_or("auto").trim().to_ascii_lowercase().as_str() {
            "" | "auto" => Ok(Self::Auto),
            "tui" | "pryx" => Ok(Self::Tui),
            "desktop" | "desktop2" | "pryx-desktop2" => Ok(Self::Desktop2),
