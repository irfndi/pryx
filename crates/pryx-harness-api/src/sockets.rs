...
//! bridge and in `pryx-desktop2`, and the two copies disagreed: the bridge
//! resolved `$XDG_RUNTIME_DIR` while the desktop always looked in
//! `~/.pryx`. The result was a desktop app that could never connect even
//! with a healthy bridge running. One definition, used by both sides, makes
//! that class of bug impossible.
//!
//! The rules match `pryx-storage::runtime_dir` so the API socket always lands
...
/// Path of the versioned harness API socket. `PRYX_API_SOCKET` overrides it.
pub fn api_socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("PRYX_API_SOCKET") {
        return PathBuf::from(custom);
    }
    runtime_dir().join("pryx-api.sock")
}

/// Path of the internal daemon socket the bridge translates onto.
/// `PRYX_SOCKET` overrides it.
pub fn legacy_socket_path() -> PathBuf {
    if let Ok(custom) = std::env::var("PRYX_SOCKET") {
        return PathBuf::from(custom);
    }
    runtime_dir().join("pryx.sock")
}
...
mod tests {
    ...
    fn the_api_socket_sits_beside_the_daemon_socket() {
        ...
        let api = runtime_dir().join("pryx-api.sock");
        let legacy = runtime_dir().join("pryx.sock");
    ...
    fn socket_names_are_stable() {
        assert_eq!(
            runtime_dir().join("pryx-api.sock").file_name().unwrap(),
            "pryx-api.sock"
        );
    }
