...
//!
//! These drive the real `pryx_base::import` pipeline against synthetic
//! `.jsonl` fixtures written into a sandboxed `PRYX_HOME`, so they never touch
...
//! environment (`PRYX_HOME`).

use pryx_base::import::{
    import_session, import_session_from_file, list_claude_code_sessions,
    list_claude_code_sessions_lazy,
};
use pryx_base::message::ContentBlock;
use pryx_base::session::Session;
use pryx_import_core::imported_claude_code_session_id;
use std::path::{Path, PathBuf};
use std::sync::Mutex;

/// Serializes tests that mutate `PRYX_HOME`. This test binary has its own
...
impl Drop for EnvVarGuard {
    fn drop(&mut self) {
        if let Some(prev) = self.prev.take() {
            pryx_base::env::set_var(self.key, prev);
        } else {
...
/// Create the sandboxed Claude project dir under `$PRYX_HOME/external/...`.
...
fn import_array_tool_result_with_image_uses_placeholder_not_base64() {
    ...
    let saved = std::fs::read_to_string(pryx_session_file(temp.path(), "img-session")).unwrap();
    assert!(
        !saved.contains(&big),
...
/// Path to where pryx persists imported sessions under the sandbox.
/// With `PRYX_HOME` set, `storage::pryx_dir()` returns it directly, and
/// sessions live at `<PRYX_HOME>/sessions/<id>.json`.
fn pryx_session_file(home: &Path, claude_id: &str) -> PathBuf {
