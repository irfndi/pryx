...
//
//   cargo test -p pryx-base --test header_cost_probe -- --nocapture --ignored
//
...
fn measure_auth_status_probe_cost() {
    ...
    let _ = pryx_base::auth::AuthStatus::check_fast();
...
///
/// This lives in `pryx-base` (its own test binary) rather than beside the
/// header tests because `invalidate_cached_status()` clears a process-global
/// cache that sibling tests in the `pryx-tui` binary share.
#[test]
fn invalidate_cached_status_bumps_the_auth_generation() {
    let before = pryx_base::auth::auth_status_generation();
    pryx_base::auth::AuthStatus::invalidate_cached_status();
    let after = pryx_base::auth::auth_status_generation();
