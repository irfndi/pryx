...
///
/// Resolution order: `PRYX_SPAWN_HOOK` env (handled by config env overrides;
...
///
/// When a spawn hook is configured (`[terminal] spawn_hook` / `PRYX_SPAWN_HOOK`),
/// the hook takes over the spawn: pryx runs `<hook> <program> <args...>` with
/// `PRYX_SPAWN_*` metadata env vars so external programs (tmux, kitty remote,
/// herd, window managers) control where and how the session appears. If the
/// hook cannot be started, pryx falls back to its built-in terminal detection.
...
mod tests {
    ...
    fn spawn_via_hook_runs_hook_with_metadata_env() {
        ...
        std::fs::write(
            &hook_path,
            format!(
                "#!/bin/sh\nprintf '%s|%s|%s|%s' \"$PRYX_SPAWN_KIND\" \"$PRYX_SPAWN_SESSION_ID\" \"$PRYX_SPAWN_SWARM_ID\" \"$*\" > {}\n",
        ...
        .spawn_env("PRYX_SPAWN_SWARM_ID", "swarm-7");
        ...
        assert_eq!(
            recorded, "swarm-agent|ses_hooked|swarm-7|/usr/local/bin/pryx --resume ses_hooked",
            "hook should receive metadata env and the pryx command as argv"
        );
    }
