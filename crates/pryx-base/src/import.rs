...
/// Convert Claude Code content blocks to pryx ContentBlocks
...
fn truncate_import_text(text: String, max_bytes: usize) -> String {
    ...
    format!(
        "{}{}",
        pryx_core::util::truncate_str(&text, prefix_budget),
        suffix
    )
}
...
fn reuse_existing_imported_session(session_id: &str) -> bool {
    // An imported snapshot becomes a normal pryx continuation as soon as the
    ...
    // doing so can discard pryx-only turns and journal state. New imports are
...
pub fn imported_session_id_for_target(
    target: &pryx_session_types::ResumeTarget,
) -> Option<String> {
    match target {
        pryx_session_types::ResumeTarget::PryxSession { session_id } => Some(session_id.clone()),
        pryx_session_types::ResumeTarget::ClaudeCodeSession { session_id, .. } => {
            Some(imported_claude_code_session_id(session_id))
        }
        pryx_session_types::ResumeTarget::CodexSession { session_id, .. } => {
            Some(imported_codex_session_id(session_id))
        }
        pryx_session_types::ResumeTarget::PiSession { session_path } => {
            Some(imported_pi_session_id(session_path))
        }
        pryx_session_types::ResumeTarget::OpenCodeSession { session_id, .. } => {
            Some(imported_opencode_session_id(session_id))
        }
        pryx_session_types::ResumeTarget::CursorSession { session_id, .. } => {
            Some(imported_cursor_session_id(session_id))
        }
    }
}

pub fn resolve_resume_target_to_pryx(
    target: &pryx_session_types::ResumeTarget,
) -> Result<pryx_session_types::ResumeTarget> {
    use pryx_session_types::ResumeTarget;
    ...
    let session_id = match target {
        ResumeTarget::PryxSession { session_id } => {
            return Ok(ResumeTarget::PryxSession {
                session_id: session_id.clone(),
            });
        }
    ...
    Ok(ResumeTarget::PryxSession { session_id })
}
...
fn import_session_from_file_with_target(
    ...
    let mut session = Session::create_with_id(pryx_session_id, None, title);
...
/// Explicitly hand a currently-running Claude Code session over to Pryx.
///
/// This is deliberately separate from normal resume. It first imports the
/// current transcript into a fresh durable Pryx session, then gracefully stops
...
/// A stop failure rolls back the staged Pryx session and leaves ordinary
...
fn take_over_live_claude_session_with_timeout(
    ...
    // Use a normal memorable Pryx session ID so the handed-off conversation
    ...
    if stop_outcome == crate::claude_live::StopLiveClaudeOutcome::ExitUnconfirmed {
        crate::session_list_cache::invalidate();
        anyhow::bail!(
            "Claude Code process {} was asked to exit, but its exit was not confirmed; prepared Pryx session {} was preserved and can be resumed",
            live.pid,
            takeover_id
        );
    }
    ...
    if let Some(err) = final_refresh_error {
        ...
        return Err(err).with_context(|| {
            format!(
                "Claude Code exited, but its final transcript could not be refreshed; prepared Pryx session {takeover_id} was preserved"
            )
        });
    }

    crate::logging::info(&format!(
        "Claude takeover complete: source_session={session_id} source_pid={} pryx_session={takeover_id}",
        live.pid
    ));
    crate::session_list_cache::invalidate();
    Ok(ResumeTarget::PryxSession {
...
fn finalize_imported_session(
    ...
    // Never overwrite a pryx-side continuation with a shorter external
...
pub fn import_cursor_session_from_path(
    ...
    let created_at =
        pryx_import_core::file_modified_datetime(session_path).unwrap_or_else(Utc::now);
