...
//!
//! pryx's static pricing tables (`pryx_provider_core::pricing`) only cover
...
//! providers, including every OpenAI-compatible profile pryx ships.
//!
//! This module mirrors the OpenRouter catalog pattern:
//!   - a 24h disk cache under `~/.pryx/cache/models_dev_pricing.json`,
...
/// from, so changing `PRYX_HOME` (tests, multi-home setups) never serves
...
fn cache_path() -> PathBuf {
    crate::storage::pryx_dir()
        .unwrap_or_else(|_| PathBuf::from(".").join(".pryx"))
        .join("cache")
        .join(CACHE_FILE)
}
...
/// Translate a pryx provider key (runtime key, activity source key, or
/// compatible-profile id) to the models.dev provider id.
pub fn models_dev_provider_id(pryx_provider: &str) -> Option<&'static str> {
    let key = pryx_provider
        .trim()
        .strip_prefix("openai-compatible:")
        .unwrap_or_else(|| pryx_provider.trim());
    Some(match key {
...
/// Strip pryx-local suffixes/prefixes a model id may carry before catalog
/// lookup (`[1m]` long-context alias, `provider/` prefixes for OpenRouter ids).
fn normalize_model_id(model: &str) -> &str {
    pryx_provider_core::model_id::strip_long_context_suffix(model).trim()
}

/// Look up live pricing for `model` under a pryx provider key. Returns `None`
...
pub fn lookup(pryx_provider: &str, model: &str) -> Option<ModelCost> {
    let provider_id = models_dev_provider_id(pryx_provider)?;
...
pub fn schedule_refresh() {
    ...
    // PRYX_FORCE_PRICING_REFRESH=1 re-enables the fetch for manual e2e checks
    // (e.g. `cargo run --example pricing_e2e_check`, which builds with
    // test-support unified in).
    let forced = std::env::var_os("PRYX_FORCE_PRICING_REFRESH").is_some();
    if !forced
        && (cfg!(any(test, feature = "test-support"))
            || std::env::var_os("PRYX_DISABLE_PRICING_REFRESH").is_some())
    {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn provider_key_mapping_covers_pryx_providers() {
