//! Typed client for the Pryx account and hosted-model billing API.
//!
...
use crate::subscription_catalog::{self, PryxTier};
...
impl SubscriptionMe {
    pub fn parsed_tier(&self) -> Option<PryxTier> {
        PryxTier::parse(&self.tier)
    }
...
impl fmt::Display for AccountApiError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Unauthorized => write!(f, "the Pryx account key is revoked or expired"),
            Self::Forbidden => write!(f, "the Pryx account request was denied"),
            Self::LegacyBackend => write!(
                f,
                "the configured Pryx API uses the legacy email-based login contract; update the backend or use the current https://api.pryx.sh/v1 endpoint"
            ),
            Self::Http { status, code } => match code {
                Some(code) => write!(f, "Pryx account API returned HTTP {status} ({code})"),
                None => write!(f, "Pryx account API returned HTTP {status}"),
            },
            Self::InvalidResponse(detail) => {
                write!(
                    f,
                    "Pryx account API returned an invalid response: {detail}"
                )
            }
        }
    }
...
pub fn configured_api_base() -> String {
    subscription_catalog::configured_api_base()
        .unwrap_or_else(|| subscription_catalog::DEFAULT_PRYX_API_BASE.to_string())
        .trim_end_matches('/')
        .to_string()
}
...
pub async fn fetch_subscription_me() -> Result<SubscriptionMe> {
    let api_key = subscription_catalog::configured_api_key()
        .context("no Pryx account credential configured (run `pryx account login`)")?;
...
mod tests {
    ...
    fn subscription_me_parses_expected_shape() {
        let json = r#"{
            ...
            "manage_url": "https://pryx.sh/account"
        }"#;
        let me: SubscriptionMe = serde_json::from_str(json).expect("parse");
        assert_eq!(me.parsed_tier(), Some(PryxTier::Flagship));
        assert!(me.has_active_paid_plan());
        assert_eq!(me.manage_url.as_deref(), Some("https://pryx.sh/account"));
    }
    ...
    async fn device_request_uses_new_contract_and_parses_public_urls() {
        let base = spawn_server(vec![(
            200,
            vec![],
            r#"{"device_code":"secret","flow_id":"public-flow","verification_uri":"https://pryx.sh/account","verification_uri_complete":"https://pryx.sh/account?flow=public-flow","verify_url":"https://pryx.sh/account?flow=public-flow","expires_in":600,"interval":3}"#.to_string(),
        )]);
        let result = request_device_authorization(&client(), &base, Some(PryxTier::Pro))
            .await
            .expect("device auth");
    ...
    async fn metered_device_request_does_not_select_a_subscription_tier() {
        ...
        std::thread::spawn(move || {
            ...
            let body = r#"{"device_code":"secret","flow_id":"public-flow","verification_uri":"https://pryx.sh/account","verification_uri_complete":"https://pryx.sh/account?flow=public-flow","expires_in":600,"interval":3}"#;
            let response = format!(
                "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\nContent-Length: {}\r\nConnection: close\r\n\r\n{body}",
                body.len()
            );
        ...
        assert!(request.contains(r#"{"client_name":"pryx-cli"}"#));
    ...
    async fn legacy_device_response_is_explained_without_echoing_body() {
        ...
        let error = request_device_authorization(&client(), &base, Some(PryxTier::Pro))
            .await
