...
fn test_rewrite_command_with_full_path() {
    ...
    if browser_binary_path().exists() {
        ...
        assert!(result.contains(".pryx/browser"));
    } else {
...
fn test_paths() {
    ...
    assert!(bdir.to_string_lossy().contains(".pryx"));
...
async fn test_inspect_browser_status_without_binary() {
    // Hold the test-env lock: this reads PRYX_HOME-derived paths, and other
    // tests mutate PRYX_HOME (and write browser fixture files) under the
    // lock. Without it, the status snapshot and the exists() check below can
    // observe different PRYX_HOME values mid-test.
    let _guard = crate::storage::lock_test_env();
...
async fn test_ensure_browser_ready_noninteractive_without_binary() {
    ...
    // that mutate PRYX_HOME under the test-env lock.
    let _guard = crate::storage::lock_test_env();
...
//
// Bug B: once ~/.pryx/browser/.setup-complete existed, setup could never
