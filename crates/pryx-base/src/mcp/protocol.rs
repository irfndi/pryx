...
pub struct McpServerConfig {
    /// Command for stdio servers. Empty for HTTP/SSE servers, which pryx does
    ...
    /// URL for HTTP/SSE servers (Claude Code compat). Unused by pryx today.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub url: Option<String>,
    /// Headers for HTTP/SSE servers (Claude Code compat). Unused by pryx today,
...
impl McpServerConfig {
    /// pryx currently only supports stdio (command-based) MCP servers. A config
...
impl McpConfig {
    ...
    /// servers survive in pryx's snapshot and would duplicate inline secrets.
    /// This only runs while ~/.pryx/mcp.json does not exist.
    fn import_from_codex_once() {
        let pryx_mcp = match crate::storage::pryx_dir() {
        ...
        let environment_value_count = imported
            ...
            .sum();
        if let Err(e) = imported.save_to_file(&pryx_mcp) {
    ...
    fn live_claude_log_message(server_count: usize, source: &str) -> String {
        format!(
            "MCP: Loaded {} server(s) live from Claude Code ({}); source values were not copied into pryx config",
            server_count, source
        )
    }
    ...
    /// order: `.pryx/mcp.json`, then `.mcp.json` (Claude Code project config),
    ...
    fn load_project_locals(project_root: &std::path::Path) -> Self {
        ...
        for relative in [".pryx/mcp.json", ".mcp.json", ".claude/mcp.json"] {
    ...
    /// Load from default locations (merges pryx global + local, local overrides),
    ...
    /// (`.pryx/mcp.json`, `.mcp.json`, `.claude/mcp.json`, and the per-project
    ...
    pub fn load_for_dir(project_dir: Option<&std::path::Path>) -> Self {
        ...
        // Load pryx's own global config (~/.pryx/mcp.json)
        if let Ok(pryx_dir) = crate::storage::pryx_dir() {
            let pryx_mcp = pryx_dir.join("mcp.json");
            if pryx_mcp.exists() {
                if let Ok(config) = Self::load_from_file(&pryx_mcp) {
        ...
        // pryx only supports stdio servers today. Drop HTTP/SSE entries (common
    ...
    /// Merge `incoming` over `existing`, except that an entry pryx cannot run
    ...
    /// working stdio server from `~/.pryx/mcp.json` and then be dropped by the
