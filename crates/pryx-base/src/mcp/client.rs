...
impl McpClient {
    ...
    pub async fn connect_in_dir(
        ...
        // config. The long-lived pryx daemon contains provider credentials in
    ...
    async fn initialize(&mut self) -> Result<()> {
        let params = InitializeParams {
            ...
            client_info: ClientInfo {
                name: "pryx".to_string(),
                version: pryx_build_meta::pkg_version().to_string(),
            },
        };
...
/// Secrets that an MCP child must not receive merely because pryx has them.
///
...
mod tests {
    ...
    fn inherited_mcp_env_scrubs_provider_credentials() {
        ...
        for key in ["PATH", "HOME", "RUST_LOG", "PRYX_OPENROUTER_API_KEY_NAME"] {
...
"#;
    ...
    async fn connect_in_dir_missing_dir_falls_back_to_inherited_cwd() {
        let client = McpClient::connect_in_dir(
            ...
            &fake_server_config(),
            Some(std::path::Path::new("/nonexistent/pryx-557")),
        )
        .await
