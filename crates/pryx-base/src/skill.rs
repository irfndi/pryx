...
impl SkillRegistry {
    ...
    ///
    /// Holds GLOBAL skills only (plugins, `~/.pryx/skills/`,
    ...
    /// Only runs if ~/.pryx/skills/ doesn't exist yet.
    fn import_from_external() {
        let pryx_skills = match crate::storage::pryx_dir() {
            Ok(dir) => dir.join("skills"),
            Err(_) => return,
        };

        if pryx_skills.exists() {
        ...
        {
            let count = Self::copy_skills_dir(&claude_skills, &pryx_skills);
            if count > 0 {
                sources.push(format!("{} from Claude Code", count));
                copied.extend(Self::list_skill_names(&pryx_skills));
            }
        }
        ...
        if let Ok(codex_skills) = crate::storage::user_home_path(".codex/skills")
            && codex_skills.is_dir()
        {
            let count = Self::copy_skills_dir(&codex_skills, &pryx_skills);
            if count > 0 {
    ...
    pub fn load_global() -> Result<Self> {
        ...
        // first, so explicit pryx/agents skills with the same name win below.
        if let Some(plugins_root) = Self::claude_plugins_root() {
            registry.load_plugin_skills_from_root(&plugins_root);
        }

        // Load from ~/.pryx/skills/ (pryx's own global skills)
        if let Ok(pryx_dir) = crate::storage::pryx_dir() {
            let pryx_skills = pryx_dir.join("skills");
            if pryx_skills.exists() {
                registry.load_from_dir(&pryx_skills)?;
            }
        }
    ...
    /// `./.pryx/skills/`, `./.agents/skills/`, and `./.claude/skills/`.
    ///
    ...
    fn load_project_local_dirs(&mut self, working_dir: Option<&Path>) -> Result<()> {
        // Load from ./.pryx/skills/ (project-local pryx skills)
        let local_pryx = Self::project_local_dir(working_dir, ".pryx");
        if local_pryx.exists() {
            self.load_from_dir(&local_pryx)?;
        }
    ...
    /// plugin never prevents pryx's own skills from loading.
    ...
    /// Reload the shared global skill sources (plugins, `~/.pryx/skills/`,
    ...
    pub fn reload_global(&mut self) -> Result<usize> {
        ...
        if let Ok(pryx_dir) = crate::storage::pryx_dir() {
            let pryx_skills = pryx_dir.join("skills");
            if pryx_skills.exists() {
                count += self.load_from_dir_count(&pryx_skills)?;
            }
        }
...
/// A skill recommended/curated by pryx that the user may want to install.
...
pub struct EndorsedSkill {
    ...
    /// Grouping label used to organize the endorsed list (e.g. "pryx",
    /// "NVIDIA CUDA-X").
...
/// Curated list of skills endorsed by pryx. Used by the `/skills` command to
...
pub const ENDORSED_SKILLS: &[EndorsedSkill] = &[
    EndorsedSkill {
        ...
        category: "pryx",
        source: "bundled in pryx repo (.pryx/skills/optimization)",
        install: None,
    },
    EndorsedSkill {
        ...
        category: "pryx",
        source: "bundled with pryx / Claude Code skills",
        install: None,
    },
    EndorsedSkill {
...
/// Return the curated list of skills endorsed by pryx.
...
mod tests {
    ...
    #[test]
    fn load_for_working_dir_reads_project_local_pryx_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".pryx", "wd-only");
    ...
    fn project_overlay_is_session_scoped_and_composes_over_globals() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".pryx", "session-skill");
    ...
    fn registry_contains_reports_loaded_skills() {
        let temp = tempfile::tempdir().expect("tempdir");
        write_test_skill(temp.path(), ".pryx", "present-skill");
    ...
    #[test]
    fn explicit_pryx_skill_wins_over_plugin_skill_with_same_name() {
        let temp = tempfile::tempdir().expect("tempdir");
        ...
        // Explicit pryx skill with the same name.
        write_test_skill(temp.path(), ".pryx", "shared-name");
        ...
        registry.load_plugin_skills_from_root(&plugins_root);
        registry
            .load_from_dir(&temp.path().join(".pryx/skills"))
        ...
        assert_eq!(
            ...
            "explicit pryx skill must override the plugin-provided one"
        );
    }
