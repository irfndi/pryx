...
//! [`AuthStatus`] plus the `PRYX_RUNTIME_PROVIDER` override on its own, keyed on
...
impl From<pryx_provider_core::ResolvedCredential> for ActiveCredential {
    fn from(value: pryx_provider_core::ResolvedCredential) -> Self {
        match value {
            pryx_provider_core::ResolvedCredential::Oauth => Self::OAuth,
            pryx_provider_core::ResolvedCredential::ApiKey => Self::ApiKey,
        }
    }
}
...
pub struct ResolvedProviderAuth {
    ...
    /// True when the active credential was pinned via `PRYX_RUNTIME_PROVIDER`
...
///
/// `runtime_provider` is the raw `PRYX_RUNTIME_PROVIDER` value (if any); it
...
pub fn resolve_dual_credential_auth(
    ...
    let dual = pryx_provider_core::DualAuthProvider::from_active_provider(provider)?;
    ...
    let forced =
        pryx_provider_core::pinned_mode_for(dual, runtime_provider).map(|mode| match mode {
            pryx_provider_core::AuthMode::Oauth => ActiveCredential::OAuth,
            pryx_provider_core::AuthMode::ApiKey => ActiveCredential::ApiKey,
        });
