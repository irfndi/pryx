...
/// Cached auth status plus the `PRYX_HOME` it was computed under.
///
/// Auth probes read credential files relative to `PRYX_HOME`. Tests swap
/// `PRYX_HOME` to per-test temp dirs, and a status computed under one home
...
/// CLI tools don't get installed/uninstalled while pryx is running, so caching
...
///
/// Without this, pryx would open a browser that does not exist (or that opens
...
/// location no installed or self-dev pryx binary ever runs from.
///
...
/// multiple browser tabs. Set `PRYX_ALLOW_BROWSER_IN_TESTS=1` to opt out
...
pub fn running_in_test_harness() -> bool {
    ...
    *IN_TEST_HARNESS.get_or_init(|| {
        if env_truthy("PRYX_ALLOW_BROWSER_IN_TESTS") {
            return false;
        }
        std::env::current_exe()
            .ok()
...
fn log_auth_status_snapshot(event: &str, status: &AuthStatus) {
    crate::logging::auth_event(
        ...
        "all",
        &[
            ("pryx", auth_state_label(status.pryx)),
...
impl AuthStatus {
    ...
    /// per-test `PRYX_HOME` swaps would poison the shared cache.
    ...
    /// pryx believes credentials are available/expired/missing without leaking
    ...
    pub fn log_snapshot(&self, surface: &str) {
        crate::logging::event_info(
            ...
            vec![
                ...
                ("pryx", self.pryx.label().to_string()),
    ...
    pub fn state_for_key(&self, key: LoginProviderAuthStateKey) -> AuthState {
        match key {
            ...
            LoginProviderAuthStateKey::Pryx => self.pryx,
        ...
        let (
            ...
            crate::provider_catalog::LoginProviderTarget::Pryx => {
                let (source, detail) = summarize_sources(vec![
                    env_source(crate::subscription_catalog::PRYX_API_KEY_ENV),
                    config_source(
                        crate::subscription_catalog::PRYX_API_KEY_ENV,
                        crate::subscription_catalog::PRYX_ENV_FILE,
                        "~/.config/pryx/pryx-subscription.env",
                    ),
                ]);
                (
                    source,
            ...
            crate::provider_catalog::LoginProviderTarget::OpenAiApiKey => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source("OPENAI_API_KEY", "openai.env", "~/.config/pryx/openai.env"),
            ...
            crate::provider_catalog::LoginProviderTarget::ClaudeApiKey => {
                ...
                // config file (`~/.config/pryx/anthropic.env`), *not* an env
                // var and *not* `~/.pryx/auth.json` (which holds the separate
            ...
            crate::provider_catalog::LoginProviderTarget::Bedrock => {
                let (source, detail) = summarize_sources(vec![
                    ...
                    config_source(
                        ...
                        "~/.config/pryx/bedrock.env",
                    ),
                    env_source("AWS_PROFILE"),
                    env_source("PRYX_BEDROCK_PROFILE"),
            ...
            crate::provider_catalog::LoginProviderTarget::OpenAiCompatible(profile) => {
                    ...
                    crate::provider_catalog::active_named_provider_profile_credential_source()
                {
                    summarize_sources(vec![
                        env_source(&key_env),
                        config_source(&key_env, &env_file, format!("~/.config/pryx/{}", env_file)),
                    ...
                    summarize_sources(vec![
                        ...
                        config_source(
                            &resolved.api_key_env,
                            &resolved.env_file,
                            format!("~/.config/pryx/{}", resolved.env_file),
                        ),
...
fn build_auth_status_uncached(mode: AuthProbeMode) -> (AuthStatus, Vec<(&'static str, u128)>) {
    ...
    record_auth_probe_step(&mut timings, "pryx", || probe_pryx_status(&mut status));
...
/// dead in `/login`, the header, onboarding, and `pryx auth status`.
///
...
fn probe_pryx_status(status: &mut AuthStatus) {
    if crate::subscription_catalog::has_credentials() {
        status.pryx = AuthState::Available;
    }
}
...
fn assessment_for_key(
    ...
    match key {
        ...
        LoginProviderAuthStateKey::Pryx
...
fn anthropic_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
    ...
    {
        return Some((
            AuthCredentialSource::PryxManagedFile,
            "~/.pryx/auth.json".to_string(),
        ));
    }
...
fn openai_oauth_source(status: &AuthStatus) -> Option<(AuthCredentialSource, String)> {
        ...
        .is_empty()
    {
        return Some((
            AuthCredentialSource::PryxManagedFile,
            "~/.pryx/openai-auth.json".to_string(),
        ));
    }
...
fn cursor_source() -> Option<(AuthCredentialSource, String)> {
    ...
    if config_source("CURSOR_API_KEY", "cursor.env", "~/.config/pryx/cursor.env").is_some() {
        return config_source("CURSOR_API_KEY", "cursor.env", "~/.config/pryx/cursor.env");
    }
    None
}
