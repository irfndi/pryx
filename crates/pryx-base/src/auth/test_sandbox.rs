...
impl AuthTestSandbox {
    pub fn new() -> anyhow::Result<Self> {
        ...
        std::fs::create_dir_all(temp.path().join("config").join("pryx"))?;
    ...
    pub fn config_dir(&self) -> PathBuf {
        self.root().join("config").join("pryx")
    }
...
mod tests {
    ...
    #[test]
    fn sandbox_isolates_pryx_home_and_config_dir() {
        let sandbox = AuthTestSandbox::new().expect("sandbox");

        assert_eq!(
            std::env::var("PRYX_HOME").ok().as_deref(),
