...
#[test]
fn pryx_auth_file_default_is_empty() {
    let auth = PryxAuthFile::default();
    assert!(auth.anthropic_accounts.is_empty());
    assert!(auth.active_anthropic_account.is_none());
}

#[test]
fn pryx_auth_file_roundtrip() {
    let auth = PryxAuthFile {
    ...
    let parsed: PryxAuthFile = serde_json::from_str(&json).unwrap();
...
#[test]
fn pryx_path_respects_pryx_home() {
    let _lock = crate::storage::lock_test_env();
    let temp = tempfile::TempDir::new().unwrap();
    let _home = EnvVarGuard::set("PRYX_HOME", temp.path());

    assert_eq!(pryx_path().unwrap(), temp.path().join("auth.json"));
    assert_eq!(
...
#[test]
fn pryx_auth_file_multi_account() {
    let auth = PryxAuthFile {
...
#[test]
fn pryx_auth_file_legacy_migration_format() {
    let legacy_json = r#"{
        "anthropic": {
            "access": "legacy_acc",
            "refresh": "legacy_ref",
    ...
    let parsed: PryxAuthFile = serde_json::from_str(legacy_json).unwrap();
...
fn update_account_profile_sets_email() {
    let mut auth = PryxAuthFile::default();
...
///   cargo test -p pryx-base --lib auth::claude::tests::live_keychain -- --ignored --nocapture
#[cfg(target_os = "macos")]
#[test]
