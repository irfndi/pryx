...
//!
//! Historically pryx discovered environment limitations *by failing*: it would
...
pub struct EnvFacts {
    ...
    /// The pryx config directory exists (or can be created) and is writable.
    ...
    pub fn precondition_message(self) -> Option<&'static str> {
        match self {
            AuthMethodChoice::BlockedConfigUnwritable => Some(
                "pryx cannot write its config directory, so a login could not be saved. Fix directory permissions (or set PRYX_HOME) and try again.",
            ),
            AuthMethodChoice::ApiKeyNonInteractive => Some(
                "This terminal is not interactive. Provide an API key via the provider's environment variable or `pryx provider add --api-key-stdin`.",
            ),
            _ => None,
        }
    }
