...
#[test]
fn config_file_path_under_pryx() {
    let path = config_file_path().unwrap();
    let path_str = path.to_string_lossy();
    assert!(path_str.contains("pryx"));
...
fn save_and_load_api_key() {
    ...
    let file = dir.path().join("pryx").join("cursor.env");
...
#[test]
fn cursor_auth_file_path_respects_pryx_home() {
    // Regression: on Linux the auth.json path previously used
    // `dirs::config_dir()` directly, ignoring PRYX_HOME. That leaked the real
    ...
    // provider correctly looked under `$PRYX_HOME/external/...`.
    ...
    assert!(
        ...
        "cursor auth path should be under PRYX_HOME/external, got {}",
        path.display()
    );
...
#[test]
fn cursor_auth_file_path_does_not_escape_pryx_home_on_windows() {
    let _guard = crate::storage::lock_test_env();
    let temp = tempfile::tempdir().unwrap();
    let old_home = std::env::var_os("PRYX_HOME");
...
#[test]
fn cursor_vscdb_paths_respect_pryx_home() {
    let _guard = crate::storage::lock_test_env();
    let prev_home = std::env::var_os("PRYX_HOME");
    let temp = TempDir::new().unwrap();
    crate::env::set_var("PRYX_HOME", temp.path());
