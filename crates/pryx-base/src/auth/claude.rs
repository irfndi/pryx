...
/// Represents a named Anthropic OAuth account stored in pryx's auth.json.
...
fn relabel_accounts(auth: &mut PryxAuthFile) -> bool {
...
pub fn pryx_path() -> Result<PathBuf> {
    Ok(crate::storage::pryx_dir()?.join("auth.json"))
}

// ---- Multi-account helpers ----

/// Read the pryx auth file, auto-migrating from legacy format if needed.
pub fn load_auth_file() -> Result<PryxAuthFile> {
    let path = pryx_path()?;
    if !path.exists() {
        return Ok(PryxAuthFile::default());
    }

    crate::storage::harden_secret_file_permissions(&path);

    let mut auth: PryxAuthFile = crate::storage::read_json(&path)
        .with_context(|| format!("Could not read pryx credentials from {:?}", path))?;
...
/// Write the pryx auth file (multi-account format).
pub fn save_auth_file(auth: &PryxAuthFile) -> Result<()> {
    let auth_path = pryx_path()?;

    let clean = PryxAuthFile {
...
/// Falls through Claude Code -> pryx accounts -> OpenCode, preferring non-expired tokens.
pub fn load_credentials() -> Result<ClaudeCredentials> {
    ...
    // prompt, so it is read once at import time and copied into pryx's own
    ...
    if let Ok(creds) = load_pryx_credentials() {
        if creds.expires_at > now_ms {
            return Ok(creds);
        }
        expired_candidates.push(("pryx", creds));
    }
    ...
    anyhow::bail!("No Claude OAuth credentials found (checked Claude Code, pryx, OpenCode)")
}

/// Load credentials for a specific pryx account by label.
...
/// Load credentials from the active pryx account (multi-account aware).
fn load_pryx_credentials() -> Result<ClaudeCredentials> {
    let auth = load_auth_file()?;
    if auth.anthropic_accounts.is_empty() {
        anyhow::bail!("No anthropic accounts configured in pryx auth.json");
    }
    ...
    let account = auth
        ...
        .context("No anthropic accounts in pryx auth.json")?;
...
/// them into a pryx-managed Anthropic account so future loads and token
/// refreshes are served from pryx's own `auth.json`.
///
...
/// rotates into pryx's store on refresh, so the copy stays usable after the
