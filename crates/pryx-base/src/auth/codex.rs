...
const ALLOW_LEGACY_AUTH_ENV: &str = "PRYX_ALLOW_CODEX_LEGACY_AUTH";
...
fn relabel_accounts(auth: &mut PryxOpenAiAuthFile) -> bool {
...
fn pryx_auth_path() -> Result<PathBuf> {
    Ok(crate::storage::pryx_dir()?.join("openai-auth.json"))
}
...
pub fn save_auth_file(auth: &PryxOpenAiAuthFile) -> Result<()> {
    let auth_path = pryx_auth_path()?;
    let clean = PryxOpenAiAuthFile {
...
fn load_pryx_credentials() -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
    if auth.openai_accounts.is_empty() {
        anyhow::bail!("No OpenAI accounts configured in pryx auth file")
    }
    ...
    let account = auth
        ...
        .context("No OpenAI accounts in pryx auth file")?;
...
/// the raw tokens plus a `last_refresh` timestamp. Without an expiry, pryx
...
pub(crate) fn expires_at_from_access_token(access_token: &str) -> Option<i64> {
    ...
    // `exp` is in seconds since epoch; the rest of pryx tracks expiry in millis.
