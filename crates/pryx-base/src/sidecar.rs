...
const CLAUDE_CODE_PRYX_NOTICE: &str = "You are pryx, powered by Claude Code. You are a third-party CLI, not the official Claude Code CLI.";
...
impl Sidecar {
    ...
    /// 3. The live agent provider (works for EVERY provider pryx supports:
    ...
    ///
    /// This is the universal path: it works for every provider pryx supports,
...
///
/// The "You are Claude Code" identity spoof and pryx notice are only valid
...
fn anthropic_sidecar_prefers_api_key() -> bool {
    match pryx_provider_core::runtime_env_pinned_mode(
        pryx_provider_core::DualAuthProvider::Anthropic,
    ) {
        Some(pryx_provider_core::AuthMode::ApiKey) => true,
        Some(pryx_provider_core::AuthMode::Oauth) => false,
...
mod tests {
    ...
    fn test_backend_selection_prefers_openai() {
        ...
        let temp = tempfile::TempDir::new().expect("create temp pryx home");
    ...
    impl crate::provider::Provider for StubProvider {
        async fn complete(
            ...
            let stream = futures::stream::once(async move {
                Ok(pryx_message_types::StreamEvent::TextDelta(reply))
            });
            Ok(Box::pin(stream))
        }
    ...
    /// Every provider pryx supports should drive the sidecar end-to-end via the
    ...
    fn test_anthropic_sidecar_prefers_api_key_respects_pinned_mode() {
        ...
        let _g =
            EnvVarGuard::set_path("PRYX_RUNTIME_PROVIDER", std::path::Path::new("claude-api"));
        assert!(
        ...
        let _g2 = EnvVarGuard::set_path("PRYX_RUNTIME_PROVIDER", std::path::Path::new("claude"));
        assert!(
            !anthropic_sidecar_prefers_api_key(),
