//! Shared control logic behind the `/remote` command and `pryx pair`.
//!
...
pub enum RemoteCommand {
    /// Activate or manage the subscription-backed Pryx Cloud host.
    Cloud,
        ...
        if self.enabled && self.host_unknown {
            out.push_str(
                "\nCould not determine a reachable address for this machine. \
                 Set `PRYX_GATEWAY_HOST` to a hostname or IP the other machine can reach.\n",
            );
        }
...
mod tests {
    use super::*;
    use pryx_gateway_types::PairedDevice;
    ...
    /// These tests mutate the process-global `PRYX_HOME`, so they must
    ...
    /// `PRYX_HOME` out from under a provider test mid-assertion, failing it
