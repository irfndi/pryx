...
pub use pryx_batch_types::{BatchProgress, BatchSubcallProgress, BatchSubcallState};
...
pub struct ModelRefreshCompleted {
    pub session_id: String,
    pub result: std::result::Result<pryx_provider_core::ModelCatalogRefreshSummary, String>,
}
...
/// background tasks, without re-coupling `pryx-base` to swarm internals.
#[derive(Clone, Debug)]
...
pub enum BusEvent {
    ...
    UsageReport(Vec<pryx_usage_types::ProviderUsage>),
    /// Progressive usage report update while providers are still loading
    UsageReportProgress(pryx_usage_types::ProviderUsageProgress),
