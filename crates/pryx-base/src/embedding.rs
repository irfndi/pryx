//! Embedding facade for pryx.
//!
//! The heavy ONNX/tokenizer implementation lives in the `pryx-embedding`
//! workspace crate so unchanged embedding code can stay cached across self-dev
//! builds. This module keeps pryx's process-wide cache, stats, and local path /
//! logging integration stable.

use anyhow::Result;
use pryx_embedding as backend;
use serde::Serialize;
...
pub fn models_dir() -> Result<PathBuf> {
    let dir = pryx_dir()?.join("models").join(backend::MODEL_NAME);
