...
impl Config {
    ...
    pub fn default_config_file_contents() -> String {
        let default_content = r##"# pryx configuration file
# Location: ~/.pryx/config.toml
#
# Environment variables override these settings.
# Run `/config` in pryx to see current settings.
...
# Spawn a fresh pryx session in a new terminal window, reusing the current
# session's working directory. Companion to the system-wide launch hotkeys.
# `pryx setup-hotkey` installs these three global launch hotkeys on macOS,
...
#   Cmd+;        new pryx in your home directory
#   Cmd+'        new pryx in your last project directory
#   Cmd+Shift+'  new pryx self-dev session (last pryx repo)
...
# How to apply the transcript inside pryx: insert|append|replace|send
mode = "send"
...
# Set false here or set PRYX_NO_EMOJI=1 for ASCII fallbacks.
emoji = true
...
# (default: true). Set false to list only pryx's own sessions.
...
# bing_api_key_env = "PRYX_BING_API_KEY"
# bing_api_key = ""
...
# this. Configure here or via the PRYX_SEARXNG_URL environment variable, then
...
# extended/full additionally emit ignorable _pryx/* extension notifications.
profile = "standard"
# Tool profile requested when `pryx acp` starts the daemon itself.
...
#   claude        = Claude via OAuth/subscription (token in ~/.pryx/auth.json)
#   anthropic-api = Claude via direct Anthropic API key (ANTHROPIC_API_KEY env
#                   or ~/.config/pryx/anthropic.env). API-key mode does NOT fall
...
# to match this value. Also overridable per-launch via PRYX_STREAM_IDLE_TIMEOUT_SECS.
...
# Env override: PRYX_SWARM_SPAWN_MODE
...
# Env override: PRYX_SWARM_MAX_CONCURRENT_AGENTS
...
# Env override: PRYX_SWARM_STRIP_LAYOUT
...
# Env override: PRYX_MEMORY_MODEL
...
# dormant instead of degrading to the no-LLM path. Env: PRYX_MEMORY_SIDECAR_ENABLED
...
# Env override: PRYX_MEMORY_EMBEDDING_BACKEND
...
# Set PRYX_TERMINAL to force a supported terminal emulator instead.
...
#
# When set, pryx runs `<spawn_hook> <pryx-binary> <args...>` instead of
# opening a terminal emulator itself. The hook receives PRYX_SPAWN_* env vars
# describing the spawn so multiplexers/wrappers can decide where it appears:
#   PRYX_SPAWN_KIND        - "swarm-agent", "resume", "selfdev", "restart", ...
#   PRYX_SPAWN_SESSION_ID  - session the window will run
#   PRYX_SPAWN_TITLE       - suggested window/tab title
#   PRYX_SPAWN_CWD         - session working directory (also the hook's cwd)
#   PRYX_SPAWN_PROGRAM     - pryx binary path
#   PRYX_SPAWN_COMMAND     - full shell-escaped command line
#   PRYX_SPAWN_SWARM_ID / PRYX_SPAWN_COORDINATOR_SESSION_ID (swarm spawns)
# If the hook fails to start, pryx falls back to built-in terminal detection.
# Env override: PRYX_SPAWN_HOOK (set empty to disable a config hook).
#
# Examples:
#   spawn_hook = "tmux new-window"                # tmux window per agent
#   spawn_hook = "kitty @ launch --type=tab --"   # kitty tab per agent
#   spawn_hook = "~/bin/pryx-spawn-router"       # custom placement script
# spawn_hook = ""
#
# External command used to focus/raise an existing session window, replacing
# the built-in wmctrl/xdotool title search. Receives PRYX_FOCUS_SESSION_ID
# and PRYX_FOCUS_TITLE env vars. Pair with spawn_hook so the program that
# placed the window also brings it to the front.
# Env override: PRYX_FOCUS_HOOK (set empty to disable a config hook).
#
# Example:
#   focus_hook = "~/bin/pryx-focus-router"
# focus_hook = ""
#
# macOS only: terminal that the Cmd+; launch hotkey and in-app session spawns
# open pryx into. One of: ghostty, iterm2, wezterm, warp, alacritty, vscode,
# terminal (Apple Terminal). Preferred over the legacy
# ~/.pryx/preferred_terminal.json file. After changing this, re-run
# `pryx setup-hotkey` so the generated launcher script (Cmd+;) picks it up.
# preferred = "ghostty"
...
[hooks]
# Lifecycle hooks: external commands pryx runs at well-defined points so other
# programs can observe or gate agent behavior. Commands are parsed shell-style
# (quotes work) but executed directly, with PRYX_HOOK_* env vars describing
# the event:
#   PRYX_HOOK_EVENT       - "turn_start", "turn_end", "session_start",
#                            "session_end", "pre_tool", "post_tool"
#   PRYX_HOOK_SESSION_ID  - the session the event belongs to
#   PRYX_HOOK_CWD         - session working directory (also the hook's cwd)
#   PRYX_HOOK_PAYLOAD     - JSON mirror of all fields
# Hook processes get PRYX_HOOKS_DISABLED=1 so nested pryx calls don't recurse.
#
# All hooks except pre_tool are observers: detached, fire-and-forget, failures
# only logged. Env overrides: PRYX_HOOK_TURN_START, PRYX_HOOK_TURN_END,
# PRYX_HOOK_SESSION_START, PRYX_HOOK_SESSION_END, PRYX_HOOK_PRE_TOOL,
# PRYX_HOOK_POST_TOOL (set empty to disable a config hook).
#
...
# think/stream window before any tool call. Extra fields: PRYX_HOOK_MODEL,
# PRYX_HOOK_SOURCE ("chat"/"resume"/"ambient").
# turn_start = "~/bin/pryx-turn-start"
#
# Runs when an agent turn completes. Extra fields: PRYX_HOOK_STATUS
# ("ok"/"error"), PRYX_HOOK_DURATION_MS, PRYX_HOOK_MODEL,
# PRYX_HOOK_LAST_ASSISTANT_TEXT (first 4000 chars), PRYX_HOOK_ERROR.
# turn_end = "~/bin/pryx-turn-notify"
#
# Runs when a session becomes active. Extra: PRYX_HOOK_SOURCE
# ("create"/"attach"/"resume").
# session_start = ""
#
# Runs when a session closes normally. Extra: PRYX_HOOK_SOURCE ("close").
# session_end = ""
#
# Gate hook before every tool call. Receives PRYX_HOOK_TOOL_NAME and the tool
# input JSON on stdin (truncated copy in PRYX_HOOK_TOOL_INPUT). Exit 0 allows
...
# pre_tool = "~/bin/pryx-tool-policy"
#
# Max milliseconds to wait for pre_tool before failing open (default: 5000).
# pre_tool_timeout_ms = 5000
#
# Runs after each tool call. Extra fields: PRYX_HOOK_TOOL_NAME,
# PRYX_HOOK_STATUS, PRYX_HOOK_DURATION_MS, PRYX_HOOK_OUTPUT_BYTES,
# PRYX_HOOK_ERROR.
# post_tool = ""
...
[power]
# Prevent automatic system sleep while any pryx session is actively working.
...
# Set PRYX_DISABLE_POWER_INHIBIT=1 to force-disable regardless of this setting.
...
# ntfy_topic = "pryx-ambient-your-secret-topic"
...
# email_from = "pryx@example.com"
# email_smtp_host = "smtp.gmail.com"
# email_smtp_port = 587
# Password via env: PRYX_SMTP_PASSWORD (preferred) or config below
# email_password = ""
...
# telegram_bot_token = ""  # From @BotFather (prefer PRYX_TELEGRAM_BOT_TOKEN env var)
...
# discord_bot_token = ""     # From Discord Developer Portal (prefer PRYX_DISCORD_BOT_TOKEN env var)
...
# PRYX_JADE_RELAY_API_BASE, PRYX_JADE_RELAY_TOKEN, PRYX_JADE_RELAY_TOKEN_ID,
# PRYX_JADE_RELAY_USER_ID, PRYX_JADE_RELAY_SESSION_ID.
...
# developer tools from Pryx's hosted integration directory. These providers
# have integrated with the agent to make setup and use seamless. Some providers
# may share revenue with Pryx when a referred user becomes a customer, but
# commercial relationships never influence recommendations.
# See https://pryx.sh/discovery-tools
# enabled = true
# endpoint = "https://api.pryx.sh/v1/discovery"
	"##;

        // Substitute platform-specific defaults from the keybinding registry.
        let p = pryx_config_types::KeybindingPlatform::current();
        let effort_increase =
            pryx_config_types::default_binding("effort_increase", p).unwrap_or("alt+right");
        let effort_decrease =
            pryx_config_types::default_binding("effort_decrease", p).unwrap_or("alt+left");
        default_content
...
mod tests {
    ...
    /// a config file that pryx itself cannot read. Parse it here.
    #[test]
    ...
    fn documented_color_example_is_valid_when_uncommented() {
        ...
        for (role, value) in &parsed.display.colors {
            assert!(
                pryx_config_types::DisplayConfig::default()
                    .colors
                    .is_empty(),
