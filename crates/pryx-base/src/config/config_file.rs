use super::*;
use crate::storage::pryx_dir;
use std::path::PathBuf;

impl Config {
    /// Get the config file path
    pub fn path() -> Option<PathBuf> {
        pryx_dir().ok().map(|d| d.join("config.toml"))
    }
    ...
    ///
    /// Scans `~/.pryx/sessions` for the directories the user works in most,
    ...
    pub fn bake_launch_hotkeys_once() -> bool {
        use pryx_import_core::repo_ranking;

        let cfg = Self::load();
        if cfg.launch_hotkeys.imported {
            return false;
        }
        let Ok(pryx_dir) = pryx_dir() else {
            return false;
        };
        let sessions_dir = pryx_dir.join("sessions");
        ...
        let entries: Vec<pryx_config_types::LaunchHotkeyEntry> = plan
            .into_iter()
            .map(|p| pryx_config_types::LaunchHotkeyEntry {
                chord: p.chord,
    ...
    pub fn external_auth_source_allowed_for_path_cached(
        ...
        // a new path-bound trust decision, or before tests switch PRYX_HOME. Fall
