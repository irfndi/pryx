...
//! doesn't expose a usage API. Data persists to ~/.pryx/copilot_usage.json.
...
pub use pryx_usage_types::{AllTimeUsage, CopilotUsageTracker, DayUsage, MonthUsage};
...
mod tests {
    ...
    #[test]
    fn usage_path_respects_pryx_home() {
    ...
    #[test]
    fn save_and_load_roundtrip_under_pryx_home() {
        let _env_lock = lock_env();
        clear_tracker();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("PRYX_HOME", temp.path().as_os_str());
