use anyhow::Result;
use pryx_provider_core::{ActiveProvider, provider_key};
...
impl ProviderActivation {
    ...
    pub fn azure_openai(model: Option<String>) -> Self {
        ...
        if let Some(model) = model.filter(|value| !value.trim().is_empty()) {
            activation.with_model_hint("PRYX_OPENROUTER_MODEL", model)
        } else {
            activation
        }
    }
    ...
    pub fn pryx_subscription(model: impl Into<String>) -> Self {
        Self::initial(RuntimeProviderId::Pryx, ActiveProvider::OpenRouter)
            .with_model_hint("PRYX_OPENROUTER_MODEL", model)
    }

    pub fn apply_env(&self) -> Result<()> {
        crate::env::set_var("PRYX_RUNTIME_PROVIDER", self.runtime_id.key());
        match self.runtime_id {
            RuntimeProviderId::Pryx => {
            ...
            RuntimeProviderId::OpenAiCompatible => {
                if std::env::var_os("PRYX_OPENROUTER_TRANSPORT_STATE").is_none() {
                    crate::env::set_var("PRYX_OPENROUTER_TRANSPORT_STATE", "direct-api-key");
                }
            }
            _ => {
        ...
        match self.selection {
            RuntimeSelection::Initial(active_provider) => {
                ...
                crate::env::set_var("PRYX_ACTIVE_PROVIDER", active_key_for_log);
                crate::env::set_var("PRYX_INITIAL_PROVIDER_EXPLICIT", "1");
            }
...
pub fn select_initial_runtime_provider_key(provider_key_raw: &str) {
    crate::env::set_var("PRYX_ACTIVE_PROVIDER", provider_key_raw);
    crate::env::set_var("PRYX_INITIAL_PROVIDER_EXPLICIT", "1");
    crate::logging::auth_event(
...
mod tests {
    ...
    fn azure_activation_preserves_identity_while_using_openrouter_slot() {
        ...
        // (e.g. anthropic_tests sets PRYX_RUNTIME_PROVIDER=claude); without
        ...
        assert_eq!(
            std::env::var("PRYX_OPENROUTER_MODEL").as_deref(),
            Ok("gpt-4.1-mini")
        );
    }
}
