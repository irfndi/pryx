...
impl PryxProvider {
    fn runtime_model_spec(model: &str) -> String {
        ...
        // as `pryx-subscription`, for every curated model.
    ...
    fn apply_runtime_profile() {
        let _ = crate::provider::activation::ProviderActivation::pryx_subscription(
    ...
    fn entitled_models_for(
        _tier: crate::subscription_catalog::PryxTier,
    ...
    fn model_routes_for(tier: crate::subscription_catalog::PryxTier) -> Vec<ModelRoute> {
...
#[async_trait]
impl Provider for PryxProvider {
    async fn complete(
        &self,
        messages: &[Message],
    ...
    fn name(&self) -> &str {
        crate::subscription_catalog::PRYX_PROVIDER_DISPLAY_NAME
    }
    ...
    fn uses_pryx_compaction(&self) -> bool {
        self.inner.uses_pryx_compaction()
    }
...
mod tests {
    ...
    #[test]
    fn pryx_provider_enables_subscription_runtime_mode() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");

        runtime.block_on(async {
            let provider = PryxProvider::new();
    ...
    #[test]
    fn pryx_provider_name_and_default_model_are_curated() {
        let _guard = crate::storage::lock_test_env();
        crate::subscription_catalog::clear_runtime_env();
        let runtime = tokio::runtime::Runtime::new().expect("tokio runtime");
        ...
        runtime.block_on(async {
            let provider = PryxProvider::new();
            assert_eq!(provider.name(), "Pryx Subscription");
    ...
    #[test]
    fn pryx_models_are_pinned_to_the_managed_transport() {
        assert_eq!(
            PryxProvider::runtime_model_spec("claude-opus-4-8"),
            "openrouter:claude-opus-4-8"
        );
        assert_eq!(
            PryxProvider::runtime_model_spec("gpt-5.5"),
            "openrouter:gpt-5.5"
        );
        assert_eq!(
            PryxProvider::runtime_model_spec("minimax-m2.5"),
            "openrouter:minimax-m2.5"
        );
    }

    #[test]
    fn pryx_provider_exposes_only_explicit_subscription_routes() {
        use crate::subscription_catalog::PryxTier;

        let plus_routes = PryxProvider::model_routes_for(PryxTier::Plus);
        let gpt_route = plus_routes
            .iter()
            .find(|route| route.model == "gpt-5.5")
            .expect("Plus tier includes GPT-5.5");
        let route_selection = pryx_provider_core::RouteSelection::from_model_route(gpt_route);
        let flagship_routes = PryxProvider::model_routes_for(PryxTier::Flagship);
        ...
        assert!(plus_routes.iter().all(|route| {
            route.provider == "Pryx Subscription"
                && route.api_method == "pryx-subscription"
                && route.available
        }));
        assert_eq!(
            PryxProvider::entitled_models_for(PryxTier::Plus)
                .map(|model| model.id.to_string())
                .collect::<Vec<_>>(),
            expected_models
                .iter()
        ...
        assert_eq!(
            ...
            pryx_provider_core::RuntimeKey::PryxSubscription
        );
        assert_eq!(route_selection.api_method, "pryx-subscription");
        assert_eq!(route_selection.provider_label, "Pryx Subscription");
