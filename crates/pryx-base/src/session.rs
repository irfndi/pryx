...
///
/// Wraps the on-disk streaming marker from `pryx-storage` (cleared on every
...
fn default_is_test_session() -> bool {
    env_flag_enabled("PRYX_TEST_SESSION")
}
...
pub fn derive_session_provider_key(provider_name: &str) -> Option<String> {
    ...
    if normalized_name == "pryx" {
        return Some("pryx".to_string());
    }
    ...
    if let Ok(active) = std::env::var("PRYX_ACTIVE_PROVIDER") {
...
request in this new forked session, using the inherited conversation only as context.\n</system-reminder>",
    ...
    /// Check if this session is working on the pryx repository
    pub fn is_self_dev(&self) -> bool {
        if let Some(ref dir) = self.working_dir {
            // Check if working dir contains pryx source
            let path = std::path::Path::new(dir);
            path.join("Cargo.toml").exists()
                && path.join("src/main.rs").exists()
                && std::fs::read_to_string(path.join("Cargo.toml"))
                    .map(|s| s.contains("name = \"pryx\""))
                    .unwrap_or(false)
        } else {
            false
        }
    ...
    pub fn strip_oversized_images(&mut self, target_total_chars: usize) -> usize {
        ...
        let stripped = pryx_compaction_core::strip_large_images_in_contents(
            &mut contents,
            target_total_chars,
        );
        if stripped > 0 {
