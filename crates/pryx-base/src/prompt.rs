...
/// Default system prompt for pryx (embedded at compile time)
...
/// [`DEFAULT_SYSTEM_PROMPT`]. Precedence: project `./.pryx/system-prompt.md`,
/// then global `~/.pryx/system-prompt.md`, then the built-in default.
///
/// This is a *replacement* hook. To merely add guidance on top of the default,
/// use `.pryx/prompt-overlay.md` instead.
pub fn load_base_system_prompt(working_dir: Option<&Path>) -> String {
    let project_dir = working_dir.unwrap_or(Path::new("."));
    let candidates = [
        Some(project_dir.join(".pryx").join("system-prompt.md")),
...
/// creating `~/.pryx/swarm-prompt.md` (global) or `./.pryx/swarm-prompt.md`
...
/// project `./.pryx/swarm-prompt.md`, then global `~/.pryx/swarm-prompt.md`,
/// then the built-in [`DEFAULT_SWARM_PROMPT`].
pub fn load_swarm_prompt(working_dir: Option<&Path>) -> String {
    let project_dir = working_dir.unwrap_or(Path::new("."));
    let candidates = [
        Some(project_dir.join(".pryx").join("swarm-prompt.md")),
        crate::storage::pryx_dir()
            .ok()
...
/// `mission` module in the upper `pryx-app-core` layer; the asset lives here
...
pub fn build_system_prompt_full_with_capabilities(
    ...
    // Add optional prompt overlays from ~/.pryx/ and ./.pryx/
    ...
    // Add optional preferred-tool guidance from ~/.pryx/ and ./.pryx/
...
impl SelfDevProductContext {
    fn from_working_dir(working_dir: Option<&Path>) -> Self {
        ...
        if path.contains("/crates/pryx-desktop") || path.ends_with("crates/pryx-desktop2") {
            Self::Desktop2
        } else {
            Self::Tui
        }
...
/// Load optional prompt overlay markdown from ~/.pryx/ and ./.pryx/
fn load_prompt_overlay_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".pryx").join("prompt-overlay.md"),
        "Project Prompt Overlay (.pryx/prompt-overlay.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }

    if let Ok(global_overlay) = crate::storage::pryx_dir().map(|dir| dir.join("prompt-overlay.md"))
        && let Some((content, size)) = load_file(
            &global_overlay,
            "Global Prompt Overlay (~/.pryx/prompt-overlay.md)",
        )
    {
        total_chars += size;
        contents.push(content);
...
/// Load optional preferred-tool guidance from ~/.pryx/ and ./.pryx/
fn load_preferred_tools_files_from_dir(working_dir: Option<&Path>) -> (Option<String>, usize) {
    ...
    let project_dir = working_dir.unwrap_or(Path::new("."));
    if let Some((content, size)) = load_file(
        &project_dir.join(".pryx").join("preferred-tools.md"),
        "Project Preferred Tools (.pryx/preferred-tools.md)",
    ) {
        total_chars += size;
        contents.push(content);
    }
    ...
    if let Ok(global_preferred_tools) =
        crate::storage::pryx_dir().map(|dir| dir.join("preferred-tools.md"))
        && let Some((content, size)) = load_file(
            &global_preferred_tools,
            "Global Preferred Tools (~/.pryx/preferred-tools.md)",
        )
    {
        total_chars += size;
        contents.push(content);
