...
/// counts, and any conflicts with pryx's configured bindings.
pub fn render_report(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> String {
    ...
    if term_count == 0 && sys_count == 0 && app_count == 0 {
        out.push_str(
            "\nNo machine bindings were discovered. pryx can read Ghostty bindings, macOS\n\
    ...
    if conflicts.is_empty() {
        out.push_str("No conflicts found between your pryx keybindings and the machine.\n");
    } else {
        out.push_str(&format!(
        ...
        out.push_str(
            "These keys may be captured by your terminal, macOS, or another app (window\n\
             manager, launcher) before pryx sees them.\n\
             To fix: rebind the pryx action in ~/.pryx/config.toml under [keybindings],\n\
...
fn render_conflict_block(c: &Conflict) -> String {
    ...
    format!(
        "  ⚠ {key}\n      pryx: {action} ({field} = \"{raw}\")\n      taken by {interceptor}\n",
        key = c.pryx.chord.display(),
        action = c.pryx.action,
        field = c.pryx.field,
        raw = c.pryx.raw,
