//! Detect conflicts between pryx's own key bindings and the bindings
//! discovered on the machine (terminal emulator + macOS system shortcuts).
//!
//! The flow is:
//!   1. enumerate pryx's configured bindings as `(field, label, KeyChord)`
//!      from [`pryx_config_types::KeybindingsConfig`] ([`pryx_bindings`]),
...
/// One configured pryx binding, tied back to its config field.
#[derive(Debug, Clone)]
pub struct PryxBinding {
...
/// A detected conflict between a pryx binding and something on the machine.
#[derive(Debug, Clone)]
pub struct Conflict {
    /// The pryx binding that may not reach the app.
    pub pryx: PryxBinding,
...
/// Enumerate pryx's configured bindings as comparable chords. Bindings that are
...
/// chord.
pub fn pryx_bindings(cfg: &KeybindingsConfig) -> Vec<PryxBinding> {
...
/// Find conflicts between pryx's configured bindings and the discovered
/// machine bindings in `snapshot`.
///
/// Conflicts are deduplicated per `(pryx field, interceptor chord, source)` so
...
pub fn detect_conflicts(cfg: &KeybindingsConfig, snapshot: &KeymapSnapshot) -> Vec<Conflict> {
    ...
    for pryx in pryx_bindings(cfg) {
        let Some(interceptors) = by_chord.get(&pryx.chord) else {
            continue;
        };
...
mod tests {
    ...
    fn detects_ghostty_ctrl_tab_conflict() {
        ...
        let fields: Vec<&str> = conflicts.iter().map(|c| c.pryx.field.as_str()).collect();
    ...
    fn detects_window_manager_prompt_jump_conflict() {
        ...
        // focus, shadowing pryx's built-in prompt navigation fallback.
        let cfg = KeybindingsConfig::default();
        let snapshot = snapshot_with(vec![
        ...
        assert!(
            conflicts
                .iter()
                .all(|c| c.pryx.field.contains("built-in fallback")),
    ...
    fn no_conflict_when_chords_differ() {
        ...
        // pryx has no cmd+t binding by default.
    ...
    fn disabled_binding_is_not_reported() {
        ...
        assert!(
            !conflicts
                .iter()
                .any(|c| c.pryx.field == "keybindings.model_switch_next")
        );
    }
