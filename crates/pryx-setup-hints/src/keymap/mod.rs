...
//! (macOS system shortcuts + terminal emulator bindings) so pryx can detect
//! when one of them intercepts a key pryx wants to use.
//!
...
//!
//! Conflict detection against pryx's own bindings is built on top of this in a
//! later layer.
...
pub use conflicts::{Conflict, PryxBinding, conflict_signature, detect_conflicts, pryx_bindings};
...
pub fn snapshot_path() -> anyhow::Result<std::path::PathBuf> {
    Ok(pryx_storage::pryx_dir()?.join("keymap-snapshot.json"))
}

/// Collect a fresh snapshot and persist it to `~/.pryx/keymap-snapshot.json`.
...
pub fn refresh_and_save() -> KeymapSnapshot {
    ...
    if let Ok(path) = snapshot_path()
        ...
        && let Err(err) = pryx_storage::write_json_fast(&path, &snapshot)
    {
        pryx_logging::warn(&format!("keymap snapshot write failed: {err}"));
    }
    snapshot
}
