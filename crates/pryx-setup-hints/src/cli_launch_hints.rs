//! Native SessionStart integrations for reminding users about Pryx's global
...
//! intercepting shell commands. The hook invokes a hidden, fast Pryx command;
...
pub(super) fn maybe_notify(source: &str) -> Result<()> {
    ...
    *shown = shown.saturating_add(1);
    ...
    let body = format!(
        "{} is open. Press {} anytime to launch Pryx.",
        source.label(),
        display
    );
    send_desktop_notification("Pryx shortcut", &body);
    Ok(())
}
...
fn install_hook(path: &Path, source: CliSource) -> Result<()> {
    ...
    bytes.push(b'\n');
    pryx_storage::write_bytes(path, &bytes)
...
fn hook_command(source: CliSource) -> Result<String> {
    let executable = trusted_pryx_executable()?;
    Ok(format!(
        "{} {HOOK_COMMAND_MARKER}{}",
...
/// bare `pryx` lookup in the external CLI's project-scoped PATH. Falling back
...
fn trusted_pryx_executable() -> Result<PathBuf> {
    #[cfg(windows)]
    {
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            let launcher = home.join(".local").join("bin").join("pryx");
            if launcher.is_file() {
                return Ok(launcher);
            }
        }
    ...
    std::env::current_exe().context("resolving the Pryx executable for lifecycle hooks")
}
    ...
    #[cfg(windows)]
    {
        ...
        let script = format!(
            "$title=\"{}\";$body=\"{}\";\
             ...
             [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('Pryx').Show([Windows.UI.Notifications.ToastNotification]::new($xml))",
            ps_quote(title),
            ps_quote(body)
        );
...
mod tests {
    ...
    fn inserts_session_start_hook_without_replacing_existing_hooks() {
        ...
        let command = "'/trusted/pryx' setup-hotkey --notify-cli-launch claude";
    ...
    fn managed_hook_update_is_idempotent() {
        let mut value = json!({});
        let command = "'/trusted/pryx' setup-hotkey --notify-cli-launch codex";
        assert!(upsert_hook(&mut value, command).unwrap());
        assert!(!upsert_hook(&mut value, command).unwrap());
            ...
            "hooks": {
                "SessionStart": [{
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "pryx setup-hotkey --notify-cli-launch old",
                            "timeout": 30
                        },
    ...
    fn rejects_malformed_hook_shape_instead_of_clobbering_it() {
        ...
        assert!(
            upsert_hook(
                &mut value,
                "'/trusted/pryx' setup-hotkey --notify-cli-launch claude"
            )
            .is_err()
        );
    }
...
"#,
        ...
        assert!(!command.starts_with("pryx "));
    ...
    fn unix_hook_executable_quoting_handles_spaces_and_single_quotes() {
        assert_eq!(
            quote_hook_executable(Path::new("/tmp/Pryx's bin/pryx")),
            "'/tmp/Pryx'\\''s bin/pryx'"
        );
    }
}
