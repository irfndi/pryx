...
//! low-level keyboard hook in the first-party Rust listener (`pryx setup-hotkey
...
/// Returns `None` for keys with no stable VK code. pryx `cmd` maps to Alt
...
/// Translate a canonical pryx key token into a Win32 virtual-key code.
...
/// User-facing chord rendering for Windows notices: pryx's `cmd` modifier
...
fn ps_dir_expr(dir: &str, last_dir_file: &str, last_repo_file: &str) -> String {
    match dir {
        "$HOME" => "(Resolve-PryxDir $null)".to_string(),
        "$LAST_DIR" => format!("(Resolve-PryxDir {})", ps_quote(last_dir_file)),
        "$LAST_REPO" => format!("(Resolve-PryxDir {})", ps_quote(last_repo_file)),
        path => format!("(Resolve-PryxFixedDir {})", ps_quote(path)),
    }
}

/// Render the full hotkey-listener PowerShell script.
///
/// `launch_exe`/`launch_args` describe the terminal command that opens pryx
/// (e.g. `wt.exe` + profile args, or `alacritty -e pryx`); `{DIR}` never
...
pub(crate) fn render_windows_listener_ps1(
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        registrations.push_str(&format!(
            "Register-PryxHotkey -Id 0x{id:X} -Mods 0x{mods:X} -Vk 0x{vk:X} -Label {label}\n",
    ...
    Some(format!(
        r#"# pryx global launch hotkey listener
# Auto-generated by pryx setup-hotkey from [launch_hotkeys] config. Runs at
# login via a startup shortcut. Do not edit; re-run `pryx setup-hotkey`.
...
function Register-PryxHotkey {{
    ...
    if ([HotKeyHelper]::RegisterHotKey([IntPtr]::Zero, $Id, $Mods -bor $MOD_NOREPEAT, $Vk)) {{
        ...
        Write-Warning "pryx: failed to register hotkey $Label (already claimed?)"
    }}
}}

# Resolve a dynamic launch dir from a tracking file, falling back to $HOME.
function Resolve-PryxDir {{
    param([string]$File)
    if ($File) {{
        try {{
...
if ($script:RegisteredIds.Count -eq 0) {{
    Write-Error "pryx: no launch hotkeys could be registered"
    exit 1
}}
...
mod tests {
    ...
    fn args_for(entry: &WindowsHotkey) -> String {
        ...
        if entry.self_dev {
            format!(r#"-e "C:\pryx.exe" --spawn-hotkey "{hotkey}" self-dev"#)
        } else {
            format!(r#"-e "C:\pryx.exe" --spawn-hotkey "{hotkey}""#)
        }
    }
    ...
    fn listener_script_registers_each_entry_and_dispatches_dirs() {
        let entries = vec![
            hk("win+shift+f23", "C:\\Users\\u\\pryx", "pryx", false),
        ...
        let script = render_windows_listener_ps1(
            ...
            "wt.exe",
            args_for,
            "C:\\Users\\u\\.pryx\\hotkey\\last_dir",
            "C:\\Users\\u\\.pryx\\hotkey\\last_repo",
        )
        .unwrap();

        // Three registrations with distinct ids.
        assert_eq!(script.matches("Register-PryxHotkey").count(), 3 + 1); // 3 calls + fn def
        ...
        assert!(script.contains("Resolve-PryxFixedDir 'C:\\Users\\u\\pryx'"));
        assert!(script.contains("Resolve-PryxDir $null"));
        assert!(script.contains("Resolve-PryxDir 'C:\\Users\\u\\.pryx\\hotkey\\last_repo'"));
