//! Resolve the global "launch a new pryx" hotkeys from config into a concrete
...
//!   [`ResolvedLaunchHotkey`]s. An empty config reproduces pryx's historical
...
/// Build a pryx launch snippet that pauses on non-zero exit so the user can
...
#[cfg(any(test, target_os = "macos"))]
fn paused_pryx_shell_command_with_args(exe_path: &str, args: &[String]) -> String {
...
pub(crate) struct ResolvedLaunchHotkey {
    /// pryx-style chord string, e.g. `cmd+;`.
    pub chord: String,
    ...
    /// fallback) before launching pryx.
    ...
    pub dir: String,
    /// Extra CLI args passed to pryx (e.g. `self-dev`).
    pub args: Vec<String>,
...
/// `launch_pryx_cmd_bracketleft.sh`. Collisions are avoided by appending the
...
fn script_name_for(chord: &str, index: usize) -> String {
    ...
    format!("launch_pryx_{index}_{slug}.sh")
}

/// The built-in default entries, used when config has none. Mirrors pryx's
...
pub(crate) fn shell_command_for(entry: &ResolvedLaunchHotkey, exe_path: &str) -> String {
    format!(
        "{}{}",
        entry.cd_prefix,
        paused_pryx_shell_command_with_args(exe_path, &entry.args)
    )
}

/// Map a pryx key token onto a `global_hotkey` `Code`. Returns `None` for
...
mod tests {
    ...
    fn shell_command_includes_cd_and_exe() {
        ...
        let cmd = shell_command_for(&resolved[0], "/bin/pryx");
        assert!(cmd.contains("/Users/jeremy/proj"));
        assert!(cmd.contains("/bin/pryx"));
    }
}
