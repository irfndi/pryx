...
fn first_three_launches_can_include_hotkey_notice_too() {
    ...
    // The notice should make clear the hotkey works globally, not just inside pryx.
...
fn default_resolved_hotkeys_match_legacy_three() {
    ...
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
        &pryx_config_types::LaunchHotkeysConfig::default(),
        "/usr/local/bin/pryx",
        "/home/u/.pryx/hotkey/last_dir",
        "/home/u/.pryx/hotkey/last_repo",
    );
    ...
    let home = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/pryx");
    assert!(home.starts_with("cd \"$HOME\"; "));
    assert!(!home.contains("self-dev"));

    let last_dir = launch_hotkeys::shell_command_for(&resolved[1], "/usr/local/bin/pryx");
    assert!(last_dir.contains("cat '/home/u/.pryx/hotkey/last_dir'"));
    assert!(last_dir.contains("cd \"$HOME\""));

    let selfdev = launch_hotkeys::shell_command_for(&resolved[2], "/usr/local/bin/pryx");
    assert!(selfdev.contains("cat '/home/u/.pryx/hotkey/last_repo'"));
    assert!(selfdev.contains("'/usr/local/bin/pryx' 'self-dev';"));
}
...
fn baked_repo_hotkey_cds_into_fixed_dir() {
    ...
    let config = pryx_config_types::LaunchHotkeysConfig {
        enabled: Some(true),
        imported: true,
        entries: vec![pryx_config_types::LaunchHotkeyEntry {
            chord: "cmd+[".to_string(),
            dir: "/Users/jeremy/pryx-github".to_string(),
            label: "pryx-github".to_string(),
            self_dev: false,
        }],
    };
    let resolved = launch_hotkeys::resolve_launch_hotkeys(
    ...
    let cmd = launch_hotkeys::shell_command_for(&resolved[0], "/usr/local/bin/pryx");
    assert!(cmd.contains("/Users/jeremy/pryx-github"));
...
fn mac_hotkey_launch_agent_plist_uses_valid_xml_quotes() {
    let plist = mac_hotkey_launch_agent_plist(
        "/Applications/Pryx.app/Contents/MacOS/pryx",
        "/tmp/pryx-hotkey.out.log",
        "/tmp/pryx-hotkey.err.log",
        "ghostty",
    );
...
#[test]
fn paused_pryx_shell_command_keeps_failures_visible() {
    let command = paused_pryx_shell_command("/tmp/pryx");
    assert!(command.contains("Press Enter to close"));
    assert!(command.contains("Pryx exited with status"));
    assert!(command.contains("pryx executable not found"));
}
...
fn macos_terminal_notice_only_fires_for_default_terminal_app() {
    ...
    assert_eq!(
        ...
        Some("Tip: Terminal.app renders pryx poorly. Try Ghostty, iTerm2, or Alacritty.")
    );
    let (title, message) = hints.display_message.expect("expected display message");
    assert_eq!(title, "Terminal");
    assert!(message.contains("Terminal.app renders pryx poorly"));
...
fn keymap_conflict_hint_full_path_debounces_and_persists_signature() {
    ...
    use pryx_config_types::KeybindingsConfig;
...
fn glyph_safe_notice_shows_once_then_debounces() {
    ...
    assert!(body.contains("PRYX_GLYPH_SAFE_MODE=off"));
