...
fn macos_launcher_script_shows_alerts_and_uses_terminal_launcher() {
    let script = macos_launcher_script(
        ...
        "/tmp/pryx",
        Path::new("/Users/test/Applications/Pryx.app"),
    );
    assert!(script.contains("display alert \"Pryx launch failed\""));
    assert!(script.contains("pryx setup-launcher"));
...
fn macos_launcher_refreshes_when_new_bundle_missing() {
    ...
    let app_dir = temp.path().join("Pryx.app");
    let legacy_app_dir = temp.path().join("pryx.app");
...
fn macos_notification_bundle_is_faceless_and_uses_multicall_binary() {
    ...
    assert!(plist.contains("<string>com.pryx.notifications</string>"));
    assert!(plist.contains("<string>pryx-notification-broker</string>"));
    assert!(plist.contains(pryx_build_meta::version()));
}
