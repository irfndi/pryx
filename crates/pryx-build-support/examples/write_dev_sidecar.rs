...
//! coordinated build queue), the freshly built `target/selfdev/pryx` has no
...
//!
//! ```sh
//! cargo run -p pryx-build-support --example write_dev_sidecar
//! ```
fn main() -> anyhow::Result<()> {
    let repo = std::env::current_dir()?;
    let state = pryx_build_support::current_source_state(&repo)?;
    let path = pryx_build_support::write_current_dev_binary_source_metadata(&repo, &state)?;
