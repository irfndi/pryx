...
/// Get the pryx repository directory
pub fn get_repo_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("PRYX_REPO_DIR") {
        let path = PathBuf::from(path);
        if is_pryx_repo(&path) {
            return Some(path);
        }
    }
...
///
/// Release archives install a tiny `pryx` wrapper script alongside the real
/// `pryx-<platform>.bin` payload (the wrapper sets `LD_LIBRARY_PATH` and execs
...
pub fn selfdev_build_command_for_target(
    ...
    let specs = match target {
        SelfDevBuildTarget::Tui => vec![("pryx", "pryx")],
        ...
        SelfDevBuildTarget::Desktop2 => vec![
            ("pryx-desktop2", "pryx-desktop2"),
            ("pryx-harness-api-server", "pryx-harness-api-bridge"),
        ],
...
fn build_target_for_paths<'a>(paths: impl Iterator<Item = &'a str>) -> SelfDevBuildTarget {
    ...
    for path in paths {
        ...
        if path == "Cargo.toml" || path == "Cargo.lock" || path.starts_with(".cargo/") {
            ...
            desktop2 = true;
            other = true;
        } else if path.starts_with("crates/pryx-desktop2/") {
            desktop2 = true;
        } else {
            other = true;
        }
...
///
/// Defaults to `~/.local/bin` on Unix, `%LOCALAPPDATA%\pryx\bin` on Windows.
/// Overridable with `PRYX_INSTALL_DIR`.
pub fn launcher_dir() -> Result<PathBuf> {
    if let Some(custom) = non_empty_env_path("PRYX_INSTALL_DIR") {
        return Ok(custom);
    }

    if let Some(sandbox_home) = non_empty_env_path("PRYX_HOME") {
    ...
    #[cfg(windows)]
    {
        if let Ok(local) = std::env::var("LOCALAPPDATA") {
            return Ok(PathBuf::from(local).join("pryx").join("bin"));
        }
        Ok(home_dir()?
            .join("AppData")
            .join("Local")
...
/// Path to the launcher binary (`~/.local/bin/pryx` by default).
...
/// Check if a directory is the pryx repository
pub fn is_pryx_repo(dir: &Path) -> bool {
    // Check for Cargo.toml with name = "pryx"
...
mod tests {
    ...
    fn repo_fixture(git_file: bool) -> tempfile::TempDir {
        ...
        if git_file {
            std::fs::write(temp.path().join(".git"), "gitdir: /tmp/pryx-test-git\n")
                .expect("git file");
        } else {
    ...
    fn find_repo_in_ancestors_finds_workspace_from_crate_dir() {
        ...
        let crate_dir = repo.path().join("crates").join("pryx-build-support");
    ...
    fn every_build_target_builds_its_own_package() {
        let repo = repo_fixture(false);
        let cases = [
            (SelfDevBuildTarget::Tui, vec!["-p pryx "]),
            (
                SelfDevBuildTarget::Desktop2,
                vec!["-p pryx-desktop2 ", "--bin pryx-harness-api-bridge"],
            ),
            (
    ...
    fn auto_routes_changed_paths_to_the_right_binary() {
        let cases: Vec<(Vec<&str>, SelfDevBuildTarget)> = vec![
            ...
            (vec!["crates/pryx-tui/src/lib.rs"], SelfDevBuildTarget::Tui),
            (
                vec!["crates/pryx-desktop2/src/main.rs"],
                SelfDevBuildTarget::Desktop2,
            ),
            (
                vec!["crates/pryx-desktop2/src/editor.rs", "src/main.rs"],
                SelfDevBuildTarget::All,
            ),
    ...
    fn porcelain_lines_are_parsed_including_renames() {
        ...
        assert_eq!(
            porcelain_path("?? crates/pryx-desktop2/src/new.rs"),
            "crates/pryx-desktop2/src/new.rs"
        );
        assert_eq!(
            porcelain_path("R  old/path.rs -> crates/pryx-desktop2/src/moved.rs"),
            "crates/pryx-desktop2/src/moved.rs"
        );
    }
    ...
    fn build_targets_parse_from_their_names() {
        for (name, expected) in [
            ...
            ("pryx-desktop2", SelfDevBuildTarget::Desktop2),
    ...
    #[test]
    fn is_pryx_repo_accepts_git_file_for_worktree() {
        let repo = repo_fixture(true);
        assert!(is_pryx_repo(repo.path()));
    }

    /// Build a release-style install dir: `pryx` wrapper script + payload.
    fn release_install_fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        let temp = tempfile::TempDir::new().expect("temp install");
        let wrapper = temp.path().join("pryx");
        let payload = temp.path().join("pryx-linux-x86_64.bin");
        std::fs::write(
            &wrapper,
    ...
    fn resolve_binary_payload_follows_channel_symlink_to_wrapper() {
        // The real layout: builds/<channel>/pryx -> versions/<v>/pryx (wrapper).
        ...
        let link = channel_dir.join("pryx");
        #[cfg(unix)]
    ...
    fn resolve_binary_payload_keeps_real_binary() {
        ...
        let temp = tempfile::TempDir::new().expect("temp install");
        let binary = temp.path().join("pryx");
        std::fs::write(&binary, vec![0x7fu8; 8192]).expect("binary");
        std::fs::write(temp.path().join("pryx-linux-x86_64.bin"), [0u8; 8]).expect("bin");
        assert_eq!(
    ...
    fn resolve_binary_payload_keeps_small_script_without_payload() {
        let temp = tempfile::TempDir::new().expect("temp install");
        let script = temp.path().join("pryx");
    ...
    fn resolve_binary_payload_refuses_ambiguous_payloads() {
        ...
        std::fs::write(temp.path().join("pryx-macos-aarch64.bin"), [0u8; 8]).expect("second bin");
        assert_eq!(
            resolve_binary_payload(&wrapper),
