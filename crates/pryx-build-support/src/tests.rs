...
fn with_temp_pryx_home<T>(f: impl FnOnce() -> T) -> T {
    ...
    pryx_core::env::set_var("PRYX_HOME", temp_home.path());
    let result = f();
...
fn test_binary_version_hash_mismatch_rejects_publish_candidate() {
    ...
    let error = validate_binary_version_matches_source_report(&report, Path::new("pryx"), &source)
...
fn test_find_repo_in_ancestors_walks_upward() {
    let temp = tempfile::tempdir().expect("tempdir");
    let repo = temp.path().join("pryx-repo");
...
#[test]
fn launcher_dir_uses_sandbox_bin_when_pryx_home_is_set() {
    with_temp_pryx_home(|| {
        let launcher_dir = launcher_dir().expect("launcher dir");
        let expected = storage::pryx_dir().expect("pryx dir").join("bin");
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_pryx_home(|| {
        ...
        let expected_launcher = storage::pryx_dir()
            .expect("pryx dir")
            .join("bin")
            .join(binary_name());
...
/// Write a distinct, real binary into `versions/<version>/pryx` with an
