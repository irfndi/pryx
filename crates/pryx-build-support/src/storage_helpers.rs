...
pub fn builds_dir() -> Result<PathBuf> {
    let dir = resolve_builds_dir(
        std::env::var_os("PRYX_HOME").map(PathBuf::from),
...
fn resolve_builds_dir(
    pryx_home: Option<PathBuf>,
    local_app_data: Option<PathBuf>,
    default_pryx_dir: PathBuf,
    is_windows: bool,
) -> PathBuf {
    if let Some(pryx_home) = pryx_home {
        return pryx_home.join("builds");
    }

    if is_windows && let Some(local_app_data) = local_app_data {
        // Keep runtime channel discovery aligned with scripts/install.ps1 and
        // the supported Windows layout under %LOCALAPPDATA%\pryx\builds.
        // Durable user state and logs still live under ~/.pryx.
        return local_app_data.join("pryx").join("builds");
    }

    default_pryx_dir.join("builds")
}
...
mod tests {
    ...
    fn windows_builds_use_local_app_data() {
        let resolved = resolve_builds_dir(
            ...
            PathBuf::from("/home/test/.pryx"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/local-app-data/pryx/builds"));
    }

    #[test]
    fn pryx_home_override_wins_on_windows() {
        let resolved = resolve_builds_dir(
            Some(PathBuf::from("/isolated-pryx")),
            Some(PathBuf::from("/local-app-data")),
            PathBuf::from("/home/test/.pryx"),
            true,
        );

        assert_eq!(resolved, PathBuf::from("/isolated-pryx/builds"));
    }

    #[test]
    fn non_windows_builds_stay_under_pryx_home() {
        let resolved = resolve_builds_dir(
            None,
        ...
        assert_eq!(resolved, PathBuf::from("/home/test/.pryx/builds"));
    }
}
