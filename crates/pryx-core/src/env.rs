...
/// Mutate the process environment for pryx runtime configuration.
///
/// Rust 2024 makes environment mutation unsafe because it can race with
/// concurrent environment access in foreign code. pryx intentionally mutates
...
where
    ...
    V: AsRef<OsStr>,
{
    // SAFETY: pryx treats these mutations as process-global configuration.
...
/// Remove a process environment variable used by pryx runtime configuration.
