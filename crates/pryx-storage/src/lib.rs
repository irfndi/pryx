...
///
/// Can be overridden with `$PRYX_RUNTIME_DIR`.
pub fn runtime_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("PRYX_RUNTIME_DIR") {
...
fn fallback_runtime_dir() -> PathBuf {
    std::env::temp_dir().join(format!("pryx-{}", runtime_user_discriminator()))
}
...
fn ensure_private_runtime_dir(path: &Path) {
    ...
    #[cfg(unix)]
    {
        let _ = pryx_core::fs::set_directory_permissions_owner_only(path);
    }
}

pub fn pryx_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("PRYX_HOME") {
...
/// to `~/.pryx/state` (respecting `PRYX_HOME`).
///
/// When `PRYX_RUNTIME_DIR` is set (tests and sandboxed temp servers), it
/// takes precedence so isolated runs never touch the real pryx home.
...
/// Resolve pryx's app-owned config directory.
///
/// Default location is the platform config dir + `pryx` (for example
/// `~/.config/pryx` on Linux). When `PRYX_HOME` is set, sandbox this under
/// `$PRYX_HOME/config/pryx` so self-dev/tests do not leak into the user's
/// real config directory.
pub fn app_config_dir() -> Result<PathBuf> {
    if let Ok(path) = std::env::var("PRYX_HOME") {
        return Ok(PathBuf::from(path).join("config").join("pryx"));
    }

    let config_dir =
        dirs::config_dir().ok_or_else(|| anyhow::anyhow!("No config directory found"))?;
    Ok(config_dir.join("pryx"))
}

/// Resolve a path under the user's home directory, but sandbox it under
/// `$PRYX_HOME/external/` when `PRYX_HOME` is set.
///
...
pub fn harden_user_config_permissions() {
    #[cfg(windows)]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let pryx_config_dir = config_dir.join("pryx");
            if pryx_config_dir.exists() {
                schedule_windows_path_hardening(&pryx_config_dir, true);
            }
        }

        if let Ok(pryx_home) = pryx_dir()
            && pryx_home.exists()
        {
            schedule_windows_path_hardening(&pryx_home, true);
        }
        return;
    }
    ...
    #[cfg(not(windows))]
    {
        if let Some(config_dir) = dirs::config_dir() {
            let pryx_config_dir = config_dir.join("pryx");
            if pryx_config_dir.exists() {
                let _ = pryx_core::fs::set_directory_permissions_owner_only(&pryx_config_dir);
            }
        }
        ...
        if let Ok(pryx_home) = pryx_dir()
            && pryx_home.exists()
        {
            let _ = pryx_core::fs::set_directory_permissions_owner_only(&pryx_home);
        }
    }
}
...
pub fn harden_secret_file_permissions(path: &Path) {
    ...
    #[cfg(not(windows))]
    {
        if let Some(parent) = path.parent() {
            let _ = pryx_core::fs::set_directory_permissions_owner_only(parent);
        }
        if path.exists() {
            let _ = pryx_core::fs::set_permissions_owner_only(path);
        }
    }
}
...
fn harden_secret_file_permissions_windows(path: &Path) {
    ...
    // Read-time hardening is opportunistic, while Pryx's own secret writes
...
fn schedule_windows_path_hardening(path: &Path, directory: bool) {
    ...
    if std::thread::Builder::new()
        .name("pryx-windows-acl-harden".to_string())
        .spawn(|| {
...
fn run_windows_hardening_worker() {
    loop {
        ...
        for path in &directories {
            let succeeded = pryx_core::fs::set_directory_permissions_owner_only(path).is_ok();
            directory_results.push((path.clone(), succeeded));
        }
        let mut file_results = Vec::with_capacity(files.len());
        for path in &files {
            let succeeded =
                !path.exists() || pryx_core::fs::set_permissions_owner_only(path).is_ok();
...
///
/// pryx intentionally avoids mutating these files. We also reject obvious risky
...
pub fn ensure_dir(path: &Path) -> Result<()> {
    if !path.exists() {
        ...
        pryx_core::fs::set_directory_permissions_owner_only(path)?;
    }
    Ok(())
}
...
fn write_bytes_inner(path: &Path, bytes: &[u8], durable: bool, secret: bool) -> Result<()> {
    if let Some(parent) = path.parent() {
        ...
        if secret {
            ...
            pryx_core::fs::set_directory_permissions_owner_only(parent)?;
        }
    }
    ...
    let result = (|| -> Result<()> {
        ...
        if secret {
            pryx_core::fs::set_permissions_owner_only(&tmp_path)?;
        }
        ...
        if path.exists() {
            ...
            if secret {
                pryx_core::fs::set_permissions_owner_only(path)?;
            }
            ...
            if secret && bak_path.exists() {
                pryx_core::fs::set_permissions_owner_only(&bak_path)?;
            }
        }
...
mod windows_hardening_tests {
    ...
    fn first_path_starts_one_worker_and_repeated_paths_are_coalesced() {
        ...
        let now = Instant::now();
        let directory = Path::new(r"C:\Users\test\.pryx");
    ...
    fn recently_attempted_paths_are_not_requeued() {
        ...
        let file = PathBuf::from(r"C:\Users\test\.pryx\auth.json");
        state
            .files
