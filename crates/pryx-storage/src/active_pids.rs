//! Tracking of active session process IDs under `~/.pryx/active_pids`.
//!
...
//! lives in the storage crate because it only needs [`pryx_dir`] and is a
...
/// Directory holding one file per active session ID (`~/.pryx/active_pids`).
pub fn active_pids_dir() -> Option<PathBuf> {
    pryx_dir().ok().map(|d| d.join("active_pids"))
}
...
pub fn streaming_pids_dir() -> Option<std::path::PathBuf> {
    pryx_dir().ok().map(|d| d.join("streaming_pids"))
}
...
pub fn internal_pids_dir() -> Option<std::path::PathBuf> {
    pryx_dir().ok().map(|d| d.join("internal_pids"))
}
...
/// List active session IDs currently tracked in `~/.pryx/active_pids`.
...
/// Live snapshot of how many pryx sessions are running, and how many of those
/// are actively streaming a model response right now. Used by the menu bar
/// indicator (`pryx menubar`) and any other presence UI.
...
mod tests {
    ...
    /// Serialize tests that mutate `PRYX_HOME`.
