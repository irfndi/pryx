...
//!
//! Shows how much RAM this window (the client) and the pryx daemon (the
...
/// Whether a `/proc/<pid>/cmdline` argv is the pryx daemon. The daemon is
/// `pryx serve ...` wherever its binary lives (selfdev target, shared-server
...
pub fn is_daemon_cmdline(args: &[&str]) -> bool {
    ...
    exe == "pryx" && args.get(1).copied() == Some("serve")
}
...
mod tests {
    ...
    fn the_daemon_is_matched_by_name_and_subcommand_only() {
        assert!(is_daemon_cmdline(&[
            "/home/j/.pryx/builds/shared-server/pryx",
            "serve",
            "--socket",
            "/run/user/1000/pryx.sock",
        ]));
        assert!(is_daemon_cmdline(&["pryx", "serve"]));
        // The desktop client, the bridge, and an interactive `pryx` must not
        // count as the server.
        assert!(!is_daemon_cmdline(&["/x/pryx-desktop2"]));
        assert!(!is_daemon_cmdline(&["/x/pryx-harness-api-bridge"]));
        assert!(!is_daemon_cmdline(&["/x/pryx"]));
        assert!(!is_daemon_cmdline(&["/x/pryx", "self-dev"]));
        assert!(!is_daemon_cmdline(&[]));
    }
