...
pub fn dispatch(args: &[String]) -> Option<Result<()>> {
    if args.iter().any(|arg| arg == "--version" || arg == "-V") {
        println!("pryx-desktop2 {}", pryx_build_meta::version());
        return Some(Ok(()));
    }
...
fn profile_transitions() -> Result<()> {
    ...
    pryx_sdk::ensure_runtime(&pryx_sdk::LaunchOptions::default(), &|_| {})?;
    let runtime = start.elapsed();

    let start = Instant::now();
    let client = pryx_sdk::PryxClient::connect(pryx_sdk::ConnectOptions {
        client_name: concat!("pryx-desktop2-profile/", env!("CARGO_PKG_VERSION")).to_string(),
...
fn check_connect() -> Result<()> {
    pryx_sdk::ensure_runtime(&pryx_sdk::LaunchOptions::default(), &|status| {
        println!("[connect] {status}")
    })?;
    let client = pryx_sdk::PryxClient::connect(pryx_sdk::ConnectOptions {
        client_name: concat!("pryx-desktop2-check/", env!("CARGO_PKG_VERSION")).to_string(),
        ensure_runtime: false,
        ..Default::default()
    })?;
    println!(
...
fn check_primary_selection() -> Result<()> {
    ...
    let sentinel = format!("pryx-primary-check-{}", std::process::id());
    ...
    if std::env::var_os("PRYX_DESKTOP2_HOLD_SELECTION").is_some() {
...
///
///   pryx-desktop2 --script 'type:alpha beta' ctrl+a shift+right shift+right
///
...
/// its bench flag:
///
///   pryx-desktop2 --script 'sessions:a=pryx,b=pryx,c=site' super+l super+j
...
fn print_keys() {
    println!("keybindings (ported from the pryx TUI)\n");
    ...
    while std::time::Instant::now() < deadline {
        ...
        match update {
            ...
            harness::HarnessUpdate::TurnDone if sent => {
                println!("\n[e2e] turn done");
                let out = std::env::temp_dir().join("pryx-desktop2-e2e.png");
...
fn check_reconnect() -> Result<()> {
    ...
    let runtime = std::env::temp_dir().join(format!("pryx-reconnect-{}", std::process::id()));
    std::fs::create_dir_all(&runtime)?;
    // SAFETY: single-threaded, before any connection thread is spawned.
    unsafe {
        std::env::set_var("PRYX_API_SOCKET", runtime.join("api.sock"));
    }
    let bridge = |kill: bool| -> Option<u32> {
        let listening = std::process::Command::new("pgrep")
            .args(["-f", "pryx-harness-api-bridge"])
            .output()
            .ok()?;
