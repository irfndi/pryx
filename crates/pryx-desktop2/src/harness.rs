...
//! explaining a lost connection) lives in `pryx-sdk`, so this file is about
...
use pryx_sdk::{ApiEvent, ConnectOptions, PryxClient, LaunchOptions};
...
pub use pryx_sdk::{SocketState, Stage, describe_disconnect, explain};
...
pub(crate) fn default_working_dir() -> Option<String> {
    if let Some(raw) = std::env::var_os("PRYX_DESKTOP2_WORKING_DIR") {
...
fn run(
    ...
    pryx_sdk::ensure_runtime(&LaunchOptions::default(), &|status| {
        ui.send(HarnessUpdate::Status(status.to_string()))
    })?;

    set_stage(Stage::Connecting);
    let client = PryxClient::connect(ConnectOptions {
        client_name: concat!("pryx-desktop2/", env!("CARGO_PKG_VERSION")).to_string(),
                ...
                let outcome = match command {
                    ...
                    Command::Peek(target) => {
                        ...
                        std::thread::spawn(move || {
                            ...
                            let preview = PryxClient::connect(ConnectOptions {
                                client_name: concat!(
                                    "pryx-desktop2-preview/",
    ...
    std::thread::spawn({
        ...
        move || {
            let Ok(client) = PryxClient::connect(ConnectOptions {
                client_name: concat!("pryx-desktop2-sessions/", env!("CARGO_PKG_VERSION"))
                    .to_string(),
                ensure_runtime: false,
                ..Default::default()
            }) else {
...
fn to_entry(session: pryx_sdk::SessionInfo) -> crate::strip::Panel {
    crate::strip::Panel {
        session_id: session.session_id,
        title: session.title,
        working_dir: session.working_dir,
...
fn to_transcript(messages: Vec<pryx_sdk::HistoryMessage>) -> crate::transcript::Transcript {
