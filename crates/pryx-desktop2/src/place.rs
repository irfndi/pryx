...
/// which checkout this window is for is what distinguishes two pryx windows
/// from each other.
pub fn window_title(working_dir: Option<&str>) -> String {
    match working_dir {
        Some(dir) => format!("{} - pryx", display_path(dir, home().as_deref())),
        None => "pryx desktop2".to_string(),
    }
}
...
mod tests {
    ...
    fn home_collapses_to_tilde() {
        assert_eq!(
            display_path("/home/j/pryx/crates/x", Some("/home/j")),
            "~/pryx/crates/x"
        );
    ...
    fn title_names_the_directory_first() {
        ...
        assert!(title.ends_with("pryx"), "{title}");
        assert_eq!(window_title(None), "pryx desktop2");
    }
}
