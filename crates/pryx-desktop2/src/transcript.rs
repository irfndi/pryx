...
//! - Markdown and LaTeX come from [`pryx_render_core`], the backend-neutral
...
fn checkbox_spans(spans: &[pryx_render_core::StyledSpan]) -> Vec<pryx_render_core::StyledSpan> {
...
fn table_lines(rows: &[Vec<String>], alignments: &[Alignment], columns: usize) -> Vec<StyledLine> {
    use pryx_render_core::{StyledSpan, TextAttrs};
...
fn strip_leading_indent(
    spans: &[pryx_render_core::StyledSpan],
) -> Vec<pryx_render_core::StyledSpan> {
    let mut spans = spans.to_vec();
    if let Some(first) = spans.first_mut() {
...
fn strip_quote_bar(spans: &[pryx_render_core::StyledSpan]) -> Vec<pryx_render_core::StyledSpan> {
    let mut spans = spans.to_vec();
    if spans
        .first()
