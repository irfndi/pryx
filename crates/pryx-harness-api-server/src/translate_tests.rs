...
fn pryx_home_test_lock() -> MutexGuard<'static, ()> {
...
impl ScopedPryxHome {
    fn new(label: &str) -> Self {
        let guard = pryx_home_test_lock();
        let previous = std::env::var_os("PRYX_HOME");
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap_or_default()
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "pryx-harness-api-{label}-{}-{unique}",
            std::process::id()
        ));
        std::fs::create_dir_all(&path).expect("create isolated PRYX_HOME");
        // SAFETY: all tests in this module that mutate PRYX_HOME share `LOCK`,
        // and this guard restores the prior value before it is released.
        unsafe { std::env::set_var("PRYX_HOME", &path) };
        Self {
            path,
            previous,
            _guard: guard,
...
impl Drop for ScopedPryxHome {
    fn drop(&mut self) {
        match self.previous.take() {
            Some(value) => unsafe { std::env::set_var("PRYX_HOME", value) },
            None => unsafe { std::env::remove_var("PRYX_HOME") },
        }
...
#[test]
fn create_session_in_a_pryx_checkout_requests_selfdev() {
...
fn session_records_are_read_from_the_instance_home() {
    ...
    assert!(
        ...
        "PRYX_HOME must scope session records, got {}",
        path.display()
    );
}
...
fn unattached_list_sessions_discovers_all_persisted_records() {
    let home = ScopedPryxHome::new("persisted-discovery");
...
fn archive_restore_and_retention_are_reversible_and_owner_only() {
    let home = ScopedPryxHome::new("archive");
...
#[test]
fn credential_provisioning_normalizes_gemini_and_supports_pryx() {
    let home = ScopedPryxHome::new("credentials");
    let config = home.path.join("config/pryx");
    ...
    let [Outbound::Legacy(notify)] = outbound.as_slice() else {
        panic!("pryx credential should notify the daemon: {outbound:?}");
    };
    assert_eq!(notify["provider"], "pryx");
    assert_eq!(
        std::fs::read_to_string(config.join("pryx-subscription.env")).unwrap(),
        "PRYX_API_KEY=pryx-secret\n"
    );
...
fn owner_only_writes_refuse_symlink_targets_and_directories() {
    ...
    let home = ScopedPryxHome::new("credential-symlinks");
    ...
    assert!(!outside_dir.join("pryx-subscription.env").exists());
}
...
fn rooted_file_operations_reject_traversal_and_symlink_escapes_and_bound_results() {
    ...
    let home = ScopedPryxHome::new("rooted-files");
    let root = home.path.join("project");
