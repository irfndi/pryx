...
impl BridgeState {
    ...
    pub fn api_request_to_legacy(&mut self, request: &Value) -> Vec<Outbound> {
        ...
        match req {
            ...
            "create_session" | "attach_session" => {
                ...
                // Sessions rooted inside a pryx checkout are self-dev
                ...
                if working_dir
                    .as_deref()
                    .is_some_and(Self::path_is_inside_pryx_repo)
                {
            ...
            "set_api_key" | "clear_api_key" => {
                ...
                let Some((provider, env_keys, file_name)) = Self::credential_binding(provider)
                    ...
                    return Self::error_reply(
                        api_id,
                        ErrorCode::InvalidRequest,
                        "unsupported API-key provider; supported: claude-api, openai-api, openrouter, cursor, gemini, pryx",
                    );
                };
    ...
    /// True when `path`, or any ancestor, looks like a pryx source checkout.
    ///
    ...
    fn path_is_inside_pryx_repo(path: &str) -> bool {
        let mut current = Some(std::path::Path::new(path));
        while let Some(dir) = current {
            if dir.join("Cargo.toml").is_file() && dir.join("crates/pryx-base").is_dir() {
                return true;
            }
            current = dir.parent();
        }
    ...
    /// plain session id.
    ///
    /// One funnel for three reasons. It honours `PRYX_HOME`, without which a
    /// launched instance reads the *user's* sessions: `peek_session` served
    /// the real transcripts of the pryx the user runs interactively, which
    ...
    fn session_record_path(session_id: &str) -> Option<std::path::PathBuf> {
        ...
        let home = match std::env::var_os("PRYX_HOME") {
            Some(home) => std::path::PathBuf::from(home),
            None => std::path::Path::new(&std::env::var_os("HOME")?).join(".pryx"),
        };
    ...
    fn pryx_home() -> Option<std::path::PathBuf> {
        std::env::var_os("PRYX_HOME")
            .map(std::path::PathBuf::from)
            .or_else(|| {
                std::env::var_os("HOME").map(|home| std::path::Path::new(&home).join(".pryx"))
            })
    }

    fn stored_session_ids() -> Vec<String> {
        let Some(home) = Self::pryx_home() else {
            return Vec::new();
        };
    ...
    fn archive_state_path() -> Option<std::path::PathBuf> {
        Some(Self::pryx_home()?.join("sdk-archive.json"))
    }
    ...
    fn save_archive_state(state: &ArchiveState) -> Result<(), String> {
        let path =
            Self::archive_state_path().ok_or_else(|| "could not resolve pryx home".to_string())?;
        let parent = path
            .parent()
    ...
    fn app_config_dir() -> Option<std::path::PathBuf> {
        if let Some(home) = std::env::var_os("PRYX_HOME") {
            return Some(std::path::Path::new(&home).join("config/pryx"));
        }
        #[cfg(target_os = "macos")]
        return Some(
            std::path::Path::new(&std::env::var_os("HOME")?)
                .join("Library/Application Support/pryx"),
        );
        #[cfg(target_os = "windows")]
        return Some(std::path::Path::new(&std::env::var_os("APPDATA")?).join("pryx"));
    ...
    fn credential_binding(
        ...
        match provider.trim().to_ascii_lowercase().as_str() {
            ...
            "pryx" | "subscription" | "pryx-subscription" => {
                Some(("pryx", &["PRYX_API_KEY"], "pryx-subscription.env"))
            }
            _ => None,
        }
    }
    ...
    fn write_credential(
        ...
        let dir = Self::app_config_dir()
            .ok_or_else(|| "could not resolve pryx config directory".to_string())?;
